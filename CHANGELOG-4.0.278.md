...
- PR #146 bestod exact-head-kildegaten i `32844951668` på `432de975` og blev merged som `8facd2d8`.
- Produktion `32845130587` bestod den fulde kæde og udgav datasæt `rr-20260825120459-210`. Den offentlige efterkontrol viser 205/210 aktive zoner, fem korrekt lokalt utilgængelige zoner, 657/673 READY-kyststrækninger og 16 lokale `WINDOW_INCOMPLETE`-forløb. Alle 673 fortsættelser er accepteret uden reset.
- Candidate G 20/50/30 er fortsat eneste offentlige profil med `rollbackProfileId: null` og `legacyPublicFallbackAllowed: false`.
- Den levende aktuelle rangering og alle fem prognosedage bruger særskilte strand-/wadersscorer. Tre af fem prognosedage har også forskellig top-5-rækkefølge; de øvrige store overlap skyldes de fælles transport- og mobiliseringskomponenter, ikke en delt beregningsvej.
- De offentlige topfelter i `data/kystdata.json` og `data/zones.geojson` er begge 4.0.278. Ingen yderligere geodataændring indgår i opfølgningen.
