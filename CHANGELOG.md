## 4.0.319-kandidat – integrated-first med measured-only historikopbygning (2026-09-01)
...
- Cold replay og warmup må ikke bruge syntese, interpolation, carry, neutral nulstrøm eller nabolån. Migrationens 40-timers sammenhængende WAM-bro forbliver migration-only. Feggesund skal bestå direkte 3 × 118; nabozoneproxyen er pensioneret for denne release uden geometri-, punkt- eller kystnormalændring.
...
- DMI-cache-backfill er data-bevarende og atomisk: en hel donor-time fra samme kompatible registry må kun kopieres ind i en manglende eller strukturelt tom time; felter og komponenter blandes aldrig mellem runs. Alle saves sker før `DMI_READY`/strict-anchor-terminalgaten. Run `33498108421` er fortsat negativ evidens for upstream stale katalog, ikke et grønt 118-timersbevis.
...
- Der tilføjes ingen ny controllerstatus, transition eller offentlig shadowmodel. Implementeringen og de målrettede lokale tests er grønne; frisk 118-timers preflight, exact-head, merge, frisk fuld produktion/releasegate/artifact/Pages, aktivering, Feggesund direkte 3 × 118 og offentlig desktop-/mobilverifikation afventer. Candidate G/4.0.316 er fortsat den eneste offentlige model.
