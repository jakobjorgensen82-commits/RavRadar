## 4.0.317-kandidat – integreret RavScore state 6 under DEC-0110/DEC-0112 (2026-08-30)
...
- **Status:** 4.0.317 er lokal releasekandidat; ikke merged, udgivet eller produktionsverificeret. Første PR #235 exact-head `33324239464`/`99291456658` stoppede på en stale state-6-feedbackfixture; den samlede remediation lukkede også stale public-fixtures, adaptiv ny-skrivning og bundle-drift og består fuld lokal `validate:source`. Anden exact-head `33329919843`/`99306529711` på `7bc848610794b87f62a6a3763564ca46a0d7528e` bestod de forudgående model-/runtime-/rollback-/DMI-dele, men stoppede fail-closed ved manglende `xarray`, fordi sourcegate-workflowet ikke installerede projektets ejede Copernicus-/geometry-requirements. Ingen merge, produktionsbuild/artifact, deploy eller offentlig ændring skete. Alle workflows med `validate:source` installerer nu de tre ejede requirements-sæt umiddelbart før gaten med samme betingelse, PR-timeout er 45 minutter, og målrettede workflow-/Copernicus-/bundletests er grønne. Offentlig 4.0.316 fortsætter på Candidate G, indtil ny exact-head, merge, frisk centralt hydreret produktion/validate/releasegate/artifact/Pages, Feggesund 3 × 118 og offentlig 210/673 mobil-/desktopkontrol er grønne. Frisk `rr-20260830091913-210` er regressionsbevis, ikke state-6-releasebevis eller dokumentation for empirisk bedre fundpræcision. Geodata er uændret bortset fra det autoriserede topversionsfelt 4.0.316→4.0.317.
