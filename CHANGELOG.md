...
- Samler næste RavScore som én modelkontrakt: `RRS-COASTAL-PROCESS-INTEGRATED-1.0.0`, state `4.0.0`, 20/50/30, verificeret gridstrøm 0,03/0,15 m/s og +10/-8, 24 timers fuld vægt med cosinusfade til 48 timer samt bølgemobilisering `Hs² × T` med 4/48-timers forløb.
...
- Gør sidste mile og vandstand eksplicit score-neutrale. `delivery = transportPotential × 1` betyder ingen estimeret scoreeffekt, ikke 100 % fysisk levering. Faldende vand kan både føre noget mobilt rav søværts og blotlægge/koncentrere rav bag revler; RavRadar mangler lokal batymetri, bølgeopløst surfzonemodel og ravets lokale partikelstate til et universelt fortegn. Aagaard 2002 viser både land- og søværts revletransport, Jalón-Rojas 2025 skelner Stokes-/undertoweksponering efter partikelposition, og Lofty 2023 målte rav som lavdensitets bedload/saltation frem for en fri overfladetracer. Vandstandstrend bliver derfor ikke en ekstra “hele vandsøjlen”-strøm oven på DMI/Copernicus-U/V.
- Genbruger eksisterende valideret, afledt Candidate G-vejrhistorik ved første schema-2→schema-4-migration, så cutover ikke kræver flerdages ny cache eller kunstig historik. Recovery vælger exact point-aktivering → gyldig integreret continuation → gyldigt checkpoint → dybt valideret Candidate G-state; ugyldig exact point stopper straks, mens en ugyldig ordinær kandidat ikke må skygge for en gyldig lavere prioritet. Hver continuation-state, checkpointet og den centrale profilselection skal matche den fulde 11-feltsbinding. Hvis ingen statekilde findes, genafspilles de 48 private, proveniensverificerede kildetimer target−48 h til target−1 h med strøm/bølge. Offentlige/syntetiske pre-target-rækker afvises, komplette data giver `READY` ved første offentlige target, og manglende/ugyldig kilde stopper med `RAVSCORE_RECOVERY_REPLAY_BRIDGE_MISSING` frem for offentlig warmup. Derefter fortsætter state fra privat syv-fils runtimebundle eller beskyttet checkpoint; offentligheden får kun fire hashbundne livefiler.
...
- Fører både `modelContractSha256` for parameterkontrakten og `modelBundleSha256` for den transitive implementeringslukning gennem hele 11-feltsbindingen og afviser hvert felt særskilt ved mismatch. Slutværdierne fastlåses først efter regeneration på den afsluttede head.
- Holder bølgernes to roller adskilt: `Hs² × T` bygger den tidslige mobilisering af allerede tilgængeligt rav, mens aktuel bølgehøjde kun påvirker metode-/sigtbarheden i søgeforhold. Jagtbarhedsvejen giver ingen ekstra mobiliserings-, transport- eller sidste-mile-kredit.
...
- **Status:** Ikke udgivet og ikke produktionsverificeret. Offentlig 4.0.308 fortsætter på Candidate G, indtil exact-head, frisk fuld produktion, releasegate og offentlig mobil-/desktopkontrol er grønne.
