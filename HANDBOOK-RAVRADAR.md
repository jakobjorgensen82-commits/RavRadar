...
## Aktuel status – 4.0.315 integreret RavScore-releasekandidat

Candidate G er fortsat den eneste produktionsverificerede offentlige model i 4.0.310. `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0` med state `5.0.0` er releasekandidat under DEC-0110; den er ikke exact-head-valideret, merged, deployet eller offentligt browserverificeret endnu.
...
Den fysiske levering er fortsat uopløst: `physicalDeliveryResolved=false` og fysisk interval `null`. DDM 50 m er kun statisk kontekst, Rainville 2026 kun buoyant-object-analogi, og der påstås ingen empirisk højere fundpræcision. Ingen geometri, kystnormal eller land-/vandpunkter flyttes.

Ægte cold start kræver præcis 48 private, verificerede timer plus den reelle targettime. Candidate G-rollback bruger samme target uden dobbelt recovery-credit. Nøddrift må kun bruge en komplet, atomisk og hashbundet continuation fra samme model i højst 72 timer eller kortere forecastudløb; cross-model fallback og interpolation er forbudt. Kun `VERIFIED_ONLY` er kalibreringsegnet, mens reconstructed/emergency og ture ikke er kalibreringsgrundlag.
...
## Status for det aktuelle modelarbejde – lokal releasekandidat, ikke produktion

Håndbogen har to tydeligt adskilte lag. De versionsmærkede afsnit om 4.0.308 og tidligere udgaver dokumenterer den offentlige historik. Kapitel 18, 54 og 55 beskriver den lokalt implementerede, integrerede RavScore-releasekandidat, som først må blive offentlig ved ét atomisk cutover efter grøn exact-head-kontrol, fuld produktion og offentlig mobil-/desktopkontrol.

Den senest produktionsverificerede offentlige baseline er fortsat 4.0.310 med Candidate G. Der findes ingen ekstra offentlig shadowmodel. Hvis det samlede DEC-0110-cutover består, bliver `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `5.0.0` den eneste offentlige model, mens Candidate G kun ligger privat som migrations-/offline-/rollback-orakel. 4.0.315 er releasekandidat og mangler exact-head, merge, frisk produktion og offentlig desktop-/mobilkontrol. Kun en senere særskilt, manuel og fuldt verificeret hel rollback kan gøre Candidate G til den ene offentlige model igen; den er aldrig en samtidig eller automatisk fallback. Denne lokale dokumentation er ikke i sig selv et produktionsbevis.
...
> **Historisk revisionsspor:** Afsnittet bevarer 4.0.286's Candidate G-kontrakt. Den aktive integrerede state-5-recovery og private cold-start-bro står i kapitel 55 og DEC-0110.
...
- [Chubarenko m.fl. (2017), DOI 10.1016/j.envpol.2017.01.085](https://doi.org/10.1016/j.envpol.2017.01.085) fremhæver, at systematisk ravmonitorering mangler. Gentagen migration mellem strand og undervandsskråning samt mulig tilbageførsel over dage behandles som en hypotese, ikke som bevis for, at alt rav er væk efter udtransport.
...
Den offentlige projektion viser de tre komponenter, 20/50/30-vægtene, deres bidrag, den aktuelle profil, supply-/delivery-/mobiliseringsforløbet samt den strukturelle sidste-mile-usikkerhed. Den markerer, at gridstrøm ikke er surfzonens undertow eller ripstrøm, og at scoren ikke er fundprocent. Hvis beregningen ikke er komplet, bliver kun den konkrete zone, søgemåde og time utilgængelig. Hverken Candidate G eller den gamle 25/40/35-model kan overtage automatisk, og profiler blandes aldrig mellem steder, timer eller jagtformer. Candidate G kan kun igen blive den ene offentlige model gennem den manuelle, fuldt verificerede hel rollback i kapitel 55.1.

### 18.7 Cold start, nøddrift og evidenstillid

Migrationen `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema5-v4` genbruger Candidate G's signerede, allerede afledte kystnormale strømstyrker og genvægter dem efter den nye currentkerne. Den læser eller kopierer ikke rå U/V og påstår derfor ikke, at resultatet er identisk med en genberegning fra rå strømvektorer. Wave-approach bygges fra 40 private præ-target-timer i ét sammenhængende DMI WAM-run pr. anvendt collection og med samme modelcelle. Kun en manglende WAM-time mellem native endepunkter højst fire timer fra hinanden må interpoleres, og kun inden for samme run, collection, gitter og celle. Den udeladte ældre EWMA-hale er højst `1/1024`, svarende til højst `0,01171875` konservativt beregnede rå scorepoint før afrunding.

Alle 673 Candidate G-states skal være gyldige schema 2, passe til det uforanderlige kystdelsregister og pege på præcis samme kanoniske targettime. Hvis targetet er blandet, en state er ugyldig eller det sammenhængende WAM-run ikke kan skaffes, stopper skiftet, og Candidate G forbliver offentlig. Der dannes ingen syntetisk eller offentlig historik. En ægte cold start er en særskilt vej og kræver præcis 48 sammenhængende private, verificerede timepositioner plus den reelle targetrække. Rollback `integrated-schema5-to-candidate-g-schema2-v2` bruger samme targettid uden dobbelt recovery-credit.
...
Candidate G-filerne, `js/core/score-engine.js`, `js/core/rule-engine.js` og gamle regelfiler findes kun til migration, rollbackkontrol, historiske sammenligninger og forskning. De er ikke en samtidig offentlig scorevej.
...
- Geodatastyrelsen, [Danmarks Dybdemodel](https://gst.dk/ansvarsomraader/soekort-og-marine-data/soeopmaaling-og-dybdedata/danmarks-dybdemodel). Det officielle 50 m-grid kan bruges som statisk dybdekontekst, men opløser ikke dynamiske revler, ripkanaler eller en bølgeopløst surfzone og er derfor ikke RavScore-input.
...
## 54. Sådan beregner RavRadar det samlede rav- og søgeindeks

*Den aktive integrerede modelkontrakt i almindeligt dansk.*
...
Hvis en nødvendig integreret beregning mangler, bliver kun den konkrete zone, søgemåde og time utilgængelig. Den vises ikke med en erstatningsscore og udelades fra de relevante ranglister, mens øvrige gyldige scorer fortsætter. Adminforsiden viser modelbinding og lokale huller. RavRadar blander ikke model-id, stateformat eller profiler mellem steder, tider og jagtformer.
...
RavRadar bygger fortsat én fuld produktionsruntime og projekterer derfra præcis fire offentlige livefiler: manifest, kompakt startpakke, detaljer og kystdele. Manifestet binder dataset-id, model-id, stateformat, kontrakter, størrelser og kryptografiske fingeraftryk. Browseren accepterer kun filer fra samme bundne datasæt.

Modelbindingen bruger to forskellige fingeraftryk. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder 34 eller flere kanonisk normaliserede transitive implementeringsfiler. Dermed kan en ændring i en evaluator, adapter eller policy ikke gemme sig bag en uændret parameterfil. De endelige værdier dokumenteres først efter regeneration på den afsluttede head.

Den fulde conditions-fil, DMI-caches, den forseglede Copernicus-current-range-cache, strømhistorik, sundhedsdata, runtime-diagnostik og vandstandsstationsdata ligger i en privat, eksakt otte-fils runtimebundle. Copernicus-cachen gør allerede indsamlet bridgehistorik og dens acquisition-/coveragebeviser genbrugelige ved næste run; den er aldrig en browserfil. Bundlen kan også indeholde den varme Candidate G-rollbackprojektion under feltet `ravScoreCandidateGRollback`; feltet er ikke en ekstra offentlig fil. Bundlen kontrolleres for eksakt 210/673-dækning, modelbinding, canonical hashes, stier og symlinks og installeres atomisk. Den komprimerede kopi ligger i den ikke-offentlige Supabase Storage-bucket `ravradar-private-production-runtime`; et beskyttet pointerdokument bevarer kun den aktuelle og den foregående kontrollerede generation, og anonym adgang skal være afvist. Bundlen kopieres aldrig til Pages, supportpakken eller kildekoden.

Et separat beskyttet checkpoint indeholder kun den afledte, kompakte state-5-fortsættelsestilstand. Ved første skift fra Candidate G bruger migrationen `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema5-v4`. Den validerer og genvægter Candidate G's signerede, allerede afledte kystnormale currentevidens uden at læse eller kopiere rå U/V og uden at påstå lighed med en rå genberegning. Wave-approach genopbygges af 40 private præ-target-positioner fra ét sammenhængende DMI WAM-run pr. anvendt collection med same-cell native provenance. Kun et WAM-gap mellem native endepunkter højst fire timer fra hinanden i samme run, collection, gitter og celle må interpoleres; cross-run, cross-cell og tvetydig retning afvises. `1/1024` er øvre grænse for den udeladte EWMA-hale, og `0,01171875` er den konservative øvre rå-scorefejl før afrunding.

Inden acquisition valideres præcis 673 schema-2-states mod det uforanderlige kystdelsregister og deres state keys. Den samme rene recoveryfunktion skal give præcis ét fælles kanonisk target. Mixed target, manglende eller ugyldig state og utilgængeligt coherent WAM-run stopper fail-closed; Candidate G forbliver offentlig. Modellen skal derfor ikke vente flere dage på en ny cache, når de krævede verificerede data allerede findes, men den opfinder heller aldrig historik for at gøre et skift muligt. En ægte cold start kræver særskilt præcis 48 sammenhængende private, verificerede timepositioner plus den reelle targetrække. Den kontrollerede WAM-interpolation er kun en privat bootstrapregel inden for ét dokumenteret run; den gælder ikke 72-timers nøddrift, som fortsat aldrig interpolerer eller relabeler data.

Hver continuation-state, checkpointet og den centrale profilselection skal matche den fulde 11-feltsbinding. Candidate G-rollback `integrated-schema5-to-candidate-g-schema2-v2` beregnes fra samme targettid uden dobbelt recovery-credit. Fremtidige opdateringer af den operationelle checkpointnøgle kopieres ikke til `admin_document_versions`, men alle eksisterende versionsrækker bevares; migration og publicering sletter ingen historik. Candidate G kan kun bruges som én kontrolleret migrations-/rollbackkilde og aldrig som automatisk offentlig nødmodel.
...
Dette afsnit er historik. Candidate G blev offentlig med `20/50/30`, modeafhængige søgeforhold, strømstyret transport og bølgeenergimobilisering; pipeline, forklaringer, daværende rollback, national slutshadow og fulde gates blev gennemført i 4.0.261–4.0.263. Candidate G er fortsat den produktionsverificerede offentlige 4.0.308-model, indtil det integrerede cutover er bevist. Mens integreret er aktiv, er Candidate G et privat migrations-/offline-/rollback-orakel; en senere manuel hel rollback kræver nyt, fuldt verificeret offentligt skift.
