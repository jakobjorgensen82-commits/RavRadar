...
## Første modelskift sker i to kontrollerede faser – lokal 4.0.320-kandidat
...
Denne tofasefunktion er lokalt implementeret og måltestet, men endnu ikke commit'et, pushet, exact-head-verificeret, merged, kapacitetsmålt eller live. Den integrerede model må derfor stadig ikke beskrives som offentlig.

## Integrated-first med verificeret historikopbygning – lokal 4.0.320-kandidat, ikke produktion
...
## Aktuel status – RavScore 4.0.320 first-cutover-kandidat

Candidate G er fortsat den eneste offentlige model i den produktionsverificerede appbaseline 4.0.316. 4.0.320-kandidaten er samlet lokalt, men endnu ikke exact-head-valideret, merged eller deployet. PR #241 lukkede den aktive bearing-`360`-blocker uden geometri- eller punktændring; mergeproduktion `33400836760` passerede denne gate og stoppede derefter fail-closed på en forældet 11-felts legacy-profilfixture. Den lokale rettelse matcher nu den faktiske 20-felts Candidate G-producent, binder manifest og conditions til samme profil og holder ukendte/miksede profiler lukkede. Målrettede cutovertests og den fulde privacy-sikre offentlige source-closure-verifikation er grønne. Schema 5 var en aldrig-offentlig kandidat og er kun en eksakt 5→6-migrationskilde.
...
## Status for det aktuelle modelarbejde – lokal 4.0.320-cutoverkandidat, ikke produktion

Håndbogen har to tydeligt adskilte lag. De versionsmærkede afsnit om 4.0.308 og tidligere udgaver dokumenterer den offentlige historik. Kapitel 18, 54 og 55 beskriver state 6 og den lokale 4.0.320-cutoverkandidat, som først må blive offentlig ved ét atomisk cutover efter grøn exact-head-kontrol, merge, fuld produktion og offentlig mobil-/desktopkontrol. 4.0.317-source blev merged, men den fejlende mergeproduktion stoppede før build/deploy og gjorde den aldrig offentlig.

Den offentlige baseline er 4.0.316 med Candidate G. Der findes ingen ekstra offentlig shadowmodel. Hvis 4.0.320 består det samlede DEC-0110/DEC-0112-cutover, bliver `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `6.0.0` den eneste offentlige model, mens Candidate G kun ligger privat som migrations-/offline-/rollback-orakel. Kandidaten mangler frisk fuld produktion og offentlig desktop-/mobilkontrol. Kun en senere særskilt, manuel og fuldt verificeret hel rollback kan gøre Candidate G til den ene offentlige model igen; den er aldrig en samtidig eller automatisk fallback. Denne lokale dokumentation er ikke i sig selv et produktionsbevis.
