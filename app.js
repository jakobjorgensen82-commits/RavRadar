...
function renderSelectedZone() {
  ...
  bindZoneInfoInteractions(infoPanel,state.selectedZone,state.mode,condition.history||{}); bindObservationForm();
}

function openZone(zone,{scroll=true}={}) {
  state.selectedZone=zone; const point=zone.pinPoint||zone.dataPoint; if(point) map.setView([point[1],point[0]],Math.max(map.getZoom(),10)); renderSelectedZone(); if(scroll) infoPanel.scrollIntoView({behavior:"smooth",block:"start"});
}
...
function bestForDay(zone,date) {
  ...
  return day.hours.map(hour=>({hour,result:resultFor(zone,hour,condition.history||{})})).filter(item=>item.result.available).sort((a,b)=>b.result.score-a.result.score)[0]||null;
}
function renderNationalForecast() {
  ...
  const render=index=>{nationalForecast.querySelectorAll(".national-day-tab").forEach((button,i)=>button.classList.toggle("active",i===index)); const rows=data[index].rows; list.innerHTML=rows.length?rows.map((item,i)=>`<button type="button" class="national-zone-row" data-zone-id="${item.zone.id}"><span class="rank">${i+1}</span><span><strong>${item.zone.name}</strong><small>Bedste tidspunkt ca. ${new Intl.DateTimeFormat("da-DK",{hour:"2-digit",minute:"2-digit"}).format(new Date(item.hour.time))}</small></span><b class="rank-score ${item.result.level}">${item.result.score}</b></button>`).join(""):'<p class="ranking-empty">Ingen prognosedata for dagen.</p>'; list.querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>openZone(state.zones.features.find(f=>f.properties.id===button.dataset.zoneId).properties)));};
...
function updateTripUi(){const trip=activeTrip();document.querySelector("#tripBar").hidden=!trip;document.querySelector("#tripButton").textContent=trip?"Tur i gang":"Start ravtur";}
...
document.querySelector("#tripButton").addEventListener("click",()=>{if(!activeTrip())startTrip();updateTripUi();});document.querySelector("#stopTripButton").addEventListener("click",()=>{stopTrip();updateTripUi();});
