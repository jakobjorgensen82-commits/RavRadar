import { calculateRavScore, exceptionalScoreMark } from "./js/core/score-engine.js?v=4.0.250";
import { selectBestTimeForDay } from "./js/core/best-time-selector.js?v=4.0.250";
import { loadConditions, loadConditionDetails, mergeConditionDetails, loadZones, loadDataManifest } from "./js/services/data-service.js?v=4.0.250";
import { submitObservation, getLocalObservations, syncPendingObservations } from "./js/services/observation-service.js?v=4.0.250";
import { predictAmberChance } from "./js/core/prediction-engine.js?v=4.0.250";
import { consumeAuthCallback } from "./js/services/auth-service.js?v=4.0.250";
import { activeTrip, answerTrip, pendingTripPrompt, resumeTripTracking, startTrip, stopTrip } from "./js/services/trip-service.js?v=4.0.250";
import { createMap, installFlowArrows, refreshZoneStyles, renderZones } from "./js/map/map-view.js?v=4.0.250";
import { projectPublicCoastlines } from "./js/map/public-coast-projection.js?v=4.0.250";
import { bindZoneInfoInteractions, showZoneInfo } from "./js/ui/info-panel.js?v=4.0.250";
import { openAccountDialog } from "./js/ui/account-panel.js?v=4.0.250";
import { openDeveloperDialog } from "./js/ui/developer-panel.js?v=4.0.250";
import { askRavRadar, QUICK_QUESTIONS } from "./js/services/rav-assistant.js?v=4.0.250";
import { loadAdaptiveModel } from "./js/core/adaptive-model.js?v=4.0.250";
import { buildLocalZoneScore, selectLocalBestForDay } from "./js/core/local-zone-score.js?v=4.0.250";
...
// RavRadar 4.0.250: versionsmanifest + sikker service-worker-opdatering.
function installAppUpdateFlow() {
  ...
  const version=window.RAVRADAR_VERSION||"4.0.250"; document.querySelector("#appVersion").textContent=version;
...
import { createPublicTripEvidenceRuntime } from './js/services/trip-evidence-runtime.js?v=4.0.250';
import { installTripEvidenceLegacyBridge } from './js/services/trip-evidence-legacy-bridge.js?v=4.0.250';
import { submitTripEvidenceObservation } from './js/services/observation-service.js?v=4.0.250';
...
function publicTripEvidenceContext(selection = null) {
  ...
  const versionText = String(globalThis.RAVRADAR_VERSION || document.querySelector('#appVersion')?.textContent || '4.0.250');
  const appVersion = versionText.match(/\d+\.\d+\.\d+/)?.[0] || '4.0.250';
  return {
