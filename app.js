import { exceptionalScoreMark, scoreRating } from "./js/core/score-presentation.js?v=4.0.395";
import { loadConditions, loadConditionDetails, mergeConditionDetails, loadZones, loadDataManifest, reevaluatePublicConditions } from "./js/services/data-service.js?v=4.0.395";
import { submitTripEvidenceObservation, syncPendingObservations } from "./js/services/observation-service.js?v=4.0.395";
import { consumeAuthCallback } from "./js/services/auth-service.js?v=4.0.395";
import { createMap, installFlowArrows, refreshZoneStyles, renderZones } from "./js/map/map-view.js?v=4.0.395";
import { projectPublicCoastlines } from "./js/map/public-coast-projection.js?v=4.0.395";
import { bindZoneInfoInteractions, showZoneInfo } from "./js/ui/info-panel.js?v=4.0.395";
import { openAccountDialog } from "./js/ui/account-panel.js?v=4.0.395";
import { openDeveloperDialog } from "./js/ui/developer-panel.js?v=4.0.395";
import { askRavRadar, quickQuestions, ravQuestionNeedsConditionDetails } from "./js/services/rav-assistant.js?v=4.0.395";
import { formatDateTime, formatNumber, getLanguage, getLocale, t } from "./js/i18n.js?v=4.0.395";
import { buildLocalZoneScore, isCurrentForecastHour, selectLocalBestForDay } from "./js/core/local-zone-score.js?v=4.0.395";
import { addNationalRanking, compareNationalRankingRows } from "./js/core/zone-ranking.js?v=4.0.395";
import { createPublicTripEvidenceRuntime } from './js/services/trip-evidence-runtime.js?v=4.0.395';
import { createPublicPageResumeHandler, createServiceWorkerControllerChangeHandler } from './js/core/public-page-resume.js?v=4.0.395';
import { forecastDateKeyInTimeZone, visibleForecastDays } from './js/core/forecast-calendar.js?v=4.0.395';
import { assertRavScoreModelBinding } from './js/core/ravscore-model-contract.js?v=4.0.395';
...
function zoneCondition(zone) {
  ...
  return state.conditions.zones?.[zone?.id] || {};
}
...
function currentScoreFor(zone){const key=`${state.mode}:${zone.id}`;if(!state.currentScores.has(key)){const local=state.conditions?.emergencyDetailsDeferred?null:localZoneScore(zone);const result=local||{available:false,score:null,level:'unavailable',label:'RavScore midlertidigt utilgængelig',reasons:['RavScore-data mangler for zonen lige nu.']};state.currentScores.set(key,result);}return state.currentScores.get(key);}
...
async function renderNationalForecast() {
  ...
  if(state.conditions?.emergencyDetailsDeferred){
    nationalForecast.innerHTML=`<p class="ranking-empty">${t('forecast.nextUpdate')}</p>`;
    return true;
  }
...
function updatePublicDataStatus(conditions){
  ...
  if(conditions?.available&&conditions?.emergencyDetailsDeferred){
    dataStatus.textContent=`${t('data.failed')} ${t('forecast.nextUpdate')}`;
...
// RavRadar 4.0.395: versionsmanifest + sikker service-worker-opdatering.
function installAppUpdateFlow() {
  ...
  const version=window.RAVRADAR_VERSION||"4.0.395"; document.querySelector("#appVersion").textContent=version;
...
function publicTripEvidenceContext(selection = null) {
  ...
  const versionText = String(globalThis.RAVRADAR_VERSION || document.querySelector('#appVersion')?.textContent || '4.0.395');
  const appVersion = versionText.match(/\d+\.\d+\.\d+/)?.[0] || '4.0.395';
