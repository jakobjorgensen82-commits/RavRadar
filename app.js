import { calculateRavScore, exceptionalScoreMark } from "./js/core/score-engine.js?v=4.0.259";
import { selectBestTimeForDay } from "./js/core/best-time-selector.js?v=4.0.259";
import { loadConditions, loadConditionDetails, mergeConditionDetails, loadZones, loadDataManifest } from "./js/services/data-service.js?v=4.0.259";
import { submitObservation, getLocalObservations, syncPendingObservations } from "./js/services/observation-service.js?v=4.0.259";
import { predictAmberChance } from "./js/core/prediction-engine.js?v=4.0.259";
import { consumeAuthCallback } from "./js/services/auth-service.js?v=4.0.259";
import { activeTrip, answerTrip, pendingTripPrompt, resumeTripTracking, startTrip, stopTrip } from "./js/services/trip-service.js?v=4.0.259";
import { createMap, installFlowArrows, refreshZoneStyles, renderZones } from "./js/map/map-view.js?v=4.0.259";
import { projectPublicCoastlines } from "./js/map/public-coast-projection.js?v=4.0.259";
import { bindZoneInfoInteractions, showZoneInfo } from "./js/ui/info-panel.js?v=4.0.259";
import { openAccountDialog } from "./js/ui/account-panel.js?v=4.0.259";
import { openDeveloperDialog } from "./js/ui/developer-panel.js?v=4.0.259";
import { askRavRadar, QUICK_QUESTIONS } from "./js/services/rav-assistant.js?v=4.0.259";
import { loadAdaptiveModel } from "./js/core/adaptive-model.js?v=4.0.259";
import { buildLocalZoneScore, selectLocalBestForDay } from "./js/core/local-zone-score.js?v=4.0.259";
import { addNationalRanking, compareNationalRankingRows } from "./js/core/zone-ranking.js?v=4.0.259";
...
function withPrediction(result,zone,weather,history){return {...result,prediction:predictAmberChance({baseScore:result.score,zone,weather,history,observations:getLocalObservations(),model:state.adaptiveModel})};}
function currentDisplayFor(zone){
  ...
  const fallback=state.conditions.coastalParts?.enabled===true;
  return {result:withPrediction(base,zone,weather,history),weather,context:{scope:fallback?'parent-fallback':'parent',time:weather.time||null}};
}
...
function bestForDay(zone,date) {
  ...
  return {...selectBestTimeForDay({day,zone,mode:state.mode,history:condition.history||{},currentWeather:condition.current||null,currentResult,adaptiveModel:state.adaptiveModel}),displayScope:'parent-fallback'};
}
...
// RavRadar 4.0.259: versionsmanifest + sikker service-worker-opdatering.
function installAppUpdateFlow() {
  ...
  const version=window.RAVRADAR_VERSION||"4.0.259"; document.querySelector("#appVersion").textContent=version;
...
import { createPublicTripEvidenceRuntime } from './js/services/trip-evidence-runtime.js?v=4.0.259';
import { installTripEvidenceLegacyBridge } from './js/services/trip-evidence-legacy-bridge.js?v=4.0.259';
import { submitTripEvidenceObservation } from './js/services/observation-service.js?v=4.0.259';
...
function publicTripEvidenceContext(selection = null) {
  ...
  const versionText = String(globalThis.RAVRADAR_VERSION || document.querySelector('#appVersion')?.textContent || '4.0.259');
  const appVersion = versionText.match(/\d+\.\d+\.\d+/)?.[0] || '4.0.259';
  return {
