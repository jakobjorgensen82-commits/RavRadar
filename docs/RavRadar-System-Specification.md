...
Denne specifikation skelner mellem den produktionsverificerede offentlige baseline 4.0.310 og den lokalt implementerede 4.0.315 RavScore-releasekandidat. Candidate G forbliver den eneste offentlige model indtil ét atomisk cutover efter grøn exact-head-kontrol, merge, frisk produktion og offentlig mobil-/desktopkontrol. Derefter er `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0` den eneste offentlige model, mens Candidate G ligger privat som migrations-/offline-/rollback-orakel. Kun en senere manuel, fuldt verificeret hel rollback kan gøre Candidate G til den ene offentlige model igen; den er aldrig samtidig eller automatisk fallback. RDKS og DEC-0110 er fortsat bindende ved konflikt.
...
Ved cutover overtager den integrerede model samme 20/50/30-hovedvægtning som én samlet model med stateversion `5.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileewma4-atten15-v4`, komponent `ravscore-components-huntability-delivery-mobilisation-v4` og forklaring `ravscore-explanation-integrated-v4`. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder den kanonisk normaliserede transitive implementeringslukning. De endelige værdier må først dokumenteres efter regeneration på den afsluttede head.
...
- Den integrerede state-5 dannes ved engangsmigrationen `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema5-v4`. Kun Candidate G's signerede, allerede afledte kystnormale currentevidens genvægtes; migrationen læser/kopierer ikke rå U/V og påstår ikke lighed med en rå genberegning.
...
- Ægte cold start er særskilt og kræver præcis 48 sammenhængende private, proveniensverificerede timepositioner plus den reelle targetrække. Den samme afgrænsede WAM-interpolation må bruges inden for identisk run/collection/grid/celle; strøm, offentlig historik og same-model nøddrift interpoleres ikke.
- Efter migrationen lever state og recovery i den private hashbundne runtime eller et beskyttet state-5-checkpoint. Hver continuation-state, checkpointet og den centrale profilselection skal matche den fulde 11-feltsbinding. Candidate G-rollback `integrated-schema5-to-candidate-g-schema2-v2` beregnes fra samme targettid og må ikke bruge samme time som ekstra recovery-credit. Den varme projektion ligger kun privat som `ravScoreCandidateGRollback`. De offentlige startup-, detalje-, state- og manifestprojektioner er fortsat dataminimerede og hashbundne.
...
2. Seneste verificerede Candidate G-state eller integrerede state-5 hentes efter modelversionen.
