...
Denne runbook er den operative ledsager til DEC-0082. DEC-0109 er historisk og operationelt erstattet af DEC-0111: den planlagte rekonstruktion blev trukket tilbage før descriptor/apply, ingen syntetiske data blev anvendt, og dens vejrproduktionsinterlock pensioneres i 4.0.315. D1-/Edge-/Worker-, privacy-, migration-, idempotens- og live-attestationskontrakterne består uændret. Runbooken må aldrig bruges til at genåbne reconstruction-operationen eller udskrive private ture eller credentials.

## 4.0.315-afgrænsning

Normal 4.0.315-vejrproduktion må ikke afhænge af et exact-D1/apply/Pages-bevis fra den pensionerede incident. Det eksisterende `trip-storage-readiness`-job bliver stående som historical exact-D1-gate for 4.0.311–4.0.314 og returnerer eksplicit `ready=true` for 4.0.315. Det separate turlagerworkflow kører fortsat D1-deploy og storageverifikation efter egne kontrakter. Defensive turkvalitetsmarkører bevares fail-closed for historisk kompatibilitet, men kan ikke skabe rekonstrueret state.

## Historisk 4.0.314-policyplan – tilbagetrukket uden anvendelse

Dette afsnit er kun revisionsspor for en plan, der aldrig blev anvendt. Der må ikke køres nyt inspect eller apply, og planen må ikke genoptages. Den planlagte inspect/apply-procedure ville have brugt `data/current-regional-proxy-policy.json` som cadenceautoritet. Kun en koordinatfri projektion skulle have været hashet i descriptoren; policyindhold, del-id'er og koordinater måtte ikke eksponeres i checkannotation eller driftslog. På 1h-dele ville eksakte målte afstande på 1, 2 eller 3 timer være blevet behandlet som eksisterende bounded continuity uden rekonstruktion af manglende interne slots. De otte `dkss_lf`-dele skulle have haft eksakt 3h på både før- og targetkanten. Et planlagt apply ville have genvalideret policyhash, descriptor, kilder og mål atomisk gennem den eksisterende CAS.

Den historiske rækkefølge var tænkt som lokal source/RDKS/review → exact-head PR → merge/no-op → exact-main D1 → read-only inspect → eventuelt descriptorbundet apply. Den nåede aldrig descriptor eller apply og blev trukket tilbage af DEC-0111. Ingen descriptor må nu føre til apply. Normal 4.0.315-produktion følger i stedet sin measured-only produktions- og releasekæde.

## Historisk 4.0.314-inspect-/releaseplan – ikke eksekverbar

Den aldrig anvendte plan ville for de otte policyklassificerede native 3-timersdele have tilladt ét eksakt målt højreanker i rollen `AFTER`, men kun efter fuldt state-replay og uafhængigt kompatibilitetsbevis på begge kanter. Nul after-punkter og singleton i andre roller ville have stoppet planen. Den daværende plan krævede et nyt grønt 4.0.314-D1-run før inspect og holdt normal produktion som no-op indtil et exact-head apply-run med Pages-bevis. DEC-0111 trak hele operationen og dette krav tilbage før anvendelse. Der må ikke bestilles nyt D1 til rekonstruktionen, inspect, apply, rollback eller cleanup; normal 4.0.315-produktion må ikke vente på dem.
...
Den uændrede 4.0.311-kørsel må ikke genstartes som genvej. 4.0.312 udførte den sikre constraint-roll-forward og beviste de efterfølgende backendtrin frem til migrationssynkroniseringen, men hele dens røde kørsel kan ikke genbruges som readinessbevis.

## 4.0.312-synkhændelsen og sikker 4.0.313-genafspilning

Den syntetisk reproducerede årsag er en formforskel, ikke en ændring i turens mening: historiske 4.0.310-rækker kan indeholde kendte nullable blade i `weather_snapshot` og `calibration_features`, mens 4.0.311-migrationens eksplicitte PostgREST-bladprojektion udelader JSON-null. Den samme tilladte historiske række kan derfor få en anden kanonisk migrationspayload og hash, selv om alle ikke-null-, kerne-, ejer- og id-felter er uændrede. Det forklarer synkstopperen uden at kræve eller tillade læsning af private produktionspayloads.

4.0.313 tillader kun denne kompatibilitet, når både den eksisterende og den indkommende kilde er `supabase-migration`, og deres hashes er forskellige. Schema v2 valideres først mod den strenge lagrede top-level- og nested-allowlist; schema v1 projiceres gennem sin dokumenterede begrænsede legacykontrakt. Derefter fjernes kun kendte null-/undefined-blade og deraf tomme kendte underobjekter. Kernefelter, ikke-null-blade, ejer, id, shard og den deterministiske projektion skal fortsat være eksakt ens. Live-klienter får ingen undtagelse.

Ved et kompatibelt replay må Worker hverken omskrive den eksisterende række, dens oprindelige payloadhash eller registrybindingen. Mangler kun registryraden, må den repareres med den gamle lagrede hash; en modstridende registryhash stopper fail-closed. Gatewayfejl forlader kun en fast intern fejlkategori og aldrig upstream-body eller andre ubetroede detaljer. En sikker genkørsel starter først på en exact-head-grøn og merged 4.0.313-main og skal bestå hele den normale initiale synk, afsluttende reconciliation, begge Edge-attestationer, Worker-reattestation, SQL og registry. Kun den samlede grønne success-kæde er readiness; recovery- eller fejlstien er det aldrig.
...
Den i 4.0.311 mergede og gennem 4.0.312/4.0.313 fremførte kontrakt har konstant `[d1]`-identitet. Supabase kan kun være en kort initial bro ved en **helt ny** installation uden varig D1-controlmarkør og uden de ti eksisterende RavRadar-shards. Efter grønt capacity- og exact-main-bevis sættes præcis ét current-run installationstype-intent umiddelbart før første Edge-deploy: `d1_edge_predeploy_intent` for eksisterende D1 eller `fresh_edge_predeploy_intent` for genuine fresh. Existing-D1 får maintenance-kapabel Edge under uændret D1-mode/gammel Worker, derpå repair-intent og en udløbende lease før Worker. En partial existing-D1 Edge-deploy går kun D1 roll-forward; en partial fresh Edge-deploy går exact-main-bundet tilbage til Supabase-secret, eksakt Edge-redeploy og dobbelt Supabase-attestation.
...
11. Vent yderligere 20 sekunder, genbekræft eksakt main og kør afsluttende reconciliation. Genverificér begge Edge-grænser, Worker, SQL, global registry og D1-mode. Først et succesfuldt eksakt-head-bundet `[d1]`-run er gyldigt readinessbevis for en 4.0.313-Pagesproduktion.
...
Kommandoen `scripts/delete-trip-owner-data.mjs --confirm-delete-owner-data` må kun køres ved en verificeret ejerhenvendelse med credentials i procesmiljøet. `TARGET_SUPABASE_USER_ID` må ikke skrives som kommandolinjeargument eller i en delt log. Den siden 4.0.311 mergede og gennem 4.0.313 roll-forward-bundne kommando:
