...
Denne runbook er den operative ledsager til DEC-0082 og DEC-0109. **Status ved det lokale 4.0.314-checkpoint:** 4.0.313 bestod PR #226 exact-head `33269501339`, blev merged som `ff62ba11`, fik korrekt no-op push `33269584236` og bestod hele exact-main D1-backend `33269631305`/`99145677813`, inklusive begge syncs, slutreconciliation, Edge, Worker, registry og SQL. Read-only inspect `33269849748`/`99146287609` stoppede før descriptor/apply på `ONE_TIME_GAP_AFTER_EVIDENCE_COUNT`; ingen data, cache, artifact eller Pages blev ændret. Lokal 4.0.314 retter kun dette målte singleton-afteranker og låser D1/inspect/apply/Pages-rækkefølgen. Den kræver nyt exact-head D1-bevis; offentlig sandhed er fortsat 4.0.310 i nøddrift, og morgenhullet er uændret. Runbooken må ikke bruges som bevis for, at rekonstruktionen er gennemført, og den må aldrig bruges til at udskrive private ture eller credentials.

## 4.0.314-inspect og release-rækkefølge

After-artifactet kan for de otte native 3-timersdele indeholde det ene eksakte målte højreanker. Det accepteres kun i rollen `AFTER`, kun efter fuldt state-replay og kun når før- og targetserien uafhængigt beviser enstemmig 3-timerskadence. Nul after-punkter og singleton i alle andre roller stopper. Et nyt 4.0.314-D1-run skal være helt grønt før inspect. Normal produktion forbliver no-op, indtil et exact-head apply-run også har et succesfuldt Pages-job; manglende eller malformed GitHub-metadata åbner intet. Fælles concurrency annullerer aldrig en kørende apply.
