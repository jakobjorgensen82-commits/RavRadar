...
Denne runbook er den operative ledsager til DEC-0082 og DEC-0109. **Status ved det lokale 4.0.314-cadencecheckpoint:** PR #231 bestod exact-head `33279317463`/`99171645787`, blev merged som `d539fc9d` og fik korrekt no-op push `33279411885`. Exact-main D1 `33279463545`/`99172031927` er helt grøn. Read-only inspect `33279639424`/`99172534863` stoppede før descriptor/apply/build/Pages på `ONE_TIME_GAP_AMBIGUOUS_NATIVE_CADENCE`; ingen data eller cache blev ændret. Lokal rettelse binder cadence til den eksisterende regionale proxy-policy og ændrer ingen data. Den kræver nyt exact-head- og D1-bevis; offentlig sandhed er fortsat 4.0.310 i nøddrift, og morgenhullet er uændret. Runbooken må ikke bruges som bevis for, at rekonstruktionen er gennemført, og den må aldrig bruges til at udskrive private ture eller credentials.

## 4.0.314-policybundet cadence og næste rækkefølge

Inspect og apply bruger eksplicit `data/current-regional-proxy-policy.json` som cadenceautoritet. Kun en koordinatfri projektion hashes i descriptoren; policyindhold, del-id'er og koordinater må ikke eksponeres i checkannotation eller driftslog. På 1h-dele accepteres eksakte målte afstande på 1, 2 eller 3 timer som eksisterende bounded continuity, uden at manglende interne slots rekonstrueres. De otte `dkss_lf`-dele kræver eksakt 3h på både før- og targetkanten. Apply genvaliderer policyhash, descriptor, kilder og mål atomisk gennem den eksisterende CAS.

Fortsæt kun i denne rækkefølge: lokal source/RDKS/review → exact-head PR → merge/no-op → exact-main D1 → read-only inspect. Kun en forseglet descriptor med de faste sikre optællinger må føre til apply. Derefter kræves frisk normal vejrhentning, fuld validate/releasegate, artifact/Pages og offentlig desktop-/mobilkontrol af præcis 210/673. Enhver anden fejl stopper uden mutation.

## 4.0.314-inspect og release-rækkefølge

After-artifactet kan for de otte policyklassificerede native 3-timersdele indeholde det ene eksakte målte højreanker. Det accepteres kun i rollen `AFTER`, kun efter fuldt state-replay og kun når både før- og targetkanten uafhængigt beviser kompatibilitet med policyidentiteten gennem mindst to eksakte 3h-intervaller på hver kant. Nul after-punkter og singleton i alle andre roller stopper. Et nyt 4.0.314-D1-run skal være helt grønt før inspect. Normal produktion forbliver no-op, indtil et exact-head apply-run også har et succesfuldt Pages-job; manglende eller malformed GitHub-metadata åbner intet. Fælles concurrency annullerer aldrig en kørende apply.
