...
Denne runbook er den operative ledsager til DEC-0082 og DEC-0109. **Status ved det lokale 4.0.313-checkpoint:** 4.0.312 bestod PR #225 exact-head CI `33266087776`/job `99136292810`, blev merged som `a5ece10d` og fik den korrekte grønne no-op-pushkørsel `33266184326` uden artifact eller Pages. Backendkørslen `33266229687`/job `99136669571` bestod kilde-, constraint-, D1-klargørings-, Edge- og Worker-gates, men stoppede fail-closed i den idempotente Supabase→D1-synk med den faste kategori `TRIP_GATEWAY_UNAVAILABLE`. Fejlens roll-forward-kæde er ikke readinessbevis, og rekonstruktion, vejrbyg, artifact og Pages blev ikke kørt. Offentlig sandhed er derfor fortsat produktionsverificeret 4.0.310 i nøddrift, og morgenhullet er uændret. Den lokale 4.0.313-kandidat retter den afgrænsede legacy-genafspilning; exact-head PR, merge, et helt grønt live `[d1]`-run, ny inspect/apply, frisk produktion og offentlig kontrol mangler. Runbooken må ikke bruges som bevis for, at cutover eller rekonstruktion er gennemført, og den må aldrig bruges til at udskrive private ture eller credentials.
