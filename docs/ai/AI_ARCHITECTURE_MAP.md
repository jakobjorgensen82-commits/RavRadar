...
- `publicConditionDetailsBytes` bliver den eksakte læsegrænse under `CODE_ONLY_MAXIMUM_PUBLIC_DETAILS_BYTES` på 512 MiB. Grænsen er fortsat eksplicit og bundet til manifestet; den er ikke en ubegrænset storfilslæsning.
...
- Fase B kræver manuelt `ravscore_integrated_first_cutover=true` plus den eksakte kapacitetstoken. Actionresolveren validerer før DMI den allerede forseglede centrale Phase-A-complete Candidate G-identitet: fuld binding og manifesthash, dataset/reference, deployment-id, implementation closure og `sourceHead` på samme `GITHUB_SHA`.
- Den tidlige centrale identitet er ikke live Pages-readback. Deployleddet genverificerer den faktiske offentlige source, manifest og implementation closure, før første begin-CAS eller central modelmutation. Gamle forseglede planer/reconciliation består; en ny direkte rowless/legacy→integrated-plan kan ikke opstå.
- Koden og de direkte tests er lokale. Fase A/exact-head/merge, 673 × 118/Feggesund, live Supabase-kapacitet, Fase B, fuld produktion og browserkontrol er ikke gennemført; Candidate G er fortsat offentlig.
...
- Adapteren er bølge-only og 50/50 energikonsistent: `Hs²` midles før kvadratrod, mens periode og cirkulær FROM-retning vægtes med `Hs²`. Retnings-, energi- og periodeuenighed klassificeres deterministisk som `LOW`, `MODERATE` eller `HIGH`; en proxytime viser fast DA/DE/EN-advarsel og låser `calibrationEligible=false` gennem mode, zone, public, tur og observation, også ved ellers `FULL_HISTORY`.
- Undtagelsen gælder ikke current, historik, recovery-backfill, geometri, land-/vandpunkter eller kystnormal og er ikke en lokal surfzonemodel. Release kræver privacy-safe 3 × 118 med `DIRECT` + proxy = 354 og `MISSING` = 0. Exact-head, frisk fuld produktion og offentlig verifikation afventer.
- Gældende 4.0.320-slutbundles er forseglet og lokalt måltestet: integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3` over 44 filer/8 deklarerede forbrugere og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d` over 56 filer. Exact-head `33577887262` fandt en fail-closed Candidate G-rollbackregression på den historiske 4.0.319-head; fem-validator-rettelsen og fire måltests blev grønne, mens ny 4.0.320-exact-head afventer. Eksplicitte 4.0.317/4.0.318-afsnit nedenfor er historiske snapshots.

## Historisk checkpoint 2026-08-31 – operational controller/recovery
...
- Den regenererede 4.0.319-binding er integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`db475a1bbb1b85fe3e0277b8687d6f1edd6dd8d74e0d6fb4df748f955d5bafe1` over 44 filer/8 consumers og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`ea22921e298a03ed1ef8787a4dbd79fd4fdf1a9b8e188d3c4b44e03f16fdceb0` over 56 filer. Outcome-v2 og P2-måltests er lokalt grønne; ny exact-head/produktion/browser udestår.
...
- Schedule/watchdog/manual run under uafsluttet first cutover vælger Candidate G-maintenance. **Den tidligere push-trigger er supersederet 2026-09-03:** push gør det samme i Fase A, og kun særskilt manuel Fase B kan vælge `integrated-cutover`. Før DMI valideres den forseglede centrale Fase-A-identitet; live Pages-source genverificeres senere før begin-CAS. Enhver identitets-/resolverfejl stopper fail-closed på sit rette trin.
...
- Feggesund-inputgaten – den lokalt implementerede, fail-closed adapter følger DEC-0114: direkte `DK-B05-11`-WAM vinder; kun en helt manglende lokal bølgetuple må afløses af komplette same-run DMI-tuples fra både `DK-B05-10` og `DK-B05-12`. Udfaldet er timevis `DIRECT`, `FEGGESUND_TWO_NEIGHBOR_WAVE_INTERPOLATION` eller `MISSING`; enhver proxyinvolveret score er ikke-kalibreringsegnet. Frisk privacy-safe 3 × 118 og efterfølgende produktions-/browserbevis er fortsat åbne releasegates.
- `scripts/ravscore-operational-activation.mjs` – beskyttet v4-controller med fire statusser, seks overgangstyper og 30 felter. Første nye integrerede plan kan kun komme fra særskilt manuel Fase B efter en current same-head central Candidate G-attestation; push/schedule/watchdog/ordinary manual vedligeholder kun Candidate G. I deployleddet observeres live source Pages før `PENDING`, target deployes/verificeres, og `ACTIVE` + central target-profil atomiseres. Crash/retry reconcilerer source/requested/third manifest fail-closed; gamle forseglede planer består, men nye direkte rowless-planer er forbudt. Rollback/return er fortsat manual-only. Assistentens Edge skiftes ikke til en Candidate-version.
