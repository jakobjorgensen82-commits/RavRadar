...
Gatehullet er lukket lokalt ved at lade både fuld validering og releasegate følge samme positive preflight-kontrakt som produktionsartifactet. En almindelig `workflow_dispatch` kan derfor ikke længere bygge frisk data og nå artifactet med triggerbetinget skipped gates. Negativ preflight stopper fortsat uden artifact/deploy. Ekstern verifikation afventer den første friske run med begge gates og deploy som `success`.
