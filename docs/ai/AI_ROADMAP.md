...
1. [x] Merge 4.0.375 via PR #317 efter sourcegate `34939186051`.
2. [x] Livebevis i `34939798892`, at komplet metadataoverførsel og atomisk privat installation består uden provider.
3. [x] Afgræns stoppet til wrapperens forkerte tekniske rodsti og bekræft, at den godkendte pilkoordinat og generatorens privacykontrol er korrekte.
4. [x] Ret alle tre rødder og bestå code-only-, 210/673-runtime- og Pages-privacykontrol.
5. [ ] Forsegl docs/version/geodatadiff, commit/push og bestå én exact-head sourcegate.
6. [ ] Merge samme head og kør providerfri code-only til privat runtime, Pages og central completion.
7. [ ] Verificér levende 4.0.376, 210/673/118, begge søgemåder, Om RavRadar og nul private lækager.
8. [ ] Genaktivér normal weather kontrolleret og bevis numeriske input/scorer, DMI-rotation og cachevedligeholdelse uden oneoff.
9. [ ] Gennemgå hele siden og revurder resten af roadmap mod faktisk produktion.
...
4. [ ] Commit/push én samlet 4.0.320-head, bestå exact-head `validate:source` én gang og gennemfør Fase A-merge med current moderne Candidate G fortsat som eneste offentlige model.
5. [ ] Lad cron på den mergede Fase A-kode akkumulere korrigeret cache; bevis derefter current for 673 dele × 118 timer, separat Feggesund wave for tre dele × 118 timer og DEC-0114''s live Supabase før/efter-kapacitet med mindst 30 procent reserve.
6. [ ] Kør først derefter særskilt manuel Fase B med bool + eksakt token, fuld produktion/aktivering og offentlig desktop-/mobilverifikation. Push, schedule, watchdog og almindelig manual kan ikke auto-cutover.

## Tidligere releasehale 2026-08-31

1. [x] PR #236 gendannede exact Candidate G/4.0.316 på `c58deb78`; exact-head `33342157517`, post-merge `33342219152` og `rr-20260830234144-210` 210/673 er grønne.
2. [ ] Afslut offentlig visuel mobil-/desktopkontrol af rollbackbasen.
3. [x] Luk lokal historical Candidate/integrated H0→H1, direct Candidate→integrated og exact-target recovery med målrettede controller/workflow/recoverytests.
...
1. [x] Bestå PR #235 exact-head `33332106627`, merge som `a584d1cf`, og klassificér produktion `33333490853` som safe fail før DMI/protected writes/artifact/Pages ved 0 READY/673 canonical warmup.
2. [x] Opret DEC-0113 og fem normative krav for source-attested cold start, source/active registry-separation, measured Candidate G-rollback, Node/Python UTC-paritet og pre-mutation-stop.
3. [x] Hydrér og validér exact public source manifest/conditions/register isoleret; materialisér aktivt centralt register separat uden geometri-/punktændring.
4. [x] Implementér nationalt aggregate modevalg: all-READY/same-context/common-target migration; canonical warmup eller legitim contextændring genuine cold start; invalid/tampered source fail-closed.
5. [x] Bind `source_validated=true` gennem workflow/statevælger og replay kun faktiske 0–48 verificerede timer som `HISTORY_INCOMPLETE`.
...
8. [x] **Historisk 4.0.319-resultat, supersederet 2026-09-03 for initiatoren:** lad almindelig drift vedligeholde Candidate G og klassificér `33334709027` og `33335078275` som sikre røde stop uden deploy. Den gældende 4.0.320-rækkefølge er Fase A med Candidate G på `main`, dernæst 673 × 118/Supabase-bevis og særskilt manuel Fase B; et push kan ikke længere initiere cutover.
...
10. [x] Regenerér endelig 4.0.321-modelkontrakt/bundle: integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`d5796289f645f1bcab6b4fe822c5ed6b0e919321013687302feb2139e814a286` over 55 filer/8 forbrugere med særskilt 78-modulers public-browserlukning og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d` over 56 filer. Versionslukning og geodatabevis ligger fortsat i slutgaten.
11. [ ] Bestå ny exact-head og Fase A-merge med Candidate G fortsat offentlig. Første historiske exact-head `33577887262` stoppede sikkert i Candidate G-rollbackens public stage; fem-validator-rettelsen og fire måltests blev grønne.
12. [ ] Bevis 673 × 118, Feggesund 3 × 118 og Supabase-kapacitet; gennemfør derefter særskilt manuel Fase B samt offentlig 210/673/current/fem døgn/begge modes/quality-warnings/assistent/admin/ekspert på desktop og mobil.
...
6. [x] Observer frisk offentlig `rr-20260830091913-210` med 210/673; Candidate G gav 0 aktive/210 `UNAVAILABLE` på grund af utilstrækkelig sammenhængende historik. Det er regressionsevidens, ikke state-6-bevis.
7. [x] Fortsæt DEC-0102/DEC-0112-producent-/forbrugermatrix og arkitekturreduktion i den samlede modelleverance.
...
2. [x] Lås `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0` og alle variant-/profil-/komponent-/forklarings-/migration-/rollbackfelter fra modelkontrakten; schema 5 er kun den aldrig-offentlige eksakte 5→6-migrationskilde.
...
5. [x] Implementér v5's signed-evidence Candidate G→schema6-reweight, aggregate 673/common-target-gate og bounded 40-timers private WAM-bootstrap med coherent run pr. collection, same-cell provenance, højst fire timers same-run-interpolation samt `1/1024`/`0.01171875`-grænser; bevar Candidate G fail-closed ved manglende state/target/run og dan ingen syntetisk/offentlig historik. State-løs recovery bruger `bounded-private-48h-history-cold-replay-v3` over de faktisk tilgængelige 0–48 positioner og er også ved 48/48 `HISTORY_INCOMPLETE`, indtil 288-timers closure; rollback får ingen dobbelt credit, og same-model atomisk nøddrift er højst 72 timer uden cross-model fallback/interpolation.
...
- [x] **Historisk 4.0.319-checkpoint:** regenerér og fastlås de daværende slutbindinger: aktiv `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`db475a1bbb1b85fe3e0277b8687d6f1edd6dd8d74e0d6fb4df748f955d5bafe1` over 44 filer/8 consumers og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`ea22921e298a03ed1ef8787a4dbd79fd4fdf1a9b8e188d3c4b44e03f16fdceb0` over 56 filer. De gældende 4.0.320-hashes står i øverste releasehale.
...
- [x] Luk det sikre stop `33029447510` med en eksakt enkeltfil-undtagelse og obligatoriske interne/score-neutrale/ikke-offentlige markører; bestå PR #172 exact-head `33030112665`, merge `7a234653`, produktion `33030166104`/Pages `98382359708` og offentlig kontrol.
