...
- **Aktuel 4.0.264-leverance:** Kontoen får **Mine ture og fund** som en begrænset RLS-læsning af de eksisterende `observations`-rækker. Der må ikke oprettes en ny tabel, serverrække eller kopi. Den direkte v2-tur erstatter den gamle GPS-baserede parallelrejse i UI, og login/magic-link-tekster samt centrale RavScore-ord er forenklet. Se DEC-0063.
- PR #104 bestod exact-head `32651048627` og blev merged som `579bd167`; produktion `32651106811` stoppede før deploy på en gammel feedback-UI-test. PR #105 bestod exact-head `32651724416` og blev merged som `7c43146f`; produktion `32651786366` stoppede også før deploy, denne gang på en gammel ordret forventning til stjerneforklaringen. Den aktive branch `codex/fix-plain-copy-validation` retter den sidste tekstkontrakt og den efterfølgende fundne gamle mobil-turkontrakt og flytter begge ind i `validate:source`. Den samlede lokale kilde-/RDKS-/releasegate er grøn; ny exact-head og produktion mangler.
