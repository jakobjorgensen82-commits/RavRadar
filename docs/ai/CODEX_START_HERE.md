...
## Aktuelt arbejdscheckpoint 2026-08-29 – integreret RavScore-releasekandidat

- Den senest produktionsverificerede offentlige baseline er `origin/main` `a93082548c4cc1ddbe9c75ce303d334530a534c4`, app 4.0.308, med Candidate G som eneste offentlige model. Exact-head `33221847955`, fuld produktion/deploy `33221916441` og offentlig mobil/desktop er grønne. Dette bevis må ikke tilskrives den lokale modelkandidat.
- Den isolerede modelbranch har lokalt implementeret én samlet kandidat: `RRS-COASTAL-PROCESS-INTEGRATED-1.0.0`, state `4.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-STRUCTURAL-LAST-MILE-HUNTABILITY-1` og profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileneutral-v3`. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder 34+ kanonisk normaliserede transitive implementeringsfiler; slutværdier afventer regeneration på afsluttet head.
- Kontrakten er 20/50/30; gridstrøm 0,03/0,15 m/s, +10/-8, fuld vægt 24 timer og cosinusfade til nul ved 48; bølgemobilisering `Hs² × T` med 4/48-timers forløb; vandstand 0 direkte point. Et stærkt udgående forløb kan gøre transportleddet 0, men den historiske 13-timers helscore-gate er fjernet.
- `delivery = transportPotential × 1` betyder ingen estimeret sidste-mile-scoreeffekt, ikke 100 % fysisk levering. Gridstrøm er ikke bølgeorbital, lokal bundstrøm, undertow, feeder-/langskyststrøm eller ripstrøm. Faldende vand kan både ledsage søværts bevægelse af mobilt rav og blotlægge/koncentrere rav bag revler. Uden lokal batymetri og surfzonemodel forbliver sidste mile og vandstand score-neutrale.
- Første schema-2→schema-4-cutover genbruger kompatibel afledt Candidate G-vejrhistorik, så kandidaten ikke skal hente flere dages ny cache eller få kunstig opvarmning. Recovery vælger exact point-aktivering → gyldig integreret continuation → gyldigt checkpoint → dybt valideret Candidate G-state; ugyldig exact point stopper straks, mens en ugyldig ordinær kandidat ikke må skygge for en gyldig lavere prioritet. Hvis ingen statekilde findes, genafspilles før første offentlige target præcis 48 timer allerede hentet, privat og proveniensverificeret strøm/bølge. Pre-target-rækker publiceres ikke, og en ufuldstændig bro stopper med `RAVSCORE_RECOVERY_REPLAY_BRIDGE_MISSING` frem for offentlig warmup. Derefter fortsætter normal drift kun fra en privat otte-fils runtimebundle, inklusive den forseglede Copernicus-range-cache, eller et beskyttet schema-4-checkpoint. Den fulde komprimerede bundle ligger i den ikke-offentlige Supabase Storage-bucket `ravradar-private-production-runtime` med current + previous og afvist anonym adgang; Pages må kun indeholde de fire hashbundne liveprojektioner. Det operationelle checkpoint kopieres ikke fremover til `admin_document_versions`, men eksisterende versionsrækker bevares, og migrationen udfører ingen destruktiv oprydning.
- Red-team-cutovergaten kræver migration `20260829010000_ravscore_operational_documents_no_history.sql` før `20260829020000_integrated_trip_calibration_binding.sql`, samlet protected readiness, eksakt Edge-`409` ved manglende/forkert binding og eksklusiv point → bundle → checkpoint → engangsimport. Første push-only cutover skriver `INTEGRATED_PENDING` med source/requested-manifesthash og bevarer central Candidate-profil under Pages-target; først efter exact implementation+210/673 sætter én RPC samtidigt `INTEGRATED_ACTIVE` og central integrated 11-feltsprofil. Retry completer ved targethash, aborterer/rekonsoliderer ved sourcehash og forbliver fail-closed ved tredje hash. Backendworkflowet genverificerer `origin/main == GITHUB_SHA` efter dry-run og umiddelbart før første eksterne skrivning; post-write fortsætter fra samme snapshot.
- Candidate G forbliver eneste offentlige model indtil atomisk cutover. Mens integreret er aktiv, ligger den varme Candidate G-projektion kun privat som `ravScoreCandidateGRollback`. Manuel rollback bruger `CANDIDATE_G_PENDING`; manuel tilbagevenden bruger `INTEGRATED_PENDING`; begge bevarer source-profil indtil atomisk `ACTIVE`+target-profil efter offentlig targetverifikation. Scheduler kan kun `CANDIDATE_G_REFRESH` på allerede aktiv uændret Candidate-binding. Der deployes ingen særskilt Candidate G-assistent-Edge; den integrerede Edge svarer `409`, klienten bruger deterministiske lokale DA/DE/EN-svar, og schema-3-ture gemmes Candidate G-bundet med `calibration_eligible=false`. Det er en tilsigtet local-only/fail-closed rollback, ikke en samtidig shadow-/nødmodel.
- Mekanik, migration-gap og målrettede regressionskæder er lokalt grønne. Release er **ikke** produktionsbevist og forbliver blokeret på samlet red-team, source/exact-head, frisk produktion, releasegate og offentlig 210/673 mobil-/desktopverifikation. Modellen må ikke kaldes empirisk mere fundpræcis uden repræsentative fund og nul-fund.
- Dual-hash-red-team-fundet er lukket lokalt: både `modelContractSha256` og `modelBundleSha256` følger den fulde 11-feltsbinding gennem generator, state, payloads, checkpoints, privat runtime, ture/observationer, central profil, Edge og gates, og de afvises særskilt ved mismatch. Slutdigests fastsættes først efter regeneration på afsluttet head; samlet red-team-, exact-head- og produktionsbevis udestår.
- Modelsporet må ikke læse eller ændre private payloads, koordinater, rå U/V, geometri eller land-/vandpunkter. Se DEC-0102, DEC-0108 og de fire integrerede research-/matrixdokumenter fra 2026-08-29.
