# NYESTE CHECKPOINT – 2026-09-05 – 4.0.324 aktiv/kandidat-cache og helvinduesvedligehold
...
- Den målrettede lokale matrix er grøn. GitHub-run `33986893042` nåede alle underliggende kode- og kontrakttests, men releasegaten fejlede alene, fordi `CHANGELOG-4.0.324.md` manglede. Changelog/status-only-opfølgningscommitten er lokalt målvalideret. Ejeren har 2026-09-05 eksplicit beordret admin-merge uden at afvente en ny exact-head. 4.0.324 får derfor ikke grønt exact-head-bevis og må ikke kaldes exact-head-grøn. Næste rækkefølge: ejerbeordret admin-merge → frisk status → active-bootstrap/runtime og fuld produktionsgate → én stor main-engangskørsel → normal catch-up og komplet 210/673/118 før modelaktivering.
...
- **Historisk supersession 2026-09-05:** To-collection-sætningen og den tidligere cadence-/én-lag-regel i dette ældre checkpoint er afløst af DEC-0116: fælles serialiseret kandidat, partial-only retention ved mindst normalt 96 timers moden/komplet fremtidshorisont og ikke-stale katalog samt tre normale DKSS-collections. Se øverste 4.0.324-checkpoint.
