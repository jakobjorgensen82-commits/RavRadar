...
- Den målrettede lokale matrix er grøn: producer, provenance, rollover, active/candidate, workflow, downstream, atomic/history, `py_compile`, YAML-/JSON-parse, RDKS/security samt releaseversion/geodata. Fuld lokal sourcegate er ikke kørt og skal ikke køres. Næste rækkefølge: PR → én exact-head `validate:source` i GitHub → sikker merge → frisk status → active-bootstrap/runtime → én stor main-engangskørsel → normal catch-up og komplet 210/673/118 før modelaktivering.
