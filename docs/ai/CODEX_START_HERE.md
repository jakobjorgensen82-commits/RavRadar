...
- **Aktuel lokal 4.0.265-kandidat:** Kontoen har **Indberet tur eller fund** uden forudgående turstart. Brugeren vælger selv dato og klokkeslæt for turens start samt varighed. Nutidens vejr bruges aldrig som historisk erstatning; uden sikkert historisk snapshot gemmes rapporten med tomme forecastfelter og `calibration_eligible=false` i den eksisterende `observations`-tabel. **Afslut uden at indberette** rydder kun den lokale aktive tur. Se DEC-0064.
- Målrettede tests er grønne. Version, PR exact-head og produktion udestår. En autentificeret testindsendelse kræver ejerens bevidste handling, fordi den opretter en virkelig Supabase-række. Candidate G, `20/50/30`, vejrdata, artifact, private data, geometri og land-/vandpunkter må ikke ændres.
- **Aktuel produktionsverificeret 4.0.264:** Kontoen har **Mine ture og fund** som en begrænset RLS-læsning af de eksisterende `observations`-rækker. Der oprettes ingen ny tabel, serverrække eller kopi. Den direkte v2-tur erstatter den gamle GPS-baserede parallelrejse i UI, og login/magic-link-tekster samt centrale RavScore-ord er forenklet. Se DEC-0063.
...
- I den produktionsverificerede 4.0.264-baseline må de to geodatafiler kun afvige ved versionsfeltet. En eventuel 4.0.265-version må tilsvarende kun løfte dette felt; geometri og land-/vandpunkter er urørlige.
