...
- Plug-and-play-fund lukket lokalt: `buildIntegratedPartPublicProjection` fører nu de eksakte schema-5-felter `lastMileWaveReferenceAt`, `lastMileMemoryReady` og `lastMileMemoryStatus` til den sikre offentlige kystprojektion. Den direkte field-set-test og 210/673-runtimeaudit er grønne; det er lokalt kontraktbevis, ikke exact-head- eller produktionsbevis.
- Sidste mile bruger en kausal energivægtet `W/N/T`-EWMA med fire timers halveringstid og en ældre hale. DMI-WAM `FROM` roteres præcis én gang +180° til `TOWARD` mod uændret kystnormal; `factor=clamp(1-0.15×W×(1-approach),0.85,1)`; `delivery=supply×factor` én gang. Bølger kan aldrig skabe/øge supply; maksimal rå totalscoredæmpning er 7,5 point før slutafrunding; vist RavScore kan derfor ændres 8 point. Aktiv direction-missing fejler lukket. Kun `waveHeightM=0` er eksakt calm og neutral; `wavePeriodS` skal stadig være finit og ikke-negativ. `waveHeightM>0` med `wavePeriodS=0` er `INVALID` og fejler lukket.
- Fysisk levering er fortsat uopløst, intervallet er `null`, og ingen empirisk fundpræcisionspåstand er tilladt. DDM 50 m er kun statisk kontekst uden dynamiske revler/surfzone, og Rainville 2026 er kun buoyant-object-analogi. Ingen kystnormal, geometri eller land-/vandpunkter flyttes. Faldende vand beskrives tosidet og score-neutralt.
