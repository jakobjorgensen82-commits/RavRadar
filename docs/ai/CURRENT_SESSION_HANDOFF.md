...
## EFTER GENSTART – lokal dokumentationslukning for 4.0.311-kandidaten

- Den spontane computer-genstart mistede ikke recoveryarbejdet. Den oprindelige branch/clone og alle ændrede/nyoprettede filer findes fortsat; aktiv redigering fortsætter i den genoprettede arbejdsbytekopi. Intet er pushet eller deployet.
- DEC-0109 og RDKS-/håndbogs-/AI-lukningen beskriver nu den lokale kildekandidat, men de må ikke læses som bevis for, at kode-, workflow-, trip-/database- eller fuld sourcegate allerede er færdige.
- Offentlig sandhed er fortsat 4.0.310 og den eksisterende målte nødvisning. Morgenhullet er ikke ændret i produktion ved dette checkpoint.
- Den lokale storage/privacy-kandidat sætter efter capacity/CAS existing-D1 eller fresh Edge-predeploy-intent lige før første Edge-deploy. Existing D1 bruger 20-/30-minutters lease, femsekunders prober, dobbeltattestation, 20-sekunders drain, 600 sekunders restlease og samlet syvminutters Worker-gate; partial Edge går D1 roll-forward. Fresh partial Edge går exact-main → Supabase-secret → eksakt Edge-redeploy → dobbelt Supabase-attestation. Uden intent ved capacity/pre-CAS-fejl sker nul recoverymutation. Dette er lokalt kandidatstatus, ikke livebevis.
- Næste sikre rækkefølge er: færdiggør agenternes kode-/testreview, saml plain-workspace-diff tilbage i recovery-clonen, kør alle målrettede gates, versions-/håndbogs-/geodatakontrol og fuld sourcegate, opret først derefter PR/exact-head, kør eksakt-head backendkæde og udfør read-only inspect før en eventuel CAS-bundet apply.
- DEC-0102-modellen skal efter recovery bevare den målte atomiske 210/673-nødvisning i højst 72 timer og aldrig efter kortere forecastudløb. Interpolation er ingen normal fallback. Schema-v2/`calibration_eligible` er endnu ikke serverbevist mod signeret public manifest, så global koefficientlæring forbliver P2-låst.
- Brug Sol/Ultra gennem slutvalidering. Ved quotaafbrydelse skal dette handoff opdateres med konkrete testresultater og fil-/branchstatus; kvalitet eller gates må ikke reduceres.

## GENSTARTSSIKKERT P0-ARBEJDSCHECKPOINT – 2026-08-29 – ejerautoriseret rekonstruktion af morgenhul
