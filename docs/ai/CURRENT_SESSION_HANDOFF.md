...
## LOKALT MODEL-CHECKPOINT – 2026-08-29 – integreret RavScore-releasekandidat

- Autoritativ offentlig baseline er `origin/main` `a93082548c4cc1ddbe9c75ce303d334530a534c4`/PR #220 og 4.0.308. Exact-head `33221847955`, fuld produktion/deploy `33221916441` samt offentlig mobil-/desktopkontrol er grønne. Candidate G forbliver eneste offentlige model indtil samlet cutover.
- Den lokale kandidat er `RRS-COASTAL-PROCESS-INTEGRATED-1.0.0`, state `4.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-STRUCTURAL-LAST-MILE-HUNTABILITY-1` og profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileneutral-v3`. Endelige `modelContractSha256`- og `modelBundleSha256`-værdier afventer regeneration på afsluttet head.
- 20/50/30, 0,03/0,15 m/s og +10/-8 er analytisk genbrugt. Gridstrøm vægtes fuldt i 24 timer og cosinusdæmpes til nul ved 48; bølgemobilisering bruger `Hs² × T` med cirka 4/48 timer. Den historiske 13-timers helscoregate er fjernet.
- Modelgridstrøm er ikke lokal bundnær strøm, undertow, feeder-/langskyststrøm eller ripstrøm. Faldende vand kan både ledsage søværts bevægelse af noget mobilt rav og blotlægge/koncentrere rav bag revler. Uden lokal batymetri/surfzonemodel er sidste mile score-neutral; vandstand giver 0 direkte scorepoint.
- Første schema-2→schema-4-cutover genbruger allerede hentet, valideret afledt vejr/state uden flerdages warmup eller kunstig historik. Recovery vælger exact point-aktivering → gyldig integreret continuation → gyldigt checkpoint → dybt valideret Candidate G-state; ugyldig exact point stopper straks, mens en ugyldig ordinær kandidat ikke må skygge for en gyldig lavere prioritet. Uden statekilde genafspilles før første offentlige target præcis 48 timer allerede hentet, privat og proveniensverificeret strøm/bølge; pre-target-rækker publiceres ikke, og manglende bro stopper med `RAVSCORE_RECOVERY_REPLAY_BRIDGE_MISSING`. Normal continuation bruger den private syvfilers runtime i beskyttet storage eller et beskyttet kompakt checkpoint. Fremtidige checkpointopdateringer kopieres ikke til `admin_document_versions`, men eksisterende historik bevares, og migrationen sletter ingen versionsrækker.
- Red-team-gaten kræver `20260829010000_ravscore_operational_documents_no_history.sql` før `20260829020000_integrated_trip_calibration_binding.sql`, samlet protected readiness, eksakt Edge-`409` og eksklusiv point → bundle → checkpoint → engangsimport. Første push-only cutover skriver `INTEGRATED_PENDING` med source/requested-manifesthash og bevarer central Candidate-profil, mens integrated Pages deployes. Exact offentlig implementation+210/673 efterfølges af én RPC, der samtidigt sætter `INTEGRATED_ACTIVE` og central integrated 11-feltsprofil. Retry completer ved targethash, aborterer/rekonsoliderer ved sourcehash og stopper fail-closed ved en tredje hash. `deploy-trip-storage` genverificerer `origin/main == GITHUB_SHA` efter dry-run umiddelbart før første eksterne write og bruger samme validerede snapshot gennem post-write-rækken.
- Senere manuel Candidate G-rollback bruger `CANDIDATE_G_PENDING`; manuel integreret return bruger `INTEGRATED_PENDING`; begge følger samme source/PENDING/target/reconcile og atomisk `ACTIVE`+central target-profil. Scheduler kan kun `CANDIDATE_G_REFRESH` på allerede aktiv uændret Candidate-binding. Der deployes ingen særskilt Candidate G-assistent-Edge; den integrerede Edge svarer eksakt `409`, klienten bruger deterministiske lokale DA/DE/EN-svar, og schema-3-ture/-observationer gemmes Candidate G-bundet med `calibration_eligible=false`. Det er fail-closed/local-only og ikke en skjult dualmodel. De separate Candidate G-kontrakt-/bundlehashes afventer slutregeneration. Dokumentation og lokale tests er ikke produktionsbevis.
- Der må ikke påstås empirisk højere fundpræcision uden et repræsentativt fund-/nulgrundlag. Ingen geometri, land-/vandpunkter, private payloads, koordinater eller rå U/V må røres eller publiceres.
- Dual-hash-red-team-fundet er lokalt lukket: både `modelContractSha256` og `modelBundleSha256` følger den fulde 11-feltsbinding og afvises særskilt ved mismatch. Slutdigests fastsættes først efter regeneration på den afsluttede head; lokal lukning er ikke release- eller produktionsbevis.
