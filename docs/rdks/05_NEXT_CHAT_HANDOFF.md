...
Arbejd i `RavRadar-4.0.396`, branch
`codex/4.0.406-bounded-public-view`, head afventer nyt commit efter
`9a2875b1`. Live 4.0.405 er 210/673 med 141 brugbare og 69 helt
utilgængelige zoner. 296 dele mangler vind trods strøm/bølger, fordi
normalrun `35205052150` aldrig nåede HARMONIE.

Mobilrettelsen og den afgrænsede HARMONIE-først-plan er måltestet. Første
exact-head `35241236001` bestod 22/24 og fandt kun fire UI-tekster i
RavScore-closure. Teksterne ligger nu i appen; modelbundle `d9ba75ed...` og
otte bindinger er uændrede. Commit/push og ny exact-head, derefter merge,
providerfri deploy og én almindelig weather. Ingen oneoff. Scheduler pauset.
DEC-0188/0189. Sol/Ekstra høj.

# HISTORISK CHECKPOINT – 2026-09-17 – lokal 4.0.405 fælles freshness-parser
...
restore, komplet migration, atomisk installation, offentlig genopbygning og
210/673-audit. Den stoppede før privat publicering og Pages, fordi
specifikationen brugte en midlertidig DMI-fil fremfor den komplette
`data/live/dmi-bulk-cache.json`.

4.0.377 bruger den kanoniske cache og tillader kun en efterfølger på samme
vejrtid med eksakt forgænger-/migrationsrapport, ni-fils inventar,
byteidentiske øvrige filer og uændrede målinger/states. Næste: slutdiff,
commit/push, én exact-head gate, merge og providerfri code-only; derefter
livekontrol og normal weather. Ingen oneoff. DEC-0158. Sol/Ekstra høj.
...
- **Første PR #282-head er negativ:** `cc06fa37`/run `34695465328` bestod alle lange model-/privacy-/runtimeled, men releasegatens statiske testplan manglede den nye migrationsbygger. Intet proof blev uploadet. Lokal opfølgning tilføjer den præcis én gang og låser package/plan med regression; push ny head og kræv ny gate.
...
- **Livebevis:** Main-run `34635781802` sluttede med 1.335 currentrester. Oneoff `34642214559` nåede 78.381/79.414 og efterlod 1.033, som alle var terminalt provider-negative hos Open-Meteo i netop kørslens fallbackforsøg; de var ikke uattempted. WAM/Feggesund var grøn, men intet handoff/cutover blev skabt.
- **Helhedskonklusion:** DMI planlagde hele 673 × 118-registeret og roterede seks collections; Baltic/AMM15 roterede også. Open-Meteo-negativ er run-/kildespecifikt upstream-fravær, ikke permanent bevis mod DMI/CP. Oneofftid alene løser ikke årsagen: 31 CP-checkpoints brugte cirka 2.376 sekunder eller 78 procent af den cirka 54 minutter lange fase.
- **Rettelse:** Hvert færdigt CP-segment gemmes atomisk i en exact-base-/reference-/register-/contentbundet journal. Journalen giver aldrig selvstændig admission, men afspilles gennem eksisterende donor-, mask-, provenance-, shadow- og source-stage-kontrol. Seks receipts konsolideres ad gangen; restbatch ved afslutning/soft budget. Journalen slettes kun efter vellykket transaktion og følger normal, oneoff, pilot og postbuild-cachetransport.
- **Kildegate:** PR exact-head kører én fuld sourcegate og uploader et contentbundet bevis. Byteidentisk merged main kan genbruge det efter live kontrol af PR, repository, head, run/attempt, job, steps og artifact. Locatorcachen er ingen autoritet; mismatch/udløb/utilgængelighed giver fuld main-gate. Post-data-validate/releasegate, adminhydrering, frisk vejr, handoff, artifact og deploy springes aldrig over.
- **Lokal evidens:** Journal-, replay-, six-batch-, failure-, CP-admission-, acquisition-, source-tree-, sourceproof- og workflowmåltests er grønne. Den syntetiske 40.120-record-prøve gav byteidentisk bank/shadow/stage og cirka 116 → 46 sekunder. Det er ikke provider-/CI-/produktionsbevis.
- **PR/CI-status:** PR #279's første head `72db0a70` fejlede run `34659873681` alene i en forældet statisk cachetest. Testen krævede den gamle énlinje-rebase og kendte ikke den durable journals acquisitions/records. Den er lokalt rettet til at bevise autoritativ bankmerge og efterfølgende fuld donorvalidering; statisk test samt funktionelle journal-/restart-/source-stage-tests er grønne. Proofupload blev korrekt sprunget over, så den røde head har ingen genbrugelig autoritet.
- **Renhedsstatus:** Anden head `581dfae9` bestod hele sourcegaten i run `34661632590`, men tracked-clean-trinnet blokerede proof. Tredje head `e1e9dd1e`/run `34664907674` viste de eksakte ændringer: releasegaten omskrev de tracked `release/RELEASE-REPORT.json` og `.md` med version/tid efter alle grønne tests. Sourcevalideringen bruger nu samme fulde releasegate med snæver `--no-write-report`; post-data-produktionsgaten og releasepakning skriver fortsat rapporterne.
- **Næste rækkefølge:** Commit/push rapportrettelsen → exact-head sourcegate med rent træ og verificeret proofartifact → merge → kontrolleret main-writer med bevarede cacher → kræv current 79.414/79.414, native WAM 79.060 og Feggesund 354/354 → fulde post-data-gates/handoff/cutover → offentlig desktop/mobil/modelkontrol → kontrolleret normalvedligeholdelse. Ved rest: gennemgå hele plan/provider/cache/journal/admission/mask/provenance/closurekæden før nyt run; ingen blind oneoff eller lempet videnskab.

# HISTORISK CHECKPOINT – 2026-09-11 – færdiggør 4.0.343 fair providerclosure
...
- **Runtime:** `34093354004` sluttede sikkert med Copernicus success og Open-Meteo 2.735 required / 1.873 retained / 750 fetched / 2.623 filled; præcis 112 critical missing stod tilbage. `34104536681` på eksakt `main` `c2ce63ff` genbrugte DMI-cachen til 67.897/79.414 på 5m24s, fik 8.372 Copernicus-par og efterlod 3.145, men stoppede før første Open-Meteo-request på `OPEN_METEO_RESIDUAL_PLAN_INVALID_DMI_LEDGER_SOURCE_INDEX_INVALID`. Copernicus-fremgangen blev gemt; ingen closure/artifact/deploy. 4.0.332 retter den ærlige null-run-ledger-path uden at tillade positive/source-bærende null-run-rækker. Normal workflow er fortsat deaktiveret under cutover, og Candidate G er offentlig.
...
- Bevar ekstern cron som primær dispatcher og eksisterende GitHub schedules som reserve; ændr ikke cadence. Run `33986893042` fejlede alene på manglende releasechangelog. Ejerens efterfølgende ordre om admin-bypass blev afvist af Codex-sikkerhedslaget, så ingen bypass eller merge skete. Run `33988058582` bestod releasegaten, men stoppede senere alene på håndbogs-/installationspariteten; den officielle synk retter én genereret SQL-payloadlinje. Efter ny grøn exact-head og merge: mål safe fetched/missing, verificér bootstrap/runtime og fuld produktionsgate, start én stor main-oneoff og overvåg candidate-save, promotion, fallbackclosure og normal catch-up. Komplet 210/673/118 er fortsat åbent.
...
- Fase A: merge/push den samlede kode efter exact-head-kildegaten, men hold Candidate G offentlig. Legacy-Candidate går gennem `candidate-legacy-maintenance`/`legacy-refresh-*` til current moderne Candidate G på samme head. Fase A må ligge før 673 × 118, så cron bygger den korrigerede cache på den mergede kode. Ingen push, schedule, watchdog eller almindelig manuel drift må starte state-6-cutover.
- Mål live Supabase før/efter under den kontrollerede Candidate G-produktion og luk DEC-0114 med faktisk øvrig egress/lager og mindst 30 procent reserve. Tokenen er aldrig bevis for dette.
- Fase B: først efter 673 × 118/Feggesund, grøn kapacitet og alle øvrige data-/releasegates, kør separat manuelt med `ravscore_integrated_first_cutover=true` og `EXECUTE-INTEGRATED-RAVSCORE-FIRST-CUTOVER-AFTER-CAPACITY-GATE`. Før DMI kræves forseglede central Phase-A-complete Candidate G med eksakt binding/manifesthash, dataset/reference, deployment-id, sourceHead, closure og profil; live public source/manifest/implementation genverificeres i deployleddet før begin-CAS.
- Preflight `33695730459` attempt 2 nåede grøn DMI-terminal med 71.526/79.414 DMI-direct og gemte 364 Copernicus-par, men timeout gav intet 673 × 118-artifact. Attempt 3 var endnu ikke terminal. Main #3913 gemte caches, men stoppede i gammel main-selector; branchens fix er lokal. Dette er progression, ikke releasebevis.
- Gamle forseglede planer og recovery består. Ingen modelbundle/hash/geodata er ændret. Candidate G er fortsat offentlig, og state 6 må ikke omtales som merged, aktiveret eller live.

# TIDLIGERE CHECKPOINT – 2026-09-03 – installerbar webapp på eksisterende Om-side
...
## Historisk checkpoint – 2026-08-31
...
- **Historisk 4.0.319-slutbinding:** integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`db475a1bbb1b85fe3e0277b8687d6f1edd6dd8d74e0d6fb4df748f955d5bafe1` over 44 filer/8 consumers og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`ea22921e298a03ed1ef8787a4dbd79fd4fdf1a9b8e188d3c4b44e03f16fdceb0` over 56 filer. Gældende 4.0.320-hashes står i øverste checkpoint.
...
- Bot-/watchdogrun `33334709027` stoppede før produktionsarbejde/Pages; pilot `33335078275` stoppede ved privat DMI-gapmatrix før pilot/Copernicus/artifact. Det er forklaringen på fejlmails, ikke skjulte deploys. **Historisk initiatorregel, supersederet 2026-09-03:** alle almindelige jobs inklusive push vedligeholder Candidate G; kun særskilt manuel Fase B må initiere first cutover.
