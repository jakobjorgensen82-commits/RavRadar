...
5. 0–3 km er foretrukket afstand. En gyldig celle mellem 3 og 5 km kan accepteres, når nærmere data ikke findes. Over 5 km er som hovedregel fail-closed. DEC-0041 indfører kun en særskilt, maskinlæsbar og højst 15 km regional DMI-proxy for otte navngivne Limfjordsdele efter fulde aktiveringsgates; den er ikke en global lempelse.
...
7. Kun strøm fra den verificerede DMI-GRIB-kæde må bruges aktivt, indtil DEC-0041-piloten og dens særskilte aktiveringsgate er bestået. Den direkte ForecastEDR-positionstjeneste må fortsat reparere vandstand og overfladetemperatur, men dens strøm må ikke bruges, fordi svaret ikke dokumenterer den krævede fælles vandkolonne og lagidentitet. Open-Meteos overfladestrøm og enhver anden gammel fallbackstrøm holdes tilsvarende ude. Manglende verificeret aktiv strøm forbliver `missing` før både score og kort.
...
10. Den viste pil bruger den viste/valgte times egen verificerede gitterkoordinat og lag. For en lokal kystdel vælges den scorepost, som runtime faktisk viser, før pilens flowpunkt beregnes ved præcis samme tid. Hverken byggetidens fallbackanker eller en anden forecasts koordinat må bruges som repræsentativ pilposition.
