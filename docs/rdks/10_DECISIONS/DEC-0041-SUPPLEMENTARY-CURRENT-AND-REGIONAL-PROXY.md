...
GitHubs native schedule leverede i den første måleperiode kun ét forsinket event. Derfor blev opsamlingen midlertidigt koblet til `requested`-eventet fra det eksternt startede produktionsworkflow. Ejerens nyere DEC-0042 erstatter nu selve produktionsstarteren med en forskudt GitHub-ejet 15-minuttersplan og en eksplicit readiness-gate. Keepalive må fortsat kun gendanne og kontrollere privat cache read-only; mangler aktuel UTC-time, må det minimale `actions: write`-job dispatch'e piloten på `main`. Ingen af disse driftsvalg ændrer aktiveringsgaten.
