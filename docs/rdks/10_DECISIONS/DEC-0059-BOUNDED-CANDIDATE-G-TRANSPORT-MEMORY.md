...
**Status:** Aktiv score-neutral beslutning efter 4.0.260; offentlig aktivering fortsat lukket
...
4. Den centrale state gemmer højst vinduets 49 timepunkter som en afledt styrke mellem -1 og 1: positiv mod kysten, negativ fra kysten, 0 verificeret neutral og `null` eksplicit missing. Rå U/V, fart, retning, koordinater, geometri, kystdels-id'er og private payloads må ikke indgå i evidensrækken.
5. Et aktiveringsklart vindue kræver præcis 48 timers sammenhængende timedækning ved den aktuelle referencetime. Missing eller et tidsgab må ikke omskrives til neutral strøm. Candidate G kan fortsat beregne et tydeligt foreløbigt diagnostisk output under opbygningen, men `transportMemoryReady=false` holder den globale profilomskifter fail-closed på legacy.
