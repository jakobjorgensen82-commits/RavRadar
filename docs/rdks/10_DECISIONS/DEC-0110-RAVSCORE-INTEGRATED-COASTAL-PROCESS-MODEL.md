...
- **Status:** Ejerbesluttet state-6-model med Fase A-koden på sourcehead `cbc4639af411ee741be938980b2d7a8c08b6b79d` merged via PR #246 efter grøn exact-head-kildegate `33706215425`; mergecommit er `7198b685f4bc9d86bd6432b049380f4279ab797c`. Candidate G er fortsat den eneste offentlige model. Den lokale 4.0.321-lukning af checkpointtransport, no-history, RLS/ACL og kapacitetsprojektion er endnu ikke pushet, exact-head-verificeret, anvendt mod live Supabase eller merged. 673 × 118, Feggesund 3 × 118, live kapacitetsmåling, særskilt manuel Fase B, frisk state-6-produktion og offentlig desktop-/mobilverifikation er fortsat åbne
- **Dato:** 2026-08-29
- **Ejer:** RavRadar
...
1. **Fase A – kode og moderne Candidate G:** Merge/push af den samlede kode må kun bygge og vedligeholde Candidate G som den ene offentlige model på den nye eksakte `main`-head. En rowless eller historisk legacy-Candidate føres gennem den eksisterende `candidate-legacy-maintenance`/`legacy-refresh-*`-bro til den moderne, current Candidate G-binding. Push, schedule, watchdog og en almindelig manuel vejrproduktion må aldrig starte første integrerede cutover.
2. **Fase B – særskilt første modelskift:** Første integrerede aktivering må kun vælges af et eksplicit manuelt `workflow_dispatch`, hvor både det boolske input `ravscore_integrated_first_cutover=true` og den eksakte bekræftelse `EXECUTE-INTEGRATED-RAVSCORE-FIRST-CUTOVER-AFTER-CAPACITY-GATE` er til stede. Dispatchkontrakten kalder denne ene operation `integrated-first-cutover`; den eksisterende interne buildaction er `integrated-cutover`. Forkert eller forældreløs bekræftelse, blanding med rollback/return eller enhver anden initiator stopper fail-closed.
3. Før Fase B må vælge den interne action, kræves den forseglede centrale Fase-A-identitet: current moderne `CANDIDATE_G_ACTIVE` på samme `GITHUB_SHA`, positiv `bindingCurrent`, `initialCutoverRequired=true`, `legacySourceRequired=false` samt eksakt aktiv version, deployment-id, `sourceHead`, implementation closure og central profil/binding. Dette er et centralt, Phase-A-complete præ-DMI-bevis; det er ikke en påstand om, at live Pages allerede er genhentet i actionresolveren. Den faktiske offentlige Candidate G-kilde, manifest og implementation closure skal genverificeres i deployleddet, før første begin-CAS eller anden central modelmutation.
4. Bekræftelsesteksten er kun udtrykkelig operationsautorisation. Den er ikke bevis for Supabase-kapacitet, 673 × 118, Feggesund, exact-head, produktion eller browser. Fase A må med vilje merges før 673 × 118-beviset, fordi Candidate G forbliver offentlig, og cron dermed kan opbygge den korrigerede cache på samme head. DEC-0114's 673 × 118-/Feggesund-gates, live Supabase før/efter-måling, øvrige egress/lager og mindst 30 procent reserve skal derefter være dokumenteret grønne, før Fase B overhovedet bestilles.
5. Eksisterende forseglede historiske planer, afbrudte `PENDING`-overgange og source/target-reconciliation bevares til deterministisk recovery. Tillægget forbyder kun oprettelse af en ny direkte rowless/legacy→integrated-plan via push; det gør ingen gammel gyldig recoveryplan ubrugelig.

Den foregående statusbeskrivelse var korrekt før PR #246: implementeringen fandtes da kun lokalt i 4.0.320-worktreeet. Tofaselåsen og den samlede Fase A-kode er nu exact-head-verificeret og merged som `7198b685f4bc9d86bd6432b049380f4279ab797c`, men ikke aktiveret som state 6. Candidate G er fortsat offentlig. Ændringen ligger uden for begge RavScore-bundles; geometri og land-/vandpunkter er uændrede.

## Bindende operationelt tillæg 2026-09-03 – checkpointmetadata uden normal fuld readback

Den lokale 4.0.321-lukning gør det beskyttede schema-4-checkpoint til en fast service-role-only compare-and-swap-kontrakt. Normal publish læser kun dokumentnøgle og version, sender derefter den validerede checkpointpayload til én server-side CAS og accepterer kun et afgrænset metadataresultat på højst 4 KiB. Den kanonisk serialiserede checkpointpayload, inklusive afsluttende newline i klientens størrelsesmåling, må højst være 16 MiB; den samlede HTTP-wrapper kan være lidt større. Den fulde payload læses kun i den særskilte restorevej, som workflowet bruger, når GitHub Actions-cachen mangler; almindelige fulde builds downloader derfor ikke først den eksisterende checkpointpayload.

PostgreSQL validerer uafhængigt checkpointets eksakte envelope, privacyfelter, kanoniske tider, fulde bindinger og præcis 673 integrerede stateposter plus 673 `READY` Candidate G-companionposter med samme delmængde. JavaScript forbliver eneautoritet for matematisk replay og de kanoniske `JSON.stringify`-baserede hashes; databasen genberegner eller foregiver ikke disse beviser. Fremtidige checkpointopdateringer opretter ingen ny række i `admin_document_versions`, mens allerede eksisterende historikrækker bevares. Restriktive SELECT-politikker blokerer direkte checkpointlæsning for `authenticated`, og CAS-, kontrakt- og validatorfunktionernes ACL/search path er låst, så kun `service_role` kan udføre de eksterne checkpointoperationer.

Dette er en lokal kontrakt- og kapacitetslukning, ikke et live kapacitetsbevis og ikke en state-6-aktivering. Den bindende kapacitetsmodel medregner to metadataresponses à højst 4 KiB pr. fuldt build og fuld payload kun ved restore/cache miss; scenarierne 1, 31, 60 og 1.860 restores pr. måned rapporteres særskilt, hvor 60 er planlægningsgrænsen og 1.860 alene en ikke-bindende stresskant. Observeret cache-miss-rate, live Supabase før/efter, øvrig egress, databasestørrelse, lager og mindst 30 procent reserve skal fortsat bevises før Fase B.
...
Produktions-outcome for 4.0.319 er `ravradar-production-workflow-outcome-v2`. Den payloadfri nested exact-key-kontrakt omfatter historical actions og recovery writer/finalizer/gate; kode og releasegate skal matche før exact-head.
...
| Parameterkontrakt | `modelContractSha256=a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b` |
| Transitiv implementeringsbundle | `modelBundleSha256=d5796289f645f1bcab6b4fe822c5ed6b0e919321013687302feb2139e814a286`; 55 kanonisk normaliserede transitive implementeringsfiler; 8 deklarerede forbrugere |

Den serialiserede runtimebinding består præcist af model-, state-, variant-, profil-, komponent-, forklarings-, rangerings-, best-time- og præsentations-id samt de to adskilte hashes: 11 felter i alt. `modelContractSha256` binder den kanoniske parameterkontrakt. `modelBundleSha256` binder den transitive implementering, så en ændring i en indlæst evaluator, adapter, policy eller anden kanonisk modelafhængighed ikke kan gemme sig bag uændret parameter-JSON. Migration og rollback er særskilte overgangskontrakter. Hver continuation-state, hvert checkpoint og den centrale profilselection skal bære og matche alle 11 felter; det samme gælder payload, cache, forklaring og release. Et model-id eller én hash alene er aldrig nok. Ovenstående state-6-værdier er reproduceret af bundlegeneratoren over præcis 55 filer og otte deklarerede forbrugere. Den faktiske offentlige browserlukning attesteres særskilt over de 78 transitive deploymoduler; det afslører direkte offentlige forbrugere, som en snævrere deklareret entrypointliste ellers kunne overse. PR #246's exact-head-kildegate beviser den tidligere 44-filers Fase A-konsistens, men ikke den efterfølgende 55-filers lukning og heller ikke state-6-produktion eller aktivering. DEC-0112 er autoritativ for den numeriske `HISTORY_INCOMPLETE`-kontrakt og for 5→6-overgangen.
...
- Ved `candidate-g-migration` skal alle præcis 673 Candidate G-states validere mod det uændrede kystdelsregister og give ét fælles kanonisk bootstrap-target. DMI-acquisition skal for hver anvendt WAM-collection finde ét `single-coherent-run`, og hver højde-/periode-/retningsrække skal have komplet same-cell native provenance. Kontrolleret tidsinterpolation er kun tilladt mellem native endepunkter højst fire timer fra hinanden inden for samme run, collection, griddefinition og celle; cross-run-, cross-cell- og tvetydig retningsinterpolation fejler lukket. Hvis migrationens state-targets er blandede, eller migrationens sammenhængende 40-timers run ikke kan skaffes og valideres, sker ingen migration. DEC-0114 er autoritativ for den særskilte `genuine-cold-start`: ufuldstændig ældre WAM-historik bliver da `HISTORY_INCOMPLETE`, mens det operationelle target..+117-handoff fortsat er en hård gate.
...
4. Ét exact-head Pages-artifact bygges og valideres med én dataset-, reference- og modelbinding. **Denne oprindelige push-/rowless-formulering er operationelt supersederet 2026-09-03:** et almindeligt push kan kun etablere moderne Candidate G i Fase A. Første `INITIAL_INTEGRATED_CUTOVER` kræver særskilt manuel bool + eksakt token og må kun starte fra den centrale, forseglede same-head `CANDIDATE_G_ACTIVE`-identitet. I deployleddet genverificeres den faktiske offentlige Candidate G-kildemanifesthash og implementation closure, før controlleren atomisk må skrive `INTEGRATED_PENDING`; en ny direkte rowless/legacy→integrated-plan må ikke oprettes. Den centrale profil **forbliver Candidate G**, mens den integrerede Pages-målversion deployes. Først efter eksakt offentlig implementeringskontrol og 210/673-verifikation må én service-role-RPC samtidigt sætte controlleren til `INTEGRATED_ACTIVE` og den centrale profil til den integrerede 11-feltsbinding. Allerede forseglede rowless/historical planer bevares kun til deres eksisterende recovery/reconciliation.
...
Candidate G-rollbacken bruger `transitionKind=CANDIDATE_G_ROLLBACK` og rollback-id `integrated-schema6-to-candidate-g-schema2-v3`; den er manual-only. En senere tilbagevenden bruger `transitionKind=INTEGRATED_RETURN`, går via `INTEGRATED_PENDING` og er også manual-only. **Fra 2026-09-03 er den oprindelige push-only-initiering af `INITIAL_INTEGRATED_CUTOVER` supersederet af den særskilte manuelle Fase B;** push kan alene udføre Fase A og Candidate G-vedligeholdelse. Scheduleren må hverken førstegangsaktivere, rulle tilbage eller returnere. Fra rowless exact legacyprofil bruger schedule/watchdog/manuelt vejr action `candidate-legacy-maintenance`, transition `LEGACY_CANDIDATE_G_REFRESH_BEFORE_INITIAL_CUTOVER` og CLI `legacy-refresh-begin/complete/abort`; begin skriver source-bevarende `CANDIDATE_G_PENDING`, complete/target-reconcile aktiverer kun exact current Candidate, og abort/source-reconcile bevarer legacy. Markøren består som lineage efter complete med exact current Candidate, fire `null`-returnfelter og `legacySourceRequired=false` og må kun arves, aldrig skabes ved relabel. En særskilt lineage efter et sikkert afbrudt integreret first-cutover-forsøg bruger `CANDIDATE_G_REFRESH_BEFORE_INITIAL_CUTOVER` og bevarer de fire return-evidence-hashes. Ordinær allerede aktiv Candidate-drift bruger `CANDIDATE_G_REFRESH`. Ingen vedligehold må skrive `INTEGRATED_PENDING`, aktivere state 6 eller skifte til integreret profil. Enhver fremmed eller uafklaret `PENDING`-status stopper normale deploys.
...
4.0.319's endelige Candidate G-rollbackbundle har sin egen `modelContractSha256=c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8` og sin egen transitive 56-filers `modelBundleSha256=ea22921e298a03ed1ef8787a4dbd79fd4fdf1a9b8e188d3c4b44e03f16fdceb0`. Begge værdier er regenereret, forseglede og regressionstestede; kontraktdigesten alene er aldrig et gyldigt bundlebevis.
...
- den private runtimebundlegate er grøn, og checkpointgaten er enten strengt grøn med 673/673 READY companion eller eksplicit attesteret `NOT_APPLICABLE_DURING_MEASURED_WARMUP` efter DEC-0114,
