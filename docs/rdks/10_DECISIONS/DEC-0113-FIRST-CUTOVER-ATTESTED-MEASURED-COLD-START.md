...
**Driftspræcisering 2026-09-01 – legacy WAM-cacheovergang:** Produktion `33471276238` på den mergede PR #244-head stoppede sikkert før beskyttede writes, artifact, Pages og deploy, fordi den fastlåste Candidate G-WAM-cache er produceret før den nu obligatoriske same-cell-proveniens og derfor gav payloadfri `MISSING_CELL`. Den strikte validator forbliver uændret. Callerlaget må for denne legacy-cacheovergang kun fjerne de private `PART::`-bølgekomponenter og deres bølgekilde/-summary, checkpointe den bevarede progression og genhente officielle målte DMI-data. Det må ikke nulstille andre vejrkomponenter eller identiteter og må ikke syntetisere, interpolere eller inferere data. Candidate G-maintenance må ikke aktivere first-cutover-forberedelse; kun `integrated-cutover` må gøre det. Watchdog skal køe og rechecke main-status umiddelbart før dispatch, så en aktiv/queued produktion ikke annulleres eller duplikeres på et forældet snapshot.
