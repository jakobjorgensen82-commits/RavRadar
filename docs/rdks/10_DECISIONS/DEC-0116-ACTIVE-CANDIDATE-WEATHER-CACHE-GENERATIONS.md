...
2. Den aktive generation bootstrapper én gang fra den nyeste tilgængelige, terminalbeviste main-scopede legacycache. En resolver læser GitHubs cacheinventar i faldende oprettelsesorden, kræver én entydig main-key med gyldig cacheversion, udleder run-id og attempt og genverificerer det immutable exact-attempt samt `success` for DMI-producent, progressiv zonecache-save og DMI-terminalkrav i korrekt rækkefølge og save-tidsvindue. Først derefter udleveres den eksakte cachekey til en main-restore uden `restore-keys`; selve indholdet skal stadig bestå strict READY og det nye eksakte registrybevis. En definitiv 404 på en enkelt kandidat må prøve næste ældre kandidat; netværks-, autorisations-, rate-limit-, server-, ufuldstændigt inventar- eller anden evidensusikkerhed stopper fail-closed. Manuel pilot og 118h-oneoff skal afvise non-main før private cache-restores. Ved lokal pre-gate-kontrol valgte kontrakten cache-id `7369179233`, version `2f5a0598…`, run `33990516150` attempt 1 og 48.847.855 byte; det er et afgrænset DMI-donorbevis, ikke et komplet produktionsbevis. Den tidligere cache fra `33984291027` var ikke længere tilgængelig i GitHubs cacheinventar; API'et beviser ikke årsagen. Hardkodning er derfor supersederet, og et wildcard-hit fra den gamle progressive familie må aldrig blive aktivt grundlag. Efter første nye active-save bruges kun active-familien.
