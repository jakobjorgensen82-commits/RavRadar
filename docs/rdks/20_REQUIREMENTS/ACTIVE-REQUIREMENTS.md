...
- **REQ-GEO-054A – IMPLEMENTERET LOKALT I 4.0.205 / AFVENTER MÅLRETTET CI:** Serverkald med en ny `sb_secret_`-nøgle må kun bruge `apikey`. Kun HTTP 401 med `PGRST303` fra denne nøgletype må genprøves én gang; alle andre eller gentagne fejl skal stoppe fail-closed. Central roundtrip skal fortsat bevise create/read/update/delete, fravær efter rollback og uændrede beskyttede ejerdata.
- **REQ-SUPABASE-QUOTA-004 – IMPLEMENTERET LOKALT I 4.0.205 / AFVENTER CI:** En fejl ved læsning af det beskyttede syncmanifest må aldrig fortolkes som et manglende manifest. Synkronisering stopper i stedet for at genskrive uændrede store dokumenter og belaste free-kvoten.
