...
- **REQ-GEO-027 – IMPLEMENTERET LOKALT I 4.0.140 / AFVENTER PRIVAT CI-PILOT:** Privat state-replay skal bruge en unik `historyKey` pr. kystdel og den faktiske score-neutrale `shadow-v2`-funktion. Parent-historik, krydslæsning, delte nøgler og scorepåvirkning er forbudt. Rå replayværdier må kun findes midlertidigt uden for artifactet og skal slettes efter validering; state forbliver deaktiveret.
