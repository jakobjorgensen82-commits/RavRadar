...
- **REQ-GEO-053 – IMPLEMENTERET LOKALT I 4.0.167 / AFVENTER PRIVAT CI:** Den samlede ejer-reviewside skal indeholde præcis 783 endelige dele og vise 752 komplette, 22 deldækkede og ni blokerede. Statusfarver må kun beskrive datadækning; delscore, scorefarve, rangering, rå vejr/state, offentlig integration og automatisk aktivering er forbudt.
- **REQ-GEO-054 – IMPLEMENTERET LOKALT I 4.0.167 / AFVENTER PRIVAT CI:** National admin-roundtrip skal skrive en unik, aldrig aktiv 783-dels tempkladde, verificere create/read/update/delete og fravær efter rollback samt bevise, at `coastline-overrides` og `direction-reviews` er uændrede i både payloadhash og version.
- **REQ-GEO-055 – IMPLEMENTERET LOKALT I 4.0.168 / AFVENTER PRIVAT CI:** Shadow-scorens vandstandstrend må kun beregnes fra et native tidspunkt og et native tidspunkt præcis tre timer senere. Den private indsamling skal hente mindst fire relevante marine assets; manglende `t+3h` stopper fail-closed og må ikke erstattes af interpolation.
- **REQ-GEO-056 – IMPLEMENTERET LOKALT I 4.0.169 / AFVENTER PRIVAT CI:** Den nationale vindgate skal hente præcis to HARMONIE-assets inden for bytebudgettet. Mindst ét skal tidsmatche et dokumenteret marint `t`/`t+3h`-par, og begge skal fortsat give komplette samme-celle wind-U/V-trin pr. del.
