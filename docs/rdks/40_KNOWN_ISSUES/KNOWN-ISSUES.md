...
- **ISSUE-CANDIDATE-G-ACTIVE-PROJECTION – LOKALT IMPLEMENTERET / EXTERNAL GATES AFVENTER:** Den globale 4.0.261-profil kræver fortsat beregnelige Candidate G-resultater for alle nødvendige rækker og begge jagtformer. Én manglende projektion skal vælge legacy for hele datasættet. Målrettede switch-, pipeline-, aktiv-shadow- og rollbacktests er grønne lokalt; exact-head, produktion, aktiv 210/673-shadow og browserkontrol udestår.
- **ISSUE-CANDIDATE-G-CENTRAL-SELECTION – LOKALT IMPLEMENTERET / CENTRAL READBACK AFVENTER:** `ravscore-profile-selection` er tilføjet som privat centralt admin-dokument med sikker engangspromotion af en nyere ejerbeslutning og efterfølgende central autoritet. Første produktionskørsel skal skrive og genlæse dokumentet identisk; derefter kan samme eller nyere central konfiguration gennemføre global rollback.
