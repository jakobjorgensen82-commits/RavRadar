...
- **ISSUE-GEODANMARK-FIRST-CI-PILOT – DELVIST VERIFICERET / AFVENTER GENKØRSEL:** #1928 bekræftede gyldig secret-injektion, central adminhydrering, maskering og pilotens isolation. Kaldet nåede capabilities, men den første parser søgte efter eksakt `Kyst` frem for entity-laget `Kyst_current`. 4.0.127 retter deterministisk valg af `_current` og tilføjer secret-fri lagdiagnose; featuredata er fortsat ikke verificeret før næste grønne pilot.
