...
- **ISSUE-RAVSCORE-1.1-RELEASE-CLOSURE – ÅBEN P0:** `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `5.0.0` er samlet lokalt under DEC-0110, men er ikke exact-head-valideret, merged, produktionsbygget eller offentligt desktop-/mobilverificeret. Candidate G forbliver eneste offentlige model.
...
- **ISSUE-RAVSCORE-EMERGENCY-AND-TRUST – LØST I KONTRAKT / AFVENTER RELEASEBEVIS:** Kun komplet atomisk same-model continuation i højst 72 timer eller kortere forecastudløb er tilladt. Cross-model fallback og interpolation er forbudt. Kun `VERIFIED_ONLY` er kalibreringsegnet; reconstructed/emergency og ture er ikke kalibreringsgrundlag. DEC-0112's separate `HISTORY_INCOMPLETE`-krav skal stadig implementeres og bevises.
...
- **ISSUE-RAVSCORE-DUAL-HASH-RUNTIME-BINDING – LØST LOKALT / AFVENTER SLUTREGENERATION OG RELEASEBEVIS:** DEC-0110 kræver både `modelContractSha256` for parameterkontrakten og `modelBundleSha256` for den transitive implementeringslukning. Begge følger nu den fulde 11-feltsbinding gennem generator, state, payloads, checkpoints, privat bundle, ture/observationer, central profil, Edge og releasegates og afvises særskilt ved mismatch. Endelige værdier må først dokumenteres efter regeneration på afsluttet head; exact-head-, produktions- og offentlig kontrol udestår.
