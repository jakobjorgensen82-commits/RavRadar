...
- **ISSUE-GITHUB-SCHEDULE-COPERNICUS-HOUR-RACE – RETTET LOKALT I 4.0.234 / NATURLIG DRIFT AFVENTER:** cron-job.org startede produktion ved hel time samtidig med ny Copernicus-indsamling og gav et sikkert 630/673-stop. GitHub-planen er forskudt til 14/29/44/59, piloten til minut 6, og en manglende aktuel time udsætter planlagt produktion uden artifact/deploy. Fuld CI og mindst én naturlig schedule-kørsel kræves, før cron-job.org deaktiveres.

- **ISSUE-PUBLIC-LOCAL-PRESENTATION-SPLIT – ÅBEN P1, SYSTEMISK BROWSERAUDIT 2026-08-18:** En direkte Chromium-kontrol gennemgik 210 zoner, 673 kystdele, 420 aktuelle zone-/jagtformvisninger og alle 2.100 femdøgnsfaner. De lokale strømvektorer, pilceller, punktpar, vindere, scorer, forklaringer og debug er internt korrekte, men zonepanelets synlige vejrkort bruger hovedzonens `condition.current`, mens vinder og score bruger den lokale kystdel. Blandt 414 direkte sammenlignelige aktuelle visninger afveg bl.a. strømretningen i 371, strømstyrken i 324 og vandtemperaturen i 414. Af 1.964 sammenlignelige femdøgnsfaner afveg den viste score fra den lokale prognose i 1.660 og bedste tidspunkt i 897. Blåvand viste konkret lokal vinder `Havsande – sydkyst` med debugstrøm NV 315°, mens det synlige strømkort viste N 11°. Rodårsagen er afgrænset til præsentationskoblingen i `app.js`/`js/ui/info-panel.js`; ingen land-/vandpunkter eller rå U/V-data er ændret. Rettelse og landsdækkende browserregression afventer næste session med Sol/Ekstra høj.

- **ISSUE-CURRENT-COMMON-HOUR-COVERAGE – ÅBEN P1 / EJERDIALOG FØR KILDEÆNDRING:** Datasæt `rr-20260818201755-210` rapporterede samlet 673/673, men den fælles aktuelle 20:00-række var ufuldstændig i tre Limfjordszoner: `DK-B05-12` 4/5, `DK-B05-17` 1/4 og `DK-B05-18` 2/5 lokale dele. UI faldt dér tilbage til hovedzonen. Dette er ikke tilladelse til at sænke 100 %-kravet eller ændre DMI → Baltic → AMM15 → otte allowlistede proxyer. Næste session skal skelne artifactets samlede proveniensdækning fra eksakt fælles timedækning og fremlægge en plan for ejeren, hvis hullet fortsat findes.
...
- **ISSUE-SUPABASE-RUNTIME-DIAGNOSTICS-STATEMENT-TIMEOUT – RODÅRSAG RETTET LOKALT I 4.0.234 / PRODUKTIONSBEVIS AFVENTER:** #2814 og senere #2830 viste HTTP 500/PostgreSQL `57014` på den voksende `runtime-diagnostics`-upsert; #2830 fejlede både første forsøg og den tilladte genprøvning. 4.0.234 gemmer den komplette rapport tabsfrit i et gzip/base64-arkiv med SHA-256 og reducerer den repræsentative kompakte payload fra 4.014.169 til 208.874 byte. Admin genskaber kun originalen efter integritetskontrol. Den snævre retry og fail-closed adfærd bevares. Central upsert/readback og download skal bestå, før issuet lukkes.
