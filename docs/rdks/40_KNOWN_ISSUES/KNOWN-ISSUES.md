...
- **ISSUE-TRIP-REPORTING-CHOICE – PLANLAGT PRODUKTFORBEDRING:** Den aktive afslutningsdialog har kun **Svar senere** og **Indsend tur**. **Svar senere** bevarer korrekt den lokale tur, men der mangler et ærligt, bekræftet **Afslut uden at indberette**, som rydder turen uden Supabase-post. Kontoen mangler samtidig en vej til at indberette uden en forudgående turstart. Den kommende løsning skal genbruge samme formular og eksisterende `observations`-række. Brugeren vælger selv korrekt dato og tidspunkt, og et historisk fund må aldrig kobles til nutidens vejr; manglende sikkert historisk grundlag giver `calibration_eligible=false`.
- **ISSUE-TRIP-COASTAL-PART-REPORTING – DELVIST LØST / KONTOVEJ AFVENTER:** Den aktive start- og afslutningsdialog filtrerer allerede kyststrækninger efter den valgte zone og gemmer begge ID'er. Den planlagte kontoindberetning skal genbruge denne funktion, så fund ikke kun placeres på den grovere zone, og en kyststrækning fra en anden zone afvises.
