...
## 4.0.315 integreret RavScore-releasekandidat
...
- **ISSUE-RAVSCORE-EMERGENCY-AND-TRUST – LØST I KONTRAKT / AFVENTER RELEASEBEVIS:** Kun komplet atomisk same-model continuation i højst 72 timer eller kortere forecastudløb er tilladt. Cross-model fallback og interpolation er forbudt. Kun `VERIFIED_ONLY` er kalibreringsegnet; reconstructed/emergency og ture er ikke kalibreringsgrundlag.

## Historisk 4.0.314 – cadencepolicy og opgivet incidentudførelse

- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – LUKKET UDEN UDFØRELSE / TEKNISK PENSIONERET:** PR #231 blev merged som `d539fc9d`, D1 `33279463545` var grøn, og inspect `33279639424` stoppede før descriptor ved `ONE_TIME_GAP_AMBIGUOUS_NATIVE_CADENCE`. Ejeren opgav derefter den fiktive rekonstruktion. Der skete ingen apply, mutation, artifact eller offentliggørelse. Workflow-inputs/job, incidentpolicy, mutator og positive eksekveringstests er fjernet; en negativ pensionsgate og historisk read-/quality-kompatibilitet består.
- **ISSUE-CANDIDATE-G-CADENCE-INFERENCE-4.0.314 – HISTORISK / PENSIONERET MED INCIDENTET:** Suffixafstanden blev fejlagtigt brugt som identitetskilde. Den lokale policyrettelse blev testet, men skal ikke exact-head-udgives eller bruges, fordi hele rekonstruktionsoperationen er fjernet.
- **ISSUE-CANDIDATE-G-CADENCE-POLICY-SWAP-4.0.314 – HISTORISK / PENSIONERET MED INCIDENTET:** Per-del-policybindingen var en lokal sikkerhed mod identitetsbytte i den aldrig udførte plan. Incidentpolicy og apply-CAS-vejen findes ikke længere operativt.
- **ISSUE-TRIP-D1-EXACT-MAIN-4.0.314 – HISTORISK BEVIST PÅ `d539fc9d`:** D1 `33279463545` var helt grøn. Der bestilles ingen ny incident-inspect; modelreleasekandidaten får sine egne normale exact-head- og produktionsgates.
...
- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829-BEFORE-PRIMARY-CHECKPOINT – HISTORISK CHECKPOINT:** PR #230 blev merged som `228725ea`, D1-genkørsel `33277510537` er grøn, men inspect `33277738135` stoppede før descriptor med `ONE_TIME_GAP_BEFORE_NOT_UNIFORMLY_READY`. Ingen apply eller mutation skete. Den aktuelle åbne hændelse føres under `ISSUE-CANDIDATE-G-MORNING-GAP-20260829` ovenfor.
...
- **ISSUE-CANDIDATE-G-BEFORE-UNIFORMLY-READY-4.0.314 – LOKALT RETTET / AFVENTER EXACT-HEAD:** Blanketgaten krævede fejlagtigt, at supportartifactets før-primary var 673/673 `READY`, selv om runnet publicerede komplet målt nødvisning separat. Kilden skal fortsat være målt schema 2.0 og eksakt replaybar, men target-reference-replayet er den eneste readinessautoritet. Positiv fixture bruger nu 673 ærlige 24-timers `WINDOW_INCOMPLETE`-suffixer; ældre hul, replaytampering, schema 2.1 og ukendt status stopper før descriptor.
- **ISSUE-TRIP-D1-EXACT-MAIN-4.0.314-BEFORE-PRIMARY-CHECKPOINT – HISTORISK CHECKPOINT PÅ `228725ea`:** Første D1 `33277253662` stoppede efter Edge på en forbigående 503, mens hele fail-closed roll-forward bestod. Den idempotente genkørsel `33277510537` er helt grøn. Den aktuelle final-SHA-gate føres under `ISSUE-TRIP-D1-EXACT-MAIN-4.0.314` ovenfor.
...
- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829-PRE-INSPECT-CHECKPOINT – HISTORISK CHECKPOINT:** Redningsproduktionen gendannede komplette aktuelle data, men offentlig primary stod ved 0/673 `READY`, 673 `WINDOW_INCOMPLETE` og 5–12/48 timer. 4.0.312 blev merged, men backend fejlede den senere migrationssynk; inspect/apply blev derfor ikke kørt. Den målte komplette nødvisning forblev den sikre vej. Den aktuelle åbne hændelse føres under det entydige issue-id øverst.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-PROVENANCE-LOSS – SOURCE EXACT-HEAD/MERGED / AFVENTER BACKEND OG LIVE:** Schema 2.1, incidentmarkør og trustprojektion skal forhindre, at afledt interpolation ommærkes som måling. Ældre kode afviser 2.1; ukendt/tampered trust og blandet modelkontekst stopper uden mutation.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-HARD-OUTFLOW – SOURCE EXACT-HEAD/MERGED / AFVENTER SAMLET LIVEGATE:** En rekonstrueret sekvens kan matematisk passere 13-timersgrænsen uden at være observeret udtransport. `hardObservedOuttransportEligible=false` og den særskilte gatesuppression skal derfor være ens i state, score og public audit.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-CAS-RACE – KENDT SIKKER STOPGREN:** En normal produktion mellem inspect og apply ændrer målets hash. Apply skal da stoppe urørt og kræve ny inspect; descriptoren må aldrig løsnes eller genbruges mod et andet mål.
- **ISSUE-RECONSTRUCTED-TRIP-CALIBRATION-CONTAMINATION – SOURCE EXACT-HEAD/MERGED / AFVENTER DATABASE- OG EDGEGATE:** Ture fra rekonstrueret score eller offentlig last-complete nødvisning skal være brugbare som brugerhistorik, men udelukkes fra kalibrering med eksakte flags/reason codes gennem browser, Edge, D1/Supabase, schema og installer. Den fundne producent-/forbrugerfejl er lukket i kilden ved at føre hele `activeManifest` til turkonteksten og kræve både startup- og kystdels-trust til stede og identiske; den samlede live-regression mangler fortsat.
- **ISSUE-LEGACY-SCHEMA-V2-TRIP-TRUST-ABSENT – FAIL-CLOSED SOURCE MERGED / AFVENTER SAMLET LIVEGATE:** Aktive/pending ture fra før 4.0.311 kan mangle trustfelterne. De bevares, men markeres `ravscore-evidence-trust-unattested` og bliver ikke-kalibrerbare; fravær må aldrig fortolkes som `VERIFIED_ONLY`.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-CLEANUP – LUKKET UDEN BEHOV / PENSIONERET:** Ingen rekonstrueret state blev anvendt, så operativ rollback/cleanup er hverken nødvendig eller autoriseret. Indgangene og mutatoren er fjernet. Historisk schema 2.1 læses fortsat fail-closed og udelukkes fra kalibrering.
- **ISSUE-TRIP-D1-CUTOVER-NOT-LIVE-4.0.311 – HISTORISK / LIVE LUKKET PÅ 4.0.313:** Kandidaten sætter installationstype-intent efter capacity/CAS før første Edge-deploy. Existing D1 bruger 20-/30-minutters lease, femsekunders prober, dobbeltattestation, drain, 600 sekunders restlease og samlet syvminutters Worker-gate; partial Edge går D1 roll-forward. Fresh partial Edge går exact-main-bundet tilbage til Supabase-secret, eksakt Edge-redeploy og dobbelt Supabase-attestation. Ingen intent ved capacity/pre-CAS-fejl betyder nul recoverymutation. 4.0.312-run `33266229687` fejlede migrationssynken, mens 4.0.313-run `33269631305` senere bestod hele D1-/Edge-/Worker-/sync-/slutattestationskæden. Rekonstruktionens separate 4.0.314-livegate står øverst.
...
Se DEC-0109. Ingen geometri, land-/vandpunkter, vejr, bølger, vandstand, rå U/V, koordinater eller private payloads er del af kandidaten.
