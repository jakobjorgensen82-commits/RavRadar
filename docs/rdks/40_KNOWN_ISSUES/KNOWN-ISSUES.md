...
- **ISSUE-WORKFLOW-FALSE-GREEN-DEPLOY – RETTET LOKALT, AFVENTER CI/PRODUKTION:** Bypasset blev bekræftet: almindelig `workflow_dispatch` kunne bygge/deploye, fordi gates krævede `push || force`, mens artifactet kun krævede positiv preflight. Gates kræver nu enhver positiv preflight, og en regressionstest beskytter både betingelsen og rækkefølgen før artifact. Først en frisk run med begge gates og deploy som `success` lukker issue produktionsmæssigt.
- **TRANSITION-NOTE – ERSTATTET:** Pre-Codex bootstrap-undtagelsen er ophørt i kode. Den bevares kun som historisk forklaring.
- **ISSUE-BULK-EMPTY-ACTIVE-ZONES – RODÅRSAG FUNDET I #1769, RETTET LOKALT:** Den nye materialisering af aktive zoner blev ophævet af `clean_and_summarize`, som slettede alle records med tom `hourly`. Frisk #1769 mistede derfor strukturelt `DK-B05-17`, `DK-B05-22`, `DK-B05-23` og `DK-B10-10`, selv om de var aktive. Tomme records bevares nu som eksplicit `missing`; ingen stale data eller nulværdier tilføjes. Afventer ny streng CI/produktion.
