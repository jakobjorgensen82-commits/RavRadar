...
## 4.0.311-kildekandidat – morgenhullet og den ejerautoriserede engangsrekonstruktion

- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – LOKAL KANDIDAT / IKKE ENDNU GENOPRETTET I PRODUKTION:** Redningsproduktionen gendannede komplette aktuelle data, men offentlig primary stod ved 0/673 `READY`, 673 `WINDOW_INCOMPLETE` og 5–12/48 timer. DEC-0109 tillader kun dette incident rekonstrueret; indtil exact-head, inspect/apply, fuld produktion og offentlig kontrol er grønne, er den målte komplette nødvisning fortsat den sikre vej.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-PROVENANCE-LOSS – LOKALT HÆRDET / AFVENTER SAMLET GATE:** Schema 2.1, incidentmarkør og trustprojektion skal forhindre, at afledt interpolation ommærkes som måling. Ældre kode afviser 2.1; ukendt/tampered trust og blandet modelkontekst stopper uden mutation.
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-HARD-OUTFLOW – LOKALT HÆRDET / AFVENTER SAMLET GATE:** En rekonstrueret sekvens kan matematisk passere 13-timersgrænsen uden at være observeret udtransport. `hardObservedOuttransportEligible=false` og den særskilte gatesuppression skal derfor være ens i state, score og public audit.
...
- **ISSUE-RECONSTRUCTED-TRIP-CALIBRATION-CONTAMINATION – LOKALT HÆRDET / AFVENTER DATABASE- OG EDGEGATE:** Ture fra rekonstrueret score eller offentlig last-complete nødvisning skal være brugbare som brugerhistorik, men udelukkes fra kalibrering med eksakte flags/reason codes gennem browser, Edge, D1/Supabase, schema og installer. Den fundne producent-/forbrugerfejl er lukket lokalt ved at føre hele `activeManifest` til turkonteksten og kræve både startup- og kystdels-trust til stede og identiske; den samlede regression mangler fortsat.
- **ISSUE-LEGACY-SCHEMA-V2-TRIP-TRUST-ABSENT – FAIL-CLOSED MIGRATION LOKALT / AFVENTER SAMLET GATE:** Aktive/pending ture fra før 4.0.311 kan mangle trustfelterne. De bevares, men markeres `ravscore-evidence-trust-unattested` og bliver ikke-kalibrerbare; fravær må aldrig fortolkes som `VERIFIED_ONLY`.
...
- **ISSUE-TRIP-D1-CUTOVER-NOT-LIVE-4.0.311 – LOKALT HÆRDET / AFVENTER EXACT-HEAD OG LIVE:** Kandidaten sætter installationstype-intent efter capacity/CAS før første Edge-deploy. Existing D1 bruger 20-/30-minutters lease, femsekunders prober, dobbeltattestation, drain, 600 sekunders restlease og samlet syvminutters Worker-gate; partial Edge går D1 roll-forward. Fresh partial Edge går exact-main-bundet tilbage til Supabase-secret, eksakt Edge-redeploy og dobbelt Supabase-attestation. Ingen intent ved capacity/pre-CAS-fejl betyder nul recoverymutation. Dette er endnu ikke livebevis.
...
- **ISSUE-TRIP-CROSS-SHARD-IDEMPOTENCY-AND-ERASURE – LOKALT HÆRDET / AFVENTER LIVE:** En global atomisk registry og owner-erasure tombstones på control-sharden lukker tværshard-dubletter, divergerende id-genbrug og sletning/write-race. Målrettede lokale kontrakter er kandidatbevis; exact-head backenddeploy og payloadfri liveverifikation mangler.
- **ISSUE-TRIP-MIGRATION-PRIVATE-OVERREAD – LOKALT HÆRDET / AFVENTER SAMLET GATE:** Den tidligere brede migrationslæsning kunne bringe private/ukendte Supabase-kolonner ind i runner-memory. Kandidaten bruger en eksplicit server-side PostgREST-bladprojektion og reconstruerer kun den dokumenterede safe allowlist; owner-id bruges kun til HMAC uden logging. CI/live-migration er ikke udført ved dette checkpoint.
