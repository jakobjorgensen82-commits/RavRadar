...
## 4.0.314 – cadencepolicy lokalt afgrænset; datahullet fortsat åbent

- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – FORTSAT ÅBEN:** PR #231 blev merged som `d539fc9d`, D1 `33279463545` er grøn, men inspect `33279639424` stoppede før descriptor ved `ONE_TIME_GAP_AMBIGUOUS_NATIVE_CADENCE`. Ingen apply eller mutation er sket. Offentlig drift forbliver 4.0.310-nøddrift indtil ny CAS-bundet inspect/apply, frisk produktion og offentlig verifikation er grøn.
- **ISSUE-CANDIDATE-G-CADENCE-INFERENCE-4.0.314 – LOKALT RETTET / AFVENTER EXACT-HEAD:** Suffixafstanden blev fejlagtigt brugt som identitetskilde. En policyklassificeret 1h-del kan lovligt have 2/3h målte afstande inden for Candidate G's ≤3h continuity. Identiteten kommer nu fra den eksisterende otte-dels `dkss_lf`-policy; kun en koordinatfri projektionshash bindes til descriptor/apply-CAS, og de manglende interne hourly slots udfyldes aldrig.
- **ISSUE-CANDIDATE-G-CADENCE-POLICY-SWAP-4.0.314 – LOKALT LUKKET / AFVENTER EXACT-HEAD:** En ren 665/8-total kan ikke opdage identitetsbytte. Per-del-policybinding, eksakt 3h-kompatibilitet for de otte dele og policyhash-CAS afviser swap eller drift før descriptor/apply.
- **ISSUE-TRIP-D1-EXACT-MAIN-4.0.314 – BEVIST PÅ `d539fc9d` / SKAL GENTAGES EFTER CADENCE-MERGE:** D1 `33279463545` er helt grøn. En ny source-SHA kræver eget D1 før inspect.
...
- **ISSUE-CANDIDATE-G-RECONSTRUCTION-CLEANUP – ÅBEN DRIFTSGATE:** Øjeblikkelig rollback og senere kausal cleanup er implementeringskrav. Koderollback før data-cleanup er forbudt, fordi schema 2.1 ellers med vilje stopper ældre kode fail-closed.
...
- **ISSUE-PREMATURE-NEXT-RAVSCORE-MERGE – PRODUKTIONSVERIFICERET TILBAGEFØRT / NY MODEL LOKALT SAMLET:** PR #215 blev merged før DEC-0102's fulde rækkefølge, men nåede aldrig deploy. Candidate G er fortsat eneste offentlige model gennem 4.0.308. DEC-0108-kandidaten er nu samlet lokalt med forskning, state, migration, runtime og producent-/forbrugerkæde; release forbliver blokeret, indtil red-team-fund, fulde gates, exact-head, produktion og offentlig kontrol er lukket.
...
- **ISSUE-RAVSCORE-COPERNICUS-SINGLE-HOUR-HORIZON – FUND / UNDER RETNING / P0-RELEASEBLOKERING:** Den første integrerede strømadapter kunne kun attestere én Copernicus-time og accepterede en kort `validTime`-nærhed i stedet for at bevise hele den kausale model- og publiceringshorisont. Det kan ikke levere en scoreklar 48-timers cold bridge plus eksakt 118-timers offentlig prognose uden efterfølgende opvarmning. Erstatningen skal forsegle en privat schema-2 range-cache med eksakt `productionReferenceAt-48h ... productionReferenceAt+117h`, fuldt DMI-gapmatrix, samme del/forælder/punkt, samme-tid/-celle/-lag U/V, rå subset-hashes, komplette acquisition-/collection-seals og nul nearest/hold/interpolation. Eksisterende verificerbar historik må genbruges; nyt produktionskald må kun udfylde dokumenterede huller. Problemet er ikke lukket, før producent, alle forbrugere, beskyttet recovery og de fulde 210/673×118 runtimegates er samlet grønne.
...
- **ISSUE-RAVSCORE-DELIVERY-DEPENDENCY – STRUKTURELT LØST / SCORE-NEUTRAL:** Den tidligere afhængige bølgelandingsandel er fjernet. Aktiv kontrakt er `delivery = transportPotential × 1`, hvor `1` ikke betyder 100 % fysisk levering, men nul estimeret numerisk last-mile-effekt. Bølgeretning er kun kontekst, og numerisk fysisk interval er `null`. Invariants, ablation og forbrugerkontrakter skal fortsat være grønne ved slutrelease.
...
- **ISSUE-RAVSCORE-EMPIRICAL-ACCURACY – KENDT BEGRÆNSNING:** Ejeren har fravalgt brugerfund som forudsætning for denne leverance. Modellen kan dokumenteres fysisk mere sammenhængende og teknisk mere robust, men må ikke kaldes empirisk mere fundpræcis uden et senere repræsentativt fund-/nulgrundlag. Se DEC-0102 og DEC-0108.
