...
- Første CI `33394343851` stoppede ved stale bundle-/binding-consumers; senere gates blev derfor ikke bevist. Bundle-/binding-consumerne er nu regenereret og målrettet lokalt verificeret; opdateret exact-head afventer. Lokale bundle-hashes er nu integrated `978415fd2b0a739b80b71c78134a79101113481817212811644b24262b6ddbd9` og rollback `4ccc2081982677aadbb47a5ee7d6f2b99fdcb7e42113e73029d5c60323a5ee96`.
- Candidate G er fortsat offentlig, indtil den opdaterede exact-head/merge/prod/browser-kæde er bevist.
# Kendte åbne og overvågede forhold

## 4.0.318 first-cutover-hærdning efter sikker 4.0.317-produktionsstop
...
- **ISSUE-RAVSCORE-COLD-START-ROLLBACK-ORACLE – LØST LOKALT / RELEASEBEVIS ÅBENT P0:** State 6 kunne allerede cold-replaye reelle input, men første cutover manglede en eksklusiv measured-only initialisering af Candidate G-rollback. Rollback-oraklet bygges nu separat fra egne faktiske timer, må ikke hybridiseres med continuation og skal være fuldt READY, før companion/checkpoint/release kan fortsætte.
...
- **ISSUE-AUTOMATED-GITHUB-FAILURES-AFTER-4.0.317 – FORKLARET / KODEHÆRDNING AFVENTER RELEASE:** Bot-/watchdogrun `33334709027` stoppede fail-closed i den operationelle actionresolver og deployede ikke. Det separate pilotrun `33335078275` stoppede ved den private DMI-gapmatrix før pilot/Copernicus/artifact. De automatiske mails er derfor reelle røde, sikre stop – ikke skjulte deploys. 4.0.318 lader ikke-push vejrdrift vedligeholde Candidate G, mens første integrerede cutover forbliver push-only.

## Integreret RavScore-modelarbejde under DEC-0110 – ikke udgivet

- **ISSUE-RAVSCORE-1.1-RELEASE-CLOSURE – ÅBEN P0:** 4.0.317-kilden til `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `6.0.0` bestod PR #235 exact-head og blev merged, men første produktion stoppede sikkert før vejrbygning/artifact/Pages. 4.0.318-hærdningen er lokal og mangler egen exact-head, merge, frisk fuld produktion/deploy og offentlig desktop-/mobilverifikation. Candidate G forbliver eneste offentlige model; schema 5 er kun den aldrig-offentlige, eksakte 5→6-migrationskilde.
...
- **ISSUE-RAVSCORE-DUAL-HASH-RUNTIME-BINDING – LØST LOKALT / BINDING FASTLÅST / RELEASEBEVIS ÅBENT:** DEC-0110 kræver både `modelContractSha256` for parameterkontrakten og `modelBundleSha256` for den transitive implementeringslukning. Begge følger nu den fulde 11-feltsbinding gennem generator, state, payloads, checkpoints, privat bundle, ture/observationer, central profil, Edge og releasegates og afvises særskilt ved mismatch. Den aktive binding er `778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7`/`978415fd2b0a739b80b71c78134a79101113481817212811644b24262b6ddbd9` over 43 filer og otte deklarerede forbrugere; exact-head-, produktions- og offentlig kontrol udestår.
...
- **ISSUE-RAVSCORE-COPERNICUS-SINGLE-HOUR-HORIZON – LØST LOKALT / AFVENTER RELEASEBEVIS:** Den private forseglede range-cache og recoverykæden kræver den eksakte kausale horisont og afviser nearest/hold/interpolation. En ægte cold start åbner kun fra præcis 48 private, proveniensverificerede timer plus den reelle targettime. Eksisterende verificeret historik genbruges, mens dokumenterede huller fortsat stopper fail-closed. Exact-head, frisk produktion og 210/673-kontrol udestår.
- **ISSUE-RAVSCORE-CANDIDATE-WAM-BOOTSTRAP – LØST LOKALT / AFVENTER RELEASEBEVIS:** Candidate G-migrationens 673 schema-2-states valideres nu samlet og skal udlede ét fælles bootstrap-target gennem samme rene recoveryfunktion. Wave-approach kræver 40 private præ-target-positioner fra ét coherent WAM-run pr. anvendt collection med same-cell native provenance; kun et gap på højst fire timer mellem native endepunkter i samme run/collection/grid/celle kan interpoleres. Tail-/scorefejlsgrænserne er henholdsvis `1/1024` og `0.01171875` rå point før afrunding. Mixed target, manglende/invalid state eller utilgængeligt run stopper før transition og bevarer Candidate G offentlig. Aggregate-resolver-, replay- og DMI-fokustests er lokale beviser; exact-head, frisk produktion og offentlig cutoververifikation udestår.
- **ISSUE-RAVSCORE-CANDIDATE-G-OPERATIONAL-ROLLBACK – LØST LOKALT / AKTIV PRODUKTIONSGATE:** Den varme Candidate G-projektion ligger kun privat som `ravScoreCandidateGRollback`. Controlleren `ravscore-operational-model-activation` bruger schema `ravscore-operational-model-activation-v4`, præcis fire statusser, seks overgangstyper og 30 felter inklusive source/requested implementation-closure-hashes. Durable source/requested-manifest- og closure-reconciliation gælder for `INTEGRATED_PENDING`, `CANDIDATE_G_PENDING` og begge aktive statusser. Central source-profil bevares under Pages-target; targetprofilen skifter først atomisk med `ACTIVE` efter eksakt offentlig implementation+210/673. Targethash completer; sourcehash må kun abortere med terminalt NOT_STARTED-bevis, mens ambiguous source-visible går til exact-target recovery; tredje hash forbliver fail-closed. Rollback/return er manual-only. Schedule/watchdog/manuelt vejr kan vedligeholde rowless legacy/current Candidate gennem de to pre-cutover-lineages og den planforseglede historical-Candidate-gren, men må aldrig initiere integreret modelskift. First cutover kan starte direkte rowless med version 0/legacy source eller efter bridge fra exact current Candidate-marker/aktuel CAS-version. Der deployes ingen særskilt Candidate G-assistent-Edge: den integrerede Edge svarer eksakt `409`, klienten bruger deterministiske lokale DA/DE/EN-svar, og schema-3-ture lagres Candidate G-bundet med `calibration_eligible=false`. Den endelige Candidate G-rollbackbinding er `modelContractSha256=c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8` og 55-filers `modelBundleSha256=4ccc2081982677aadbb47a5ee7d6f2b99fdcb7e42113e73029d5c60323a5ee96`. Måltests og bindingstjek er lokalt grønne; 4.0.318's fulde slutmatrix, PR exact-head, central readback, deploy, offentlig verifikation og produktions-fault-injection er åbne.
...
- **ISSUE-LIMFJORD-WAVE-MISSING-B05-11 – ÅBEN PART-LEVEL-GATE / EJERAUTORISERET SIDSTE-UDVEJSHYPOTESE:** Limfjordsmodellen har ingen bølgekomponent, `wam_dw` har intet accepteret gyldigt parent-punkt ved Feggesund, og eksisterende fallback er også missing. Sanitiseret `rr-20260830104132-210` viser derfor 118/118 parent-wave-missing. De tre aktive part-id'er findes imidlertid, har `marineCoverage=full`, og Candidate G-current er tilgængelig for strand/waders. Da den integrerede model producerer fra part-level 673-serier, skal en frisk integreret produktion først afgøre, om der overhovedet er et hul i de tre deles eksakte 118 timer. Kun ved et reelt part-level-hul, og kun hvis korrekt direkte DMI/egnet officiel data dokumenteret er umulig, har ejeren autoriseret en vurdering af konservativ nabozoneinterpolation for netop denne ene zone. Det er ikke implementeret eller generel fallback og kræver særskilt RDKS/provenance/usikkerhed/kapacitet/rollback/test/produktion; ingen punktflytning, strømlån eller historieskabelse.
