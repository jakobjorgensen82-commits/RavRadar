...
## 4.0.313 – legacy-replay blokerer stadig den endnu ikke udførte rekonstruktion

- **ISSUE-TRIP-D1-LEGACY-NULL-REPLAY – RODÅRSAG REPRODUCERET / LOKAL RETTELSE GRØN / AFVENTER EXACT-HEAD OG LIVE D1:** Backend `33266229687` fejlede sync, fordi 4.0.310-rækker kunne bevare kendte nested nullblade, mens 4.0.311's bounded PostgREST-leafprojektion udelader dem. 4.0.313 accepterer kun migration→migration efter streng stored-kontrakt og eksakt non-null/core-lighed; row/hash/registry omskrives ikke.
- **ISSUE-TRIP-D1-REGISTRY-LEGACY-HASH – LOKALT LUKKET / AFVENTER LIVE:** Den globale registry bærer korrekt den gamle hash. Genkørsel skal verificere/bevare den; manglende registry må kun backfilles med gammel selvhash-verificeret hash, og en modstridende registry skal stoppe uden rowmutation.
- **ISSUE-TRIP-GATEWAY-ERROR-BODY-LEAK – LOKALT LUKKET / AFVENTER EXACT-HEAD:** Kun faste kategorier må forlade gatewayklienten. Ubetroet/malformed non-2xx og 200-body, serverdetaljer og timeout er dækket syntetisk uden bodyudsnit.
- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – FORTSAT ÅBEN:** Rekonstruktionens inspect/apply er ikke kørt. Offentlig drift forbliver 4.0.310-nøddrift, indtil 4.0.313 exact-main backend, ny CAS-bundet inspect/apply, frisk produktion og offentlig verifikation er grøn.
- **ISSUE-RECOVERY-4.0.314-OVERTAKE – OPERATIV STOPGATE:** Versionsinterlocken ophører bevidst ved 4.0.314. Ingen senere versionsmerge må overhale den krævede grønne 4.0.313-backend/rekonstruktionskæde.

## Historisk 4.0.312-roll-forward efter 4.0.311-backendstop – morgenhullet og engangsrekonstruktionen

- **ISSUE-CANDIDATE-G-CONSTRAINT-DEPARSE-VERIFY – LØST, EXACT-HEAD OG MERGED I 4.0.312:** PR #224-head `4c4699fe` bestod exact-head `33263734108`/`99129959870` og blev merged som `7c168b00`; push `33263858078` var korrekt no-op. Backend `33263892151`/`99130384780` modtog HTTP 201 for én atomisk CHECK-transaktion, men en formatteringsfølsom regex afviste bagefter PostgreSQLs semantisk ækvivalente deparserform. 4.0.312's balancerede strukturelle verifier blev derefter exact-head-valideret i PR #225 og merged som `a5ece10d`; backend `33266229687` passerede denne gate. Det senere migrationssyncstop følges kun i det aktuelle 4.0.313-afsnit ovenfor.
- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – HISTORISK CHECKPOINT, AFLØST AF DEN AKTUELLE 4.0.313-POST OVENFOR:** Redningsproduktionen gendannede komplette aktuelle data, men offentlig primary stod ved 0/673 `READY`, 673 `WINDOW_INCOMPLETE` og 5–12/48 timer. 4.0.312 blev merged, men backend fejlede den senere migrationssynk; inspect/apply blev derfor ikke kørt. Den målte komplette nødvisning forblev den sikre vej.
...
- **ISSUE-TRIP-D1-CUTOVER-NOT-LIVE-4.0.311 – SOURCE MERGED / 4.0.312 NÅEDE D1/EDGE/WORKER / FULD READINESS AFVENTER 4.0.313:** Kandidaten sætter installationstype-intent efter capacity/CAS før første Edge-deploy. Existing D1 bruger 20-/30-minutters lease, femsekunders prober, dobbeltattestation, drain, 600 sekunders restlease og samlet syvminutters Worker-gate; partial Edge går D1 roll-forward. Fresh partial Edge går exact-main-bundet tilbage til Supabase-secret, eksakt Edge-redeploy og dobbelt Supabase-attestation. Ingen intent ved capacity/pre-CAS-fejl betyder nul recoverymutation. Backend `33266229687` nåede D1/Edge/Worker, men fejlede migrationssynken; kun en helt grøn 4.0.313-kæde kan lukke livegaten.
