...
## 4.0.312-roll-forward efter 4.0.311-backendstop – morgenhullet og engangsrekonstruktionen

- **ISSUE-CANDIDATE-G-CONSTRAINT-DEPARSE-VERIFY – RETTET OG FULDT LOKALT SOURCEGATE-GRØN I 4.0.312 / AFVENTER PR, MERGE OG BACKEND:** PR #224-head `4c4699fe` bestod exact-head `33263734108`/`99129959870` og blev merged som `7c168b00`; push `33263858078` var korrekt no-op. Backend `33263892151`/`99130384780` modtog HTTP 201 for én atomisk CHECK-transaktion, men en formatteringsfølsom regex afviste bagefter PostgreSQLs semantisk ækvivalente deparserform. CHECK'en er med høj sandsynlighed fuldt committed/valideret/kommenteret; eneste alternativ er fuld rollback. `VALIDATE` kan have scannet rækker internt, men ingen observationspayload blev hentet til runneren eller logget, og der skete ingen rækkemutation; D1, Edge, Worker, sync, vejr, artifact og Pages blev ikke nået. 4.0.312 bruger balanceret strukturel udtrækning og afviser fortsat ombytning, dublet, ekstra predicate og tvetydighed; målrettede tests, fuld lokal source/release/RDKS/håndbog/version og geodatakontrol er grønne, mens exact-head og produktionskæden mangler.
- **ISSUE-CANDIDATE-G-MORNING-GAP-20260829 – 4.0.311-KILDE MERGED / IKKE ENDNU GENOPRETTET I PRODUKTION:** Redningsproduktionen gendannede komplette aktuelle data, men offentlig primary stod ved 0/673 `READY`, 673 `WINDOW_INCOMPLETE` og 5–12/48 timer. DEC-0109 tillader kun dette incident rekonstrueret; indtil 4.0.312-backend, inspect/apply, fuld produktion og offentlig kontrol er grønne, er den målte komplette nødvisning fortsat den sikre vej.
...
- **ISSUE-TRIP-D1-CUTOVER-NOT-LIVE-4.0.311 – SOURCE MERGED / BACKEND STOPPEDE FØR D1/EDGE:** Kandidaten sætter installationstype-intent efter capacity/CAS før første Edge-deploy. Existing D1 bruger 20-/30-minutters lease, femsekunders prober, dobbeltattestation, drain, 600 sekunders restlease og samlet syvminutters Worker-gate; partial Edge går D1 roll-forward. Fresh partial Edge går exact-main-bundet tilbage til Supabase-secret, eksakt Edge-redeploy og dobbelt Supabase-attestation. Ingen intent ved capacity/pre-CAS-fejl betyder nul recoverymutation. Run `33263892151` nåede aldrig disse trin; livebevis afventer 4.0.312-roll-forward.
