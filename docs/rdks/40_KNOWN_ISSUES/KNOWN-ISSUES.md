...
- **ISSUE-BLAAVAND-INDEPENDENT-WEATHER-SERIES – GRIDAFKLARET / INTEGRATION ÅBEN:** #1987 viser forskellige celler for alle seks kontrollerede komponentfelter. Den resterende risiko er systemisk: separate serier må først fremstilles efter korrekt provenance, merge, score/state, UI og admin-roundtrip.
- **ISSUE-COASTAL-PART-CROSS-MERGE – AFDÆKKET / BLOKERET I 4.0.138-KANDIDAT:** Den nuværende multi-anker-score sammenholder én zones vejrværdi med flere kystretninger. Hvis separate partdata blev koblet direkte på, kunne nordlig vejrsampling blive vurderet mod sydøstlig kyst og omvendt. Shadow-kontrakten forbyder derfor krydsmerge og part-score, indtil partidentitet følger hele kæden.
