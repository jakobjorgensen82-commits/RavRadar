...
- **ISSUE-PLAIN-COPY-STALE-TESTS – LØST I LOKAL OPFØLGNING / AFVENTER EXACT-HEAD:** Produktion `32651786366` stoppede før deploy, fordi `test-score-presentation` stadig forventede den erstattede formulering "exceptionelt gode forhold". Den efterfølgende lokale gennemgang fandt også, at mobiltesten stadig krævede den gamle `startTrip()`-rejse. Begge tests følger nu den nye almindelige stjernetekst og direkte v2-tur og indgår i `validate:source`.
