...
- **ISSUE-WORKFLOW-FALSE-GREEN-DEPLOY – P0 / ÅBEN VED CODEX-HANDOFF:** Almindelige `workflow_dispatch`-vejropdateringer kan efter frisk opbygning nå Pages artifact/deploy, mens `Validate full project after fresh weather and current provenance` og `Run release governance gate after refreshed data validation` er `skipped`. #1760 bekræfter mekanismen på `a164b6e…`. Konsekvens: grøn topstatus på sådanne runs er ikke fuldt releasebevis. Første Codex-opgave er at lukke bypasset uden at svække valideringen; derefter kræves en frisk run med begge gates `success`.
- **TRANSITION-NOTE:** Den sidste pre-Codex ZIP ændrer med vilje ikke workflowet. Dette er kun en bootstrap for at få den dokumenterede handoff ind i repositoryet; undtagelsen ophører ved første Codex-kodeændring.
