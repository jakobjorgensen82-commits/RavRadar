...
- Den produktionsverificerede offentlige sandhed er fortsat 4.0.310 med Candidate G. `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `5.0.0` er lokal releasekandidat under DEC-0110 og bliver først offentlig efter grøn exact-head, merge, frisk fuld produktion/deploy og offentlig 210/673 desktop-/mobilkontrol.
...
- Ægte cold start kræver præcis 48 private verificerede timer plus reel target. Rollback bruger samme target uden dobbelt credit. Nøddrift er kun same-model, komplet, atomisk og hashbundet i højst 72 timer; cross-model fallback og interpolation er forbudt. Kun `VERIFIED_ONLY` er kalibreringsegnet.
- Ejeren opgav den planlagte fiktive udførelse af morgenhullet efter read-only inspect `33279639424` og før descriptor, apply, mutation, artifact eller offentliggørelse. DEC-0109 bevares kun som historisk incident-/trustkontrakt. Workflow-inputs/job, incidentpolicy, mutator og positive eksekveringstests er fjernet; en negativ pensionsgate bevarer kun historisk read-/quality-kompatibilitet. De efterfølgende 4.0.311–4.0.314-afsnit er revisionsspor.
...
- Modelsporet bygger én samlet efterfølger til Candidate G under DEC-0110: `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `5.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileewma4-atten15-v4`, komponent `ravscore-components-huntability-delivery-mobilisation-v4` og forklaring `ravscore-explanation-integrated-v4`. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder den transitive implementeringslukning; endelige værdier dokumenteres først efter regeneration på afsluttet head. 4.0.315 er en releasekandidat: Candidate G forbliver den eneste offentlige model, indtil exact-head, merge, frisk produktion og offentlig desktop-/mobilkontrol er grønne.
...
- Overgangen bruger `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema5-v4`. Den genvægter kun signeret, allerede afledt kystnormal Candidate G-currentevidens; migrationen læser/kopierer ikke rå U/V og hævder ikke rå genberegningslighed. Alle 673 schema-2-states skal bestå `candidateGStateKey` og give ét fælles kanonisk target. Wave-approach bruger 40 private præ-target-timepositioner fra ét sammenhængende DMI WAM-run pr. anvendt collection med same-cell native provenance; højst fire timers interpolation er kun tilladt inden for samme run, collection, gitter og celle. Udeladt EWMA-hale er højst `1/1024`, og konservativ rå-scorefejl er højst `0.01171875` før afrunding. Hvis state, fælles target eller coherent run ikke kan bevises, stopper cutover fail-closed, og Candidate G forbliver offentlig. Der dannes ingen syntetisk eller offentlig historik.
- Ægte cold start er særskilt og kræver præcis 48 sammenhængende private, verificerede timepositioner plus den reelle targetrække; WAM må kun bruge samme kontrollerede same-run/same-cell-interpolation. Candidate G-rollback `integrated-schema5-to-candidate-g-schema2-v2` beregnes fra samme targettid og må ikke give samme time dobbelt recovery-credit. Efter cutover fortsætter et kompakt state-5-checkpoint samt den private hashbundne produktionsbundle. Manglende eller ugyldig state fejler lukket og skaber aldrig kunstig historik.
...
- Dual-hash-red-team-fundet er lukket lokalt: den fulde 11-feltsbinding fører både `modelContractSha256` og `modelBundleSha256` gennem generator, state, payloads, checkpoints, privat runtime, ture/observationer, central profil, Edge og releasegates, og hvert felt har særskilte mismatchprøver. Slutværdierne er fortsat bevidst ulåste, indtil den afsluttede implementering er regenereret og hele exact-head-/produktionskæden er grøn.
