...
PR #286-head `d30faf93`/sourcegate `34724378629` bestod bindingspreflight, den fulde releasegate og alle tidligere led, men stoppede senere i den nye current-historikfixture. Fixturet manglede først den allerede beregnede `currentDirectInputAvailable`; derefter viste den samme reproduktion tre afledte auditfejl, fordi auditten sammenlignede et 47/48-timers konservativt scoreview med continuation-statens særskilte nulværdi for sammenhængende memory readiness. Rettelsen fører fixturefeltet igennem og lader auditten genbruge `buildCurrentSupplyScoreBounds` på den kompakte evidence, inklusive native-hold-bevis, og kræver eksakt match med continuationens lower/upper bounds. Den fulde lokale 210/673-regression er grøn; modelbundle og migration 13 er uændrede. En ny exact-head-CI er nødvendig, den røde head genkøres ikke.
