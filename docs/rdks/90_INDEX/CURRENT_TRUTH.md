...
## 4.0.317 – lokal integreret RavScore-releasekandidat, endnu ikke offentlig
...
- Grøn-no-op-/skipped-semantikken er lokalt lukket med det payloadfri schema `ravradar-production-workflow-outcome-v1` og terminalerne `NOOP`, `DEFERRED`, `BUILT`, `DEPLOYED`, `FAILED`. `DEPLOYED` kræver Pages, eksakt offentlig model-/implementation-/210/673-verifikation og afsluttet activation/reseal; `FAILED` uploades som statusbevis og holder runnet rødt. Monolitisk workflowkobling og spredt version/docs/kontraktmetadata er fortsat bindende arkitekturroadmap og ikke del af 4.0.316-P0-hotfixen.
...
## 4.0.317-modelarbejde – state 6 og konservativ score ved ufuldstændig historik
...
- Candidate G er fortsat den eneste offentlige model og der findes ingen offentlig shadowmodel. Den lokale proportionale matrix er grøn, men state 6 er ikke udgivet før PR exact-head, merge, frisk fuld produktion/deploy, Feggesund 3 × 118 og offentlig mobil-/desktopkontrol af 210/673 og alle kvalitetsflader er bevist. Offlinebevis dokumenterer teknik og fysisk sammenhæng, ikke empirisk bedre fundpræcision.
...
## 4.0.315 integreret RavScore-releasekandidat – endnu ikke exact-head-/produktionsverificeret
...
- Dual-hash-red-team-fundet er lukket lokalt: den fulde 11-feltsbinding fører både `modelContractSha256` og `modelBundleSha256` gennem generator, state, payloads, checkpoints, privat runtime, ture/observationer, central profil, Edge og releasegates, og hvert felt har særskilte mismatchprøver. 4.0.317-afsnittet øverst fastlåser de regenererede aktive og Candidate G-rollback-digests; exact-head-/produktionskæden er fortsat åben.
