...
## Lokal integreret RavScore-releasekandidat – endnu ikke offentliggjort

- Modelsporet bygger én samlet efterfølger til Candidate G: `RRS-COASTAL-PROCESS-INTEGRATED-1.0.0`, state `4.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-STRUCTURAL-LAST-MILE-HUNTABILITY-1` og profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileneutral-v3`. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder 34+ kanonisk normaliserede transitive implementeringsfiler; endelige værdier dokumenteres først efter regeneration på afsluttet head. Candidate G forbliver den eneste offentlige model, indtil hele kandidaten har bestået exact-head, frisk produktion og offentlig browserkontrol.
...
- Sidste nærkystled er strukturelt eksplicit, men score-neutralt: `delivery = transportPotential × 1`. Faktoren betyder ikke 100 % fysisk levering; den markerer, at RavRadar mangler lokal batymetri og en bølgeopløst surfzonemodel. `physicalDeliveryResolved=false`, og der udstilles intet opdigtet fysisk interval eller lokal landingspræcision.
...
- Overgangen genbruger den eksisterende, modelbundne vejr- og statehistorik gennem en engangs Candidate G→state-4-migration. Recovery vælger exact point-aktivering → gyldig integreret continuation → gyldigt checkpoint → dybt valideret Candidate G-state. Ugyldig exact point stopper straks; en ugyldig ordinær kandidat må ikke skygge for en gyldig lavere prioritet. Hvis ingen statekilde findes, genafspilles før første offentlige target præcis 48 timer allerede hentet, privat og proveniensverificeret strøm/bølge; pre-target-rækker publiceres ikke, og manglende bro stopper med `RAVSCORE_RECOVERY_REPLAY_BRIDGE_MISSING`. Den kræver derfor ikke flere dages ny offentlig warmup. Efter cutover fortsætter et kompakt, hash- og mål-bundet checkpoint samt en privat otte-fils produktionsbundle inklusive den forseglede Copernicus-range-cache; den fulde komprimerede bundle ligger kun i den ikke-offentlige Supabase Storage-bucket `ravradar-private-production-runtime`, hvor et beskyttet pointerdokument bevarer current + previous, og anonym adgang skal være afvist. Checkpointets operationelle nøgle skaber ikke nye `admin_document_versions`-kopier; alle eksisterende versionsrækker bevares, og migrationen udfører ingen destruktiv oprydning. Manglende eller ugyldig state fejler lukket og skaber aldrig kunstig historik.
...
## Historisk startgrundlag før DEC-0108 – én samlet ny RavScore-model
