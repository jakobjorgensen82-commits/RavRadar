...
## Merged 4.0.306-kildekandidat – første produktion stoppede sikkert før deploy

- Den samlede modelkandidat bestod PR #215 exact-head `33206386764` på `8547c1f9` og blev merged til `main` som `cfb9142062218a3ac8afc58a3de95560017450b2`. Aktiv rettelsesbranch `codex/ravscore-4.0.306-runtime-audit-fix` er baseret på denne eksakte, senest fetchede `origin/main`. Offentlig produktion er fortsat 4.0.305.
- Push-produktion `33206467775` bestod central adminhydrering, eksisterende checkpoint/state, DMI/Copernicus-cache og frisk runtimebygning, men stoppede før deploy, fordi `.geometry-v2-work/ravscore-public-runtime-audit.json` ikke havde en eksisterende forældermappe. Ingen modelaudit, fuld produktionsgate eller Pages blev omgået.
- Den afgrænsede lokale rettelse opretter auditrapportens forældermappe rekursivt og låser den hidtil utestede CLI-adfærd med en system-temp-baseret nested-output-regression. Den er endnu ikke sendt til GitHub.
- Rettelsen består audit-self-test, hele den målrettede RavScore-suite, 288 offlinecases, RDKS, releasegate og fuld lokal `validate:source`. PR'ens eksakte head skal fortsat bestå GitHubs egen sourcegate.
...
- Den gamle kompakte state-, provenance-, gap-, cadence-, privacy-, checkpoint- og recoverykæde genbruges med eksplicit migration og modelbinding. Allerede hentet verificeret vejrhistorik og DMI/Copernicus-cache genbruges ved første release, så modellen ikke skal vente flere dage på ny historik. Ingen rå U/V, koordinater, vejr eller gammel score kopieres ind i state; inkompatibilitet stopper eller udløser replay af de allerede hentede verificerede prøver, aldrig kunstig historik. Konto/ture/observationer gemmer nyt model-/state-/forklarings-id, mens ældre poster er læsbare.
...
- Producent-/forbrugerkæden er integreret i generator, central profil, startup/detaljer/hashes, ranglister, bedste tid, fem dage, DA/DE/EN, lokal/Edge-assistent, konto/ture/observationer, admin, håndbøger, workflow, checkpoint/recovery og audits. Ingen særskilt datahentning er nødvendig for den valgte model. Hvis senere evidens gør nye data nødvendige, må de kun anskaffes i et separat score-neutralt forløb efter konfliktkontrol mod nyeste `main`; det er ikke en skjult restleverance. Ny exact-head, frisk 210/673-produktion og offentlig desktop/390 px er resterende gates.
