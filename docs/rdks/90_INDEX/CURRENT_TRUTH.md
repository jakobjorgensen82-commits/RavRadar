...
- 4.0.137-kandidaten genbruger produktionens faktiske DMI STAC/GRIB-parser, nearest-valid-cell-søgning, afstandsgrænser og fælles fysiske U/V-gridregel til en privat kontrol af Blåvands to vandpunktkandidater mod `wam_nsb` og `dkss_nsbs`. Rapporten gemmer kun gridkoordinater, afstande og provenance; produktionsgeometri, admin, vejrsampling og RavScore forbliver uændrede. CI-artifactet skal bevise resultatet, før DMI-gridgaten kan kaldes bestået.
...
- Ingen Blåvand-geometri eller punkter er produktionsgodkendt. Ortofotogaten er bestået i #1982; 4.0.137-kandidaten udfører nu den efterfølgende private DMI-gridkontrol. Selv et positivt gridresultat er ikke tilladelse til selvstændig sampling eller admin-/vejrintegration. De øvrige otte zoner forbliver ved redesigngaten.
