# NYESTE SANDHED – 2026-09-06 – 4.0.328 per-pair-verificeret vejrfallback
...
- Kun en partial kandidat må fastholde sit native run hen over et seks timers modelskift, og kun mens den stadig har mindst den krævede modne/komplette fremtidshorisont, normalt 96 timer, og kataloget ikke er dokumenteret stale. Manglende eller allerede READY kandidat vælger nyeste komplette native run; ingen kandidat kan pinnes til cirka +120 timers alder.
...
- DMI-cachegenbrug bevarer nyere progression, men backfiller kun fra en strict-verificeret donor med samme samplingregistergeneration; hver vejrtime kopieres atomisk som hel række til en manglende/strukturelt tom time. Komplementære U/V- eller bølgefelter fra forskellige runs/acquisitions må aldrig blandes. Alle tre saves ligger før terminalgaten, og selector kræver `DMI_READY` plus strict current anchor.
...
- **Historisk supersession 2026-09-05:** Cadence-/én-lag-reglen og to-collection-sætningen ovenfor er ikke længere current truth. DEC-0116 kræver fælles serialiseret kandidat, partial-only retention ved mindst normalt 96 timers moden/komplet horisont og ikke-stale katalog samt tre normale DKSS-collections.
