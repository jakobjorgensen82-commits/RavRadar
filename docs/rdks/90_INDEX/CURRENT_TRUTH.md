# NYESTE SANDHED – 2026-09-05 – 4.0.324 active/candidate uden cache-nulstart
...
- Ikke-annulleret partial kandidat gemmes før terminalen. Active og deploykæden må først ændres eller fortsætte efter producer-success, allowlistet status, `DMI_READY`, strict current-anchor, `candidate_promoted=true` og nyt eksakt registrybevis. Den gamle progressive wildcardfamilie kan aldrig bootstrappe active; kun den eksakte livecache `dmi-zone-cache-v1-Linux-2026-W36-33984291027-1` fra run `33984291027` må bruges ved migrationen.
- Det samlede run `33984291027` fejlede senere i Copernicusleddet, men DMI-producenten, den progressive DMI-cache-save og DMI-terminalkravet var alle `success`. Det er et afgrænset DMI-cachebevis, ikke et komplet produktionsbevis; restore skal fortsat bestå READY og den nye eksakte registrykontrol, som stopper fail-closed ved enhver afvigelse.
...
- De to P1-rettelser og den målrettede lokale matrix er grønne. Run `33986893042` fejlede alene på manglende releasechangelog. Ejeren beordrede derefter et admin-bypass efter risikoforklaring, men Codex-sikkerhedslaget afviste handlingen, så ingen bypass eller merge skete. Run `33988058582` bestod releasegaten, men stoppede senere alene på håndbogs-/installationspariteten. Den officielle synk retter én genereret SQL-payloadlinje; ny exact-head, merge, active-bootstrap/runtime, fuld produktionsgate og komplet 210/673/118 er åbne. Candidate G er fortsat offentlig.
