# NYESTE SANDHED – 2026-09-13 – 4.0.351 main og lokal H0-audit-hotfix før cutover

PR #286's endelige head `b6f06310b59ce6c074361735ec9175da435d0e21` bestod exact-head-sourcegate `34726624728` og blev merged byteidentisk som main `6d4adbb2afddfd103566cf0a80c0a57408be1ca4`. Backendrun `34727884447` anvendte/readback-verificerede migration 13. Integrated/rollback/continuation forbliver `79d5118a…` / `84311c92…` / `9d396013…`.

Låst cache-only-preflight `34728026044` genbrugte target `2026-09-12T08:00:00Z` uden providerhentning og bestod current, WAM, freshness samt modelbygning på 210 zoner/673 kystdele. Intet blev cut over eller deployet, fordi slutauditten stoppede med `MODE_RECONSTRUCTION_MISMATCH`, `PART_LAST_MILE_STATE_METADATA_MISMATCH` og `PUBLIC_PROFILE_NOT_READY`.

Den tilbageværende årsag er nu reproduceret præcist. Ved et ægte manglende direkte H0-current danner producenten med vilje en ren lokal `UNAVAILABLE`-tilstand med `null` current-historikgrænser. Auditten krævede fejlagtigt endelige tal, kastede inde i en samlet evaluation/last-mile-blok og gav derefter en afledt mode-fejl. Profilkontrollen blandede desuden offentlig scoretilgængelighed sammen med rå memory-status.

Den lokale hotfix rekonstruerer samme `CURRENT_DIRECT_INPUT_MISSING` som producenten uden at opfinde bounds eller score. Coverage, memory og migration bevises fortsat uafhængigt, og first-cutover kræver fortsat migration/continuation på alle 673 dele. Den målrettede fulde 210/673-audit og dens negative fixtures er grøn. Modelkode, fysik, bundle, migration og vejrcache er uændrede.
