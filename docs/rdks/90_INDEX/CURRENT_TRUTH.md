# NYESTE SANDHED – 2026-09-15 – 4.0.367 genskaber den manglende centrale cutover

Main er 4.0.366 på `62fe62eee58b016d7ce79c2be867a5f4a6bfb0a2` efter PR #307 og #308. De to code-only-forsøg bekræftede først det nødvendige 128 MiB-loft og derefter den egentlige blokering: Pages viser det integrerede historiske artifact fra cutover `34877443841`, men Supabase har stadig ingen operationel RavScore-række, og den centrale profil står derfor fortsat på legacy Candidate G.

Den tidligere formulering om, at cutover `34877443841` afsluttede Supabase-overgangen, var forkert. Buildet fortsatte efter en fejlet privacyaudit og en manglende plan, uploadede et ufuldstændigt handoff og kørte aldrig central begin/complete-CAS. Pages blev publiceret, og terminalen blev alligevel falsk grøn, fordi den kun så på deploy og offentlig readback. Offentlig modelkode er altså til stede, men installationen er ikke centralt afsluttet. Dataset `rr-20260914180039-210` er strukturelt 210/673, men alle 420 aktuelle modes og alle 49.560 time/mode-resultater er utilgængelige; det er ikke numerisk komplette data.

4.0.367 løser præcis denne splittelse. Code-only-workflowet accepterer kun den eksakte manglende række/legacyprofil og den eksakte offentlige manifesthash fra run `34877443841`. Alle historiske evidenshashes og artifactfelter er fastlåst, den faktiske offentlige model verificeres igen, og først efter en ny exact-main-kontrol skrives den allerede offentlige historiske binding atomisk som central version 1. Derefter fortsætter samme code-only-kørsel gennem den eksisterende historisk-til-aktuel vedligeholdelse. Ingen DMI, Copernicus, Open-Meteo, oneoff eller gentagelse af den gamle cutover indgår. DEC-0149.

Den genbrugelige Pages-terminal kræver nu både Pages, offentlig verification, checkpoint og det konkrete centrale completion- eller reconciliation-led. Fejlsamling må fortsætte, men kan ikke skabe en falsk grøn installation. Den permanente adskillelse mellem rettelsesdeploy og normal vejrdrift består uændret efter DEC-0148. Lokal recovery/state-machine, code-only-kontrakt, YAML-parse og de virkelige historiske beviser er grønne; exact-head PR-gate, merge, code-only-deploy og offentlig kontrol mangler. Først derefter køres normal weather tidsbegrænset for at bevise numeriske scorer, rotation og cache.
