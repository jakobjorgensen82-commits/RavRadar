...
# HISTORISK SANDHED – 2026-09-26 – lokal 4.0.491, privat R2-overgang
...
Baltic NEMO og AMM15. Rå totaler fra 04:00 og 07:00 er forskudte
vinduer og kan ikke bruges som direkte fremgang. Den efterfølgende
eksakte sammenligning på 115 fælles timer × 673 dele = 77.395 par
viste nul gyldig→tom for vind, bølger, strøm og vandstand, men 274
gyldig→tom for vandtemperatur på fire timer (214 dele, 86 zoner).
Det er en reel offentlig regression, ikke blot en forskudt horisont.
...
Lokal 4.0.485 retter aflæsningen og tester den faktiske projektion.
...
den eksakte 4.0.484-kilde `eec1f0d8`/`rr-20260924084821-210`, mens
modelbinding, continuation, public projection og arkivkontrol består.
Måltests grønne; exact-head og kode-only levering afventer. Cron
forbliver pauset. Ingen overlappende eller blind vejrkørsel.
...
og providerfri code-only `35320190547`.

Normalrun `35320738621` gennemførte DMI, Copernicus, Open-Meteo, closure,
historik, central weather, proveniens og offentlig runtime. Den gamle
prognoserevisionskonflikt kom ikke igen. Runtimeauditten stoppede derefter på
673 ens `LAST_MILE_STATE`-replayfund; 420 aktuelle modes var utilgængelige.
Alle senere kontroller blev sprunget over, og den friske vejrpakke kom ikke
online.

Lokal 4.0.410 gør normale driftskontroller fortsættende og samler alle
udfald i én payloadfri rapport. Et score-/test-/readinessfund kan derfor
ikke alene gøre gyldige prognoser gamle. Target/main, byggeligt artifact,
beskyttet state, privacy og forseglet deploy er fortsat hårde grænser.
Runtimeauditten uploades i normalruten og opdeler continuation i skema, tid,
momenter, readiness, reference og seneste vektor samt parent-time og
historikgrænser. Den målrettede deploysuite, workflowrækkefølge, RDKS,
håndbog, version, YAML, diff og geodataversionsbevis er grønne. Modelbundlen
er uændret `d9ba75ed...`.

Næste: commit/push, exact-head, merge, providerfri kode og én normal weather
på gemte cacher. Ingen oneoff; scheduler pauset.
DEC-0193.
...
Samme run stoppede i næste klassifikationstrin, før migration, privat runtime eller nyt Pages-artifact. Årsagen var én konkret outputfejl: jq-udtrykket `.[$field] // ""` behandler en ægte boolsk `false` som tom værdi. `bindingCurrent=false`, `initialCutoverRequired=false` og `legacySourceRequired=false` blev derfor alle sendt videre som tom tekst, selv om central readback var korrekt.

4.0.368 bruger eksplicit felttilstedeværelse og bevarer boolske `false`-værdier. Samme mønster fandtes ikke andre steder i workflows/scripts. Næste code-only-run ser den allerede genskabte centrale version 1, springer recoveryen over og fortsætter direkte som `integrated-historical-maintenance` til migration 16, privat runtime og Pages. Ingen DMI, Copernicus, Open-Meteo, oneoff eller gammel cutover gentages. DEC-0148 og DEC-0149.

Pages-terminalen kræver fortsat både Pages, offentlig verification, checkpoint og det konkrete centrale completion- eller reconciliation-led. Exact-head PR-gate, merge og code-only-deploy af 4.0.368 mangler. Først efter central og offentlig 4.0.368-verifikation køres normal weather tidsbegrænset for at bevise numeriske scorer, rotation og cache.

# HISTORISK SANDHED – 2026-09-14 – integreret model er online, numeriske scorer mangler

PR #306 er admin-merget efter specifik ejergodkendelse som main `fa418f43bbd070c446ed19b6587541b93af89599`. Cutover `34877443841` publicerede privat runtime og Pages, men afsluttede ikke Supabase-overgangen. Sourcegate, providerhentning og brede valideringer blev sprunget over. Dette er kun bevis for det offentlige artifact, ikke en fuldført central installation eller et numerisk fungerende produkt.
...
Cutoveren stoppede bagefter, fordi checkpoint-leddet krævede `.datasetId` i auditrapporten, mens auditproducenten ikke skrev feltet. 4.0.365 tilføjer alene det allerede validerede `full.datasetId`. Bundles `327b989b…`/`1ccbb10e…`, migration 15, score, vejr, rotation og cache er uændrede. Exact-head, merge, handoff, cutover og offentlig kontrol mangler. Ingen oneoff/weather. DEC-0147.
...
Låst cache-only-preflight `34733358422` genbrugte target `2026-09-12T08:00:00Z` uden providerhentning og bestod current, WAM, freshness, modelbygning på 210 zoner/673 kystdele og rekonstruktionen af alle 1.346 aktuelle modes. De tidligere H0-/last-mile-fejl var væk. Intet blev cut over eller deployet, fordi kun `PUBLIC_PROFILE_NOT_READY` stod tilbage.

Resolveren valgte dokumenteret `genuine-cold-start` for alle 673 dele, fordi den kanoniske Candidate G-kilde ikke var migrationsklar. Det er den eksplicit tilladte DEC-0113/0114-vej. Producentens profil kaldte dog kun state klar efter tidligere migration eller continuation, så den godkendte første opstart kunne aldrig blive grøn.
...
Vindfejlen var en kontraktfejl i adapteren: fuldt verificeret DKSS `windTail` blev afvist, fordi adapteren hardcodede komponentnavnet `wind`. Lokal 4.0.350 accepterer kun det faktisk deklarerede `wind` eller `windTail` og kontrollerer proof mod samme navn. Rettelsen gælder både den gemte cache og fremtidige almindelige vejrdata.

Otte af de 14 currentdele er de godkendte regionale proxydele. De havde gyldige closure-bundne `NATIVE_CADENCE_HOLD`, men deres private præ-H0-kildereference nåede ikke RavScore. Live-current fører nu en eksakt valideret reference fra cache/proof/grid/lag/U/V/hash ind i privat scoring og recovery. Den publiceres ikke og kan ikke opfinde strøm, retning eller tidspunkt. En separat sorteringsrettelse følger producentens kanoniske `(validTime, partId)`.

Direkte inputmangel stopper ikke længere hele modellen. Den fælles offentlige kontrakt gør kun den berørte kystdel, jagtform og time `UNAVAILABLE` med null-score og årsag; øvrige scorer fortsætter, og utilgængelige dele kan ikke vinde rangering. Manglende historik forbliver særskilt `HISTORY_INCOMPLETE`. Samme kontrakt håndhæves i conditions, manifest, audit, Pages, browser, admin og ture. De seks øvrige H0-currentdele skal klassificeres i ny same-main-runtime; lokal utilgængelighed er et ærligt tilladt resultat, ikke et skjult hul.

Efter ejerens udtrykkelige godkendelse kører `integrated-cutover` runtimeaudit, strenge referencezoner, fuld `validate`, `release:gate` og `validate:data` som fem uafhængige kontroller. Alle køres, en payloadfri rapport samler resultaterne, og enhver fejl stopper før database/checkpoint/privat runtime/Pages. Hvis alle fem er grønne, fortsætter den eksisterende cutover automatisk. Almindelige vejrjobs beholder deres stoprækkefølge. Kildegaten køres én gang på exact PR-head; fulde post-data-gates består.

Ny append-only migration `20260912194206_local_unavailable_cutover_binding.sql` fører kun model-/continuationforseglinger og readbackversion frem; migration 11 er byteuændret. PR #285-head `fe4969f1`/run `34717671774` fandt og lukkede den historiske hash-test. Head `66338d63`/run `34717905077` bestod dette og samlede derefter tre Candidate-policy-/gatemarkørfejl. En fælles modelafhængig validator tillader nu kun integrated lokal `UNAVAILABLE` eller Candidate G exact komplet/full-history, og releasegaten følger den fælles `HISTORY_INCOMPLETE`-validator. Hurtige mål-/hashkontroller er grønne; den lange lokale fuldpakketest blev stoppet efter ejerens besked og gentages ikke. 4.0.350 er ikke online: ny exact-head-CI, merge, backendreadback, cache-only cutover, offentlig 210/673-verifikation og efterfølgende normaldriftsbevis mangler. Se DEC-0132.

# NYESTE SANDHED – 2026-09-12 – 4.0.349 main efter komplet cache og senere scorepakkestop

4.0.349-head `fa5e648c5fbbb4263dfae539a4707b920fe8646c` bestod PR #283's exact-head-sourcegate `34700907469` og blev merged med byteidentisk tree som main `187e599879e39d20014277e9afc4cb96029a78b3`. Backendrun `34702305208` genbrugte beviset uden dobbelt fuld kildegate, anvendte alene migration 11 og bestod central apply/readback. Normalworkflowet er stadig `disabled_manually`; den gamle `34613079069` er fortsat jobløs/inert.

Cache-only-run `34702471040` brugte target `2026-09-12T08:00:00Z`. DMI- og Copernicus-acquisition blev sprunget over, Open-Meteo var reuse-only, og ingen ny providerindsamling fandt sted. Exact currentclosure 79.414/79.414, WAM, freshness og kontrolleret live-current-selection bestod. Kørselen passerede det gamle v1/v2-stop; 4.0.349-rettelsen virker i den virkelige cache.

Modelbygningen stoppede senere med `Public RavScore horizon is not one complete 210/673 package`. Conditions-pakken blev bygget efter gennemgang af alle zoner, men den nuværende assertion udskriver ikke de allerede beregnede lokale årsager. WAM-gaten kræver retning ved positive bølger og er derfor ikke løs. Manglende onlinevandstand i cache-only er ikke bevist som scoreårsag; vandstand er score-neutral kontekst. Intet handoff, artifact, cutover eller deploy blev dannet.

Den lokale drifts-hotfix på 4.0.349 tilføjer kun en bounded, privacy-sikker fejlrapport med offentlige id'er, antal og årsagskoder. Den ændrer ikke modelbundle, releasekontraktmetadata, backendbinding, score, fysik, vejr, grid, afstande, geometri eller punkter. Kørselen forbliver rød og kan ikke udstede handoff/deploy.
...
Ejeren har præciseret, at Candidate G ikke skal bevares som praktisk backup, hvis den offentlige score allerede er ubrugelig. Den ønskede retning er én integreret offentlig model med komplette 210/673 strukturer og tydelig lokal `UNAVAILABLE`, mens fejl samles og rettes efterfølgende. Den gældende atomiske gate ændres ikke blindt: først hentes den konkrete fejlfordeling, derefter udføres et Astra/Ultra-review og en eksplicit, testet kontraktændring. Næste trin er exact-head for rapport-hotfixen, byteidentisk merge og samme cache-only-kontrol uden providerhentning.
...
Wrapper 15/15, supervisor 12/12, checkpoint 32/32, compile, modeldownload, freshness og workflowkontrakt er grønne lokalt. PR #281's første exact-head-run `34680013012` bestod releasegaten og stoppede senere alene i en forældet statisk WAM-test, som stadig søgte de gamle direkte `return 2/0`-linjer. Testen følger nu terminalhelperen og er grøn 63/63; produktionskoden er uændret. Næste sekvens er nyt dokumenteret PR-head, én ny exact-head sourcegate, merge, ny oneoff på genvaliderede cacher og kun ved 79.414 current + 79.060 native WAM + 354 Feggesund et same-head-handoff og kontrolleret cutover. Fuld post-data validate/releasegate og offentlig modelkontrol består; normaldriftskapacitet skal bevises efter launch. Se DEC-0129.
...
Normalrun `34635781802` roterede alle seks DMI-currentcollections og nåede 57.803 direkte DMI-par; unionen før Copernicus var 74.093/79.414. Open-Meteo sluttede med 1.335 mangler: 976 tidligere terminalt negative og 359 ikke terminalt afklarede i den kørsel. Det betyder ikke, at DMI kun søgte kendte positive par; DMI-planen dækkede hele det autoritative 79.414-register, mens budget, kildeegnethed og providerresultater bestemte udfaldet.

Oneoff `34642214559` beviste det nyere mønster: 2.274 manglede før Copernicus; 31 segmenter tilføjede 965; Open-Meteo sluttede med 1.033 provider-negative par. Alle 1.033 var forsøgt hos Open-Meteo og gav intet gyldigt grid-/værdipar for det eksakte request, men det er kun upstream-fravær for den kilde/kørsel. Det er ikke et bevis for, at DMI eller Copernicus aldrig kan levere dem. Current var 78.381/79.414. WAM og Feggesund var grønne. Ekstra tid alene er derfor ikke et launchbevis.

Copernicus brugte cirka 54 minutter; cirka 2.376 sekunder/78,2 % lå i gentagne admission/checkpoints. Lokal 4.0.345 journalfører nu hvert segment varigt og samler seks ad gangen gennem den uændrede strenge bank→shadow→stage-transaction. En syntetisk 40.120-record prøve målte seks segmenter 115,905 → 46,438 sekunder, med 0,127 sekunders journal og byteidentisk slutresultat. Positive, negative, batch, restart, failure og workflowtransport er måltestet grønne; livekapacitet afventer main.
...
Anden head `581dfae9` beviste cachetestrettelsen ved at bestå hele sourcegaten i run `34661632590`, men ren-tree-trinnet blokerede korrekt artifactet. Tredje head `e1e9dd1e`/run `34664907674` viste årsagen: releasegaten omskrev `release/RELEASE-REPORT.json` og `.md` med aktuel version/tid efter alle grønne tests. Sourcegaten muterede dermed det tracked indhold, den skulle forsegle. `validate-source-once` kalder nu den samme fulde releasegate med `--no-write-report`, som alene undertrykker disse to slutwrites. Post-data-produktion og `release:package` bruger fortsat almindelig `npm run release:gate` og producerer rapporterne.
...
Lokal pakke er 4.0.344 efter DEC-0126. Main er stadig `5587001b` / 4.0.343, og Candidate G er offentlig. Læsende run `34623745943` er grøn: faktisk migration +656/−0, fast rest 1.002, heraf 676 uden afsluttet relevant CP-forsøg og 326 forsøgt uden accepteret par; alle input er uændrede. Fem produkt/shard-requests har 37 nødvendige native timer over 118 med 80 tomme timer. CP-segmentering ved mindst 24 hele tomme timer er frosset og måltestet med fair pass, exact-pair-fejlisolation og kollektiv immutable attempt-retirement. Den specifikke AMM-deferred-regression er grøn sammen med de tre samlede CP-måltests: et fejlet segment spærrer ikke et senere segment, som et nyt Baltic-resultat åbner. Tidligere afsnits ventende diagnose/implementation er dermed afklaret. Katalogterminal, DMI/LF-integration, CP-precommit og cadence er måltestet; ingen leverbarheds- eller normaldriftsgaranti er udledt heraf.

Releaseversion, browsercacheidentitet og workflow-User-Agent er synkroniseret; geodatadiff er rent topversionsfelt. DEC-0122-undtagelsen følger snævert 4.0.344 uden ændrede materielle grænser. Changelog, DEC-0126, krav/issues/MASTER_LOG og begge håndbøger er opdateret. Måltests, RDKS, runtimebinding, modelbundle og browserimports er grønne. Pakken er committed/pushet i PR #278; en ny exact-head-CI efter testopfølgningen, merge og alle produktionsbeviser mangler fortsat. Ingen ny vejr-writer, cache-reset, køb eller deploy er udført.

Første PR #278-kildekontrol på head `1fd4d8a3` stoppede efter de tidligere grønne gates i én stale schedulertest: den forventede STAC-forespørgslens gamle højre kant i stedet for den allerede besluttede separate +120-terminalobservation. Den ene testforventning og kommentar tilpasses kontrakten; produktionskode, required ledger og 118-timersakse ændres ikke. Det fejlede run genstartes ikke; en ny exact head kræver én ny CI.

Anden kildekontrol på head `362709cd` passerede den rettede schedulertest og stoppede senere i en statisk cachebevaringstest. Den søgte recovery-hash/quarantine direkte i `commit_donor_bank`, men den fælles kontrol ligger nu i `prepare_recovered_donor_replacement`, som kaldes før både klassisk og prepared commit. Den eksisterende funktionelle crashfixture beviser bytebevaring. Kun kildetestens helper-/rækkefølgeforventning ændres; produktionskode og cacheformat er uændret. Hele workflowkontraktpakken og de efterfølgende source-tail-kontroller er grønne lokalt før næste exact head.

Main er uændret `5587001b` / 4.0.343, og integreret model er ikke online. Læsende `34615189896` beviser 912 regionale huller af 1.658; 656 kan klassificeres med allerede lagrede data ved korrekt tilknytning til de originale retained LF-outcomes i et isoleret kontrafaktisk forsøg (216 eksakte, 440 hold; intet tab, ingen cacheændring). 256 regionale huller består. I den DMI-filbundne CP-journal har 1.004 slutpar intet afsluttet forsøg fra en relevant CP-kilde, mens 654 blev forsøgt på returnerede native timer uden accepteret par. AMM-domæneudvidelse kan højst være relevant for 48 slutpar; produktrektangel er ikke vådcellebevis. OM-diagnosens 4.516 afvisninger er alle forklaret af diagnosens punktafrunding, ikke produktionskorruption. Samme runtime-samplingidentitet som det komplette 4.0.333-run er nu bevist ved immutable active-cache/target-binding.

Rettet læsende `34615949813`/`69a86b78` er grøn med uændrede inputhashes. DMI-originaloutcome viser 1.107 slutpar lokalt sprunget over i alle tre currentfamilier, 477 IDW-spatialUnavailable med mindst én anden familie lokalt sprunget over og 74 upstreamAbsent i alle tre. Leadrotation alene er utilstrækkelig: loopet giver også skæv tidsfordeling inden samme kørsel. Faktisk migrationsreplay `34619730789`/`cc2d1e59` er nu grøn med +656/−0, serialiserings- og idempotensbevis og uændrede inputhashes; den faste rest efter den isolerede prøve er 1.002, ikke en ny produktionsstatus. Af de 256 regionale rester har 240 relevante ubearbejdede officielle LF-assets; 16 ligger ved venstre kildekant, hvis katalog ikke var undersøgt. Lokal regional prooflevetid, migration/EOF/no-loss, fair tidsdeling og LF-prioritering er implementeret og måltestet; uafhængig main-flow-review og positiv CP-checkpointoptimering fortsætter. Ingen af rettelserne er committed/merged eller produktionsverificeret. Den ejerautoriserede almindelige opdatering `34613079069` stod endnu queued/jobs[] ved seneste kontrol; tidsplanen er disabled_manually igen. Ingen rollback/cutover. Detaljer i `docs/ai/WEATHER_RESIDUAL_AUDIT_2026-09-11.md`.

Efterfølgende main-flow-review har fundet og lokalt rettet katalogets terminalmismatch: observation til modelRun+120 adskilles fra producerkøens target+117, og original `nativeTerminalAsset` bæres separat uden at udvide 118-nævneren. Faktisk datetime-filtrerende katalogtest, ægte ledgerbuilder/strict-reader, serialisering og tamperkontrol er grønne. Regional EOF/replan/observation er nu måltestet med 29 transaktionstests; in-axis nulresultater kan genbruges som schedulerhint, aldrig som dataadmission. Højst ét on-phase præ-target-asset kan fortsat genprøves. CP's reviderede checkpoint på 40.120 rækker er byteidentisk med strict; efter readback-rettelsen målt 27,536 mod 45,926 sekunder og cirka 77,1 MB ekstra instrumenteret peak. Det er lokal Windows-fixtureevidens, ikke driftens tidsbudget. CP-kadencerotation og læsende post-migrations-sparsitetanalyse fortsætter før beslutning om eventuel tidsopdeling. Current-closure (6 tests), acquisition-plan (17), DMI-native-provenance og DMI-modeldownload/workflowkontrol er grønne. Ingen ny release er skabt.

# HISTORISK SANDHED – 2026-09-11 – første 4.0.343-residualaudit

Main er `5587001b45ffea056addaf6cd20084719540336a` / 4.0.343, CI-valideret og merged via PR #277. Oneoff `34588002366` sluttede failure med 77.756/79.414 currentpar og 1.658 rester, før friskhedstrinnet. Provider-saves lykkedes; native WAM-inspektion accepterede 79.060 par. Ingen integreret handoff/cutover. Læsende diagnose `34608174221` er nu grøn på fem eksakte generationer: unionen fandt 240 rå shadow-identiteter, men nul ekstra accepterede par i den faste 1.658-rest. Råtælleren inkluderer research; antallet af policygodkendte regionale samples er ikke målt. DMI-/CP-/OM-råpar havde nul overlap med resten før positiv validering. De 1.418 uden rå eksakt sample er ikke et bevis for nødvendig genhentning, fordi regionale holdtimer ikke indgår i råtallet. Regionalvejen kan højst omfatte 944 positioner, hvoraf 32 allerede er fyldt uden for resten; mindst 746 af resthullerne skal derfor løses ad anden vej. Ingen cachedata blev ændret. OM-reference/projections har en endnu uafklaret diagnoseadmissionsafvisning; diagnosens lavere total må ikke erstatte oneoffens drifttal. Se `docs/ai/WEATHER_RESIDUAL_AUDIT_2026-09-11.md` og `docs/ai/CP_CHECKPOINT_PROFILE_2026-09-11.md` fra repositoryroden. Regional genbrugsfejl, DMI-service og målt CP-checkpointoverhead skal løses samlet; hverken regionalrettelsen alene eller tidligere lokalt grønne schedulertests beviser tilstrækkelig vedligeholdelse.

Historisk kodekontrol mod 4.0.333/`57a4c91405` (komplet strømserie i run `34161930631`) viser uændrede strøm-afstandsgrænser: DMI 5 km, CP 5 km, OM 15 km og regional 5–15 km. CP's nearest_shared_uv/recordvalidator og OM's build_record/recordvalidator er AST-identiske. Ny donorbank/positiv proof/conflict masks kom i 4.0.340 og CP-checkpointarbejdet er blevet væsentligt tungere; fairness og WAM-admission er også ændret. Ingen evidens begrunder at udvide afstandsgrænser eller lempe admission på gæt. Checked-in targetfil er uændret; central adminhistorik er ikke dermed verificeret.

# HISTORISK SANDHED – 2026-09-11 – lokal 4.0.343-providerrettelse
...
# NYESTE SANDHED – 2026-09-09 – samlet 4.0.340 før tredje CI
...
- PR #273/head `a3786e6d` fejlede sourcegate `34362702197` på en forældet immutable migrationshash. Den lokale opfølgning fastlåser den korrigerede fil og beviser, at fjernelse af netop to parenteser reproducerer originalens SHA. Hele migrations-testgruppen og den dokumentationsuafhængige CASE-test er grønne; ny exact-head-CI mangler.
- Ejeren har udtrykkeligt godkendt nødvendig overførsel af DEC-0122 til 4.0.340 og senere launchrettelser uden ny forespørgsel. Aktuelt fastholdes 4.0.339. Policyen skal stadig dokumenteres og matche én eksakt release/main-head fail-closed; alle størrelses-, integritets- og driftsgrænser består.
- Weather `34350872447` er terminalt fejlet på merged 4.0.338 med alle provider-saves grønne. OM genbrugte 5.166, hentede 537 og mangler 301; WAM MISSING_HOUR. Beregnet 79.113/79.414 er ikke slutattesteret, og intet handoff er produceret. Cachedata må donere til 4.0.339. Candidate G er fortsat offentlig.
- [Ekstra Astra-audit](../../ai/LAUNCH_CHAIN_ASTRA_REVIEW_2026-09-09.md): SQL-funktionskald positivt/negativt bevist; ingen ny readerfejl. OM-historik ved større targetskift og bæredygtig cachetransport er konkrete efter-launch-opgaver, som skal påbegyndes straks. Mere OM-tid alene garanterer ikke de sidste huller.
- Åben vej: afsluttet udvidet Astra-audit, exact-head sourcegate og merge efter aktiv weather-progress-save. Backend skal se 3 applied/5 pending og bevise DB/D1/Edge/readiness; en ny 4.0.339-oneoff kan fylde bevarede cacher parallelt. Før cutover kræves begge beviser, 79.414/79.414, WAM/Feggesund 354/354, fulde gates, artifact/deploy og offentlig modelverifikation. Det komplette handoff beholder producentens reference; gamle 90/150-minuttersmål er kun advarsler inden for det gyldige vindue.
...
- Den tidligere codec-NO-GO er rettet: fælles Python/Node-codec bevarer hele den logiske schema-2-visning, unknown/nested/Unicode/proto-felter, afviser før ekspansion og muterer ikke input. Alle kendte produktionsreaders/writers, inklusive oneoff-progresskontrollen, er omlagt. Read-only job `34288231609` kørte den eksakte codec fra `bce970af` på 760.487.472 byte/578.063 sourceposter, skrev 94.150.151 midlertidige byte, bevarede logisk hash/count, ændrede ikke input og bestod Node-readback. Den separate gamle Open-Meteo-overlapprobe fejlede fortsat; det er ikke codecfejl eller produktionsruntimebevis.
...
- PR #269 er merged som `269db74b…`. Exact-head-run `34245761528` bestod selve releasegaten, men stoppede senere i protected-handbook-testen, fordi det nye webhåndbogsafsnit ikke var ført ind i den statiske Supabase-installationskopi. Kopien er nu synkroniseret bytepræcist, og den præcise test er grøn lokalt; nyt exact-head-bevis mangler. Runnet åbnede ingen vejrcache og deployede intet.
...
- Main-oneoff `34127986853` ved target `2026-09-07T13:00:00Z` genbrugte de private cacher. DMI dækkede 65.409/79.414 og afleverede 14.005 operationelle huller; den terminale DMI-outcome var `DMI_LOCALLY_SKIPPED_DKSS_ASSET` og `RUNTIME_BUDGET_REACHED`, mens verificerede par fortsatte downstream. De 2.451 DMI-history gaps var rådgivende og ikke operationelle currenthuller. Copernicus dækkede 12.661 og efterlod 1.344. Efter regional DMI modtog Open-Meteo 472 par, bevarede 76, hentede 114 og sluttede 190/472 med 282 missing. Den tidlige 70.280/79.414-linje var donorrestore ved gammel reference, ikke slutstatus. Ingen closure, artifact, deploy eller cutover skete.
- Runnet beviste cachegenbrug, delvis Open-Meteo-fremgang og 282 ærlige rester, men ikke den konkrete payload-/stopårsag, fordi 4.0.333-diagnostikken endnu ikke fandtes. Kodegennemgangen identificerede samtidig en selvstændig under-batch-risiko: delvist accepterede svar blev checkpointet, mens den uløste rest kunne genbestilles samlet under et globalt tre-runders loft. 4.0.333 checkpoint'er fortsat hvert succespar straks og genbestiller alene uløste `(partId, validTime)` gennem en bounded FIFO/BFS-kø med binær fan-out på højst to helt ned til singleton. Næste main-oneoff skal klassificere den faktiske restårsag.
- Transport har højst tre forsøg pr. eksakt work-item, content får ét same-work-forsøg, hele kaldsmængden er højst 1.024, pending-køen højst 2.048 og alt deler én monotonic deadline. Retryable HTTP bruger provider-wide cooldown; numerisk eller HTTP-date `Retry-After` begrænses til 15 sekunder. HTTP 400 og ukendte permanente statusser stopper providerfamilien; kun 413/414 isoleres ved split.
- Den udvidede lokale test beviser 8- og 50-dels BFS, poisoned time, per-item retry, request-/queue-/deadline-stop, udtømt 429-cooldown, global 400 og eksakt residualbevarelse. Hele den målrettede DMI→Copernicus→regional→Open-Meteo→closure- samt live builder/adapter/runtime-kæde er grøn, og to uafhængige reviews er GO. Dette er ikke CI- eller runtimebevis for 4.0.333.
- Uændret slutkrav er 673 × 118 = 79.414, én kilde pr. par, nul overlap/missing og alle fulde produktionsgates. Normal workflow forbliver deaktiveret; Candidate G er offentlig. Næste rækkefølge er exact-head CI, merge, frisk main-oneoff og kun ved 79.414/79.414 run-bundet handoff samt kontrolleret modelcutover.

# NYESTE SANDHED – 2026-09-07 – 4.0.332 bruger horizon, ikke alder, og kan genbruge et eksakt run ved cutover

- Ejerens bindende regel er nu DEC-0119: den nyeste strukturelt valide current-/future-række er brugbar uanset acquisition-, generation-, target- eller checkpointalder, så længe den forespurgte time ligger i den forseglede pakkes egen horizon. `validUntil`/`operationalRangeEndAt` er inklusive; `+1 ms` er udløbet. Alder er en ærlig advarsel samt signal til emergency, tillid, tur og `calibrationEligible=false`, aldrig alene missing, `UNAVAILABLE` eller deploystop.
- Et nyt weather-artifact kræver fortsat præcis 673 × 118 = 79.414 unikke par, én godkendt kildeklasse pr. par, nul overlap/missing og eksakt target-/registry-/model-/hashbinding. Run `34083611297` lukkede derfor korrekt uden deploy ved 78.856/79.414 og 558 missing.
- Første integrerede cutover må genbruge de fem private sourcecacher fra én eksakt grøn og komplet `main`-producentkørsel gennem et run-/head-/attempt-/target-/registry-/hashbundet handoff. Consumeren genbeviser closure og alle filer. Dette erstatter kun gentaget providerindsamling; sourcegate, central hydrering, bounded `update:weather`, Feggesund/spatial audit, kapacitet, fuld validate/releasegate, artifact/privacy, Pages og offentlig verifikation består.
- DMI-, Copernicus- og Open-Meteo-cache salvages granulært: control-plane/topidentitetskorruption stopper, mens en ugyldig leaf/proof-enhed fjernes som et ærligt hul og canonical hashes/seals genbygges fra uafhængige positive beviser. Den nyeste verificerede tuple vinder atomisk efter DMI → Baltic → AMM15 → policyregional DMI → Open-Meteo; fallback er ikke kildelås.
- Op til 48 timers verificeret historik er rådgivende. Mangler giver numeriske `HISTORY_INCOMPLETE`-scorer for alle aktive zoner, reason codes og kalibreringslås, ikke availability-stop. Den gamle generelle rå 72-timersregel og de hårde 90/150/240-minutters freshnessgates er **SUPERSEDERET**.
- Run `34093354004` sluttede sikkert med Copernicus success og Open-Meteo 2.735 required / 1.873 retained / 750 fetched / 2.623 filled; præcis 112 critical missing stod tilbage. Cachen blev gemt, men closure, artifact og deploy skete ikke.
- Run `34104536681` på eksakt `main` `c2ce63ff` genbrugte DMI-cachen til 67.897/79.414 på 5m24s, lod Copernicus dække 8.372 og efterlod 3.145, men stoppede før Open-Meteos første request med `OPEN_METEO_RESIDUAL_PLAN_INVALID_DMI_LEDGER_SOURCE_INDEX_INVALID`. Årsagen var, at regionalleddet afviste en ærlig valgt DMI-collection med `modelRun=null` og nul positive/source-bærende rækker. 4.0.332 accepterer kun netop denne negative tilstand som residual, bevarer eksakte retained old-run-proofs og afviser fortsat enhver positiv eller kildebærende null-run-række. Normal workflow forbliver deaktiveret under den kontrollerede cutover. Candidate G er offentlig, indtil exact-head, runtime, gates, deploy og Phase B faktisk er grønne.
- Source-transition-overlap/hysterese og en durable immutable multi-artifact-historik er separate post-launch-issues; ingen blending eller generel historikselector er godkendt i 4.0.332.
...
# HISTORISK SANDHED – 2026-09-07 – 4.0.330 helkædevedligeholdelse; supersederet af 4.0.333-topblokken
...
- På dette historiske trin var 4.0.330 en lokalt valideret releasekandidat. Den fokuserede kontraktmatrix samt versions-, RDKS-, håndbogs- og workflowkontroller var grønne, og den særskilte geodatadiff viste alene de autoriserede topversionsfelter. Den var på dette tidspunkt ikke exact-head-CI-valideret, merged eller produktionsverificeret; den aktuelle 4.0.333-status står alene i topblokken.
- Den bindende indsamlingsrækkefølge er DMI → Baltic → AMM15 → policybundet regional DMI → Open-Meteo. Hver providers verificerede resultat bevares, og den eksakte rest må fortsætte. Partial status er gyldig progression, aldrig releasebevis.
- DMI prioriterer operationelle missing/invalid/expired, interne huller og hale. Først derefter må bounded vedligeholdelsesrefresh af stadig gyldige rækker ske. En ældre gyldig currenttuple bliver stående, indtil en nyere U+V+currentproveniens er fuldt valideret og kan erstatte den atomisk.
- Copernicus isolerer provider-/datafejl pr. shard og fortsætter senere shards. Validerede records og afsluttede nul-resultatforsøg bevares som eksakt bundet `IN_PROGRESS`; stale attempts må ikke udvide den aktuelle residual eller skabe falsk COMPLETE. Target-/modelrunrollover sletter ikke fysisk validerede records.
- Ved en times reference-rebase kan et fortsat kryptografisk, produktdomæne- og tidsvalidt Baltic-prerequisite inden for højst fire timer bære et overlappende AMM15-fallbackpar, så et rent envelopeskift ikke skaber missing. Kun exact-current-reference-attempt må undertrykke frisk Baltic-retry eller postbuild source-upgrade; AMM15 er ikke kildelåst.
- Copernicus-primary må genbruge eksisterende verificeret target−48..−1-historik, men må ikke netværkshente historikken. Al history/advisory-netværksrefresh ligger i et kort bounded postbuild-job, som validerer en separat kandidat før atomisk promotion, bevarer aktiv historik ved fejl, ikke blokerer artifact/deploy og aldrig kommer foran reelle operationelle huller.
- Valgfri regional shadowevidens med defekt header, anchor eller sample isoleres til det konkrete par som missing. Policy, centrale targets, registry, DMI-ledger/attestation og gapmatrix er control plane og forbliver fatal ved fejl.
- Open-Meteo schema v2 deler privat progresscache mellem normal og oneoff og bevarer faktisk `acquiredAt` pr. record. Kun valideret overlap genbaseres til nyt target. Der checkpointes før første request og efter hver batch.
- Den kritiske Open-Meteo-rest behandles breadth-first i batches på højst 50 lokationer med bounded retry og én fælles monotonic deadline. HTTP-truncation, provider-, transport- eller payloadfejl isoleres til batchen. Proaktiv refresh starter kun ved nul kritiske huller, ældst først og mindst to timers recordalder; fejl bevarer den gamle record.
- Normal og oneoff bruger samme provider-, cache-, residual-, provenance- og closurekode. Normal drift er den varige updater; oneoff er accelerator og deployer ikke. Shared cachewrites kræver exact `main` og eksplicit write authority.
- Den separate scheduled Copernicus-pilot fjernes. Ekstern cron forbliver primær payloadfri dispatcher af normal drift; GitHubs native schedule er reserve, og tunge writers er fortsat serialiserede.
- Public/runtime/deploy kræver fortsat præcis 673 × 118 = 79.414 operationelle par, én kilde pr. par og nul overlap/missing. Højst 48 timers verificeret historik er rådgivende og kan give `HISTORY_INCOMPLETE`, men må ikke syntetiseres eller blokere et komplet forecast.
- Den integrerede scoremodel må først aktiveres efter frisk 79.414/79.414, Feggesund 354/354, hydreret spatial audit, live kapacitetsbevis, fuld post-data `validate` og `release:gate`, artifact/deploy og den særskilte autoriserede Phase B. Ingen scoreformel, geometri, kystnormal eller land-/vandpunkter ændres af 4.0.330.

# NYESTE SANDHED – 2026-09-06 – cache gemt, DMI-proof-kontinuitet brudt

- 4.0.328 er exact-head-godkendt i `34040547841` og merged via PR #261 som `31b98428dea163c11ded1fc1e428e27a0218a8f2`. Dens første main-runs nåede DMI og gemte progression, men nåede ikke Copernicus, regional DMI, Open-Meteo, closure, artifact eller deploy.
- Run `34041885030` havde 8.918 kanonisk attesterede cached DMI-par og 9.541 par klassificeret positive af current asset-outcome-proofs; delta 623. Run `34044178502` havde 22.357 mod 25.826; delta 3.469 efter det observerede kanoniske modelrunskift. Begge cache-save-forløb gennemførte.
- Gammel-main `34049794693` og `34051318868` på head `31b9842` nåede DMI og Copernicus, men Open-Meteo fejlede deterministisk med `OPEN_METEO_RESIDUAL_PLAN_INVALID_SHADOW_NATIVE_CADENCE_INVALID` ved `2026-09-06 18:16:28Z` og `18:42:49Z`; artifact/deploy blev korrekt skipped. Producenten skrev en hourly off-phase regional `dkss_lf`-sample, mens consumeren gjorde selected-run `lead % 3 != 0` fatal. Dette er præcis negativ regional producer/consumer off-phase-evidens, ikke retained-proof-admission-evidens. Begge paths er lokalt fokustestede; ingen har exact-head runtimebevis endnu.
- Det er derfor ikke dokumenteret, at cacheindholdet blev nulstillet. Den dokumenterede P0-fejl er, at kompatible rækker fra et tidligere modelrun mistede et fortsat gyldigt attestation-proof under den nye ledger, samtidig med at et nyt assets partOutcomeProof kunne påstå positiv dækning uden en faktisk række med denne sourceidentitet.
- 4.0.329 skal bevare og genvalidere konkret per-pair/source-proof for hver genbrugt række på tværs af modelruns. Processing-signatur skal være kompatibel, og native source-lead er højst 120 timer. Den positive DMI-mængde skal være præcis actual canonical attestation af faktiske rækker med gyldigt aktuelt eller retained source-proof; et nyt assets generelle spatial-proof må ikke skabe positive rækker.
- Operational complement er altid den eksakte inverse i det fulde registry på 79.414 par. Der er ingen kardinalitetsgrænse: en rest fra 0 til 79.414 må fortsætte til de næste leverandører under deres eksisterende datakontrakter. Et stort hul er ikke i sig selv en stopårsag.
- Active-donor skal valideres ved donorledgerens egen `productionReferenceAt` og præcis dens +117-timersvindue. Missing/expired, interne huller og hale behandles før retained refresh. En officielt revideret asset i samme modelrun gør gammel revision utilstrækkelig; atomisk currenttuple-udskiftning kræver verificeret ny asset, outcome og provenance.
- Whole-row donor-merge er supersederet. Kun exact-proof-gated U+V+`sources.current` merges atomisk; primary-proof vinder, halv/uattesteret donor afvises, og primarys wave/wind/øvrige fields+sourcer+`processedSteps` bevares. Complete donor-current-summary erstatter; ellers bevares complete same-grid primary-summary, mens alene partial/mismatch current-summary fjernes.
- En canonical selected-run off-phase regional sample er ineligible, ikke fatal. Producer skriver den ikke. Consumer validerer dict, exact collection/modelRun/validTime og `run <= validTime`, ignorerer derefter off-phase før capturedAt/hash/sampleKey/spatial/vector og sender exact pair som `MISSING` til Open-Meteo. Legacybytes prunes ikke. Malformed identitet og alle on-phase hash-/binding-/spatial-/vektorbeviser forbliver fail-closed. Dette er en scoped DEC-0117-supersession via DEC-0118/4.0.329.
- `RETAIN_PREFERRED_NATIVE_RUN` udledt af `ledger.ready` er supersederet. Exact retained per-pair/source-proofs gør rollover tabsfrit, så normal og oneoff vælger selectorens nyeste modne, native-complete run med `RETAIN_PREFERRED_NATIVE_RUN=false`, selv ved non-READY kandidat. Den gamle 96-timers-pin må ikke forsinke refresh op til cirka 24 timer eller skabe unødig fallback-tail; `true`-path er alene dormant test/helper.
- Retained availability medfører `ready=false` og kan ikke promoveres eller publiceres, mens den eksakte residual må fortsætte. Strict active-promotion, 79.414/79.414, kildeunikhed, nul overlap/missing, privacy og fulde gates er uændrede. Providerkæde, ydre budgetter, cron/reserve-dispatchkadence, score, state, geometri, punkter og 48-timers historik består; canonical regional off-phase-admission er den afgrænsede interne undtagelse.
- Status: runtimeårsag og bindende design er dokumenteret; lokal implementering og final fokustests er grønne (`py_compile` 8 produktionsscripts; provenance, current-field-shadow, regional-current-operational, Open-Meteo-fallback, targetregistry, transactional 19/19 og diff-check). GitHub exact-head sourcegate, merge, positiv main-runtime, 79.414/79.414, fuld validate/releasegate og deploy afventer. Candidate G er fortsat offentlig.

# HISTORISK SANDHED – 2026-09-06 – 4.0.328 per-pair-verificeret vejrfallback, supersederet af 4.0.329

- Ejeren har godkendt DEC-0118. Den lokale 4.0.328-kandidat flytter den strenge alt-eller-intet-grænse fra DMI-/Copernicus-mellemleddene til den samlede closure: verificerede par bevares, og kun den eksakte rest går videre.
- DMI-kandidaten genbruger kompatible gyldige par over hele target..+117. Download-, parse- eller behandlingsfejl isoleres til det eksakte asset og rulles transaktionelt tilbage; senere assets fortsætter. Watchdoggen bruger collection/run/time/item samt kanonisk URL- og revisionshash, genstarter bounded med kun denne assetrevision sprunget over og må aldrig gætte ud fra en malformed markør.
- Availability-ledgeren er den eksakte inverse af verificeret DMI-availability og omfatter interne huller, hale, lokal filfejl og registrybundet total outage. Den er ærlig partial evidens, ikke `DMI_READY`; active-promotion kræver fortsat strict producent-success, current-anchor, `candidate_promoted=true` og registrybevis.
- Copernicus `READY` eller target/DMI/shadow-bound `IN_PROGRESS` kan sende sin eksakte rest videre. `IN_PROGRESS` med nul attempts betyder nul dokumenteret Copernicus-dækning, ikke udtømning. Kun ugyldig privat Copernicus-shadow/-stage må karantæneres; registry, DMI-ledger og centralt hydrerede targets stopper fail-closed.
- Open-Meteo forbliver sidste særskilte `open-meteo-combined-current`, target..+117-only, højst 15 km, UTC/m/s/grader og altid `calibrationEligible=false`. 48 timers verificeret historik er rådgivende og må mangle som `HISTORY_INCOMPLETE` uden at svække den operationelle akse.
- Public/runtime/deploy kræver stadig præcis 673 × 118 = 79.414 unikke currentpar, én kildeklasse pr. par, nul overlap/missing og alle øvrige gates. Den målrettede lokale matrix er grøn; fuld exact-head `validate:source` i GitHub, merge, frisk mainkørsel, 79.414/79.414, 354/354 Feggesund, fulde produktionsgates og offentlig verifikation mangler. Candidate G er fortsat offentlig.
- Runtime fuldvaliderer closure-/advisorybeviset én gang pr. dokument og bruger derefter et `(partId, validTime)`-indeks; hvert konkret opslag bevarer hash- og assignmentbinding. Spatial slutgate reproducerer public strøm fra privat closurebundet U/V og afviser rå U/V i public/runtime. Den hydrerede spatial-audit mangler fortsat GitHub-bevis.
- Aktuelle deterministiske modelclosures er integrated `4346bf2de26a0dde25c3ef8dc72e741d6259f15282801e62a95a31a8f6594c0d` over 55 filer, Candidate G rollback `71a093a4b419891cb41f582de2ab926a2ea23e5abbe16015cc2b6f4b3ae8be0f` over 56 filer og continuation `5456d603a687e03b8983b5a97712b4acd305011a6029edb90df72d1d3e4f702f`; lokale hashchecks er grønne.
- Oneoff `34032882646` på gammel main gemte DMI 71.522/79.414 og løftede Copernicus fra 108 til 5.960 af 7.892 restpar; 1.932 stod tilbage, men den gamle READY-only-gate blokerede Open-Meteo. Normal `34034258619` genbrugte cachen og havde et separat, korrekt 13:00-downstream-register med 8.514 DMI-gappar; Copernicus gik fra 5.890 til 5.973 og blev gemt, men gammel main gjorde derefter den afgrænsede 360-sekunders timeout fatal. 7.892 var donorbeviset ved 12:00, ikke samme register. 4.0.328 bygger downstream-registret fra DMI-kandidaten og lader kun samme-reference-verificeret `READY`/`IN_PROGRESS` fortsætte. Oneoff `34037360371` kører på gammel main; ingen af kørslerne er 4.0.328-bevis.
- Ekstern cron er primær dispatcher, GitHub-schedules reserve, og normal drift er den varige fuldvinduesvedligeholder; oneoff er kun genopfyldning. Efter stabilisering og model-online måles leverandørrækkefølge/tid/friskhed løbende. Større pipelineoptimering er udskudt til efter modellanceringen.
...
- Partial kandidat gemmes efter et faktisk startet, ikke-annulleret producentforsøg før terminalen. Active og deploykæden må først ændres eller fortsætte efter producer-success, allowlistet status, `DMI_READY`, strict current-anchor, `candidate_promoted=true` og nyt eksakt registrybevis. Når active endnu mangler, vælger resolveren den nyeste entydige main-key/version, hvis dens immutable exact-attempt livebeviser grøn DMI-producent, tilhørende save og terminaltrin. Kun den eksakte valgte key restores på main; active-bootstrap har ingen wildcard og ingen hardkodet kortlivet key.
- Den lokale liveprøve valgte cache-id `7369179233`, version `2f5a0598…`, run `33990516150` attempt 1 og 48.847.855 byte. Den er et afgrænset DMI-donorbevis, ikke et komplet produktionsbevis; restore skal fortsat bestå READY og eksakt registrykontrol. Den tidligere cache fra `33984291027` var ikke længere i GitHubs inventar, men API'et kan ikke bevise hvorfor. En definitiv kandidat-404 prøver næste cache; netværks-, autorisations-, rate-limit-, server-, inventar- eller anden evidensusikkerhed stopper fail-closed.
- Historisk 4.0.324-regel, supersederet af 4.0.329: en partial kandidat kunne fastholde sit native run ved mindst normalt 96 timers moden/komplet fremtidshorisont og ikke-stale katalog. Gældende drift vælger i stedet selectorens nyeste modne, native-complete run med retentionflag `false`; exact retained pair/source-proof bevarer kontinuiteten.
- Hver normal kørsel genbruger kompatible gyldige rækker og scanner hele target..+117 for interne huller, ufuldstændige/ugyldige/udløbne trin og hale. Normal DKSS-ramme er tre collections. Oneoffen accelererer samme kandidatmekanisme, men normal drift er den fremtidige updater.
- Historik og kildeorden er uændret: privat DMI-buffer er normalt 60 timer og mindst 54, rå zonehistorik mindst 72, Copernicus 168 timer/target−48..+117, og modellen bruger højst 48 timers verificeret mobiliserings-/transportforløb. DMI → Baltic → AMM15 → regional DMI → Open-Meteo består; missing syntetiseres ikke.
- Ekstern cron 8348098 fortsætter watchdog-dispatch ved 04,19,34,49 UTC. GitHub-schedules bevares som reserve; concurrency og kadence ændres ikke. Den større pipelineoptimering afventer komplet opfyldning og målt leverandørtid.
- PR #257 bestod exact-head `33989875253` og blev merged som `948ba60b`. Post-merge-run `33991028274` blev stoppet under checkout. `33991952081` bestod sourcegaten, men ramte derefter den gamle hardkodede cachemiss og re-savede unødigt 2.778.397.542 byte GRIB efter skipped DMI. No-skipped-save, resolver-unit, active/candidate-kontrakt, workflowrækkefølge og live read-only API-prøve er grønne; 4.0.325 exact-head, merge, active-bootstrap/runtime, fuld produktionsgate og komplet 210/673/118 er åbne. Candidate G er fortsat offentlig.
...
- Samlet fejlklasse-review er afsluttet; præcis post-Copernicus-restfordeling afventer ny runtime. Lokal fælles CLI udskriver kun resttællere pr. prognosetime/kildemangelsklasse/policyscope efter eksisterende regional-stop; samme fejl og 118-gate bevares. Fem måltests og eksisterende closure-test er grønne. Normal `33894480067` gemte DMI-fremgang: LF/NSBS 117 VERIFIED, IDW 10/107 lokalt ubehandlede, én upstream-hale pr. collection ved 16Z. Ejeren kræver autonom samlet PR-opfølgning, grøn merge og prioriteret stor opfyldning; ventende normalruns kan ryddes, aktivt arbejde/scheduler bevares. Se residual-reviewet 2026-09-04.

- Nyeste #252-kontrol `33892021429` nåede fuld releasegate på `2fd5aa02` og fandt én statisk referencefejl: outcome-testen ledte i gammel fil frem for den fælles testliste. Den kontrollerer nu den reelt udførte liste; øvrige assertions er bevaret. Ingen ny runtime-/modelændring. Normal `33888805489` nåede videre gennem Copernicus (8.572/9.756, 1.184 rester) og stoppede derefter `DMI_GAP_NOT_FALLBACK_ELIGIBLE`; regional politik må ikke udvides. Ny samlet CI og fuld vejrakse afventer.
...
- Continuation-implementation normaliseres deterministisk som `utf8-bomless-lf-v2`; Windows og GitHub/Linux giver samme aktuelle hash `35c45f8f1f701695923b3195d60a6b8931aad4d2d08b05c93900b88401eca95c`.
- En samtidig 4.0.320-produktion kan nå at skrive predecessor-hash `082a5187f569518c0474590e924ccd17fce760d494a1da4a593de551e440cf91`. Kun denne eksakte hash, attesteret til sourcehead `7198b685f4bc9d86bd6432b049380f4279ab797c`, må genattesteres ved samme target, og kun continuation-hash samt top-/companion-generation må ændres. Alle states, bindinger, tider, privacy og øvrige felter skal være identiske; ukendte forgængere eller andet same-target-indhold stopper fail-closed.
- Continuation-, protected restore/CAS-, readiness-, installer-, binding- og release-metadata-måltests er grønne, inklusive remote predecessor → lokal genattestering → same-target update → idempotent retry samt negativ state-konflikt. Remote Supabase dry-run/apply/readback, exact-head, 673 × 118, Feggesund, live kapacitet, merge, frisk produktion og offentlig state-6-kontrol er fortsat åbne.

# NYESTE CHECKPOINT – 2026-09-03 – tofasede release- og aktiveringsgrænser

- Fase A er kode-/Candidate G-releasen: merge/push må kun etablere current moderne Candidate G på den nye eksakte `main`-head. Legacy-Candidate bruger den bevarede `candidate-legacy-maintenance`/`legacy-refresh-*`-bro. Fase A må merge før 673 × 118, så cron kan opbygge den korrigerede cache på den mergede kode. Push, schedule, watchdog og almindelig manuel vejrdrift kan ikke udføre første integrerede cutover.
- Fase B er særskilt og manual-only. Den kræver både `ravscore_integrated_first_cutover=true` og `EXECUTE-INTEGRATED-RAVSCORE-FIRST-CUTOVER-AFTER-CAPACITY-GATE`. Før DMI valideres den forseglede centrale Fase-A-identitet: current Candidate G på samme `GITHUB_SHA` med eksakt binding/manifesthash, dataset/reference, aktiv version/deployment, `sourceHead`, implementation closure og profil. Faktisk live public source/manifest/implementation genverificeres i deployleddet før begin-CAS eller central modelmutation.
- Bekræftelsen er ikke et kapacitetsbevis. Før Fase B skal 673 × 118, Feggesund, DEC-0114's live Supabase før/efter-måling, faktisk øvrig egress/lager og mindst 30 procent reserve være dokumenteret grønne.
- Preflight `33695730459` attempt 2 på `eb7cd0c6` nåede grøn DMI-terminal (`DMI_READY`/`NONE`) med 71.526 af 79.414 direkte DMI-par og 7.888 eksakte Copernicus-restpar. Den gemte 364 Copernicus-par før 20-minutters timeout, men byggede intet 673 × 118-artifact. Attempt 3 startede 01:26Z og var endnu ikke terminal ved checkpointet. Main #3913 gemte DMI/caches grønt, men stoppede i den gamle main-selector, som branchens lokale fix retter. Dette er progression, ikke releasebevis.
- Gamle forseglede planer og `PENDING`-recovery bevares; nye direkte rowless/legacy→integrated-planer er forbudt. Ingen nye controllerfelter/statusser/kinds, modelbundles, hashes, geometri eller punkter indgår. Implementeringen og de direkte tests er lokale; intet af dette delta er commit'et, pushet, merged eller live. Candidate G er fortsat offentlig.

# TIDLIGERE CHECKPOINT – 2026-09-03 – 4.0.320-modelbundles og bindinger forseglet

- Den integrerede modelkontrakt er `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`, og den separate 44-filers implementeringsbundle er `3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3`; alle otte deklarerede bindingsforbrugere er synkroniseret.
- Candidate G-rollbackens kontrakt er uændret `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`, mens dens 56-filers bundle er regenereret som `7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d`. Candidate G er fortsat offentlig, indtil exact-head, 673 × 118, merge, frisk produktion og offentlig verifikation er grønne.
- Hashene er genereret i separate Node-processer i rollback→integrated-rækkefølge og derefter ført gennem binding og release metadata. Den samlede fokuserede slutmatrix er grøn; commit/push og GitHubs exact-head er næste gate. Historiske 4.0.317–4.0.319-hashes nedenfor bevares som revisionsspor.

# TIDLIGERE CHECKPOINT – 2026-09-03 – installerbar hjemmeskærmswebapp lokalt lukket

- Den eksisterende RavRadar-webapp er færdiggjort med relative `id`/`start_url`/`scope`, `display: standalone`, ikke-transparente 192/512-pixels manifestikoner og 180-pixels Apple-touch-ikon. Forsiden og den eksisterende **Om RavRadar**-side linker samme manifest/appidentitet; service-workerens lette første installation fra DEC-0099 bevares uden ny popup eller ekstra runtime.
- Om-siden forklarer trin for trin iPhone/Safari og Android/Chrome på DA/DE/EN og gør klart, at friske prognoser kræver internet. Den eksisterende måltest kontrollerer tekst, oversættelser, manifest, links og faktiske PNG-dimensioner og er grøn lokalt.
- Dette ændrer ingen RavScore-, Candidate G-, vejr-, cache-, state-, recovery-, brugerdata-, geometri- eller punktkontrakt. Offentlig mobil-/desktopkontrol følger den samlede 4.0.320-release; funktionen kaldes ikke produktionsverificeret før da.

# TIDLIGERE CHECKPOINT – 2026-09-02 – 4.0.320 atomisk DMI-progression lokalt lukket; frisk 118-bevis åbent

- Kildeordenen er uændret og bindende: lokal DMI først, derefter Baltic og AMM15 for alle eksakte DMI-restpar. Source-stage v2 beviser pr. par det valgte produkts eget domæne og samme-par Baltic-forudsætning før AMM15; timeout eller manglende stage er ikke udtømning. Først dokumenteret udtømning af begge Copernicus-kilder må sende konkrete rester blandt præcis de otte policybundne Limfjordsdele til regional DMI-native eller et kausalt 1–3 timers derived-state hold. Holdet er state-only, eksakt tidsbundet og må hverken skabe vektor/pil, autorisere nabotimer eller bruges i historikken. Den tidligere foreløbige genvej uden disse beviser er forkastet og er ikke releaseevidens.
- Den endelige currentrevision skal klassificere alle 673 × 118 positioner med præcis én kilde, ingen overlap og ingen missing. Hold må ikke skabe ny U/V, hastighed, retning eller pil. Target−48..target−1 forbliver særskilt rådgivende historik af verificerede, faktisk hentede kildefelter uden syntese; Copernicus-bidrag er fortidige modelfelter, ikke målinger. Ufuldstændig historik fjerner ikke forecast eller femdøgnsvisning ved gyldige operationelle input.
...
- Main-run `33682062077` opdaterede DMI-bulkcachen på cirka 11 minutter og gemte DMI-zone-/GRIB-/researchcache samt bevarede Copernicus-cachen. Gammel main stoppede derefter ved exact-gap-selector før Copernicus-fill og 673 × 118; intet artifact eller deploy blev lavet. Den nye selector-/closurekontrakt er lokal og måltestet, men frisk branch-run mangler.
...
- Required DKSS-currentfelter behandles på hver eksakt time; valgfrie marine felter følger fortsat tretimersstride. Samplingpunkt, spatial-first, dybeste fælles lag, 5 km, missing og DMI-first/Copernicus-exact-gap er uændrede.
- Hvert asset behandles transaktionelt mod afgrænsede public/private/shadow-/outcome-/diagnosticstages. Kun en komplet grøn assetvalidator committer; interruption, exception eller validatorfejl ruller hele assettet tilbage uden `processedStep` eller falsk success. Én fælles controller skriver bulkcache og sidecars kompakt/atomisk ved 8 committede assets/60 sekunder; fuld clean/summarize/ocean-diagnostics og pretty write sker én gang ved terminal finalisering.
- Main-runs `33591129416`, `33608982473` og `33617117320` hentede DMI-filer, men stoppede før artifact/Pages på main-parserens string-`round`. Exact-gap-selectorens afvisning af bred Copernicus var korrekt. Den lokale numeric-only-rettelse lukker parserfejlen uden at skjule eller ommærke provenance.
- Tolv dynamiske transaction/checkpointtests, 21 WAM-bootstraptests, producent-/workflowtesten, native provenance, syntaks/diff og uafhængig slutreview er grønne uden P1/P2-blokker.
- `origin/main 28f24d1c1fc2c9d971b5acb43cf91bddd80fb950` er hentet og er allerede ancestor til modelbranchens head. Candidate G er fortsat offentlig; commit/push, exact-head, frisk isoleret 673 × 118-currentpreflight, separat Feggesund tre dele × 118-waveledger, merge, fuld produktion/aktivering og offentlig desktop-/mobilkontrol mangler. Der påstås ingen empirisk bedre fundpræcision.

# TIDLIGERE CHECKPOINT – 2026-09-02 – Feggesund direct-first wave proxy
...
- Begge naboer skal levere komplette, direkte, samme-time/samme-run, provenanceverificerede DMI `wam_dw`-tuples. Højde sammenlægges som `sqrt(mean(Hs²))`, periode vægtes med `Hs²`, og FROM-retning sammenlægges cirkulært med `Hs²`-vægt. Resultatet er et energikonsistent ingeniørestimat med `LOW`/`MODERATE`/`HIGH` uenighed, ikke lokal batymetri, surfzonemodel eller ravkalibrering.
- Den offentlige kompaktprojektion bærer kun kildeklasse, usikkerhed og notice-id. DA/DE/EN viser, at bølgerne er estimeret fra to samtidige naboområder; advarslen forsvinder automatisk ved direkte lokal input. Enhver mode-/zone-/public-score, tur eller observation, som faktisk bruger proxyen, har `calibrationEligible=false`, også hvis historikkvaliteten ellers er `FULL_HISTORY`.
- Releasegaten kræver et privacy-sikkert, hashbundet 3 × 118-bevis: alle 354 positioner er `DIRECT`, `FEGGESUND_TWO_NEIGHBOR_WAVE_INTERPOLATION` eller `MISSING`, direct + proxy = 354 og missing = 0. Beviset indeholder ingen koordinater, rå kildeværdier, rå U/V eller private payloads.
- Undtagelsen gælder kun operationelle bølger. Den skaber eller udfylder ingen historik, current, state-/recovery-backfill, blander ingen deployed/progressiv cache og flytter ingen zone, geometri, kystnormal eller land-/vandpunkt. Alle generelle forbud mod nabolån består.
- Slutbundles er forseglet og lokalt måltestet for 4.0.320: integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3` over 44 filer/8 deklarerede forbrugere og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d` over 56 filer. Exact-head `33577887262` stoppede sikkert i Candidate G-rollbackens public stage, fordi `FULL_HISTORY` med `calibrationEligible=false` ramte den integrerede calibration-ceiling. Fem-validator-rettelsen og fire måltests er grønne. Exact-head `33580532775` passerede hele model-/rollbackkæden og stoppede kun på to forældede DMI-testassertions; den runtime-neutrale testrettelse er grøn 21/21. Restkontrollen rettede derefter kun en gammel syntetisk live-current-fixture til den gældende downloadindholdsbinding; testen er grøn, mens hydreringstestens lokale WindowsApps-Python-fejl er afvist med bundled-Python-bevis. Ny exact-head, slutligt 118-bevis, merge, frisk fuld produktion og offentlig browserkontrol er endnu ikke bevist. Candidate G er fortsat offentlig.
- Implementeringen findes lokalt i den samlede 4.0.319-kandidat; målrettet slutvalidering, exact-head, frisk produktion/releasegate og offentlig desktop-/mobilverifikation er åbne. Candidate G er fortsat den eneste offentlige model, indtil hele den atomiske cutover er bevist. Der påstås ingen empirisk bedre fundpræcision.
- Den afsluttende lokale releasegate fandt to stale tekstmarkører fra før de stærkere DMI-refaktoreringer. Kun gatebindingerne er justeret til den faktiske cache-key/row/native-time-kontrol og nested `resetWaveRowCount`-registrering; runtimekode, geometri, punkter og dataadfærd er uændret.

# TIDLIGERE CHECKPOINT – 2026-09-01 – DEC-0114 integrated-first, measured history afterward

- Ejeren har erstattet den tidligere releaseblokering på fuld ældre WAM-/Candidate G-rollbackhistorik: `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `6.0.0` skal online, når dens egne direkte input, exact operational WAM bridge og coherent `productionTarget..+117 h` er grønne for 210 zoner/673 kystdele og begge modes. Den afgrænsede implementering og måltests er lokalt grønne; modellen er ikke exact-head-verificeret, merged eller live.
- Historisk `NO_COHERENT_RUN`, `MISSING_HOUR` eller `INTERPOLATION_GAP` ved `genuine-cold-start` er `HISTORY_INCOMPLETE`, ikke en releaseblokering efter operational-gaten. Migrationens coherent 40-timers WAM-bevis forbliver strict og migration-only. Proveniens-, celle-, felt-, retning-, run-, register- og direct current/future-fejl stopper fortsat fail-closed.
- Candidate G-rollback-oraklet fortsætter privat som `BUILDING_MEASURED_ONLY` gennem den beskyttede runtimebundle. Schema-4-checkpointet forbliver uændret 673/673 READY-strict; checkpoint build/save/protected-publish er lokalt implementeret som eksplicit `NOT_APPLICABLE_DURING_MEASURED_WARMUP`, aldrig tilfældigt skipped eller ghost proof.
- Første integrerede cutover og same-binding maintenance må fortsætte under measured warmup. Manuel Candidate G-rollback/return er utilgængelig, indtil en senere frisk produktion forsegler 673/673 READY. Før/during cutover bevares source/PENDING-reconciliation; efter activation bevares same-model integrated runtime/state-less recovery. Tilstedeværende invalid/tampered state er fortsat fatal.
- Ved gyldige direkte input vises hele 118-timersfladen med numerisk `HISTORY_INCOMPLETE`, bounds og tydelig DA/DE/EN-advarsel. `calibrationEligible=false` føres lokalt gennem controller, trip/observation, admin, hydrering og reconciliation; kun en forseglet, hashbundet 210/673/privacy-grøn audit med alle aktuelle modes `FULL_HISTORY` må sætte `true`. Ingen syntese, rekonstruktion, interpolation, carry-forward eller nabo-/kystdelslån er tilladt.
- Copernicus-current schema 3 forsegler hard `OPERATIONAL_COMPLETE` på DMI-first-gapmatricen `target..+117`; `target−48..target−1` er en særskilt rådgivende historikmatrix af verificerede, faktisk hentede kildefelter uden syntese. Copernicus-bidrag klassificeres som fortidige modelfelter, ikke målinger; matricen må være ufuldstændig og kun påvirke historikkvalitet/bounds.
- En isoleret branch-valgt `operational_118_preflight` er lokalt workflowtestet: DMI→persistente saves→payloadfri terminalgate→operationel selector/rangechecker→`update:weather`→integreret 210/673/118-audit. Den kører ingen source/full/release/admin/Pages/deploy/browser og uploader kun én filtreret syv-dages safe rapport.
- Historisk 4.0.320-regel, supersederet af 4.0.329: DMI-cachegenbrug kopierede hver vejrtime atomisk som hel række til en manglende/strukturelt tom time. Gældende drift må alene merge exact-proof-gated U+V+`sources.current` komponentatomart; wave/wind/øvrige fields+sourcer+`processedSteps` bevares, og halv/uattesteret donor afvises. Alle tre saves ligger fortsat før terminalgaten, og strict promotion kræver `DMI_READY` plus strict current anchor.
- Runs `33510636195` og `33512163102` behandlede HARMONIE/WAM, men nul DKSS-trin. `DMI_STRICT_CURRENT_ANCHOR_MISSING` beviser derfor ikke en bred DMI-mangel. Cachelinjen førte tilbage til det negative run `33498108421`, men den beviste rodårsag var lokal: en preferred progressionsrun kunne blive afvist som stale uden skift til en nyere moden run, preflightens donor var en kopi af samme progressive cache, og gamle DKSS-`processedSteps` kunne undertrykke nødvendig genbehandling uden strict anchor.
- Run `33520738058` på `3a26ba0c` nåede producenten, men hard gate stoppede fortsat med `DMI_STRICT_CURRENT_ANCHOR_MISSING`. Den yderligere lokale årsag var faseuoverensstemmelse: tidlig cache-health kunne godkende en cache, som den senere autoritative sampling-/gridvektoroprydning korrekt fjernede current fra. Working-tree-fixet gør nu `coastal_part_current_cache_reusable` fasekonsistent med det autoritative `samplingPoint` og top-level `current-u`/`current-v`-gridpar. Direkte provenance-, bulk- og schedulertest samt Python-syntakskontrol er grønne lokalt; ny eksakt GitHub-run er endnu ikke grøn.
- Den lokale rettelse binder ét eksakt jobtarget før DMI og genbruger det gennem hele snapshotbeviset, også over et UTC-timeskifte. På dette historiske trin blev en ældre preferred run kun beholdt ved kendt cadence og højst ét lag; denne delregel er supersederet af DEC-0116. Uden strict anchor står DKSS først i den normale collection-loop, og DKSS genbehandles. En deployed Candidate G-DMI-donor er valgfri og bruges kun efter strict kompatibilitetskontrol; frisk officiel DMI fortsætter uden den. Ved første cutover ligger den særskilt checkpointede WAM-historikbootstrap fortsat før den normale seks-collection-loop og kan fortsætte over flere forsøg. I loopet står DKSS foran WAM alene, mens strict anchor mangler; normal drift havde på dette historiske trin to collections; denne del er supersederet af DEC-0116.
- **Historisk supersession 2026-09-06:** DEC-0116 gjorde cadence-/én-lag- og to-collection-reglen historisk og indførte fælles kandidat, partial-only 96h-retention og tre DKSS-collections. 4.0.329 supersederer siden retentionen med nyeste modne native-complete run og exact retained pair/source-proof; tre collections består.
- `33498108421` og `33520738058` er negative run-/cachelineagebeviser, ikke bevis for generel upstreamfejl. DMI er primær i både Candidate G-drift og den nye integrerede model. Copernicus må kun supplere eksakte dokumenterede DMI-tuples efter grøn DMI-terminalgate og må aldrig erstatte en systemisk DMI-fejl. Et grønt frisk 118-timers preflightbevis mangler fortsat.
- Denne sektions oprindelige Feggesund-pensionering er supersederet af ejerpræciseringen 2026-09-02 øverst: frisk produktion skal bevise 3 × 118 accepterede wave-positioner under direct-first/two-neighbor-policyen, med direct + proxy = 354 og missing = 0. Ingen geometri, zoner, kystnormaler eller land-/vandpunkter ændres.
- DEC-0114 er den autoritative operationelle supersession. Lokale måltests for warmup/checkpoint/recovery/calibration, schema-3 operational current, isoleret preflight og DMI atomic-cache/terminalgate er grønne. Disse er ikke exact-head-, preflight-run-, merge-, produktions- eller browserbevis; Candidate G er fortsat den eneste offentlige model.

# TIDLIGERE CHECKPOINT – 2026-09-01 – PR #244 merged; legacy WAM-cacheovergang og watchdogkø lokalt rettet

- PR #244's opdaterede exact-head-kørsel `33452730102` bestod, og ændringen blev merged som `origin/main 27906d7d83883622d87d66b141869302b016d6c6`. Dette lukker PR #244's egen kildeevidens, men er ikke et frisk offentligt produktionsbevis for den efterfølgende remediation.
- Den første relevante `workflow_dispatch`-produktion på den mergede head, `33471276238`, stoppede sikkert før beskyttede cache-/stateskrivninger, artifact, Pages og deploy. Candidate G og den offentlige side blev derfor ikke ændret. Den primære fejl var payloadfri `MISSING_CELL`: den fastlåste legacy Candidate G-WAM-cache stammer fra før kravet om fuld same-cell-proveniens og mangler derfor de nye cellefelter. Den sekundære Copernicus-fejl med nul strikte par ændrer ikke denne klassifikation og giver heller ingen tilladelse til at lempe validatoren.
- Cacheovergangen håndteres nu som et afgrænset, genopbyggeligt cold-cache-miss: kun de private `PART::`-bølgekomponenter samt deres bølgekilde/-summary nulstilles, progressionen checkpointes, og de officielle målte DMI-data hentes igen. Strøm, vind, vandstand, temperatur, identiteter og øvrig cache bevares. Der syntetiseres, interpoleres eller infereres ingen vejrdata, og ingen geometri, kystnormal eller land-/vandpunkt ændres.
- Candidate G-maintenance er isoleret fra den integrerede first-cutover-bootstrap. Kun den eksplicitte `integrated-cutover`-action må aktivere den integrerede resolver, cold-start-reserver, force-refresh/WAM-mode og first-cutover-miljø; schedule-, watchdog- og manuel Candidate G-drift må ikke starte modelcutover indirekte.
- Watchdog-racet er lokalt lukket med en reel kø (`queue: max`), en fail-closed, ufiltreret runhistoriklæsning med lokal main-filtrering og en ny kontrol umiddelbart før dispatch. En aktiv eller queued main-produktion må derfor ikke annulleres eller duplikeres på baggrund af et forældet branch-filtreret snapshot.
- De målrettede DMI/WAM-cache-, bootstrap-, Candidate G-isolations- og integrerede generatorregressioner er grønne. Den strikte same-cell-validator er uændret og skal fortsat stoppe, hvis den målte genhentning ikke kan etablere komplet provenance.
- Den aktuelle remediations **egen** exact-head, sikre merge, friske fulde produktion/deploy og offentlige current-/femdøgns-/desktop-/mobilverifikation er åbne. Candidate G forbliver eneste offentlige model, indtil hele kæden er bevist.

# TIDLIGERE CHECKPOINT – 2026-09-01 – vejrpipeline lokalt rettet; releasebevis åbent
...
# TIDLIGERE CHECKPOINT – 2026-08-31 – PR #242 merged; first cutover nåede DMI/WAM
...
- Endelige lokale bundle-hashes er integrated `e880d5425e6f7b93d8afc99cddf491e58ad5a4a2ab055f8e4455193609c90a73` og Candidate G-rollback `4ccc2081982677aadbb47a5ee7d6f2b99fdcb7e42113e73029d5c60323a5ee96`.
- Candidate G/4.0.316 er fortsat den eneste offentlige model. Opdateret exact-head, merge, frisk 4.0.319-produktion og offentlig desktop-/mobilbrowserverifikation er stadig åbne gates.
# Current truth – gældende projektviden

## 2026-08-31 – frisk Candidate G-base og lokal 4.0.318-slutprotokol

- Nyeste offentlige Candidate G-bevis er produktion `33368963614` på uændret `origin/main 8c03e25d`, afsluttet grønt 2026-08-31 07:44:39Z gennem build, frisk fuld validate, releasegate, protected sync/artifact og Pages. Datasæt `rr-20260831074016-210` (genereret 07:40:16Z, gyldigt til 2026-09-05 03:00Z) er komplet 210 zoner/673 kystdele. Det er kun bevis for den offentlige 4.0.316/Candidate G-baseline, ikke for state 6.
- PR #236 gendannede exact 4.0.316/Candidate G-tree på `origin/main c58deb78`. Exact-head `33342157517` og post-merge `33342219152` var grønne gennem fuld validate/releasegate/artifact/Pages. Normal produktion `33345476979`/`rr-20260831010337-210` var første grønne recoverybevis. Et tidligere external-watchdog-`workflow_dispatch`, `33347230240`, bestod ligeledes fuld DMI, validate, releasegate, storage gate og Pages og publicerede `rr-20260831012407-210` komplet 210/673 med `evidenceTrust=VERIFIED_ONLY` og `syntheticSampleCount=0`. Candidate G har 0/210 aktive zoner på grund af utilstrækkelig historikmemory; det er ærlig Candidate G-status, ikke et datahul eller state-6-bevis. Candidate G er fortsat eneste offentlige model; visuel desktop-/mobilkontrol er stadig åben.
- Kørsel `33343469247` og genforsøg `33344823000` stoppede begge sikkert på en forbigående HTTP 503 i build/prepare; deploy var skipped, og offentlig Candidate G blev ikke ændret. Den snævre bounded-retry-hotfix bestod PR #237 exact-head `33352520408`, blev merged som `8c03e25d`, og backend-/fuld produktion `33352661061`/`33352634365` var grøn.
- 4.0.318 er ikke offentlig. Lokalt er workflowet opdelt i eksplicitte orchestrator/build/deploy-roller, alle 40 direkte workflowreaders er migreret, og role-aware workflowkontrakterne er grønne. Public-integrated-kæden er grøn for 210/673 og 78 browsermoduler; profil, cutover og alle 8 bindingsforbrugere er grønne, assistentens state-6-fixture ligger nu fast i `validate:source`, og slutreviewet fandt ingen P0/P1. Fuld slut-`validate:source`, exact-head, merge, frisk state-6-produktion, Feggesunds policybundne 3 × 118 og offentlig desktop-/mobilbrowserkontrol mangler.
...
- Den endelige lokale 4.0.318-binding er regenereret og grøn: `modelContractSha256=778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7`, `modelBundleSha256=e880d5425e6f7b93d8afc99cddf491e58ad5a4a2ab055f8e4455193609c90a73`, 43 transitive filer og 8 bindingsforbrugere. Den separate Candidate G-rollbackbinding er `modelContractSha256=c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`, `modelBundleSha256=4ccc2081982677aadbb47a5ee7d6f2b99fdcb7e42113e73029d5c60323a5ee96`, 55 filer.
...
## 4.0.319 – lokal first-cutover-hærdning efter sikker 4.0.317-stop; Candidate G er fortsat offentlig

- Den offentlige sandhed er fortsat 4.0.316/Candidate G. 4.0.317-kilden bestod PR #235 exact-head `33332106627` på `30306a51c4e360c5054368f1b0167e3aaa3862ee` og blev merged som main `a584d1cf1a53692b10b0f01244eab4fb91ca89b1`, men den integrerede model blev ikke aktiveret.
- Push-produktion `33333490853` stoppede ved den aggregate Candidate G-bootstrapresolver: den offentlige kilde havde 0 `READY` og 673 kanoniske warmupstates. Resolveren krævede fejlagtigt READY migration i stedet for at skelne gyldig source-attestation fra migrationsegnethed. Stoppet skete før DMI/Copernicus, scorebygning, protected state/cache/checkpoint/adminskrivning, artifact, Pages og activation; WAM-gatefejlen var kun en efterfølgende kaskade, og WAM kørte ikke.
- 4.0.319 validerer først manifest, conditions og det eksakte publicerede Candidate G-source-register samlet og opbevarer registeret isoleret. Efter central adminhydrering materialiseres det aktive register separat. Alle 673 source-states valideres mod source-stateKey; aktiv samplingkontekst valideres for sig. Ingen geometri, zoner, kystnormaler eller land-/vandpunkter flyttes.
- Når alle 673 er READY, source/active-kontekst er identisk, og migrationstarget er fælles, vælges `candidate-g-migration`. Når alle states er kanoniske, men mindst én er legitim warmup/non-READY eller source/active-kontekst er ændret, vælges national `genuine-cold-start` ved produktionstarget. Invalid, tampered, ukendt, fremtidig eller bindingsforkert source stopper fortsat fail-closed.
- Cold start kræver aggregate `source_validated=true`, må ikke maskere en afvist integreret source og replay'er kun de faktisk tilgængelige private, verificerede 0–48 timer plus reel target. State 6 bliver ærligt `HISTORY_INCOMPLETE`; der dannes ingen syntetisk, interpoleret, lånt eller carry-forwardet historik.
- Candidate G-rollback bygges ved cold start gennem en separat eksklusiv measured-only vej. Den må ikke hybridiseres med legacy/continuation og kan kun blive READY companion efter sin egen fulde faktiske 48-timerskontrakt. Efter DEC-0114 stopper non-READY checkpointforsegling og manuel rollback-readiness, men ikke selve first cutover, når privat `BUILDING_MEASURED_ONLY`-runtime, eksplicit checkpoint-N/A og alle integrerede direct+118-h-gates består.
- Bootstrapoutput er payloadfrit og begrænset til mode, `target_hour`, 673 og source-attestation. UTC-timen normaliseres til `YYYY-MM-DDTHH:00:00Z` og testes direkte gennem den produktionskritiske Python-parser. Progressive reelle DMI-zonecachefiler gemmes efter ikke-annullerede delvise forsøg, men giver aldrig alene publicerings- eller rollbackbevis.
- **Historisk 4.0.319-initiatorregel, supersederet 2026-09-03:** bot-/watchdogrun `33334709027` stoppede på den gamle operationelle actionresolver og deployede ikke; pilotrun `33335078275` stoppede ved den private DMI-gapmatrix før pilot/Copernicus/artifact. Begge røde runs er sikre stop og forklarer de automatiske GitHub-mails. Den gældende regel er, at push og al almindelig vejrdrift vedligeholder Candidate G i Fase A; kun særskilt manuel Fase B må initiere den integrerede cutover.
- DEC-0113 og de fem nye aktive first-cutover-krav er bindende. Den endelige 4.0.319-modelbinding er regenereret og lokalt grøn. Version-/docs-/testlukning, egen exact-head/merge, frisk fuld produktion/releasegate/artifact/Pages samt offentlig 210/673/current/femdøgns- og desktop-/mobilverifikation udestår. Der må ikke påstås empirisk bedre fundpræcision.
- Det efterfølgende 4.0.317-afsnit er nu det præ-hærdningscheckpoint, som forklarer den mergede kandidat og dens lokale beviser; det må ikke læses som aktuel release-status.

## Historisk 4.0.317 – lokal integreret RavScore-releasekandidat, aldrig offentlig

- Den lokale appversion er 4.0.317. Modellen er `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0`, med parameterkontrakt `778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7` og reproducerbar 43-filers implementeringsbundle `74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`.
- Candidate G-migrations-/rollbackkilden er forseglet til den faktiske produktionsverificerede 4.0.316-baseline: head `49dd4cb454656bdf629e5df760176705e38d2cb0`, tree `975c3e9432cea7780564ffd56766bc1f0a0a9763`, central switch `RAVSCORE-PROFILE-SWITCH-4.0.316`, source contract `2f888a16190e9e43e44536536029f1b0021a1b850195524aa2312664ca74810b` og 53-filers source closure `a366b4a64fc3ccc8f1b94f3fed24b3ce03ea23d906396bc8bea183338c5d2606`. PR-, build- og deploygaten henter den eksakte pinned source commit før attestationen.
- Historikmangel med gyldige direkte input giver fortsat numerisk `HISTORY_INCOMPLETE` lower/upper og synlig auto-forsvindende advarsel; direkte inputmangel er `UNAVAILABLE`. Sekundære bedste tider og alternative kystdele fører nu deres egen kvalitet, bounds, coverage og reasons: incomplete vises med kompakt markør/interval, unavailable uden score.
- Point-staging schema 3 er dual-state og atomisk: eksakt integreret state 6 og eksakt Candidate G-rollback-companion bindes med separate hashes og fælles parhash. Companionen kan kun vælges af `POINT_ACTIVATION`; cross-part, cross-target, mixed-generation, tamper og privacy-afvigelser afvises. Den forseglede Candidate G-binding er `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`fd3f7e70ec3706818c153c26140ae592e4f0ad2acc6c157183984689f74a2207` over 54 filer. Ved samme reference sammenligner checkpoint publish/restore også `generationSha256` og hele den validerede companion før mutation; divergens afvises.
- Native-hold-slutrevisionen fandt ingen P1/P2/P3. Kun eksakt ejerautoriseret regional native cadence kan attestere højst tre timer; almindelige spring over én time bliver unknown bounds, t0→t3 kan ikke retroaktivt lukke et hul, og normal DMI/Copernicus-evidens tilbagekalder hold. Syv målrettede tests er grønne.
- Produktionsworkflowet udsteder én payloadfri terminal `NOOP`, `DEFERRED`, `BUILT`, `DEPLOYED` eller `FAILED`; terminalerne kan ikke bære bevis fra senere faser, `DEPLOYED` kræver den fulde offentlige kæde, og `FAILED` holder runnet rødt.
- Historisk 4.0.317-bevis: den daværende proportionale matrix var grøn. Historical/recovery/outcome/P2-deltaet i 4.0.319 kræver en ny fuld slutmatrix og nye digests.
- PR #235's første exact-head-run `33324239464`/job `99291456658` stoppede korrekt i `validate:source` på en forældet state-6-feedbackfixture; der skete ingen merge, intet produktionsbuild/artifact og intet deploy. Den efterfølgende samlede lokale gennemgang fandt desuden andre stale public-fixtures, en aktiv adaptiv observationsskrivning og en ændret bundlelukning. Remediation har pensioneret ny-skrivningen, opdateret state-6 quality/bounds-/payload-/forklaringsregressionerne og synkroniseret alle bindingsforbrugere til bundle `74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`.
- Den remediatede arbejdsgren har derefter bestået en ny fuld lokal `validate:source` fra RDKS/security gennem 210/673-runtime, rollback, DMI/Copernicus, privacy og den indlejrede `release:gate` i kildekontekst. Dette er lokalt kildebevis, ikke frisk centralt hydreret `npm run validate`/produktionsreleasegate, exact-head- eller offentlig releaseevidens.
- PR #235's anden exact-head `33329919843`/job `99306529711` på `7bc848610794b87f62a6a3763564ca46a0d7528e` bestod de forudgående model-, public-runtime-, rollback- og DMI-dele og stoppede derefter fail-closed ved manglende `xarray` i `test:copernicus-target-registry`. Rodårsagen var CI-dependency-paritet, ikke RavScore/runtime/data: sourcegate-workflowet udelod de ejede geometry-/Copernicus-requirements. Der skete ingen merge, produktionsbuild/artifact, deploy eller offentlig ændring.
- Alle tre workflows med `validate:source` installerer nu DMI-, geometry- og Copernicus-requirements umiddelbart før gaten under samme betingelse. Regressionen håndhæver dette på hele workflowinventaret, PR-timeout er hævet 30→45 minutter, og målrettede workflow-/Copernicus-/modelbundletests er grønne. En ny exact-head er fortsat nødvendig.
- Slutrevisionen bandt releasegaten til faktisk arkitektur: atomisk to-dokument-RPC/SQL-CAS inklusive service-role-rettigheder, eksakt native-time DMI-matrix -48..+117, dynamisk 15/45-minutters watchdog, eksplicit public-fallbackafvisning og faktiske DA/DE/EN-tekster. Fresh-startup-regressionen tester schema-4-envelope, bytes/hash/modelbinding og fail-closed fallback; SQL/RPC-installationsparitet er obligatorisk gennem `test:ravscore-integrated-profile`/`validate:source`. Krævede fil-/JSON-fejl aggregeres i én slutrapport og har en isoleret sourcegate-test. Den fulde lokale `release:gate` er grøn efter justeringen, men er ikke exact-head- eller produktionsbevis.
- Frisk final fetch bekræfter `origin/main=49dd4cb454656bdf629e5df760176705e38d2cb0`; der er ingen nyere main-delta eller ejerbeslutning at integrere ved dette checkpoint.
- Resterende releasegates er: commit/push af dependency-pariteten til PR #235 og ny exact-head, merge, frisk fuld produktion/releasegate/artifact/Pages, Feggesunds policybundne 3 × 118 part-level, offentlig 210/673/current/fem døgn og desktop-/mobilbrowser. Indtil hele kæden er grøn, er offentlig 4.0.316/Candidate G eneste offentlige model.
- Geodata er uændret bortset fra den særskilt autoriserede rene topversionssynk 4.0.316→4.0.317; geometri, zoner og land-/vandpunkter er ikke ændret.

## Offentlig produktionsbaseline – 4.0.316 frisk measured-only primary må ikke kræve en gyldig ældre fallback

### Lokal same-version Edge-verifierhotfix 2026-08-31

- Main-head `c58deb7827848918d58d68ba9969bfcb923311e5` er kildegrundlaget. Runs `33343469247`, `33344823000` og `33348745681` stoppede alle ved HTTP 503 i det normale ikke-indloggede turlogkald; det tredje efter grøn vejrproduktion, 210/673, fuld validate og releasegate, men før deploy. Kodeaudit viste, at normalruten forbruger rate limit før auth; den er derfor ikke no-write og må ikke genkøres. Fejlene modbeviser ikke selve 401-responsekontrakten.
- Efter de tre ovenstående hændelser oprettede grønt external-watchdog `33351078871` præcis én ny produktion `33351090164`. Den bestod vejr/DMI/Copernicus, 210/673, fuld validate, releasegate og datagates, men gammel step 69 ramte igen normal unauthenticated `trip-log` HTTP 503 før deploy. Dette fjerde uafhængige datapunkt bekræfter transient-gatewayfejlen; det er ikke et livebevis for den lokale rettelse.
- Den lokale patch genkører kun OPTIONS og en privat HMAC-signeret, statefri `trip-log`-GET uden body ved 429/502/503/504 eller bundet timeout/transportfejl, højst tre forsøg med 0/250/750 ms. Helperen ejer fast signature-path, per-attempt cachebuster og no-store-descriptor. Gammel/current normalrute afviser GET med ikke-transient 405 før rate limit; normal `trip-log` rate/auth/storage og alle observation-/kvalitetsprober forbliver single-shot.
- Den signerede probe ligger før rate/auth/storage og returnerer kun fast minimal 401 `LOGIN_REQUIRED` med kontrakt-/modeheader. Den beviser Edge-liveness og response-kontrakt, ikke udført auth eller storage-read.
- Samme aktive pre-write gate kører derfor også eksisterende Worker-health og en HMAC-signeret `/v1/trips/count`; dette er det faktiske live D1-readiness-bevis. Secrets bindes kun via step-env.
- Count-readiness har en separat fast `POST {}`-helper med samme tre snævre transientforsøg. Usigneret skal være 401; signeret får frisk timestamp/HMAC per forsøg og skal være 200 med `ok=true` og ikke-negativt safe-integer-count. Workerens count er ren `SELECT`; kun dens interne readfejl returnerer fast datasikker 503. Ikke-transiente status-/bodyfejl prøves aldrig igen.
- Signeret 200 accepteres kun med exact-key-body `{ok, trip_count}`; ekstra felter, malformed body eller ugyldigt count stopper straks og forbliver uden payloadlog.
- Ikke-transient status/header/body stopper straks, udmattelse er rød, og body/cause logges ikke. Versionen forbliver 4.0.316, fordi ingen offentlig app, Pages-payload, model/state, turdata, geodata eller produktionsartifact ændres. Kandidaten er kun lokal; ingen push, PR, merge eller livepåstand findes endnu.
...
- DEC-0112 fastlægger samtidig state-6-modellens numeriske kontrakt: `HISTORY_INCOMPLETE` scorer konservativt hele current+femdøgnsfladen, når direkte input er gyldige, og fører lower/upper, årsager, dækning, auto-forsvindende DA/DE/EN-advarsel samt `calibrationEligible=false`. Manglende direkte input er separat `UNAVAILABLE`.
- Den historiske v1-outcome lukkede grøn-no-op/skipped-semantikken før recoveryudvidelsen. 4.0.319's gældende releasekrav er det payloadfri schema `ravradar-production-workflow-outcome-v2` med terminalerne `NOOP`, `DEFERRED`, `BUILT`, `DEPLOYED`, `FAILED`, historical actions og recovery writer/finalizer/gate. Kode, releasegate og måltests er lokalt synkroniseret; exact-head-, produktions- og offentligt slutbevis udestår.
- 4.0.316/Candidate G er offentlig. Det friske datasæt `rr-20260830091913-210` havde 210 zoner/673 kystdele, men 0 aktive zoner og 210 `UNAVAILABLE`, fordi sammenhængende strømhistorik var utilstrækkelig. Det er regressionsevidens for Candidate G's all-or-nothing-historik, ikke produktionsbevis for state 6.

## Historisk 4.0.317-modelarbejde – state 6 og konservativ score ved ufuldstændig historik

- Den aktive lokale 4.0.317-releasekandidat er én model: `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-historybounds12d-lastmileewma4-tail40-atten15-v5`, komponent `ravscore-components-huntability-delivery-mobilisation-bounds-v5` og forklaring `ravscore-explanation-integrated-bounds-v5`. Den fulde binding bruger kontraktdigest `778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7` og 43-filers bundledigest `74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`.
- Tre kvaliteter er bindende: `FULL_HISTORY`, `HISTORY_INCOMPLETE` og `UNAVAILABLE`. Gyldige direkte scoretimeinput med ufuldstændig historik giver en numerisk konservativ lower-bound-score og en medfølgende upper bound. Manglende/ugyldigt direkte strøm-, bølge- eller jagtbarhedsinput giver fortsat `UNAVAILABLE`/`null`; ingen interpolation, carry eller zonelån er tilladt.
- Feggesund/`DK-B05-11` har 118/118 wave-missing i sanitiseret parent-zone-forecast `rr-20260830104132-210`, men de tre aktive part-id'er findes, har `marineCoverage=full`, og Candidate G-current er tilgængelig for strand/waders. Denne historiske åbne hypotese er nu afløst af den faste direct-first-kontrakt øverst: kun hele local wave-tuple missing må bruge begge verificerede DMI-WAM-naboer, og et privacy-sikkert 3 × 118-bevis skal være komplet før release.
- Strømmens aktive scorevindue er fortsat 48 timer; ukendte historiske intervaller omsluttes med stærkest tilladt ud-/indstrømningsbane under samme 0,03/0,15, +10/-8 og 24/48-kerne. Bølgemobiliseringens usikkerhedshale lukkes efter 288 timer med højst `0,46875` råt scorepoint tilbage, og last-mile-momenterne efter 40 timer med højst `1/1024` udeladt andel.
- 168 timers datasikker researchretention er kun replay-/auditgrundlag og har eksakt ingen scoreeffekt. Den forlænger ikke 48-timers currentvinduet og er ikke skjult syvdøgnshistorik i RavScore.
- Aktiv spatial current bruger fortsat det provenanceverificerede U/V-par ved hvert eksisterende centralt godkendte havpunkt og den uændrede kystnormal. Den private roterende 0/5/15-km-flerlagscache bevares score-neutralt til offline lag-/afstand-/tidslagsanalyse; ydre afstande og ekstra lag bliver ikke lagt oven i RavScore uden landsdækkende dækning og en valideret kausal rum-/tidskobling. Det kræver ingen ny fetch eller fler-dages warmup ved state-6-cutover, og ingen punkter flyttes.
- Schema 5 var en lokal, aldrig-offentlig releasekandidat. Kun en eksakt `READY`-state med de fastlåste historiske hashes kan migreres én gang gennem `integrated-schema5-ready-point-to-schema6-history-bounds-v1`; Candidate G migreres direkte gennem `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema6-v5`. Aktiv privat runtime, cache, checkpoint og recovery er schema 6. Rollback er `integrated-schema6-to-candidate-g-schema2-v3`.
- Kvalitet, bounds og kalibreringslås føres gennem startup/detaljer/hashes, aktuelle/fem døgn, rangering/beste tidspunkt, strand/waders, DA/DE/EN, lokal/Edge-assistent, evidens/faste svar, konto/tur/observation, admin/ekspert, central profil, scheduler, recovery/rollback, audits og releasegates. Trip-/observationspersistens er implementeret gennem klient, immutable snapshot, DTO, Edge og SQL. Sekundære tider/dele bevarer deres egen kvalitet og viser incomplete som markør/interval, mens unavailable aldrig viser score. Checkpointparret READY Candidate G-companion og dual-state point-staging er serializer-/protected-storage-/tamper-/privacy-testet. Numerisk score rangerer først; `FULL_HISTORY` vinder kun et eksakt scoretie.
- Candidate G er fortsat den eneste offentlige model og der findes ingen offentlig shadowmodel. Historisk 4.0.317-matrix var grøn; 4.0.319-slutmatrixen er åben. State 6 er ikke udgivet før exact-head, merge, frisk fuld produktion/deploy, Feggesunds policybundne 3 × 118 og offentlig mobil-/desktopkontrol af 210/673 og alle kvalitetsflader er bevist. Offlinebevis dokumenterer teknik og fysisk sammenhæng, ikke empirisk bedre fundpræcision.
...
## DEC-0110-modelarbejde – integreret RavScore er ikke udgivet; offentlig Candidate G består

- Den offentlige sandhed er 4.0.316 med Candidate G. `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`/state `6.0.0` er lokal releasekandidat under DEC-0110/DEC-0112 og bliver først offentlig efter grøn exact-head, merge, frisk fuld produktion/deploy og offentlig 210/673 desktop-/mobilkontrol. Den aldrig-offentlige state-5-kandidat er kun en eksakt migrationskilde.
- Releasekandidaten bruger det afgrænsede kausale energivægtede wave-approach-led med fire timers halveringstid og en ældre hale, én DMI `FROM`→`TOWARD`-rotation og én `delivery=supply×factor`-anvendelse med faktor 0,85–1. Bølger kan ikke skabe/øge supply og kan højst fjerne 7,5 rå totalscorepoint før slutafrunding; vist RavScore kan derfor ændres 8 point. Fysisk levering forbliver uopløst, DDM er kun statisk kontekst, Rainville 2026 kun buoyant-object-analogi, og der påstås ingen empirisk højere fundpræcision.
- Ægte state-6-cold start bruger `bounded-private-48h-history-cold-replay-v3`, registrerer forventede/komplette/ukendte kausale positioner og forbliver `HISTORY_INCOMPLETE` ved alle 0–48 timer. Selv 48/48 (`VERIFIED_CAUSAL_HISTORY_WINDOW`) bliver først `FULL_HISTORY` efter 288 timers kausal tail closure eller attestert migration/continuation; kortere/gappede forløb er `UNKNOWN_HISTORY_INTERVAL`. Rollback bruger samme target uden dobbelt credit og kræver ved checkpoint-only recovery en særskilt beskyttet READY Candidate G-companion fra samme generation. Under manuel Candidate G-rollback må kun en eksakt `READY`/`memoryReady` Candidate G-runtime projicere sin egen mode-score som exact full-history med collapsed bounds/coverage 48; mismatch stopper, og `calibrationEligible=false` består. Nøddrift er kun same-model, komplet, atomisk og hashbundet i højst 72 timer; cross-model fallback og interpolation er forbudt.
- DEC-0109/DEC-0111 og morgenhullets aldrig anvendte operation er alene historisk incident-/trustdokumentation og indgår ikke i 4.0.317-releasegates. De efterfølgende 4.0.311–4.0.314-afsnit er revisionsspor.
...
- PR #230's første head `e8f579ba` fejlede source-run `33276791132`/`99164804850` kun i testharnessen, fordi normale child-cases arvede GitHub-runnerens `GITHUB_ACTIONS=true`. Harnessen isolerer nu miljøet eksplicit. Den syntetiske 210/673-regression består både med og uden nedarvet Actions-flag; ny exact-head afventer, og ingen produktion eller data blev rørt.
...
- Kun lineær interpolation af Candidate G's allerede afledte signerede kystnormale `strength` mellem eksakte run-/artifactbundne ankre er tilladt. Vejr, vind, bølger, vandstand, rå U/V, fart, koordinater, geometri, punkter, scoreoutput og private payloads interpoleres eller publiceres ikke.
- Målt state forbliver schema 2.0.0. State med levende rekonstrueret evidens er schema 2.1.0 og bærer eksplicit trust gennem score, startup/detaljer, manifest og hashes. Den er ikke kalibreringsegnet og må ikke alene udløse den hårde observerede udtransportgate.
- `inspect` er read-only og hashforsegler en dataminimeret descriptor. `apply` genverificerer kilder og mål med CAS, skriver privat rollback før mutation og beviser isoleret apply→direkte rollback uden at røre produktionsmålet. Eksakt rollback er kun gyldig mod den byte-/hashidentiske umiddelbare post-apply-state; senere descendants bruger kausal cleanup, som fjerner incidentets syntetiske prøver, bevarer nyere målinger og vender ærligt tilbage til schema 2.0/warmup. Reconstruction-mode må ikke skrive delt continuation-, checkpoint- eller last-ready-cache, og alle 673 scoreveje forbliver fail-closed indtil en obligatorisk frisk normal produktion.
- Last-verified nødvisning forbliver målt-only. Ture bindes gennem hele det faktisk aktive manifest; startup- og kystdels-trust skal begge findes og være identiske. Rekonstruktion eller vist nødvisning får eksakte kvalitetsmarkører og `calibration_eligible=false` gennem klient, Edge, D1/Supabase, schema og installer. Ældre aktive/pending schema-v2-ture uden trust bevares fail-closed som `ravscore-evidence-trust-unattested` og bliver ikke-kalibrerbare. Allerede gemte pre-4.0.311 schema-v2-observationer ændres ikke i databasen; den lokale prediction-/kalibreringsforbruger medtager kun en række ved samtidig `appVersion >= 4.0.311`, `calibration_eligible=true` og eksakt attesteret `data_quality_flags=[]`. Dette er en konservativ klientattesteret udelukkelseslås, ikke serverbevist manifestproveniens eller empirisk evidens; global koefficientlæring forbliver forbudt. Edge/D1 bruger en type-/intervalvalideret nested allowlist, og migration/readback læser kun eksplicitte server-side blade; historiske fri-form-snapshots kan kun krydse grænsen gennem en deterministisk sikker projektion, mens eksisterende D1-rækker forbliver uændrede.
- D1 bruger én global atomisk registry på control-sharden til klient-/trip-id, ejerbinding, payloadhash og målshard. Ejer-sletning skriver en global tombstone før rows/registry fjernes, så samtidige eller senere writes for samme pseudonyme ejer stoppes. Direkte identitet, GPS/lokation, geohash/UTM, rå U/V, fri tekst/billeder og ukendte kolonner må ikke læses ind i migrationsrunneren eller føres ud ved readback.
- Før første 4.0.312-Pagesproduktion med denne kontrakt kræves exact-head `[d1]`-backendbevis; trust-/tripprotokollens kompatibilitetsgrænse forbliver 4.0.311. Efter schema, capacity og CAS sættes umiddelbart før første Edge-deploy enten `d1_edge_predeploy_intent` eller `fresh_edge_predeploy_intent`. Existing D1 får maintenance-kapabel Edge under uændret D1-mode/gammel Worker, derefter repair-intent og 20-minutters lease med 30-minutters max/automatisk D1 ved udløb. Femsekunders Edge-prober, dobbeltattestation, 20-sekunders drain og mindst 600 sekunders restlease kræves før den samlede højst syv minutter lange Worker-secret/deploy/health-gate. Partial existing Edge går D1 roll-forward; partial fresh Edge før activation går exact-main → Supabase-secret → eksakt Edge-redeploy → dobbelt Supabase-attestation. Uden intent ved capacity/pre-CAS-fejl sker nul recoverymutation. Efter D1-aktivering følger attestation, drain og reconciliation; manglende bevis giver no-op uden artifact/Pages.
- 4.0.311-head `4c4699fe3a87a3b804da1d8beea204e4144a7a76` bestod PR #224 exact-head `33263734108`/job `99129959870` og blev merged som `7c168b00af535415117c968a8c021a493b083137`. Push `33263858078` blev korrekt no-op uden artifact/Pages.
- Backend `33263892151`/job `99130384780` modtog HTTP 201 for den atomiske Candidate G-trip-quality-CHECK-transaktion og fejlede derefter kun den læse-only katalogverifiers flade canonical-order-regex. PostgreSQLs transaktionsgrænse betyder, at CHECK'en enten er fuldt committed/valideret/kommenteret — den højst sandsynlige tilstand — eller fuldt rullet tilbage. `VALIDATE` kan have scannet rækker internt, men ingen observationspayload blev hentet til runneren eller logget, og der skete ingen rækkemutation. D1-, Edge-, Worker-, sync-, vejr-, artifact- og Pagesfamilierne blev ikke nået.
- 4.0.312 har lokalt erstattet den flade regex med balanceret, deparser-tolerant strukturel verifikation af præcis ét relevant JSONPath-kald og eksakt tokenrækkefølge. Målrettede tests er grønne; uændret 4.0.311-backend må ikke genkøres.
- DEC-0109 registrerer kontrakten. Den kommende integrerede model under DEC-0102 skal bevare trust/provenance/checkpoint/cleanup/trip-binding, men må ikke arve interpolation som normal missingregel. Den har en bindende nøddriftsacceptgate: kun én komplet, målt-only, atomisk, hash-/model-/statebundet 210/673-state må vises i højst 72 timer og aldrig efter en kortere prognoseudløbsgrænse, med tydelig DA/DE/EN-advarsel, ikke-kalibrerbare ture og automatisk retur til en ny komplet primary; ukendt, rekonstrueret, tampered eller udløbet state lukker fail-closed.

4.0.312's målrettede tests, fulde lokale source-/RDKS-/håndbogs-/versions-/releasegate og særskilte geodatabevis er grønne. PR exact-head, merge, grøn exact-main-backend, inspect/apply, frisk produktion, Pages og offentlig desktop/mobil/210/673 mangler fortsat og må ikke foregribes i status.
...
## Historisk 4.0.315 integreret RavScore-releasekandidat – aldrig exact-head-/produktionsverificeret

- Modelsporet bygger én samlet efterfølger til Candidate G under DEC-0110/DEC-0112: `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-historybounds12d-lastmileewma4-tail40-atten15-v5`, komponent `ravscore-components-huntability-delivery-mobilisation-bounds-v5` og forklaring `ravscore-explanation-integrated-bounds-v5`. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder den transitive implementeringslukning; endelige værdier dokumenteres først efter regeneration på afsluttet head. Candidate G forbliver den eneste offentlige model, indtil exact-head, merge, frisk produktion og offentlig desktop-/mobilkontrol er grønne.
- Modellen bevarer de fagligt og teknisk stærke dele af Candidate G, herunder 20/50/30, strømgrænserne 0,03/0,15 m/s og +10/-8, men forbedrer den tidslige kausalitet: strømbevis vægtes fuldt i 24 timer og cosinusfader til nul ved 48 timer, mens bølgemobilisering bygger med 4 timers halveringstid og aftager over 48 timer. Den tidligere 13-timers nul-gate for hele scoren er fjernet; transport kan være nul uden at udslette mobilisering og jagtbarhed.
- Verificeret DMI/Copernicus-current bruger det beviste U/V-par som eneste numeriske sandhed: kanonisk fart til 0,01 m/s og toward-retning afledes fra samme par, 360° normaliseres til 0°, og modstridende parallelle cachefelter ignoreres. Hele den aktive model-/state-/migration-/recovery-/runtime-/public-kæde kræver endelige JSON-tal; numeriske strenge og andre coercible typer bliver invalid/missing og kan ikke skabe evidens eller score.
- Sidste nærkystled bruger en kausal energivægtet bølgeapproach med fire timers halveringstid, afledt som `W/N/T` med en ældre hale. DMI-WAM-retningen er `FROM` og roteres præcis én gang +180° til `TOWARD` mod den uændrede eksisterende kystnormal. `normalAlignment` er det energivægtede normalmoment divideret med aktivitet, `approach=clamp((normalAlignment+0,25)/1,25,0,1)`, `factor=clamp(1-0.15×W×(1-approach),0.85,1)`, og `delivery=supply×factor` anvendes præcis én gang. Bølger kan aldrig skabe eller øge supply, og den rå totalscore kan højst dæmpes 7,5 point før slutafrunding; den viste heltalsscore kan derfor ændres 8 point. Aktiv retningsmissing fejler lukket. Kun `waveHeightM=0` er eksakt calm og neutral; `wavePeriodS` skal stadig være finit og ikke-negativ. `waveHeightM>0` med `wavePeriodS=0` er `INVALID` og fejler lukket. Den fysiske sidste levering er fortsat uopløst: `physicalDeliveryResolved=false`, fysisk interval er `null`, og der påstås hverken lokal surfzonepræcision eller empirisk højere fundpræcision.
- Faldende vandstand behandles tosidet. Noget mobiliseret rav kan føres ud, samtidig med at lavere vand kan blotlægge materiale, som allerede er afleveret eller fastholdt bag revler, og gøre et mindre område lettere at afsøge; det beviser ikke, at vandstandsfaldet har koncentreret ravet. Feltstudier viser både lavvandsaktiveret offshore riptransport, recirkulerende ripflow og stærk afhængighed af revle-/kanalgeometri, bølgebrydning, setup og lavfrekvente bevægelser. Ravlitteraturen understreger samtidig manglende systematisk monitorering og mulig gentagen migration/tilbageførsel over dage. Kilderne giver ingen universel dansk ravregel, og søværts bevægelse beviser ikke “alt rav væk”. Vandstand ændrer derfor ikke RavScore direkte; den bruges som jagt- og tidskontekst samt som tie-break.
- DDM's officielle 50 m-dybdegrid kan bruges som statisk forsknings- og forklaringskontekst, men ikke som scoreinput: det opløser ikke dynamiske revler, ripkanaler eller en bølgeopløst surfzone. Rainville m.fl. 2026 bruges kun som buoyant-object-analogi for bølgeapproach, aldrig som ravkalibrering. Ingen geometri, kystnormal eller land-/vandpunkter flyttes.
- Migrationen bruger `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema6-v5`. Den genvægter kun signeret, allerede afledt kystnormal Candidate G-currentevidens; migrationen læser/kopierer ikke rå U/V og hævder ikke rå genberegningslighed. Alle 673 schema-2-states skal bestå `candidateGStateKey` og give ét fælles kanonisk target. Migrationens wave-approach bruger 40 private præ-target-timepositioner fra ét sammenhængende DMI WAM-run pr. anvendt collection med same-cell native provenance; højst fire timers interpolation er kun tilladt inden for samme run, collection, gitter og celle. Udeladt EWMA-hale er højst `1/1024`, og konservativ rå-scorefejl er højst `0.01171875` før afrunding. Hvis migrationens state, fælles target eller coherent run ikke kan bevises, stopper migrationen fail-closed. DEC-0114 tillader i stedet en attesteret `genuine-cold-start` med ærlig `HISTORY_INCOMPLETE`, men lemper ikke operational target..+117 eller direct-input-gaterne. Der dannes ingen syntetisk eller offentlig historik.
- Ægte state-6-cold start genafspiller 0–48 faktisk tilgængelige private, verificerede timepositioner plus den reelle targetrække og giver `HISTORY_INCOMPLETE`, indtil 288 timers conservative tail reset eller attestert migration/continuation. WAM må kun bruge de kontrollerede provenanceveje; ukendte historiske positioner udfyldes ikke. Candidate G-rollback `integrated-schema6-to-candidate-g-schema2-v3` opbygges fra samme targettid uden dobbelt credit. Under `BUILDING_MEASURED_ONLY` bevares fremdriften kun privat i runtimebundlen; checkpoint-only recovery og manuel Candidate G-rollback kræver fortsat en særskilt beskyttet READY companion fra samme generation. Manglende direkte input fejler lukket; manglende historik med gyldige direkte input scorer konservativt uden kunstig historik.
- Den offentlige runtime bliver én atomisk schema-4-projektion med eksakt fire livefiler og fælles model-/state-/datasæt-/hashbinding. `20260829010000_ravscore_operational_documents_no_history.sql` anvendes før `20260829020000_integrated_trip_calibration_binding.sql`; protected readiness binder begge efter samlet database-/Edge-readback. Efter dry-run genverificerer backendworkflowet `origin/main == GITHUB_SHA` umiddelbart før første eksterne skrivning og fortsætter fra samme checkout/migrationssnapshot. Ny Edge svarer eksakt `409` ved manglende/forkert binding, så gammel klient bruger lokal Candidate G. Fra 2026-09-03 kræver første cutover særskilt manuel Fase B fra en forseglede central Phase-A-complete current Candidate G-identitet; i deployleddet genverificeres live sourcemanifest/implementation før controlleren skriver `INTEGRATED_PENDING` med `INITIAL_INTEGRATED_CUTOVER`. Central Candidate-profil bevares under integreret Pages-deploy. Efter eksakt offentlig implementation+210/673 sætter én service-role-RPC samtidigt `INTEGRATED_ACTIVE` og central integreret 11-feltsprofil. Retry fuldfører ved målhash, aborterer/rekonsoliderer ved kildehash og forbliver fail-closed ved en tredje hash.
- Nøddrift må kun bruge en komplet, atomisk og hashbundet continuation fra den samme integrerede model og højst i 72 timer eller kortere forecastudløb. Der findes ingen cross-model fallback eller interpolation. Kun `VERIFIED_ONLY` er kalibreringsegnet; rekonstrueret og emergency er ikke, og ture er ikke i sig selv kalibreringsgrundlag. Den tidligere autoriserede fiktive udførelse af morgenhullet blev opgivet før descriptor/apply/mutation/publicering; DEC-0109 bevares kun som den historiske, afgrænsede sikkerhedskontrakt.
- Mens integreret er aktiv, findes Candidate G's varme projektion kun privat som `ravScoreCandidateGRollback`. Manuel Candidate G-rollback går via `CANDIDATE_G_PENDING`; manuel integreret tilbagevenden går via `INTEGRATED_PENDING`. Begge bruger source/PENDING/target/reconcile og atomisk `ACTIVE` + central target-profil. Scheduleren kan kun udføre `CANDIDATE_G_REFRESH` for allerede `CANDIDATE_G_ACTIVE` med samme binding; den kan ikke aktivere, rollbacke eller returnere. Der deployes ingen Candidate G-assistent-Edge; den integrerede Edge svarer `409`, hvorefter klienten bruger deterministiske lokale DA/DE/EN-svar. Schema-3-ture/-observationer lagres Candidate G-bundet med `calibration_eligible=false`. Det er fail-closed/local-only, ikke en skjult dualmodel; efter bestået rollback er Candidate G igen den ene offentlige scoremodel. Den separate Candidate G-binding er nu forseglet som kontraktdigest `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8` og 54-filers bundledigest `fd3f7e70ec3706818c153c26140ae592e4f0ad2acc6c157183984689f74a2207`.
- Forskning, BEVAR/FORBEDR/ERSTAT/FJERN-klassifikation, integreret design, producent-/forbrugermatrix, offline scenarier/ablation/følsomhed og de nødvendige adaptere er implementeret lokalt. Der påstås fysisk og teknisk forbedring, men ikke empirisk bedre fundpræcision uden et repræsentativt fund-/nulgrundlag. Endelig red-team-, regressions-, versions-, exact-head-, produktions- og browserlukning udestår. Se DEC-0110 og `docs/research/RAVSCORE_INTEGRATED_MODEL_DESIGN_2026-08-29.md`.
- Dual-hash-red-team-fundet er lukket lokalt: den fulde 11-feltsbinding fører både `modelContractSha256` og `modelBundleSha256` gennem generator, state, payloads, checkpoints, privat runtime, ture/observationer, central profil, Edge og releasegates, og hvert felt har særskilte mismatchprøver. Det nyeste 4.0.319-checkpoint øverst fastlåser de regenererede aktive og Candidate G-rollback-digests; exact-head-/produktionskæden er fortsat åben.

## Produktionsverificeret 4.0.308 – naturligt sikkerhedsspørgsmål og alle zonehits
...
- PR #220 bestod exact-head `33221847955` på sourcehead `698f2e15b32aaff711c8b4e1907236b81a4a6327`, blev merged som `a93082548c4cc1ddbe9c75ce303d334530a534c4` og bestod fuld produktion/deploy `33221916441`. Offentlig mobil- og desktopkontrol bekræftede det naturlige DA-fosforspørgsmål samt alle delstrengsmatches i den bevarede zonerullemenu ved både afslut tur og manuel indberetning.
...
## Historisk startgrundlag før DEC-0110 – én samlet ny RavScore-model

- Den offentlige baseline er produktionsverificeret 4.0.308/Candidate G fra mergecommit `a93082548c4cc1ddbe9c75ce303d334530a534c4`. Hele 4.0.307-ejerpakken er bevaret; smårettelsessporet ændrede ingen modelmatematik, DMI/Copernicus, state/cache/recovery, geometri, land-/vandpunkter eller private data.
- Dette var startkravet til den nu lokalt implementerede DEC-0110-kandidat: modelgenerationen skulle bygges autonomt ende til ende i en isoleret worktree og først udgives som én samlet kandidat. Den skulle være en analytisk videreudvikling, hvor alt veldokumenteret i Candidate G først blev identificeret og genbrugt, og kun dokumenteret svage eller ufuldstændige led blev ændret.
- Det faglige startpunkt er, at bølgeorbitaler, lokal modelgridstrøm og surfzonens undertow/rip-/langskyststrømme ikke er det samme. RavRadars verificerede U/V-gridkolonne er et relativt transportbevis frem mod kystzonen, ikke en direkte måling af lokal bundstrøm eller de sidste meter gennem revler, render og brydningszone.
- Ved start skulle modellen samle tilførsel, mobilisering, transport, nærkystlevering/retention, jagtbarhed og usikkerhed uden dobbelt-tælling. 20/50/30, +10/-8, 13-timers nul-gate, 4/48-timers mobilisering og bølgelandingsandelen var analysepunkter, ikke tavse naturkonstanter. DEC-0110 og det øverste aktuelle afsnit dokumenterer de efterfølgende beslutninger.
...
- Det andet isolerede worktree leverede PR #220 uden ændring af modelkæden. Modelsporet har integreret den grønne merge `a9308254` og skal fortsat hente og integrere seneste grønne `origin/main` før samlet slutvalidering.
...
- Historisk 4.0.287 tillod `TRIP_STORAGE_MODE=supabase` som eksplicit manuel rollback uden normal dual-write. Denne post-cutover rollback afløses af 4.0.311-protokollens énvejs D1-kontrakt; source er merged, men backend-/livebevis afventer 4.0.312-roll-forward. Efter intent må kun roll-forward/reconciliation bruges.
...
- Cloudflare deploy-/audit-token har fortsat de præcise mindst-mulige rettigheder og er sat til **No expiration**. Den historiske Supabase-PAT-rotation bestod hele D1-kæden i `33024408547`, før gamle tokens blev tilbagekaldt; payloadfri audit `33024621109`/`98362935528` bekræftede ti shards og 0 % uden turlæsning.
- Ejerens driftspræcisering 27. august gør `SUPABASE_ACCESS_TOKEN` behovsstyret: normal Auth/Edge-runtime, login, turindsendelse, D1-lagring og daglig monitor bruger det ikke. Det installerede PAT må udløbe uden fornyelse eller varsel. Ved en konkret Edge-deploy, migration eller rollback-deploy oprettes et kortlivet PAT gennem godkendt kanal og tilbagekaldes efter grøn exact-main-verifikation.
- Det tidligere secret-frie PAT-udløbsworkflow, som historisk bestod PR #169/exact-head `33025102301`, merge `1e402834` og manuel prøve `33025289153`, er derfor pensioneret. Der findes ingen aktiv PAT-mail-, issue-, kalender- eller Codex-påmindelse.
- Frisk produktion `33025210517`/Pages `98367528389` og offentlig `rr-20260827000855-210` er grønne på 4.0.287, 210/210 aktive zoner, fem **Bedste områder** og fuld 210/673/420/2.100-browseraudit uden fejl.
- Sammenligningen af offentligt synlige Ravudsigten-/RavRadar-resultater er startet som en intern, longitudinel og score-neutral analyse. Første snapshot omfatter aktuelle top-fem, alle synlige ikke-røde femdøgnssignaler og RavRadar-komponentforklaringer. Ét snapshot er ikke validering. Analysen omgår ingen adgang, har `scoreImpact=false`/`publicRuntime=false` og må kun være synlig i RDKS, roadmap og changelog. Se `docs/rdks/30_FEATURES/INTERNAL-RAVRADAR-RAVUDSIGTEN-ANALYSE.md`.
- PR #171 bestod exact-head `33029393300` og blev merged som `f15f5892`. Produktion `33029447510` bestod frisk vejr og den faktiske runtimeaudit, men stoppede før Supabase-sync/artifact/Pages, fordi kildeneutralitetstesten også afviste den godkendte interne RDKS-kildeangivelse. Den eksakte enkeltfil-afgrænsning bestod derefter PR #172 exact-head `33030112665`, merge `7a234653` og produktion `33030166104`/Pages `98382359708` gennem fuld validering, releasegate og beskyttet Supabase-sync. Offentlig `rr-20260827013448-210` er komplet med 210/210 aktive zoner og 673/673 scoreklare kystdele; begge søgemåder viser fem rangliste- og prognoserækker uden synlig runtimefejl.
...
- Afstandsgrænsen, administratorens punkter, RavScoreformlen, fail-closed-nullerne og den geografiske gate var uændrede. Den private 168-timers 0/5/15-km-flerlagscache fortsætter. Den daværende åbne analyse er nu lukket af DEC-0110/state 6: eksisterende havpunkt/kystnormal og nærmeste verificerede gridcelle forbliver aktivt strømgrundlag; 0/5/15 km og ekstra lag bevares kun til privat offline lag-/tidslags-/ablation-/følsomhedsanalyse, fordi den roterende dækning og manglende kausale rum-/tidskobling er utilstrækkelig til score og kan dobbelt-tælle currentforcing. Ingen ny fetch eller release-warmup kræves.
...
- På dette tidspunkt var forskningen og et eventuelt scoremodul åbent. DEC-0110/state 6 har siden analyseret hele kæden og besluttet, at det aktive signal fortsat er den nærmeste verificerede gridcelle bundet til det uændrede havpunkt og kystnormalen. 0/5/15-km-flerlagscachen genbruges privat og score-neutralt; uden komplet fælles dækning, valideret tidslag/advektion og kontrol mod dobbelttælling er den utilstrækkelig som aktiv scoreinput. Se også DEC-0040 og DEC-0029.
...
# TIDLIGERE CHECKPOINT – 2026-09-02 – Supabase måles før integreret cutover

- Den progressive DMI-/Copernicus-/GRIB-cache ligger i GitHub Actions, ikke i Supabase. Supabase bærer den beskyttede state-6-runtime med current/previous samt checkpoint.
- Før cutover skal den isolerede preflight måle produktionsækvivalent komprimeret runtime, rå bundle og checkpoint som rene byteantal uden at uploade eller logge payload, poster, koordinater eller rå U/V.
- Normal fuld produktion budgetteres foreløbigt som mindst 2 × komprimeret objektstørrelse i uncached egress; rollback-/previous-vej som 3 ×. Faktisk buildfrekvens og live forbrug før/efter en kontrolleret fuld produktion skal fremskrives for hele billingperioden med mindst 30 procent reserve.
- Det historiske cirka 0,516/5 GB dashboardsnapshot er ikke live releasebevis. Ukendt eller utilstrækkelig kapacitet blokerer den nye model, indtil restore/readback er reduceret sikkert eller bundlen er formindsket/opdelt uden svækket rollback, checkpoint, hashes, privacy eller recovery.
# NYESTE CHECKPOINT – 2026-09-03 – 4.0.321 metadata-CAS lukker checkpoint-egress lokalt

- Phase A-koden fra PR #246 bestod exact-head-kildegaten `33706215425` og er merged på `origin/main` som `7198b685f4bc9d86bd6432b049380f4279ab797c`. Candidate G er fortsat den eneste offentlige model; merge er ikke et 673 × 118-, kapacitets-, produktions- eller state-6-aktiveringsbevis.
- Den lokale 4.0.321-lukning erstatter checkpointets normale fuld-payload readback efter hver publicering med en service-role-only compare-and-swap, som returnerer højst 4 KiB metadata. Den kanonisk serialiserede checkpointpayload er fortsat højst 16 MiB, mens HTTP-wrapperen kan være lidt større; fuld payload læses kun ved reel restore/cache-miss, og et retry efter tabt HTTP-svar er idempotent.
- Migration `20260903010000_ravscore_checkpoint_metadata_cas.sql`, `supabase/schema.sql` og sikkerhedsinstalleren bærer samme genererede kontrakt. Checkpointet får ingen versionshistorikkopi; eksisterende historik slettes ikke. Restriktiv RLS skjuler checkpointpayload og eventuelle gamle checkpointversioner for authenticated-læsning, mens CAS og metadata-readback kun kan udføres af service role.
...
# NYESTE SANDHED – 2026-09-23 – lokal 4.0.477; P04-id rettes uden at fjerne fastholdelse
...
Lokal 4.0.477/DEC-0247 retter diagnosen med C07 for status og
C08 for beregnet dækning. Statisk producent/SQL-krydstjek påviste
dernæst den konkrete P04-spærre: Den forseglede private Candidate G-
ledsager producerer v2-rollback-id, mens SQL forlangte den aktive
integrerede controllers separate v3-overgangs-id. Append-only
`20260923160000` ændrer kun ledsager-id-sammenligningen i checkpointets
validator og skrivefri diagnose til den eksakte frosne v2-identitet.
Alle andre CAS-betingelser forbliver. Otte autoriserede Limfjord-dele må stadig holde en verificeret
