...
- 4.0.318 validerer først manifest, conditions og det eksakte publicerede Candidate G-source-register samlet og opbevarer registeret isoleret. Efter central adminhydrering materialiseres det aktive register separat. Alle 673 source-states valideres mod source-stateKey; aktiv samplingkontekst valideres for sig. Ingen geometri, zoner, kystnormaler eller land-/vandpunkter flyttes.
...
- Candidate G-rollback bygges ved cold start gennem en separat eksklusiv measured-only vej. Den må ikke hybridiseres med legacy/continuation og kan kun blive READY companion efter sin egen fulde faktiske 48-timerskontrakt. Ellers stopper checkpoint/release.
...
- DEC-0113 og de fem nye aktive first-cutover-krav er bindende. Den endelige 4.0.318-modelbinding er regenereret og lokalt grøn. Version-/docs-/testlukning, egen exact-head/merge, frisk fuld produktion/releasegate/artifact/Pages samt offentlig 210/673/current/femdøgns- og desktop-/mobilverifikation udestår. Der må ikke påstås empirisk bedre fundpræcision.
...
- Historisk 4.0.317-bevis: den daværende proportionale matrix var grøn. Historical/recovery/outcome/P2-deltaet i 4.0.318 kræver en ny fuld slutmatrix og nye digests.
...
- Candidate G er fortsat den eneste offentlige model og der findes ingen offentlig shadowmodel. Historisk 4.0.317-matrix var grøn; 4.0.318-slutmatrixen er åben. State 6 er ikke udgivet før exact-head, merge, frisk fuld produktion/deploy, Feggesund 3 × 118 og offentlig mobil-/desktopkontrol af 210/673 og alle kvalitetsflader er bevist. Offlinebevis dokumenterer teknik og fysisk sammenhæng, ikke empirisk bedre fundpræcision.
...
- Overgangen bruger `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema6-v5`. Den genvægter kun signeret, allerede afledt kystnormal Candidate G-currentevidens; migrationen læser/kopierer ikke rå U/V og hævder ikke rå genberegningslighed. Alle 673 schema-2-states skal bestå `candidateGStateKey` og give ét fælles kanonisk target. Wave-approach bruger 40 private præ-target-timepositioner fra ét sammenhængende DMI WAM-run pr. anvendt collection med same-cell native provenance; højst fire timers interpolation er kun tilladt inden for samme run, collection, gitter og celle. Udeladt EWMA-hale er højst `1/1024`, og konservativ rå-scorefejl er højst `0.01171875` før afrunding. Hvis state, fælles target eller coherent run ikke kan bevises, stopper cutover fail-closed, og Candidate G forbliver offentlig. Der dannes ingen syntetisk eller offentlig historik.
- Ægte state-6-cold start genafspiller 0–48 faktisk tilgængelige private, verificerede timepositioner plus den reelle targetrække og giver `HISTORY_INCOMPLETE`, indtil 288 timers conservative tail reset eller attestert migration/continuation. WAM må kun bruge samme kontrollerede same-run/same-cell-interpolation. Candidate G-rollback `integrated-schema6-to-candidate-g-schema2-v3` beregnes fra samme targettid uden dobbelt credit; checkpoint-only recovery kræver en særskilt beskyttet READY rollback-companion fra samme generation. Manglende direkte input fejler lukket; manglende historik med gyldige direkte input scorer konservativt uden kunstig historik.
...
- Dual-hash-red-team-fundet er lukket lokalt: den fulde 11-feltsbinding fører både `modelContractSha256` og `modelBundleSha256` gennem generator, state, payloads, checkpoints, privat runtime, ture/observationer, central profil, Edge og releasegates, og hvert felt har særskilte mismatchprøver. Det nyeste 4.0.318-checkpoint øverst fastlåser de regenererede aktive og Candidate G-rollback-digests; exact-head-/produktionskæden er fortsat åben.
