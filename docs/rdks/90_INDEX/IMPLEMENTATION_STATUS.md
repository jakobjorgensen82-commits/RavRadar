...
- [x] Merge 4.0.490/PR #453 på `main` efter grøn exact-head; den
  komponentbevarende rettelse afventer stadig livebevis.
- [x] Opret dedikeret `RAVRADAR_R2_ACCOUNT_ID` som GitHub-secret;
  den eksisterende Cloudflare-binding til andre funktioner er urørt.
- [ ] Opret scoped R2-token og de to nøgle-secrets, bestå exact-head CI,
...
- [ ] Mål Supabase og R2 over flere døgn. Anbefal først Free, når
  ...
  og øvrige Free-kvoter har reserve. Kvotevarsel hver sjette time
  er oprettet; det er ikke en hård betalingsspærre.
...
- [x] Bekræft levende 4.0.381, integreret model, 210/673 og præcis én manglende closurefil.
- [x] Ret admin til den publicerbare payloadfrie decodersti og fjern den gamle fil.
- [x] Kontrollér færdig `_site` mod forseglet closure i begge Pages-buildere før upload.
- [x] Bestå målrettede decoder-, closure-, kode-only- og YAML-kontroller.
- [ ] Synkronisér resterende docs/geodatabevis, commit/push og én exact-head sourcegate.
- [ ] Merge, providerfri 4.0.382, offentlig 79/79-verifikation og central maintenance-completion.
- [ ] Derefter én normal tidsbegrænset weather for tal, scorer, DMI-rotation og cache. Ingen oneoff.
...
- [x] Brug hele sourcegate `34742976226`: alle kontroller før den gamle workflowinventartest var grønne. Afgræns dens artifactoptælling til det oprindelige preflight-job, ret fire bogstavelige patchmarkører i shellkommandoerne og bestå de to direkte tests samt YAML-kontrol.
...
- [x] Klassificér tredje head `e1e9dd1e`/run `34664907674`: releasegaten omskrev alene de to tracked, tidsstemplede release-rapporter efter alle grønne tests.
- [x] Undertryk kun dynamisk rapportskrivning i sourcegavens releasegatekald; bevar fuld testudførelse og almindelig rapportskrivning i post-data-produktion/`release:package`.
...
- [x] Bevar N−1 gode proofs ved én ugyldig række og afvis same-asset-konflikt uden last-write-wins.
- [x] Implementér komponentvis atomisk donorbackfill for current, bølge, vind, vandstand og temperatur.
...
- [x] Klassificér `34041885030` som negativ runtime med cache-save, actual attestation 8.918, outcome-proof 9.541 og delta 623; klassificér `34044178502` som samme fejl efter det observerede kanoniske modelrunskift med cache-save, 22.357 mod 25.826 og delta 3.469.
- [x] Klassificér gammel-main `34049794693` og `34051318868` på `31b9842`: DMI+Copernicus nået; Open-Meteo fejlede deterministisk med `OPEN_METEO_RESIDUAL_PLAN_INVALID_SHADOW_NATIVE_CADENCE_INVALID` ved `2026-09-06 18:16:28Z` og `18:42:49Z`; artifact/deploy korrekt skipped. Producer skrev hourly off-phase regional `dkss_lf`, mens consumer gjorde selected-run `lead % 3 != 0` fatal. Bevar som negativ regional producer/consumer off-phase-evidens, ikke retained-proof-admission- eller hotfixbevis.
- [x] Fastlæg kontrakten: et genbrugt par skal bevare og genvalidere sit konkrete per-pair/source-proof på tværs af modelruns med kompatibel processing-signatur og højst 120 timers native source-lead; et nyt valgt assets part-proof må ikke skabe positiv coverage for en cached række, som assettet ikke ejer.
- [x] Fastlæg, at operational complement er den eksakte inverse af actual canonical attestation i 79.414-parsdomænet uden count-cap, minimumsdækning eller anden kardinalitetsgate for fallback.
- [x] Implementér retained proof-kontinuitet og actual-attestation-partition i den fælles normal/oneoff-path. Valider active-donor ved egen `productionReferenceAt` og eget eksakte +117-vindue. Bevar providerkæde, ydre budgetter, cron/reserve-dispatchkadence, data og score; afgræns kun intern canonical regional off-phase-admission, og prioritér inden for DMI reelt missing/expired, interne huller og hale før retained refresh.
- [x] Supersedér ledger.ready-afledt `RETAIN_PREFERRED_NATIVE_RUN`: normal og oneoff vælger selectorens nyeste modne, native-complete run med `RETAIN_PREFERRED_NATIVE_RUN=false`, også ved non-READY kandidat, mens exact retained pair/source-proof bevarer kompatible cached rækker. `True`-path er kun dormant test/helper. Regressionen dækker gammel 96h-pin, op til cirka 24h forsinket refresh og unødig fallback-tail.
- [x] Afvis gammel revision ved revideret officiel same-modelrun-asset, og erstat først currenttuple atomisk efter verificeret ny asset, outcome og provenance. Retained proof holder DMI non-READY, mens den eksakte residual fortsætter.
- [x] Supersedér absolut whole-row donor-merge: genbrug kun exact-proof-gated U+V+`sources.current` atomisk; primary-proof vinder, halv/uattesteret donor afvises, øvrige felter/kilder/`processedSteps` bevares, og current-summary følger complete donor → complete same-grid primary → ellers fjern partial/mismatch.
- [x] Scoped supersedér DEC-0117 selected-run off-phase blanket-fatal: producer skriver ikke canonical off-phase regional sample; consumer validerer dict + exact collection/modelRun/validTime + `run <= validTime`, ignorerer derefter før deep proof og sender exact pair som `MISSING` til Open-Meteo. Ingen fysisk prune; malformed identitet og alle on-phase checks forbliver fatal.
- [x] Forsegl hvert progressivt bulkcheckpoint før write med en ny availability-ledger fra validerede retained proofs + aktuelle canonical processedSteps/officielt katalog; validér persisted candidate/ledger samlet og afvis ledgerløs trust-reconstruction samt processedValidTimes/complement som selvstændigt proof.
- [x] Tilføj fokuserede regressioner for retained proof, processing-signatur, max 120h lead, donorens referencevindue, currenttuple-atomicitet/primary-prioritet/summary, tamper/missing proof, current-asset-overclaim, revised same-run asset, sealed checkpoint/afvist ghost coverage, missing-first, stor residual og strict READY-adskillelse.
- [x] Final fokustest lokalt: `py_compile` 8 produktionsscripts; provenance, current-field-shadow, regional-current-operational, Open-Meteo-fallback og targetregistry; transactional 19/19 samt diff-check er grønne.
- [ ] Commit/push, bestå én ny exact-head `validate:source`, merge og kør frisk main til DMI-proof/attestation-paritet, faktisk downstream-admission og til sidst 79.414/79.414. Feggesund 354/354, fulde produktionsgates, kapacitet og særskilt modelaktivering forbliver efterfølgende krav; Candidate G er offentlig.

# HISTORISK IMPLEMENTERINGSSTATUS – 2026-09-06 – 4.0.328 per-pair fallback, supersederet af 4.0.329
...
- [x] Historisk 4.0.320-implementering, supersederet af 4.0.329: DMI compatible-cache backfill brugte atomiske hele vejrtime-rækker. Gældende implementering merger alene exact-proof-gated currenttuple U+V+`sources.current` atomisk og bevarer øvrige komponenter/kilder/`processedSteps`; strict promotion kræver fortsat `DMI_READY` plus strict current anchor.
...
- [x] **Historisk supersession:** DEC-0116 afløste først to-collection-/preferred-run-delen med tre normale DKSS-collections og partial-only 96h/non-stale retention. 4.0.329 supersederer siden retentionen med nyeste modne native-complete run og exact retained pair/source-proof; tre collections består.
...
- [ ] Afslut visuel desktop-/mobilkontrol af den gendannede 4.0.316-side; må ikke overclaims før da.
- [x] Implementér planforseglet Candidate H0→H1 og integrated H0→H1 begin/complete/abort/source+target-reconcile uden nye controllerfelter/statusser/kinds; bevar ordinary exact-current.
...
- [x] Fastlås hele 11-feltsbindingen: parameterkontrakt `778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7` og reproducerbar 43-filers implementeringsbundle `74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`.
...
- [x] Stop det ejerautoriserede rekonstruktionsspor uden ny exact-head, apply, mutation, artifact eller offentliggørelse; den fiktive udførelse af morgenhullet er opgivet.
- [x] Bevar DEC-0109 som en historisk, hændelsesafgrænset sikkerhedskontrakt uden at gøre interpolation til en normal fallback- eller modelregel.
- [x] Pensionér alle aktuelle workflow-inputs og inspect/apply/rollback/cleanup-job samt incidentpolicy, mutator og positive eksekveringstests. Bind en negativ pensionsgate ind i source/release, men bevar historisk read-/quality-klassifikation og den separate Candidate G-helrollback.
...
- [x] Luk den historiske same-version-hotfix som kildebevis uden at kræve en ny incidentrelease; den efterfølgende ejerbeslutning ophæver behovet for ny exact-head/D1 alene til rekonstruktionen.
- [x] Aflys ny read-only inspect og CAS-apply efter ejerbeslutningen; der blev ikke udført mutation, artifact, Pages eller offentlig incidentverifikation.
...
- [x] Bind model `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `5.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileewma4-atten15-v4`, komponent `ravscore-components-huntability-delivery-mobilisation-v4` og forklaring `ravscore-explanation-integrated-v4` som én kontrakt.
...
- [x] Implementér fuld 11-felts model-/state-/hashbundet state-5-runtime for hver continuation-state, checkpoint og central profilselection, med fire offentlige schema-4-livefiler, kompakt checkpoint, privat produktionsbundle, beskyttet driftscheckpoint og fail-closed restore. Stop fremtidig versionskopiering for det allowlistede operationelle checkpoint uden at slette eksisterende `admin_document_versions`-historik.
- [x] Implementér samme-model atomisk nøddrift i højst 72 timer eller kortere forecastudløb, uden cross-model fallback eller interpolation. Kun `VERIFIED_ONLY` er kalibreringsegnet; reconstructed/emergency og ture er ikke kalibreringsgrundlag.
...
Se DEC-0110 og forskningspakken `docs/research/RAVSCORE_INTEGRATED_*_2026-08-29.md`. Offentligheden bruger fortsat Candidate G/4.0.310; 4.0.315 er releasekandidat og mangler exact-head, merge, frisk produktion og offentlig kontrol.
...
Se DEC-0102, DEC-0110 og `docs/research/RAVSCORE_NEXT_GENERATION_WORK_BASIS_2026-08-28.md`. Offentlig model forbliver Candidate G, indtil den samlede kandidat er verificeret.
