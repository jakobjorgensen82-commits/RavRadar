...
Eksakt 113×673-sammenligning med forrige deploy viste nul tab af
...
# HISTORISK – 2026-09-23 – lokal 4.0.479, cache/CP/checkpoint samlet
...
`d53fb174`. Providerbootstrap `35081537023` gennemførte alle
...
implementerer reglen. Normal `35102336423` nåede ingen providers eller
writes, fordi runtimebindingen endnu manglede. Providerfri code-only
`35102784460` beviste den korrekte ene pending migration
`20260916120000`, men en gammel helperforventning stoppede før write.
Forventningen er rettet lokalt; næste er exact-head, merge, code-only og én
normal weather på den bevarede cache. Scheduler er pauset. DEC-0173.
...
PR #286's endelige 4.0.351-head `b6f06310b59ce6c074361735ec9175da435d0e21` bestod sourcegate `34726624728` og blev merged byteidentisk som main `6d4adbb2afddfd103566cf0a80c0a57408be1ca4`. Backend `34727884447` anvendte/readback-verificerede migration 13.

Cache-only-preflight `34728026044` hentede intet providervejr og bestod current, WAM, freshness samt 210/673-modelbygningen. Intet blev cut over eller deployet, fordi slutauditten stoppede på `MODE_RECONSTRUCTION_MISMATCH`, `PART_LAST_MILE_STATE_METADATA_MISMATCH` og `PUBLIC_PROFILE_NOT_READY`.
...
PR #284's 4.0.349-rapporthotfix er merged som main `512f889dbb301bc7fb801478358625f5404586b2`. Cache-only-run `34706453561` genbrugte `2026-09-12T08:00:00Z` uden providerhentning og beviste igen current 79.414/79.414 med DMI 67.686, Copernicus 8.668, regional 944 og Open-Meteo 2.116. Den samlede modelrapport viste 659 kystdele i begge jagtformer med manglende accepteret vindinput og 14 dele i begge jagtformer med `CURRENT_DIRECT_INPUT_NOT_READY`.

Vindfejlen var systemisk: `sanitizeWind` bad altid proofvalidatoren om komponenten `wind`, selv når den virkelige DKSS-række korrekt deklarerede `windTail`. Gyldige data blev således behandlet som manglende. 4.0.350 læser alene deklareret `wind|windTail` og validerer proof mod samme komponent. Den konkrete produktionsadaptertest bygger en rigtig DKSS-prognoserække og afviser desuden falsk ommærkning. Rettelsen gælder både den låste cache og fremtidige vejrdata.

Otte af de 14 currentdele er de godkendte regionale proxydele. Deres H0 kunne være en closure-bundet `NATIVE_CADENCE_HOLD`, men RavScore kunne ikke se den private præ-H0-kildetuple. Live-current bygger nu `regionalReferenceEntries` med eksakt cache-, proof-, grid-, lag-, U/V-, hash-, part- og tidsbinding. Kun privat score/recovery kan slå dem op. Standardmerge og offentlig runtime udelader dem. En fundet tværimplementationskant er samtidig rettet: Pythonproducentens `(validTime, partId)`-rækkefølge er nu også JavaScriptvalidatorens sorteringsregel.

De resterende seks currentdele antages ikke løst uden runtimebevis. I stedet er den allerede fagligt besluttede lokale missingregel gjort til én håndhævet kontrakt: berørt del, jagtform og time får `UNAVAILABLE`, `score=null`, antal og årsag; den kan ikke vinde ranking, mens andre gyldige dele og hele 210/673-strukturen fortsætter. Manglende historik er fortsat særskilt `HISTORY_INCOMPLETE`. Generator, manifest, conditions, public audit, Pages, browserdataservice, admin og ture genvaliderer samme kontrakt. Emergency må kun genberegne availability ved den faktisk valgte, validerede reference.

`integrated-cutover` kører nu de fem godkendte kontroller i én samlet blok. En payloadfri artifact-rapport indeholder kun exact source/run/action samt trin-id, required og success/failure. Manglende `set -e` kunne først have ladet et efterfølgende echo skjule en rød samlet kontrol; en eksplicit afsluttende check og `exit 1` lukker denne kant. Alle senere writes bruger fortsat `success()` og ligger efter barrieren. Alle fem grønne fortsætter automatisk.

Ny append-only migration `20260912194206_local_unavailable_cutover_binding.sql` fører alene integrated bundle `a575f767…`, rollbackbundle `ca184522…`, continuation `dce13d51…` og readbackversion frem. Migration 11 er byteuændret. Den oprindelige lokale slutmatrix og de tunge 210/673-regressioner var grønne før den senere Candidate-policyopfølgning. Efter ejerens stop for testspiralen gentages ingen lang lokal fuldpakketest; hurtige tur-, hash-, binding-, migration- og releasemarkørkontroller er grønne, og næste exact-head GitHub-sourcegate leverer det samlede bevis.

PR #285's første head `fe4969f158d440e263f0259089c8932126dda218` fejlede sourcegate `34717671774` før proof/upload. Den historiske kædetest erstattede measured-warmup-hashes direkte med den aktuelle 4.0.350-binding, selv om det kontrollerede filled var den allerede anvendte 4.0.349-migration. Rettelsen fastlåser 4.0.349 integrated/rollback/continuation samt migrationens normaliserede SHA-256 og tilføjer derefter et særskilt 4.0.350-led mod aktuelle bindings. Den gamle migration er fortsat byteuændret; migrationskæde, begge builders, releasecontractmetadata og installkontrakt er lokalt grønne. En ny exact-head-CI er obligatorisk.

PR #285-head `66338d63`/run `34717905077` passerede migrationsrettelsen og samlede derefter tre fejl i samme releasegate: Candidate G-public-stage og Candidate G-ture kaldte den nye integrated-only availabilityvalidator, mens den statiske 118-timersgate stadig ledte efter den udgåede inline-`HISTORY_INCOMPLETE`-kode. Rettelsen tilføjer én fælles policy-dispatch: integrated må have eksakt lokal `UNAVAILABLE`; Candidate G kræver fortsat alle 210 zoner, 420 modes, fuld historik og ingen unavailable. Public generator, browserdataservice, nødruntime og ture bruger samme dispatch. Den statiske gate binder til den fælles fail-closed-validator. Ingen policy eller datakrav lempes.

# NYESTE EJER- OG IMPLEMENTERINGSDELTA – 2026-09-12 – 4.0.349 main, komplet cache og samlet modelfejlrapport

Ejeren fastholder autonomt arbejde, helikopterperspektiv, ingen blind ny tretimers-oneoff og én sourcegate for byteidentisk kilde. Ejeren har desuden præciseret, at en ubrugelig Candidate G ikke har praktisk værdi som backup, og at cutoveren bør gennemføre modelanalysen, samle alle lokale fejl og derefter rette dem samlet. Arbejdet fortsætter på GPT-5.6 Sol/Ekstra høj; en afgrænset Astra/Ultra-gennemgang bruges først, når den konkrete fejlfordeling foreligger.
...
4.0.349-head `fa5e648c5fbbb4263dfae539a4707b920fe8646c` bestod PR #283's exact-head-sourcegate `34700907469`. Den byteidentiske tree blev merged som main `187e599879e39d20014277e9afc4cb96029a78b3`. Backendrun `34702305208` genbrugte det levende exact-content-bevis uden en anden fuld kildegate, anvendte alene migration 11 `20260912141641_state_only_hold_closure_v2_binding.sql` og bestod central apply/readback.

Cache-only-run `34702471040` fastholdt `2026-09-12T08:00:00Z` og hentede intet nyt vejr. DMI-/Copernicus-producere blev sprunget over, Open-Meteo var reuse-only, og gemt currentclosure 79.414/79.414, native WAM, freshness samt den kontrollerede live-current-selection bestod. 4.0.349 passerede dermed det gamle v1/v2-stop; rettelsen er produktionsbevist.

Kørselen stoppede senere i `writePublicRuntimeFromFull` med den samlede fejl `Public RavScore horizon is not one complete 210/673 package`. Modelbygningen havde allerede gennemgået alle zoner og skrevet den private conditions-pakke, men den ydre assertion loggede ikke, om afvigelsen var et antal eller en lokal scoreårsag. WAM-hypotesen er afkræftet ved kodebevis: operational-handoff-gaten kræver bølgeretning for enhver positiv bølgehøjde. De netværksspærrede vandstandskald er heller ikke bevist som årsag; vandstand er score-neutral kontekst i den integrerede evaluator.

En lokal 4.0.349-hotfix ændrer ikke model, fysik, data eller backendbinding. Den lader den eksisterende fulde zonegennemgang skrive en strengt bounded rapport med alene offentlige zone-/kystdels-id'er, antal og fejlkoder, viser den i loggen, uploader kun rapporten og bevarer samme røde stop uden handoff/deploy.
...
Måltests og model-/metadatahashkontrol er grønne. Næste sekvens er exact-head-sourcegate, merge, samme cache-only-kontrol, analyse af den samlede fejlfordeling og derefter den mindst mulige model-/cutoverrettelse. Før en ændring af den atomiske all-zones-active-regel udføres den aftalte Astra/Ultra-helikoptergennemgang.
...
PR #282's første exact head `cc06fa37`/run `34695465328` bestod de lange model-, 210/673-, privacy-, runtime- og migrationsled, men stoppede før rent træ/proof, fordi releasegatens statiske testinventar ikke var opdateret med den nye migrationsbygger. Lokal opfølgning tilføjer byggeren præcis én gang, retter packageforventningen og tilføjer en regression mod ny drift. Den røde head må ikke genbruges; ny exact-head-CI kræves.
...
Den næste head `362709cd` passerede schedulerrettelsen og stoppede i CI `34629975674` ved en statisk cachebevaringstest. Testen forventede recovery-hash/quarantine i den gamle commitfunktion; den fælles helper beskytter nu både klassisk og prepared commit før atomisk replace. Funktionel crash-/bytebevaring er allerede grøn. Kun helper-, builder- og kald-rækkefølgeforventningen ændres; produktionskode/cacheformat ændres ikke. Den korte samlede workflowkontrakt og de efterfølgende source-tail-kontroller er grønne lokalt før ny exact head.
...
4.0.341 blev merged på `main` som `caa49c42`. Backendrun `34534769955` blev rapporteret grøn. Oneoff `34534764449` gav intet handoff og ingen cutover: `wam_dw` kunne lukke, men alle 91 `wam_nsb`-assets blev rullet tilbage, når enkelte partmål ikke kunne accepteres. Den isolerede candidate virkede som sikkerhedsgrænse, men whole-asset-admissionen var for grov og forhindrede reel progression.

Lokal 4.0.342 bevarer derfor DEC-0123's isolation, same-target/pair-superset/no-new-lineage-promotion, own-run fallback og faseatomiske qualityrefresh. Den nye admission sker pr. komplet `(partId, nativeValidTime)`-bølgetuple efter fuldt, ikke-afbrudt assetgennemløb med kendt denominator og same-asset-lineage. Rejected slices skal være uændrede; har de allerede komplet anden native lineage, eller er asset-/tidsakse-/control-plane-identiteten ugyldig, afvises hele stagen. Privat bootstrap må bevare samme sikre tuples, men en delvis time er aldrig locked/history-complete, og genuine cold-start er uændret.

Copernicus-current får tilsvarende granularitet: et validt shard kan checkpoint'e de eksakte native timer, der faktisk findes, mens fraværende bestilte par forbliver rest til næste collection og Open-Meteo. Attempts gemmer `observedNativeValidTimes` i nested v2, accepterer legacy 4.0.341 og bevarer immutable Baltic-prerequisite efter pruning af sidste søskende. Tom/subsekund providerakse er retryable malformed, ikke no-record. Tidsinterpolation/hold er fortsat forbudt, og non-hourly/duplikeret tidsakse eller out-of-request-rækker stopper shardet.

Currentclosure forbliver præcis 79.414/79.414. Bølgeclosure forbliver 79.060 native WAM plus Feggesund 354/354 med nul missing og nul uløste lineage-konflikter. Partial cacheprogression er aldrig handoff-, artifact-, deploy- eller model-online-bevis. Testforløbet fandt først to fixturefejl i en 55-test-WAM-suite (`53/55`) og bekræftede derefter rettelserne `2/2`; den udvidede aktuelle WAM-suite er samlet grøn `63/63`, inklusive lineage-evidence og en reel parser→provenance→summary→admission-kæde med to PARTs. WAM-historik er grøn `35/35`, checkpoint er grøn `21/21`, og de tre Copernicus-måltests, Python compile samt code diff-check er grønne. Privacy-/releasepakningskontrol, version, exact-head-CI, merge og al main-/produktionsverifikation er fortsat åbne.

DEC-0124 er ny bindende detaljekontrakt og supersederer alene DEC-0123's whole-asset-admission. DEC-0122's first-cutover-undtagelse flyttes under stående ejerautorisation til exact-release 4.0.342. Normalworkflow og watchdog/shadow-dispatch forbliver deaktiveret gennem den kontrollerede sekvens; cachetransport, ekstern cron, providerforbrug og normal vedligeholdelseskapacitet består efter launch.
...
- Ejeren godkendte først exact-releaseflytningen af DEC-0122 til 4.0.339 og derefter udtrykkeligt nødvendig overførsel til 4.0.340 og senere launchrettelser uden ny forespørgsel. Én dokumenteret exact-releasebinding ad gangen; de materielle grænser er uændrede. 4.0.339 fastholdes i den aktuelle PR.
- PR #273/head `a3786e6d` fejlede sourcegate `34362702197` på en gammel immutable migrationshash. Den lokale opfølgning pin'er korrigerede bytes OG beviser, at alene fjernelse af de to parenteser reproducerer originalen. Hele migrations-testgruppen og CASE-regressionen er grønne. CASE-testen er afgrænset til validatoren for at undgå falske fejl fra håndbogstekst.
- Ejeren bad om en ekstra stor Astra-helikoptergennemgang. Verificeret: behold aktive cache-saves før merge; senere ubetinget readiness beskytter cutover trods tidligt versionsinterval; handoff fastholder producentens reference, og 90/150-minuttersmål afviser ikke brugbare fremtidstimer. Produktionsbeviser må fortsat ikke erstattes af lokal optimismе.
- Weather `34350872447` sluttede med alle provider-saves grønne, men OM missing 301 og WAM MISSING_HOUR. Direkte log beviser retained 5.166/fetched 537, ikke den fejlagtige tidligere opsummering 5.680/23. Beregnet 79.113/79.414 er ikke en slutattesteret closure; intet handoff. De bevarede cacher kan donere til ny exact-main-fuldførelse.
- Astra-helkædegennemgangen er afsluttet og gemt i `docs/ai/LAUNCH_CHAIN_ASTRA_REVIEW_2026-09-09.md`. Den faktiske SQL-funktion accepterede fuld/partial gyldig state og afviste manipulation på PostgreSQL 16.4. Ingen WAM-readerrettelse er begrundet. OM-historik ved >1h-spring kan påvirke konservativ score, og bæredygtig transport er stadig uafsluttet; begge skal håndteres straks efter launch.
...
- Helkædekontrollen fandt en særskilt pre-normalization-fejl: 256 MiB-readeren kunne afvise den bevarede 760 MB legacycache, før den tabsfri codec fik lov at komprimere den. Alle fire relevante workflowveje bruger nu en bounded separat materialisering før første reader; originalen bevares ved fejl, og overstor encoded input afvises fortsat.
- Lokal målmatrix er grøn. Diagnosejob `34288231609` beviste den eksakte tabsfri Python/Node-codec på 760.487.472 inputbyte og 578.063 sourceposter til 94.150.151 encoded byte uden cacheændring. Main/offentlig produktion er uændret; endelig exact-head, merge, komplet oneoff og modelcutover afventer.
...
- Main-oneoff `34127986853`, target 13Z, genbrugte cache og nåede DMI 65.409/79.414, Copernicus 12.661/14.005 og Open-Meteo 190/472 efter regionalleddet; 282 operationelle par manglede ved slutgaten. Open-Meteo bevarede 76 og hentede 114 nye. Ingen artifact, deploy eller modelcutover skete. Den tidlige 70.280/79.414-optælling var gammel-reference-donorrestore og er ikke runnets slutresultat.
- Runnet beviste ingen cache-nulstilling: eksisterende data blev genbrugt, og ny Open-Meteo-fremgang blev checkpointet. Det historiske run havde ikke 4.0.333-diagnostikken og beviser derfor ikke den konkrete payload-/stopårsag. Helikoptergennemgangen fandt en selvstændig koderisiko, hvor samme samlede delvise rest kunne genbestilles under et globalt tre-runders loft. 4.0.333 indfører en bounded FIFO/BFS-kø, som fjerner accepterede par straks og binært isolerer resten helt ned til eksakt kystdel/time; næste main-oneoff skal klassificere den faktiske rest.
- Tre transportforsøg pr. exact work, ét content-retry, requestcap 1.024, pending-cap 2.048, fælles deadline, provider-wide HTTP-cooldown, bounded Retry-After samt globalt stop på 400/ukendt permanent status forhindrer både starvation og requesteksplosion. 413/414 kan fortsat splittes. Alle stop efterlader en ærlig residual til næste kørsel.
- Den målrettede kode-/provider-/closure-/live-runtime-matrix er grøn, og to uafhængige reviews er GO. Releasepakken er lokalt klar, men ikke exact-head-CI-, merge- eller runtimeverificeret. Uændret næste kæde er sourcegate → merge → main-oneoff → 79.414/79.414 → run-bundet handoff → fulde gates og kontrolleret modelcutover. Candidate G er fortsat offentlig, og normal workflow er deaktiveret.
...
- Ejeren afviser en arkitektur, hvor cachearbejdet praktisk bliver usynligt ved hvert seks timers DMI-modelrunskift. Gyldige data må erstattes af nyere data, når de faktisk er hentet og verificeret, men et rollover må ikke tvinge genhentning af hele vinduet eller skjule tidligere bevist progression.
- 4.0.328 bestod exact-head `34040547841` og blev merged som `31b98428dea163c11ded1fc1e428e27a0218a8f2`. Runs `34041885030` og `34044178502` gemte DMI-cacheprogression, men stoppede før downstream på henholdsvis 8.918 vs. 9.541 (delta 623) og 22.357 vs. 25.826 (delta 3.469) mellem actual attestation og outcome-proof. Sidste fund følger det observerede kanoniske modelrunskift.
- Gammel-main `34049794693` og `34051318868` på `31b9842` nåede DMI/Copernicus, men Open-Meteo fejlede deterministisk med `OPEN_METEO_RESIDUAL_PLAN_INVALID_SHADOW_NATIVE_CADENCE_INVALID` ved `2026-09-06 18:16:28Z` og `18:42:49Z`; artifact/deploy blev korrekt skipped. Producenten skrev en hourly off-phase regional `dkss_lf`-sample, mens consumeren gjorde selected-run `lead % 3 != 0` fatal. Det er negativt regional producer/consumer off-phase-bevis, ikke retained-proof-admission-bevis. Begge 4.0.329-paths er lokalt fokustestede; exact-head runtime afventer.
- Den beviste fejl er ikke slettede cachebytes, men manglende proof-kontinuitet: tidligere modelruns gyldige cached rækker kunne ikke længere attesteres under den nye current-ledger, mens det nye assets generelle spatial-proof overklassificerede de faktiske rækker.
- Ejerkravet bindes i 4.0.329: bevaret og genvalideret per-pair/source-proof følger den faktiske cached række over modelrun med kompatibel processing-signatur og højst 120 timers native source-lead; DMI-positive par er eksakt actual attestation; alle øvrige registrypar er den matematiske inverse rest uden antalstærskel.
- Active-donor valideres ved egen `productionReferenceAt` og eget eksakte +117-vindue. Reelt missing/expired, interne huller og hale prioriteres før retained refresh. En officielt revideret same-modelrun-asset må ikke fastholde gammel revision; atomisk currenttuple-replacement kræver ny asset, outcome og provenance.
- Den tidligere absolutte whole-row-donorregel er supersederet af komponentatomar currenttuple-merge. Kun exact-proof-gated U+V+`sources.current` flyttes samlet; primary-proof vinder, halv/uattesteret donor afvises, og wave/wind/øvrige fields+sourcer+`processedSteps` bevares. Complete donor-current-summary erstatter; ellers bevares complete same-grid primary-summary, mens partial/mismatch current-summary fjernes.
- DEC-0117's selected-run off-phase blanket-fatal-regel er scoped supersederet. Producer skriver ikke canonical off-phase regional sample. Consumer validerer først dict + exact collection/modelRun/validTime + `run <= validTime`, ignorerer derefter off-phase før capturedAt/hash/sampleKey/spatial/vector og sender exact pair som `MISSING` til Open-Meteo. Ingen fysisk prune; malformed identitet og alle on-phase proofs forbliver fatal.
- Den tidligere ledger.ready-afledte `RETAIN_PREFERRED_NATIVE_RUN` supersederes. Exact retained pair/source-proof bærer cachekontinuiteten, mens både normal og oneoff vælger selectorens nyeste modne, native-complete run med `RETAIN_PREFERRED_NATIVE_RUN=false`, også ved non-READY kandidat. Dermed kan en gammel 96h-pin ikke forsinke refresh op til cirka 24h eller skabe en unødig fallback-tail; `true`-path er alene dormant test/helper.
- Retained availability er fortsat non-READY DMI, men dens eksakte rest må gå videre gennem Copernicus-prioriteterne, policybundet regional DMI og Open-Meteo. Public/runtime/deploy kræver 79.414/79.414, én kilde pr. par og nul overlap/missing. Lokal implementering og final fokustests er grønne (`py_compile` 8 produktionsscripts; provenance, current-field-shadow, regional-current-operational, Open-Meteo-fallback, targetregistry, transactional 19/19 og diff-check). GitHub exact-head, merge og frisk runtime afventer; Candidate G er fortsat offentlig.

# NYESTE EJERDELTA – 2026-09-06 – per-pair fallback og filisoleret vejrhentning

- Ejeren godkender 4.0.328/DEC-0118: RavRadar skal bevare hvert gyldigt DMI-par, isolere en defekt fil og lade den eksakte currentrest fortsætte gennem Copernicus, regional DMI og Open-Meteo. Robust indsamling må ikke forveksles med en lempet slutgate.
- DMI-assets rulles nu tilbage enkeltvis ved download-/parse-/processingfejl. Supervisoren kan bounded genstarte med kun den eksakte collection/run/time/item/URL-/revisionsbundne asset skipped; malformed marker kan ikke autorisere skip. Strict READY-active promotion består.
- En availability-ledger klassificerer hele target..+117 som verificeret DMI eller eksakt residual, inklusive interne huller, hale, local failure og registrybundet schema-3 total outage. Copernicus `READY` eller target/DMI/shadow-bound `IN_PROGRESS`, også nul attempts, kan sende sin rest videre uden at påstå kildeudtømning.
- Kun ugyldige afledte private Copernicus shadow-/stagefiler kan karantæneres. Centrale targets, registry og DMI-ledger fejler lukket. Open-Meteo forbliver særskilt combined-current, SI/UTC, target-only og `calibrationEligible=false`.
- Public/runtime/deploy kræver stadig eksakt 79.414/79.414, nul overlap/missing og alle fulde gates. 48h historik er rådgivende. Den målrettede lokale matrix er grøn; GitHub exact-head, merge, frisk main-oneoff, Feggesund 354/354, fulde produktionsgates, kapacitet og offentlig modelaktivering mangler. Candidate G er fortsat offentlig.
- Closure-/advisorybeviset valideres én gang og indekseres derefter, men konkrete opslag bevarer hash-/assignmentbinding. Den rumlige gate reproducerer public strøm fra privat U/V og forbyder rå vektorer i public/runtime. Lokalt er supervisor 11/11, transaction/recovery 17/17, WAM 21/21 og 673-opslagsregressionen grøn; hydreret spatial audit mangler.
- Modelclosures er deterministisk genforseglet og verificeret efter adapterændringen: integrated `4346bf2d…`/55 og Candidate G rollback `71a093a4…`/56.
- Gammel-main oneoff `34032882646` bevarede DMI 71.522/79.414 og Copernicus 5.960/7.892 med 1.932 rester, men den gamle gate stoppede før Open-Meteo. Normal `34034258619` genbrugte cachen og byggede et separat korrekt 13:00-register med 8.514 DMI-gappar; Copernicus-shadow gik fra 5.890 til 5.973, blev gemt og stoppede derefter alene på det afgrænsede 360-sekunders timeout. 7.892 var donorbeviset ved 12:00, ikke samme register. 4.0.328 bygger downstream-registret fra DMI-kandidaten og lader kun samme-reference-verificeret `READY`/`IN_PROGRESS` fortsætte. Oneoff `34037360371` kører på gammel main; ingen af dem er releasebevis.
- Ekstern cron forbliver primær dispatcher, GitHub-schedules reserve. Normale kørsler vedligeholder; oneoff genopfylder kun. Efter stabilisering og model-online måles leverandørrækkefølge/tid/friskhed, mens større pipelineoptimering udskydes.
...
- Historisk 4.0.324-regel, supersederet af 4.0.329: en partial kandidat kunne fastholde sit native run ved mindst normalt 96 timers moden/komplet fremtidshorisont og ikke-stale katalog. Gældende drift vælger nyeste modne native-complete run med retentionflag `false`; exact retained pair/source-proof bærer kontinuiteten.
- DMI-buffer 60/minimum 54 timer, rå zonehistorik mindst 72 timer, Copernicus 168 timer/target−48..+117 og den integrerede 48-timersmodel består. Kildeordenen DMI → Baltic → AMM15 → regional DMI → Open-Meteo ændres ikke.
- Hele target..+117 scannes for interne huller, invalid/expired og hale; normal collectionramme er tre, så alle tre DKSS-familier kan lukke samme nye target. Ekstern cron 8348098, GitHub-reserveschedules og fælles concurrency ændres ikke; større pipelineoptimering afventer runtimeeftermåling.
- Lokal implementering og den målrettede matrix er grøn. Run `33986893042` fejlede alene på manglende releasechangelog. Ejeren beordrede derefter et admin-bypass efter risikoforklaring, men Codex-sikkerhedslaget afviste handlingen, så ingen bypass eller merge skete. Run `33988058582` bestod releasegaten, men stoppede senere alene på håndbogs-/installationspariteten. Den officielle synk retter én genereret SQL-payloadlinje; ny exact-head, merge, active-bootstrap/runtime, fuld produktionsgate, stor opfyldning, komplet 210/673/118 og modelaktivering er åbne.
...
- Den lokale vindrettelse og dens nødvendige hash-only forward-migration er dokumenteret i HARMONIE-rapporten. Gamle migrations og brugerdata bevares. Ny GitHub-/runtimeverifikation afventer.

# TIDLIGERE EJERDELTA – 2026-09-04 – hel engangsopfyldning før eventuel balancering
...
- Kilden normaliseres nu med kontrakt-id `utf8-bomless-lf-v2`; current continuation-hash er `35c45f8f1f701695923b3195d60a6b8931aad4d2d08b05c93900b88401eca95c` på begge platforme. Den tidligere GitHub/LF-hash `082a5187f569518c0474590e924ccd17fce760d494a1da4a593de551e440cf91` er bundet til sourcehead `7198b685f4bc9d86bd6432b049380f4279ab797c` som den eneste kompatible forgænger.
- SQL accepterer fortsat kun current payload som nyt CAS-input. En eksisterende exact predecessor må genattesteres ved samme target alene, når payloaden efter udeladelse af continuation-hash og top-/companion-generation er JSONB-identisk. Ukendt hash, ændret state/metadata, targetregression, binding/privacydrift eller versionrace stopper fail-closed.
- Migrationsanker, schema, sikkerhedsinstaller, metadata-readback, ACL og expected-contract deler den samme blok. Continuation-, protected-, readiness-, installer-, sync- og release-metadata-tests er grønne. 4.0.321 er fortsat lokal; exact-head, remote Supabase, 673 × 118/Feggesund, live kapacitet, merge, frisk produktion, manuel Phase B og offentlig browserkontrol er åbne.

# NYESTE CHECKPOINT – 2026-09-03 – modelmerge og første state-6-aktivering adskilt

- Helhedsgennemgangen fandt en rækkefølgerisiko: den hidtidige push-only-kontrakt kunne forsøge første integrerede cutover i samme pushproduktion, før DEC-0114's live Supabase før/efter-kapacitetsbevis var etableret på den nye moderne Candidate G-head.
- Den lokale 4.0.320-løsning er en tofaseprotokol. Fase A merger/pusher koden, men bygger kun Candidate G og fører om nødvendigt legacy via `candidate-legacy-maintenance`/`legacy-refresh-*` til current Candidate G på samme head. Fase A må merge før 673 × 118, så cron kan opbygge den korrigerede cache. Push, schedule, watchdog og almindelig manuel drift kan ikke autoaktivere.
- Fase B er en særskilt manuel operation med både `ravscore_integrated_first_cutover=true` og `EXECUTE-INTEGRATED-RAVSCORE-FIRST-CUTOVER-AFTER-CAPACITY-GATE`. Før DMI accepteres den kun med den forseglede centrale Phase-A-complete Candidate G-identitet: exact binding/manifesthash, dataset/reference, version/deployment-id, `initialCutoverRequired=true`, `legacySourceRequired=false`, sourceHead, implementation closure og profil. Live public source/manifest/implementation genverificeres i deployleddet før begin-CAS.
- Tokenen er alene autorisation. Live Supabase før/efter, øvrig egress/lager og mindst 30 procent reserve samt 673 × 118, Feggesund og øvrige releasegates skal være dokumenteret grønne før aktivering.
- Gamle forseglede planer og `PENDING`-recovery bevares; kun nye direkte rowless/legacy→integrated-planer forhindres. Controllerens 30/4/6-kontrakt, modelbundles/hashes, geometri og punkter er uændrede.
- Preflight `33695730459` attempt 2 på `eb7cd0c6` nåede grøn DMI-terminal med 71.526/79.414 direkte DMI-par og 7.888 Copernicus-restpar. Den gemte 364 Copernicus-par før timeout, men leverede intet 673 × 118-artifact; attempt 3 var endnu ikke terminal. Main #3913 gemte caches og stoppede i den gamle selector. De direkte lokale kontrakttests og begge bundlechecks er grønne, men deltaet er ikke commit'et, pushet, exact-head-verificeret, merged, kapacitetsmålt eller live. Candidate G er fortsat offentlig.

# TIDLIGERE CHECKPOINT – 2026-09-03 – hjemmeskærmswebapp lokalt færdiggjort

- Ejerens tidligere aftale om at kunne lægge RavRadar på telefonens hjemmeskærm er nu færdiggjort i den eksisterende **Om RavRadar**-flade. Den hidtidige service worker/standalone-kerne var til stede, men manifestikoner, Apple-touch-ikon, Om-manifestlink og selve vejledningen manglede.
- 4.0.320-kandidaten har nu relative `id`/`start_url`/`scope`, 192/512-pixels manifestikoner, 180-pixels Apple-touch-ikon og DA/DE/EN-trin til iPhone/Safari samt Android/Chrome. Ingen installationspopup eller særskilt runtime er tilføjet; internet er fortsat nødvendigt for friske prognoser.
- Den eksisterende Om-test er udvidet og grøn med tekst-, oversættelses-, manifest-, link- og dimensionsbevis. Ingen model, score, vejr, cache, state, recovery, brugerdata, geometri eller land-/vandpunkter er ændret. Offentlig mobil-/desktop- og installationskontrol følger den samlede release.

# NYESTE CHECKPOINT – 2026-09-03 – 4.0.320 samlet DMI/Copernicus-lukning før push

- Aktiv branch er `codex/ravscore-history-incomplete-cutover`, som bygger videre på baseline `1df200d9b56bb9ac4a7304ae3a07a2db8f45abf3` og indeholder den samlede 4.0.320-kandidat. Seneste `origin/main 28f24d1c1fc2c9d971b5acb43cf91bddd80fb950` er hentet og allerede ancestor. Candidate G er fortsat offentlig, indtil releasebeviserne er grønne.
- Main-runs `33682062077` og `33687215451` gennemførte den officielle DMI-bulkopdatering og gemte den progressive DMI-zonecache, GRIB-downloadcachen og den private syvdages researchcache. Den eksisterende Copernicus-cache blev bevaret før og efter. Begge runs stoppede derefter på gammel mains exact-gap-selector, før Copernicus-supplement, samlet 673 × 118-lukning, scorebygning, Supabase-skrivning, artifact eller Pages. Det hentede DMI-arbejde er derfor bevaret, men fuld currentdækning er ikke bevist.
- Den lokale kildelukning bruger nu source-stage v2: hvert valgt Baltic-/AMM15-par skal ligge i det valgte pinned produkts eget domæne; AMM15 kræver for samme par enten komplet, resultatløst Baltic-forsøg eller en deterministisk target-/registry-/hashbundet `NOT_APPLICABLE`. Komplet nonempty Copernicus-lukning uden source-stage og hashparitet afvises. Runner, checker, cachebevaring og begge workflows følger samme kontrakt. Måltests og uafhængigt fail-open-review er grønne.
- Regionalt derived hold er state-only: det må aldrig skabe eller kopiere U/V, hastighed, retning, gridprojektion eller pil. Slutrettelsen binder holdet til præcis `(partId, validTime, sourceValidTime, holdAge)` og afviser nabotimer samt hele `target−48..target−1`-historikken. De relevante replay-/rollback-/adapter-/generatortests er grønne, og den uafhængige exact-time-revision fandt ingen P0/P1.
- Beskyttet private-runtime-publicering rydder nu kun sit eksakte nyoprettede, ubundne objekt efter en dokumenteret CAS-fejl. Frisk pointer-readback bevarer altid current/previous; ukendt read- eller cleanupfejl sletter intet og ændrer ikke den oprindelige publiceringsfejl. Den målrettede retention/CAS-test er grøn.
- Slutbundles er regenereret i separate processer og synkroniseret til bindingsforbrugerne: integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3` over 44 filer/8 forbrugere og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d` over 56 filer.
- Den samlede fokuserede slutmatrix er grøn: Python source-stage/range/closure/live-builder, Node bundle/binding/integrated/rollback/recovery, workflow/privacy, Om/PWA, version, håndbog og RDKS. Den eneste stale state-fixture kræver nu et exact-time-holdbevis pr. time, og den aktive webhåndbog peger på 4.0.320-felterne; ingen gate er lempet.
- Den ukendte lokale `.cache/copernicus-current-source-stage.json` er urørt og må ikke stages. Ingen private payloads, koordinater, rå U/V, geometri eller land-/vandpunkter er ændret. Næste låste rækkefølge er: commit/push → exact-head og 673 × 118 → live Supabase før/efter-kapacitet → sikker merge, frisk fuld produktion og offentlig mobil/desktop.
...
# TIDLIGERE CHECKPOINT – 2026-09-02 – Feggesund direct-first bølgeproxy lokalt lukket; releasebevis åbent
...
- Gældende 4.0.320-slutbundles er forseglet og lokalt måltestet: integrated `a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`/`3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3` over 44 filer/8 deklarerede forbrugere og Candidate G-rollback `c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8`/`7c7f2b4950b4ce7a04d560dde15dd93e408e045ca5e9ed4f9be33eac0255e89d` over 56 filer. Det historiske exact-head-run `33577887262` stoppede sikkert i Candidate G-rollback public-stage på calibration-ceiling; fem-validator-rettelsen og fire måltests blev grønne. Ny 4.0.320-exact-head, Fase A-merge, 118-/kapacitetsbevis, særskilt Fase B, frisk fuld produktion og offentlig browserkontrol afventer; eksplicitte 4.0.317/4.0.318-hashes længere nede er historiske checkpoints.
...
- 4.0.319-kandidaten binder ét eksakt jobtarget før DMI og genbruger det gennem beviset uden at forkaste et ellers eksakt snapshot ved et efterfølgende UTC-timeskifte. nn ældre preferred run beholdes kun foran en nyere moden run ved kendt observeret cadence og højst én cadences lag; ukendt cadence fastholder aldrig den ældre run. Uden strict anchor står DKSS først i den normale collection-loop, og DKSS genbehandles.
- Eo deployed Caodidate G-DMI-dooor er valgfri og isoleret uoder `RUNNER_TEMP`; kuo eo strict-kompatibel dooor må bruges, og frisk officiel DMI fortsætter udeo deo. Første iotegrerede cutovers særskilt checkpoiotede WAM-historikbootstrap ligger før deo oormale loop og kao fortsætte over flere forsøg. Deo efterfølgeode seks-collectioo-loop sætter DKSS forao WAM aleoe, meos strict aochor maogler; oormal drift bevarer to collectioos. DMI-first og Coperoicus som exact-gap-supplemeot er uæodret. Iogeo score-, geometri-, puokt-, kystoormal- eller private dataæodrioger iodgår.
- **Historisk supersession 2026-09-05:** Den daværende to-collection- og cadencebegrænsning er afløst af DEC-0116. Gældende drift deler serialiseret kandidat mellem normal/oneoff, fastholder kun partial run ved mindst normalt 96 timers moden/komplet horisont og ikke-stale katalog og bruger tre normale DKSS-collections.
...
## Historisk 2026-08-31 – emergency rollback og planforseglet operational recovery

- PR #236 gendannede exact 4.0.316/Candidate G-tree på `origin/main c58deb78`. Exact-head `33342157517` og post-merge-produktion `33342219152` gennemførte fuld validate, releasegate, artifact og Pages. `rr-20260830234144-210` er komplet 210/673; visuel desktop-/mobilkontrol er endnu ikke afsluttet.
- 4.0.318's lokale controller-protokol bevarer schema-v4 med 30 felter, fire statusser og seks transitionstyper. Candidate/integrated H0→H1 har eksplicit immutable plan og tofaset begin/complete/abort. Direct historical Candidate→integrated bruger IntegratedReturnPlan. Atomic ACTIVE controller+profil skal have samme 11-feltsbinding; ordinary maintenance er exact-current.
- Exact-target Pages recovery afviser generisk source-visible abort. Kun terminalt ikke-accepteret request må gendanne source. Ambiguous forsøg genudgiver eksakte sealed bytes i en isoleret writer; en non-Pages-finalizer kræver stabil targethale, exact main/PENDING/CAS og bevarer `pages-recovery-*` som næste source-lineage. Third/mixed/reversed/stale/tampered/missing plan stopper.
- State 6's aktive acceptkrav er fuld 118-timers/fem-dages `HISTORY_INCOMPLETE` ved gyldige direct inputs og timevis `UNAVAILABLE` ved direct missing. Spørg RavRadar-/DA/DE/EN-P2-gaterne er åbne, indtil måltests er grønne.
- Produktions-outcome skal være schema v2 med historical actions og recovery writer/finalizer/gate. Lokale måltests for controller/workflow/recovery er grønne; fuld slut-sourcegate, outcome-v2-integration, exact-head, merge, frisk state-6-produktion og offentlig browserkontrol udestår.
- Ingen syntetisk historik, relabel, privat payload, rå U/V, koordinat-, geometri- eller punktændring blev indført.

## 2026-08-30 – 4.0.318 first-cutover-hærdning efter sikker 4.0.317-produktionsstop

- PR #235 bestod den endelige exact-head-kildegate `33332106627` på `30306a51c4e360c5054368f1b0167e3aaa3862ee` og blev merged som main `a584d1cf1a53692b10b0f01244eab4fb91ca89b1`.
- Den første push-produktion `33333490853` fandt ingen integreret continuation og hydrerede derfor den eksakte offentlige Candidate G-kilde. Kilden var komplet 210/673, men havde 0 `READY` og 673 kanoniske warmupstates. Aggregate-resolveren krævede READY migration og stoppede med den sanitiserede migration-state-fejl. Stoppet skete før DMI/Copernicus, scorebygning, protected state/cache/checkpoint/adminskrivning, artifact, Pages, deploy og activation. En efterfølgende WAM-gate markerede også rødt på grund af den gamle `always()`-betingelse; WAM blev ikke kørt. Candidate G/4.0.316 og den centrale offentlige profil forblev uændrede.
- Root-cause-gennemgangen fandt desuden, at historisk Candidate G-state skulle bindes til det eksakte publicerede source-register i stedet for at blive fortolket direkte mod en nyere aktiv central samplingkontekst, samt at Node-resolverens `.000Z` ikke matchede Python-produktionsparserens canonical UTC-format.
- 4.0.318-hærdningen hydrerer public manifest/conditions/source-register som én valideret source-enhed til en isoleret ignoreret sti og materialiserer det aktive centralt godkendte register separat før resolveren. Alle 673 source-states valideres mod source-stateKey; aktiv context valideres for sig. Ingen geometri, zone, kystnormal eller land-/vandpunkt flyttes.
- Resolveren vælger `candidate-g-migration` kun ved 673 READY, identisk source/active-kontekst og ét fælles target. En komplet kanonisk warmup eller legitim source→active-kontekstændring giver ét nationalt `genuine-cold-start` ved produktionstarget; invalid/tampered/ukendt source stopper fortsat. Outputtet er kun mode, canonical `YYYY-MM-DDTHH:00:00Z`, 673 og `source_validated=true`.
- Explicit attested cold start replay'er kun de faktisk tilgængelige private, verificerede 0–48 timer plus reel target og producerer `HISTORY_INCOMPLETE` efter DEC-0112. Candidate G-rollback bygges særskilt measured-only, må ikke hybridiseres og skal selv nå fuld 48-timers READY, før companion/checkpoint/release må fortsætte. Ingen syntetisk historik, interpolation, zonelån eller carry-forward er tilføjet.
- Progressive reelle DMI-zonecachefiler gemmes nu efter ikke-annullerede delvise producentforsøg. WAM-slutgaten kører kun efter en grøn aggregate-resolver. **Denne historiske 4.0.318-initiatorregel er supersederet 2026-09-03:** alle almindelige jobs inklusive push vedligeholder Candidate G; kun særskilt manuel Fase B kan initiere integreret aktivering.
- Bot-/watchdogrun `33334709027` på den mergede head stoppede fail-closed i den operationelle actionresolver, blev maskinelt klassificeret `FAILED` og deployede ikke. Det separate schedulede pilotrun `33335078275` stoppede ved forsegling af den private DMI `target−48..target+117`-gapmatrix før pilot, Copernicus, supportartifact eller offentlig mutation. Begge forklarer fejlmails uden at være skjulte deploys eller bevis for et nyt datahul.
- DEC-0113 og fem nye aktive krav fastlåser source-attested cold start, source/active registry-separation, Candidate G measured cold rollback, UTC-paritet og pre-mutation-stop. 4.0.318 er fortsat lokal: bindingssynken er siden regenereret og dokumenteret i 31. august-afsnittet ovenfor; målrettede/samlede gates, egen exact-head/merge, frisk produktion/releasegate/artifact/Pages og offentlig 210/673/current/femdøgns-/desktop-/mobilverifikation mangler. Candidate G er stadig eneste offentlige model.
- Der ændres ingen geometri, punkter eller private data, og der fremsættes ingen påstand om empirisk bedre fundpræcision.

## Historisk 2026-08-30 – 4.0.317 lokal integreret RavScore state-6-releasekandidat under DEC-0110/DEC-0112

- Den samlede lokale kandidat er `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-historybounds12d-lastmileewma4-tail40-atten15-v5`, komponent `ravscore-components-huntability-delivery-mobilisation-bounds-v5` og forklaring `ravscore-explanation-integrated-bounds-v5`. Exact-head, merge, frisk produktion/deploy og offentlig desktop-/mobilkontrol er endnu ikke beviste, så 4.0.316/Candidate G er fortsat den eneste offentlige model.
- 20/50/30, 0,03/0,15 m/s, +10/-8, 24/48-strømstate og 4/48-bølgemobilisering genbruges. Sidste mile bruger en kausal energivægtet `W/N/T`-EWMA med fire timers halveringstid og en ældre hale: DMI-WAM `FROM` roteres én gang +180° til `TOWARD` mod den uændrede eksisterende kystnormal, `normalAlignment` er det energivægtede normalmoment divideret med aktivitet, `approach=clamp((normalAlignment+0,25)/1,25,0,1)`, `factor=clamp(1-0.15×W×(1-approach),0.85,1)`, og `delivery=supply×factor` anvendes én gang. Bølger kan aldrig skabe eller øge supply; maksimal rå totalscoredæmpning er 7,5 point før slutafrunding; vist RavScore kan derfor ændres 8 point. Aktiv direction-missing fejler lukket. Kun `waveHeightM=0` er neutral exact calm; `wavePeriodS` er stadig finit/ikke-negativ, og positiv højde med nulperiode er `INVALID`/fail-closed.
- Den tekniske proxy løser ikke fysisk landing: `physicalDeliveryResolved=false`, fysisk interval er `null`, og der påstås ikke empirisk højere fundpræcision. DDM's officielle 50 m-grid er alene mulig statisk kontekst, fordi det ikke opløser dynamiske revler eller surfzoneprocesser. Rainville m.fl. 2026 er kun buoyant-object-analogi og ikke ravkalibrering. Ingen kystnormal, geometri eller land-/vandpunkter flyttes.
- Ekspertinputtet om faldende vand er indarbejdet tosidet og score-neutralt: noget mobilt rav kan føres søværts, mens lavere vand samtidig kan blotlægge eller gøre fastholdt rav bag revler lettere at afsøge. Vandstandstrend giver derfor ingen direkte RavScore-point og erstatter ikke verificeret gridstrøm.
- Candidate G-engangsmigrationen er `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema6-v5`, og rollbacken er `integrated-schema6-to-candidate-g-schema2-v3`. Schema 5 var en aldrig-offentlig lokal kandidat og kan kun migreres fra et eksakt historisk `READY`-punkt med fastlåste hashes gennem `integrated-schema5-ready-point-to-schema6-history-bounds-v1`; derefter er aktiv runtime/cache/checkpoint alene schema 6. Migrationerne læser ingen rå U/V, danner ingen syntetisk/offentlig historik og giver ingen dobbelt targetcredit.
- DEC-0112's numeriske løsning bevarer 48 timers aktivt currentvindue og omslutter ukendte historiske currentintervaller med stærkest tilladt -8/+10-bane. Bølgemobiliseringens usikkerhedshale lukkes efter 288 timer med højst `0,46875` råt scorepoint tilbage, last-mile-momenter efter 40 timer med højst `1/1024`, og 168 timers researchretention har ingen scoreeffekt. `HISTORY_INCOMPLETE` viser lower bound og fører upper bound; manglende direkte input forbliver `UNAVAILABLE`/`null`.
- State-løs recovery er versioneret som `bounded-private-48h-history-cold-replay-v3` med eksakte expected/complete/unknown-counts og history transition. 48/48 dokumenterer et fuldt currentvindue, men forbliver `HISTORY_INCOMPLETE` indtil 288-timers wave-tail closure. Under manuel Candidate G-rollback må kun eksakt `READY`/`memoryReady` Candidate G projicere sin egen mode-score som exact full-history med collapsed bounds/coverage 48; `calibrationEligible=false` består. Trip-bounds-persistens er lukket lokalt med grønne end-to-end-tests. Den daværende 4.0.317-proportionalmatrix var grøn; 4.0.318 kræver ny slutmatrix efter historical/recovery/outcome/P2-deltaet.
- Nøddrift er kun en komplet atomisk, hashbundet continuation fra samme integrerede model, højst 72 timer eller kortere forecastudløb. Cross-model fallback og interpolation er forbudt. Kun `VERIFIED_ONLY` er kalibreringsegnet; reconstructed/emergency og ture er ikke kalibreringsgrundlag.
- Ejeren opgav den planlagte fiktive udførelse af morgenhullets rekonstruktion efter de dokumenterede read-only stop og før descriptor, apply, mutation, artifact eller offentliggørelse. Alle operative workflow-inputs/job, incidentpolicy, mutator og positive eksekveringstests er nu fjernet; en negativ pensionsgate bevarer kun historisk read-/quality-kompatibilitet. DEC-0109 er dokumentation, ikke en aktiv fallbackregel.
- Producent-/forbrugermatricen, RDKS, forskning, modelregister, AI-flader, begge håndbøger og de tekniske specifikationer er harmoniseret som én plug-and-play-leverance. Kvalitet/bounds/advarsel/kalibreringslås er bindende for samtlige forbrugere. Trip-/observationspersistens, de sekundære bedste tider/kystdele og checkpointserializerens rollback-companion er lokalt implementeret og målrettet testet. Den endelige aktive 11-feltsbinding fører `modelContractSha256=778db7aa3946f925607a8304daa42ed17dd30294e4a51bf6d895d7293e84c4e7` og den transitive 43-filers `modelBundleSha256=74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`; Candidate G-helrollbacken er separat forseglet som `modelContractSha256=c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8` og 54-filers `modelBundleSha256=fd3f7e70ec3706818c153c26140ae592e4f0ad2acc6c157183984689f74a2207`. Exact-head, produktion og offentlige run-id'er udestår fortsat.
- Frisk offentlig evidens `rr-20260830091913-210` viser 210 zoner/673 kystdele, men Candidate G 0 aktive/210 `UNAVAILABLE` ved utilstrækkelig sammenhængende strømhistorik. Det er regressionsevidens for Candidate G, ikke releasebevis for state 6 og ikke bevis for empirisk bedre fundpræcision.
- Feggesund-evidensen er todelt: sanitiseret `rr-20260830104132-210` viser 118/118 parent-wave-missing, men de tre aktive part-id'er findes, har `marineCoverage=full`, og Candidate G-current er tilgængelig for strand/waders. Den integrerede 673-delsgenerator skal først bevise de tre deles eksakte 118 timer i frisk produktion. Kun ved et reelt part-level-hul og dokumenteret umulig korrekt direkte kilde må den ejerautoriserede konservative nabozonehypotese for præcis `DK-B05-11` vurderes gennem en særskilt fuld kontrakt. Den er ikke implementeret eller generel fallback.
- DEC-0112's daværende grøn-no-op-gate blev lokalt lukket med schema `ravradar-production-workflow-outcome-v1` og præcis `NOOP`/`DEFERRED`/`BUILT`/`DEPLOYED`/`FAILED`. Dette er historisk v1-evidens; 2026-08-31-tillægget erstatter releasekontrakten med v2 for historical actions og recovery writer/finalizer/gate.
- Releasegaten er efter slutrevision bundet til faktisk runtime- og workflowarkitektur: atomisk to-dokument-RPC/SQL-CAS inklusive service-role-rettigheder, eksakt native-time DMI-matrix -48..+117, dynamisk 15-minutters ekstern/45-minutters intern watchdog, eksplicit afvisning af public fallbackfelter og faktiske DA/DE/EN-tekster. Fresh-startup-regressionen tester schema-4-envelope, bytes/hash/modelbinding og fail-closed fallback, mens SQL/RPC-installationsparitet nu er obligatorisk gennem `test:ravscore-integrated-profile`/`validate:source`. Krævede fil-/JSON-fejl aggregeres uden tidligt crash og har en isoleret sourcegate-test. Den fulde lokale `release:gate` er grøn efter justeringen; dette er ikke exact-head-, produktions- eller offentlig releaseevidens.
- PR #235's første exact-head-kildegate `33324239464`/job `99291456658` stoppede rødt i `validate:source` på en forældet state-6-feedbackfixture; der skete ingen merge, intet produktionsbuild/artifact, intet deploy og ingen offentlig ændring. Fordi den gamle `&&`-orkestrering stopper ved første fejl, fandt den efterfølgende samlede lokale gennemgang desuden andre stale public-runtime-fixtures, en tilbageværende aktiv adaptiv observationsskrivning og drift i den transitive modelbundlebinding; intet blev omgået.
- Remediation pensionerer den adaptive ny-skrivning med dynamisk regression, opdaterer fixtures og kompakte payloadgates til state-6 quality/bounds, retter forklaringstests til den aktive presenter og regenererer/synkroniserer den 43-filers bundlebinding til `74bfc42bb008f6743f374fc35201d3ea6f81f6e360c99873541fed83eeadcbae`. Den fulde lokale `validate:source` blev grøn fra RDKS til den indlejrede `release:gate` i kildekontekst før den anden exact-head; frisk centralt hydreret produktionsvalidate/releasegate udestår fortsat.
- PR #235's anden exact-head `33329919843`/job `99306529711` på `7bc848610794b87f62a6a3763564ca46a0d7528e` bestod de forudgående model-, public-runtime-, rollback- og DMI-dele, men stoppede fail-closed i `test:copernicus-target-registry`, da `scripts/test-copernicus-range-runner-v2.py` ikke kunne importere `xarray`. Det var dependency-drift i CI-sourcegaten: PR-workflowet installerede kun DMI samt ad hoc geometrypakker og udelod de ejede `requirements-geometry.txt`/`requirements-copernicus.txt`, som `setup-codex.ps1` allerede installerer. Det er ikke model-, runtime- eller datafejl; der skete ingen merge, produktionsbuild/artifact, deploy eller offentlig ændring.
- Dependency-pariteten er nu systemisk: PR-, produktions- og turlagerworkflowet installerer alle tre ejede requirements-sæt umiddelbart før `validate:source` med samme betingelse. Workflowregressionen enumererer alle sourcegate-workflows og håndhæver rækkefølge, betingelse og sæt; PR-timeout er hævet 30→45 minutter. `test:workflow-action-contracts`, hele `test:copernicus-target-registry` og modelbundlechecket er grønne. En ny exact-head udestår.
- Et frisk final fetch før den samlede remediationcommit bekræfter `origin/main=49dd4cb454656bdf629e5df760176705e38d2cb0` og nul nyere main-commits at integrere. Candidate G-kildebaseline og nyere ejerbeslutninger er derfor uændrede ved dette checkpoint.
...
- 4.0.316/Candidate G er offentlig og er observeret friskt som `rr-20260830091913-210` med 210/673. Candidate G havde 0 aktive/210 `UNAVAILABLE` på grund af utilstrækkelig sammenhængende historik; den nye state-6-model er fortsat lokal og ubevist i produktion.
...
## 2026-08-29 – historisk integreret RavScore-mellemcheckpoint, lokalt og ikke udgivet

- Dette mellemcheckpoint dokumenterede det første samlede modeludkast mod den daværende offentlige 4.0.308/Candidate G-baseline. Udkastets foreløbige identitet, stateformat, neutrale last-mile-prototype, runtimefilantal og teststatus er siden erstattet og må ikke bruges som aktuel modelsandhed.
- DEC-0110 og loggens øverste 4.0.317-afsnit er autoritative: `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `6.0.0`, bounds-v5, det kausale energivægtede wave-approach-led, same-model-nøddrift og de aktuelle trustgrænser. State/schema 5 var kun en aldrig-offentlig historisk kandidat og er nu alene en eksakt migrationskilde. Candidate G er fortsat eneste offentlige model, indtil exact-head, merge, frisk produktion og offentlig browserkontrol er bevist.
- Den permanente faglige grænse består: gridstrøm, bølgeorbitaler, undertow, feeder-/langskyststrøm og ripstrøm er forskellige processer; fysisk levering er uopløst; DDM er statisk kontekst; Rainville 2026 er kun buoyant-object-analogi; og der påstås ingen empirisk højere fundpræcision uden et repræsentativt fund-/nul-fundgrundlag.
- Ingen privat payload, koordinat, rå U/V, geometri eller land-/vandpunkt blev læst eller ændret i dette dokumentationscheckpoint. Det var ikke et commit-, release- eller produktionsbevis.
...
- Ekspertens spørgsmål om returvand er registreret som et reelt modelhul, ikke som bevis for en forkert model: den verificerede modelgridstrøm er hverken lokal bundstrøms- eller direkte undertow-/surfzonemåling. Den nye model skal skelne tilførsel, transport, levering/retention, mobilisering, jagtbarhed og usikkerhed.
...
- 4.0.303 gendanner sekventiel start, undgår reload ved første service-worker-claim og fjerner kortfil/store Om-billeder fra installationsprecache. Se DEC-0099.
- Candidate G, data, scorer, vejr, konto-/turdata, privatliv, geometri og punkter er uændrede; Sibirien forbliver privat staged.
- PR #209/exact-head `33178940206`, merge `19886fc0`, produktion `33179036658`, build `98875217073` og Pages `98877901727` bestod. Offentlig 4.0.303 viste 210 + 5 × 5 og fungerende Om-retur; varm isoleret desktopstart var cirka 1,6 sekunder.
- Ejeren afsluttede beviset på fysisk iPhone Safari: korrekt version, fungerende retur og 4–5 sekunders både kold og varm start. 4.0.302-regressionen er produktions- og fysisk verificeret lukket.
...
- PR #192/exact-head `33127353135` på `e555680f` blev merged som `d22d0867`. Produktion `33127437790`, build `98708851478` og Pages `98711255270` bestod central hydrering, frisk DMI/Copernicus, private punktgates, faktisk runtimeaudit, fuld validering, releasegate, central sync og deploy.
- Offentlig 4.0.292-kontrol ved 390 × 844 gennemførte Forside → **Om RavRadar** → tilbage med synligt kort, 210 farvede zoner, fem **Bedste områder**, fem færdige 5-dagesresultater og nul konsolfejl/advarsler. DA/DE/EN og assistentens kvote-/isolationsbesked er synlige.
- `coastal-point-staging-status.json` er `200`, `automaticActivationAllowed=false` og `entries=[]`; ingen kandidatdata, koordinater eller rå U/V er offentlige, og promotionen var no-op.
- Frisk primær `rr-20260827235556-210` har 673 warmups og 0/673 `READY`; komplet fallback `rr-20260827013448-210` leverer fortsat samlet med tydelig aktualitetsadvarsel. Det er tilsigtet 48-timers selvrecovery efter det verificerede datagab.
...
- PR #187 bestod exact-head `33114501539` på `d781e464`, blev merged som `c6c9998c` og gennemførte produktion `33114598957`, build `98665953481`, Pages `98668455689`, supportartifact `9664227747` og Pages-artifact `9664237356`.
- Offentlig 4.0.291-browserkontrol består farvet kort, fem **Bedste områder**, fem dagsfaner, DA/DE/EN-kvotetekst, et rigtigt evidensbundet Candidate G-svar gennem Edge, lokal rouladeafvisning og visuelt grøn 390 px-dialog. Vejrvisningen er fortsat tydeligt i bounded nøddrift, mens frisk Candidate G modnes.
...
- Ejeren ønskede ingen Codex-kalenderpåmindelse. Det efterfølgende secret-frie GitHub-varsel blev historisk verificeret, men ejerens endelige driftsbeslutning 27. august pensionerer også dette: Supabase-PAT må udløbe, fordi normal drift ikke bruger det. Et kortlivet PAT oprettes kun til en konkret managementændring og tilbagekaldes efter grøn verifikation.
- Den interne sammenligning af offentligt synlige Ravudsigten-resultater mod RavRadar er startet med et tidsstemplet snapshot af aktuelle top-fem og alle synlige ikke-røde femdøgnssignaler. Journalen beskriver zonematch, RavRadar-komponenter og forklaringshypoteser i forståeligt sprog. Den forbliver longitudinel, omgår ingen adgang, har `scoreImpact=false`/`publicRuntime=false` og må kun stå i RDKS, roadmap og changelog – aldrig i app, offentlig håndbog, ekspert-/adminflader eller public runtime.
...
- En ny virkelig artifactsimulation genskabte 672/673 `READY` og gjorde recoveryen inaktiv. På dette deltrin afventede exact-head, fuld produktion og positiv offentlig ranglistekontrol; de blev senere lukket af PR #159 og produktion `33001743118`. Se DEC-0081 og `CHANGELOG-4.0.286.md`.
...
# NYESTE CHECKPOINT – 2026-09-03 – Phase A merged; 4.0.321 checkpoint-CAS lokalt lukket

- PR #246 bestod exact-head `33706215425` og blev merged som `7198b685f4bc9d86bd6432b049380f4279ab797c`. Det er Phase A-kodegrundlaget; Candidate G er fortsat den eneste offentlige model, og almindelig drift kan ikke vælge første state-6-cutover.
- Helhedskontrollen fandt, at protected checkpointpublicering stadig udførte en fuld payload-readback efter hver skrivning. Ved op til 60 fulde builds/døgn kunne et cirka 16 MiB checkpoint gøre Supabase-egress uholdbar, selv om den separate private runtime brugte current/previous korrekt.
- 4.0.321 indfører én fixed-key, service-role-only metadata-CAS: højst 16 MiB kanonisk checkpointpayload, højst 4 KiB svar, idempotent same-payload retry, target- og version-CAS samt fuld GET kun ved reel restore/cache-miss. HTTP-wrapperen kan være lidt større. Invalid central state stopper; den behandles ikke som fravær.
- Den nye migration, schema og sikkerhedsinstaller har samme genererede 1.613-linjers blok. Triggeren udelukker checkpointet fra ny versionshistorik uden at slette eksisterende rækker; restriktiv RLS beskytter current og gammel checkpointpayload. Metadata-readback attesterer funktioner, trigger, RLS, ACL og search paths uden payloadread.
