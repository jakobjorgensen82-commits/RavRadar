## 2026-08-30 – ikke udgivet integreret RavScore-modelcheckpoint under DEC-0110

- Den samlede lokale kandidat er `RRS-COASTAL-PROCESS-INTEGRATED-1.1.0`, state `5.0.0`, variant `COASTAL-SUPPLY-MOBILISATION-BOUNDED-WAVE-APPROACH-HUNTABILITY-2`, profil `cn-003-015-in10-out8-full24-cos48-gap3-wave4-48-coldrestart-gapcredit1-lastmileewma4-atten15-v4`, komponent `ravscore-components-huntability-delivery-mobilisation-v4` og forklaring `ravscore-explanation-integrated-v4`. 4.0.315 er releasekandidat; exact-head, merge, frisk produktion/deploy og offentlig desktop-/mobilkontrol er endnu ikke beviste, så Candidate G er fortsat eneste offentlige model.
...
- Engangsmigrationen er `candidate-g-schema2-signed-current-reweight-bounded40h-wave-approach-to-integrated-schema5-v4`: signeret afledt kystnormal Candidate G-currentevidens genvægtes uden rå U/V eller påstand om rå genberegningslighed; præcis 673 gyldige schema-2-states skal give ét fælles target; wave-approach bruger 40 private præ-target-positioner fra coherent WAM-run pr. collection med same-cell provenance og kun højst fire timers same-run/same-cell-interpolation. Tail-/rå-scoregrænser er `1/1024` og `0.01171875` før afrunding. Fejl bevarer Candidate G offentlig, og ingen syntetisk/offentlig historik dannes. Ægte cold start er særskilt: 48 sammenhængende private verificerede timepositioner plus reel target. Rollback `integrated-schema5-to-candidate-g-schema2-v2` bruger samme target uden dobbelt credit.
...
- Producent-/forbrugermatricen, RDKS, forskning, modelregister, AI-flader, begge håndbøger og de tekniske specifikationer harmoniseres som én plug-and-play-leverance. Endelige hashes og grønne run-id'er må først føres ind efter faktisk exact-head og produktion. Den separate 4.0.316-post nedenfor er autoritativ for den aktuelle offentlige P0 og produktionsfejl.
