...
- Seneste produktionsverificerede sandhed er fortsat `origin/main` `a93082548c4cc1ddbe9c75ce303d334530a534c4` / 4.0.308 / Candidate G. Den lokale integrerede kandidat har endnu ingen exact-head-, produktions-, deploy- eller offentlig browserverifikation.
- DEC-0108 binder den samlede kandidat til `RRS-COASTAL-PROCESS-INTEGRATED-1.0.0`, state `4.0.0` og én variant/profil. `modelContractSha256` binder parameterkontrakten, mens `modelBundleSha256` binder 34+ kanonisk normaliserede transitive implementeringsfiler; slutværdierne afventer regeneration på afsluttet head. 20/50/30, 0,03/0,15, +10/-8 og bølgemobiliseringens 4/48-princip genbruges analytisk; gridstrøm bruger nu 24 timers fuld vægt og cosinusfade til nul ved 48, og den gamle 13-timers helscore-gate er fjernet.
- Sidste mile er ærligt score-neutral: `delivery = transportPotential × 1` betyder nul estimeret effekt, ikke 100 % fysisk levering. Gridstrøm er ikke bølgeorbital eller lokal bundstrøm, og surfzonens undertow, feeder-/langskyststrøm og ripstrøm holdes som forskellige processer. Faria 2000, Reniers 2009, Gallop 2018 (DOI `10.1016/j.margeo.2018.07.015`), MacMahan 2010, Aagaard 1997/2002 og Moulton 2017 understøtter, at revle-/surfzonecirkulation og nettoretning er sted-, geometri- og faseafhængig. Aagaard 1997 og Mouragues 2020 er feltkilder; Haller 2002 er et laboratorieforsøg om morfologiafhængig feeder-/ripcirkulation og ikke et lavvandsfeltbevis. Jalón-Rojas 2025 viser partikelpositionens betydning for Stokes versus undertow, mens Lofty 2023 målte rav som lavdensitets bedload/saltation, ikke en fri overfladetracer. Kilderne er mekanistisk evidens, ikke en dansk rav-landingsandel eller numerisk last-mile-faktor.
- Ejerens ekspertinput er indarbejdet tosidet og score-neutralt: `waterLevelTrendCm3h` er fremadrettet modelændring fra den viste time til tre timer senere, aldrig aflæst ebbe, flod eller tidevandsfase. Faldende vand bestemmer ikke strømretningen, men vises sammen med verificeret udgående, indgående eller langs/for svag kystnormal gridstrøm inden for ±0,03 m/s – eller ukendt/native-hold. Det kan falde sammen med søværts transport af noget mobilt rav. Lavere vand kan samtidig blotlægge allerede afleveret eller fastholdt rav bag revler og gøre et mindre område lettere at afsøge, uden at bevise fysisk koncentration. Fælleskonteksten ændrer ikke score, state eller transportpotentiale. Waders-tie-breaken bevares og viser nu den eksakte årsag; lavere vand ved samme RavScore er en søgbarhedsprioritet, ikke tegn på mere rav eller en sikkerhedsvurdering. DMI DKSS-/Copernicus NEMO-gridstrøm og vandstandskontekst har ingen direkte dubletterm i RavScore, men kan have residual korrelation gennem fælles drivere; derfor bygges ingen ekstra “hele vandsøjlen”-strøm.
- Den score-neutrale sidste-mile-kontekst og waders-årsagen er målrettet regressionstestet lokalt. `scripts/test-ravscore-last-mile-water-context.mjs` dækker alle fire strømklasser, native-hold/uverificeret strøm, de eksakte ±0,03-grænser samt transport 0 med mobilisering 100 og beviser identisk rå/slutscore og transport. Best-time-, public-model-, public-runtime-, DA/DE/EN-forklarings-, 210/673×femdøgns-, håndbogs-, assistent- og RDKS-tests er grønne. Den samlede integrerede modeltest stopper som forventet på den bevidst endnu ikke regenererede kontraktdigest og afventer planlagt slutregeneration af kontrakt-/bundlehash; den påstås ikke grøn på de midlertidige hashes.
- Chubarenko 2017 understreger manglende systematisk ravmonitorering og beskriver gentagen migration mellem strand og undervandsskråning med mulig tilbageførsel over dage som hypotese. Det giver ingen simpel faldende-vand-score og intet bevis for “alt rav væk”.
- Første kontrollerede schema-2→schema-4-skift genbruger eksisterende valideret, afledt vejrhistorik, så modellen ikke kræver flerdages ny cache eller kunstig opvarmning. Recoveryprioriteten er exact point-aktivering → gyldig integreret continuation → gyldigt checkpoint → dybt valideret Candidate G-state. Ugyldig exact point stopper straks; en ugyldig ordinær kandidat må ikke skygge for en gyldig lavere prioritet. Hver continuation-state, checkpointet og den centrale profilselection skal matche den fulde 11-feltsbinding. Hvis ingen statekilde findes, genafspiller kystdelen præcis de private, proveniensverificerede kildetimer target−48 h til target−1 h. Offentlige/syntetiske pre-target-rækker afvises, komplette data giver `READY` ved første offentlige target, og en manglende eller ugyldig kilde stopper build/release med `RAVSCORE_RECOVERY_REPLAY_BRIDGE_MISSING`. Normal drift bruger derefter privat otte-fils runtimebundle, inklusive forseglet Copernicus-range-cache, eller beskyttet schema-4-checkpoint; Pages må kun indeholde fire hashbundne liveprojektioner.
- Migrationens lange bølgegap er lokalt rettet med tidsbundet seed, konservativ `MIGRATION_RESTARTED_AFTER_GAP` og højst én times ny opbygningskredit. `scripts/test-ravscore-wave-mobilisation-state.mjs` og `scripts/test-ravscore-integrated-state-pipeline.mjs` er grønne lokalt; samlet source-/release-/produktionsbevis mangler fortsat.
- P1-egenkontrol lukkede to integritetskanter lokalt: verificeret DMI/Copernicus-U/V afleder nu selv kanonisk 0,01 m/s-fart og toward-retning med 360°→0° i stedet for at stole på parallel cached fart/retning, og den aktive model-/state-/migration-/recovery-/runtime-/public-kæde afviser numeriske strenge og andre coercible typer. Målrettede negative fixtures findes; slutpakken og bundlehashen genkøres efter samlet hardening.
- Aktiv RDKS, Markdown-håndbog, webhåndbog, changelog og plain-language-kontrakt er harmoniseret med den integrerede kontrakt. Bølgehøjdens tidslige mobiliseringsrolle og aktuelle metode-/sigtbarhedsrolle er dokumenteret som to adskilte kausale udfald uden ekstra transport-/last-mile-kredit. Schema 3 accepterer kun eksakt integreret eller forseglet Candidate G-11-feltsbinding; ukendte/forfalskede bindinger afvises. Konto-DTO'en bærer kun den privacy-sikre eksakte `model_binding`, og Pages udleder current/historical/ineligible mod det aktive overlay uden at ommærke turen. Under Candidate G-rollback er Candidate G current-ineligible og integrerede ture historiske. Historiske Candidate G-afsnit bevares som versionsspor; Candidate G forbliver eneste offentlige model indtil atomisk cutover. Mens integreret er aktiv, er Candidate G kun privat migration-/offline-/rollback-orakel; kun et senere manuelt, fuldt verificeret helskift kan igen gøre den til den ene offentlige model. Dokumentationsgates genkøres efter slutdigests.
- Red-team-fundet om tværflade-cutover er gjort til en åben releasegate: `20260829010000_ravscore_operational_documents_no_history.sql` anvendes før `20260829020000_integrated_trip_calibration_binding.sql`, protected readiness binder begge efter samlet database-/Edge-readback, og ny Edge giver eksakt `409` ved manglende/forkert binding. Den første migration undlader kun fremtidige versionskopier for allowlistede operationelle dokumenter; eksisterende `admin_document_versions` bevares, og der udføres ingen destruktiv oprydning. `deploy-trip-storage` genverificerer efter dry-run `origin/main == GITHUB_SHA` umiddelbart før første eksterne backendskrivning og fortsætter post-write fra samme validerede checkout/migrationssnapshot. Første cutover er push-only og går source-manifest → `INTEGRATED_PENDING` med bevaret central Candidate-profil → integrated Pages → exact implementation+210/673 → atomisk `INTEGRATED_ACTIVE` + central integrated profil. Dokumentation er ikke produktionsbevis.
- Den operationelle v3-controller er lokalt kontraktbundet til fire statusser og fire overgangstyper. Candidate G-rollback og integreret return er manual-only; Candidate-refresh er kun tilladt for allerede `CANDIDATE_G_ACTIVE`. Alle skift bruger source/requested-manifest-reconcile: target completer efter genverifikation, source aborterer/rekonsoliderer med source-profil, og tredje hash forbliver fail-closed `PENDING`. Der deployes ingen særskilt Candidate G-assistent-Edge; integreret Edge giver eksakt `409`, og klienten bruger deterministiske lokale DA/DE/EN-svar. Backend accepterer kun eksakt integrated eller forseglet Candidate G-11-feltsbinding; Candidate G gemmes altid med `calibration_eligible=false`. Separate Candidate G-kontrakt-/bundlehashes og samlet exact-head-/produktionsbevis afventer.
- Håndbogens statiske installationskopi er synkroniseret efter den afsluttende indholdsændring. Plain-language/paritet, RDKS, produktionsadapter, schema-4-state, recovery, cutover-readiness, beskyttet dokumentation, Candidate G-rollbackforberedelse og v3-controllerens transition/reconcile-tests er grønne lokalt. Den integrerede modeltest stopper fortsat korrekt på den bevidst endnu ikke regenererede transitive bundlehash; hele pakken gentages efter slutregeneration. Intet af dette er central readback, deploy eller produktionsbevis.
- Dual-hash-red-team-fundet er lukket lokalt: `modelContractSha256` og `modelBundleSha256` følger nu den fulde 11-feltsbinding gennem alle aktive producenter og forbrugere og har særskilte negative mismatchtests. Slutværdierne forbliver ulåste, indtil den afsluttede implementeringshead er regenereret og genvalideret.
- Den fulde syvfilers runtime er dokumenteret efter den faktiske private storagekontrakt: komprimeret bundle i den ikke-offentlige bucket `ravradar-private-production-runtime`, beskyttet current + previous-pointer, service-role-only og obligatorisk anonym afvisning. Det kompakte schema-4-checkpoint er fortsat et separat, dataminimeret recoveryled.
- Ingen privat payload, koordinat, rå U/V, geometri eller land-/vandpunkt er læst eller ændret i dokumentationsarbejdet. Der er ikke committed, pushet, åbnet PR eller påstået produktion.
