...
## Bindende operationelt designtillæg 2026-09-03 – Fase A før særskilt Fase B

Fase A merger 4.0.320-koden med Candidate G fortsat som eneste offentlige model. Push, schedule, watchdog og almindelig manuel vejrdrift kan kun vedligeholde Candidate G; legacy føres først gennem den eksisterende bro til current moderne Candidate G på samme head. Fase A må ske før 673 × 118-beviset, så cron kan opbygge den korrigerede cache på den mergede kode.

Fase B er en særskilt manuel operation med både `ravscore_integrated_first_cutover=true` og den eksakte bekræftelsestoken. Før DMI valideres den allerede forseglede centrale Phase-A-complete Candidate G-identitet med fuld binding og manifesthash, dataset/reference, deployment-id, implementation closure og `sourceHead`; dette er ikke live Pages-readback. Den faktiske offentlige source, manifest og implementation closure genverificeres i deployleddet, før første begin-CAS eller central modelmutation. 673 × 118, Feggesund og DEC-0114's live Supabase-kapacitetsbevis med mindst 30 procent reserve blokerer Fase B. Gamle forseglede planer/recovery består; en ny direkte rowless/legacy→integrated-plan er forbudt.

Dette tillæg supersederer kun de ældre push-only- og direkte-rowless-initieringsformuleringer nedenfor. Modellen, 30/4/6-controllerkontrakten, atomisk `PENDING`→`ACTIVE`, øvrig recovery og alle faglige gates består. Implementeringen er lokal og endnu ikke commit'et, merged eller live; Candidate G er fortsat offentlig.
...
| Parameterkontrakt | Aktiv 4.0.319-værdi `modelContractSha256=a226e7d10f5c9fa94e122c0e4e3dc1367f1d5e44e763593e4568ac8a3ed1b14b`; historisk state-5-værdi `0cd7c263727721696253ae57c45aa3485b4081ff2cbb5b01a1f022b31b1aa7da` |
| Transitiv implementeringsbundle | Aktuel lokal 4.0.320-værdi `modelBundleSha256=3192db304a6e613059cd66d1ae983583c3aaff832293bda978cdc03991bb49c3` over 44 filer og 8 bindingsforbrugere; 4.0.319-værdien `db475a1bbb1b85fe3e0277b8687d6f1edd6dd8d74e0d6fb4df748f955d5bafe1` og state-5-værdien `27a744e820038d5e508597d02fd0a600479f160a5a5a4a66bdc252e7ea8b3bcd` er historiske |
...
### 4.5 Feggesund: direct-first og én fast wave-only proxy

Den sanitiserede parentserie viser et vedvarende lokalt WAM-hul for `DK-B05-11`/Feggesund. Direkte lokal, provenanceverificeret DMI WAM vinder fortsat for hver del og time. Kun når **hele** den lokale tuple `(Hs, period, mean FROM direction)` mangler, må samme times og samme runs komplette direkte DMI-tuples fra både `DK-B05-10` og `DK-B05-12` danne `FEGGESUND_TWO_NEIGHBOR_WAVE_INTERPOLATION`. Delvis lokal tuple, én manglende nabo, fallbackafledt nabo eller provenance-/tids-/runmismatch giver `MISSING`.

Proxyen er et 50/50 energikonsistent ingeniørestimat: `Hs_proxy = sqrt((Hs1²+Hs2²)/2)`; perioden vægtes med `Hs²`; og mean wave **FROM** direction sammenlægges cirkulært med `Hs²`-vægt. Eksakt antipodal tvetydighed afgøres af størst `Hs²` og ved lighed fast kilde-id-rækkefølge. Samlet usikkerhed er maksimum af tre klasser: retningsforskel ved 45°/90°, relativ energiforskel `|Hs1²−Hs2²|/(Hs1²+Hs2²)` ved 1/3 og 0,6 samt perioderatio ved 1,25 og 1,5. Resultatet mærkes `LOW`, `MODERATE` eller `HIGH`.

En proxytimes afledte score bærer fast kilde-/notice-id, synligt DA/DE/EN-varsel og `calibrationEligible=false` gennem mode, zone, offentlig projektion, tur og observation – også ved ellers `FULL_HISTORY`. Direkte DMI følger den normale historikregel. Undtagelsen leverer kun bølger; aldrig current, historik, recovery-backfill eller kunstig state. Den flytter ingen geometri, land-/vandpunkter eller kystnormal. Før release skal tre dele × 118 timer give præcis 354 privacy-sikre udfald med direct + proxy = 354 og missing = 0.

[DMI WAM](https://www.dmi.dk/friedata/dokumentation/data/forecast-data-wave-model-wam) dokumenterer timeoutput, fire runs dagligt, cirka 5,5 døgn og `wam_dw` omkring 1 km, men ikke wave-current interaction eller varierende tide-/stormvandstand i bølgemodellen. [ECMWF's bølgeparameterdefinitioner](https://confluence.ecmwf.int/download/attachments/59774192/wave_parameters.pdf?version=1) knytter signifikant bølgehøjde til spektralmomentet og dermed energi til `Hs²`. [Copernicus Marine](https://help.marine.copernicus.eu/en/articles/6175153-how-to-describe-wave-height-period-and-direction-parameters) beskriver mean wave direction som en FROM-retning. Det begrunder proxyens matematik og konvention – ikke lokal surfzonefysik eller empirisk fundpræcision.
...
Candidate G-rollback (`CANDIDATE_G_ROLLBACK`) og integreret tilbagevenden (`INTEGRATED_RETURN`) er manual-only. Fra 2026-09-03 er første `INITIAL_INTEGRATED_CUTOVER` ligeledes manual-only gennem den særskilte Fase B; et push udfører kun Fase A/Candidate G. Fra rowless exact legacyprofil er scheduler/manuelt vejr reachable gennem action `candidate-legacy-maintenance` og `LEGACY_CANDIDATE_G_REFRESH_BEFORE_INITIAL_CUTOVER`, aldrig gennem integreret PENDING. Efter legacybroens complete fortsætter samme marker med exact current Candidate-binding og fire `null`-returnfelter; den må kun arves, ikke relabeles. Et selvstændigt sikkert afbrudt initialt integreret forsøg bruger derimod `CANDIDATE_G_REFRESH_BEFORE_INITIAL_CUTOVER` med fire forseglede return-evidence-hashes. Normal allerede aktiv drift bruger `CANDIDATE_G_REFRESH`. I alle tilfælde følger `legacySourceRequired` den faktiske sourcebinding. Scheduleren må aldrig skrive `INTEGRATED_PENDING`, aktivere state 6, rulle tilbage eller returnere en model. Rollbackplanens `calibrationEligible` er `false`, og persistente schema-3-ture/-observationer er Candidate G-bundne med `calibration_eligible=false`. 4.0.319's historiske særskilte Candidate G-binding er `modelContractSha256=c73dac1b4376005e792580791d84eb79c9370e905a2a7fd0bdee857506a20cf8` og `modelBundleSha256=ea22921e298a03ed1ef8787a4dbd79fd4fdf1a9b8e188d3c4b44e03f16fdceb0` over 56 filer.
...
2. **Én offentlig aktivering:** den integrerede runtime vælges eksklusivt fra point-aktivering → gyldig bundle → gyldigt checkpoint → engangs-Candidate G-import. Før DMI kræver den særskilte manuelle Fase B den forseglede centrale Phase-A-complete current Candidate G-identitet på samme head; nye direkte rowless legacy-planer er forbudt. I deployleddet genverificerer controllerforløbet den faktiske offentlige Candidate G-kildes manifest og implementation closure, før det atomisk skriver `INTEGRATED_PENDING` med `transitionKind=INITIAL_INTEGRATED_CUTOVER` og eksakte kilde-/målhashes. Den centrale profil bevares som den valgte Candidate G-kilde, mens det integrerede Pages-artifact publiceres. Efter eksakt offentlig implementerings- og 210/673-verifikation sætter én service-role-RPC samtidigt `INTEGRATED_ACTIVE` og den integrerede centrale 11-feltsprofil. Eksisterende forseglede rowless/historical planer bevares kun til recovery/reconciliation.
...
5. Manglende obligatorisk vind- eller strømevidens og manglende bølgeevidens uden den eksakte Feggesund-undtagelse giver `UNAVAILABLE`; manglende historik giver `HISTORY_INCOMPLETE` med scoreLower/scoreUpper og må ikke skjule prognosen.
...
25. Feggesund-proxyen kan kun opstå ved hel lokal tuple-missing, bruger præcis B05-10+B05-12 fra samme time/run, er wave-only, mærkes synligt og gør alle berørte downstream-scorer ikke-kalibreringsegnede – også ved `FULL_HISTORY`.
