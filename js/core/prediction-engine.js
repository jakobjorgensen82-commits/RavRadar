import { loadAdaptiveModel, modelAdjustment } from './adaptive-model.js?v=4.0.111';
...
function comparableRows(observations,zone,weather){
  ...
  const water=Number(weather?.waterLevelCm),wind=Number(weather?.windSpeedMps);
  const similar=observations.filter(x=>(!Number.isFinite(water)||!Number.isFinite(Number(x.water_level_cm))||Math.abs(Number(x.water_level_cm)-water)<=20)&&(!Number.isFinite(wind)||!Number.isFinite(Number(x.wind_speed_mps))||Math.abs(Number(x.wind_speed_mps)-wind)<=3));
...
export function predictAmberChance({baseScore,zone,weather={},history={},observations=[],model=loadAdaptiveModel()}={}){
  ...
  const fields=['windSpeedMps','waveHeightM','waterLevelCm','currentSpeedMps'];const completeness=fields.filter(k=>Number.isFinite(Number(weather[k]))).length/fields.length;
