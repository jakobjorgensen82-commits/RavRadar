import { loadActiveZoneCollection } from './zone-registry.js?v=4.0.291';
export { createForecastSnapshotReference } from './trip-evidence-contract.js?v=4.0.291';
...
export async function loadConditions({manifest=null}={}){try{
  ...
  if(recovery?.status==='active-last-verified'){
    try{
      ...
      const fallback=await fetchJson(fallbackUrl,{ttlMs:2*60*1000,noStore:true});
...
export async function loadConditionDetails({manifest=null,conditions=null}={}){
  ...
  const data=await fetchJson(url,{ttlMs:2*60*1000,noStore:true});
