import { PUBLIC_CONFIG } from '../../config.js?v=4.0.243';
import { currentSession } from './auth-service.js?v=4.0.243';
import { assertTripEvidencePrivacy } from './trip-evidence-contract.js?v=4.0.243';
...
export function remoteObservationPayload(row){const {gps:localGps,route,track,position,coordinates,latitude,longitude,location,sync_status,sync_error,synced_at,...remote}=structuredClone(row||{});return {...remote,gps:null};}
async function postRemote(row){const session=currentSession();const response=await fetch(`${PUBLIC_CONFIG.supabaseUrl}/rest/v1/observations`,{method:'POST',headers:{apikey:PUBLIC_CONFIG.supabasePublishableKey,Authorization:`Bearer ${session?.access_token||PUBLIC_CONFIG.supabasePublishableKey}`,'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(remoteObservationPayload(row))});if(!response.ok)throw new Error(`HTTP ${response.status}`);}
...
export async function submitTripEvidenceObservation(columns){
  ...
  const row={
    ...
    zone_id:columns.zone_id,
    zone_name:columns.zone_name||columns.zone_id,
    coast_type:null,
    ...
    coastal_part_id:columns.coastal_part_id,
