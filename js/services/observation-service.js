...
import { authorizedFetch, currentSession, requireFreshSession } from './auth-service.js?v=4.0.306';
import { assertTripEvidencePrivacy } from './trip-evidence-contract.js?v=4.0.306';
import { ACCOUNT_TRIP_REPORT_SOURCE, HISTORICAL_SNAPSHOT_UNAVAILABLE } from './account-trip-report-contract.js?v=4.0.306';
...
export function buildImmutableWeatherSnapshot(weather,scoreResult,prediction,{capturedAt=new Date().toISOString()}={}){return structuredClone({schemaVersion:SNAPSHOT_SCHEMA_VERSION,capturedAt,sourceGeneratedAt:weather?.generatedAt??null,forecastTime:weather?.time??null,provider:weather?.provider??null,current:publicWeatherSnapshot(weather),score:{baseScore:scoreResult?.baseScore??scoreResult?.score??null,finalScore:scoreResult?.score??null,level:scoreResult?.level??null,modelVersion:scoreResult?.scoreProfileId??prediction?.modelVersion??null,modelContractVersion:scoreResult?.modelContractVersion??null,stateSchemaVersion:scoreResult?.stateSchemaVersion??null,explanationContractVersion:scoreResult?.explanationContractVersion??null},prediction:publicPredictionSnapshot(prediction||scoreResult?.prediction),matchedRuleIds:(scoreResult?.ruleEvaluation?.matches??[]).map(match=>String(match?.id??match?.ruleId??'').slice(0,120)).filter(Boolean).slice(0,40)});}
