...
function scoreAnswer(ctx){const r=ctx.result,w=ctx.weather||{};if(!r)return'Vælg først en zone, så kan jeg forklare dens score.';const rows=(r.explanations||r.reasons||[]).slice(0,5).map(x=>typeof x==='string'?x:x?.text||x?.explanation).filter(Boolean);return`RavScore ${r.score} for ${ctx.zone?.name||'zonen'} skyldes især:

${rows.map(x=>'• '+x).join('\n')||'• Samspillet mellem transport, mobilisering og jagtbarhed.'}
...
export async function askRavRadar(question,context={}){const safe=String(question||'').trim().slice(0,1200);if(!safe)throw new Error('Skriv et spørgsmål først.');return await remoteAnswer(safe,context)||localAnswer(safe,context);}
