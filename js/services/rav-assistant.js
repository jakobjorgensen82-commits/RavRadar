import { PUBLIC_CONFIG } from "../../config.js?v=4.0.289";
import { buildLocalZoneScore, selectLocalBestForDay } from "../core/local-zone-score.js?v=4.0.289";
...
function shortText(value,max=160){return typeof value==='string'?value.trim().slice(0,max):null;}
export function publicAssistantContext(value={}){const ctx=value&&typeof value==='object'?value:{};const zone=ctx.zone&&typeof ctx.zone==='object'?ctx.zone:{};const result=ctx.result&&typeof ctx.result==='object'?ctx.result:{};const weather=ctx.weather&&typeof ctx.weather==='object'?ctx.weather:{};const reasons=(Array.isArray(result.explanations)?result.explanations:Array.isArray(result.reasons)?result.reasons:[]).map(item=>typeof item==='string'?item:shortText(item?.text||item?.explanation)).filter(Boolean).slice(0,5).map(item=>item.slice(0,180));return{mode:ctx.mode==='beach'?'beach':'waders',zone:{id:shortText(zone.id,80),name:shortText(zone.name,100),coastType:shortText(zone.coastType,60)},result:{available:result.available!==false,score:finite(result.score),level:shortText(result.level,40),reasons},weather:{time:shortText(weather.time,40),provider:shortText(weather.provider,60),windSpeedMps:finite(weather.windSpeedMps),windDirectionDeg:finite(weather.windDirectionDeg),waveHeightM:finite(weather.waveHeightM),wavePeriodS:finite(weather.wavePeriodS),waterLevelCm:finite(weather.waterLevelCm),currentSpeedMps:finite(weather.currentSpeedMps),currentDirectionDeg:finite(weather.currentDirectionDeg),waterTemperatureC:finite(weather.waterTemperatureC)}};}
async function remoteAnswer(question,ctx){if(PUBLIC_CONFIG.ravAssistantRemoteEnabled!==true||!PUBLIC_CONFIG.supabaseUrl||!PUBLIC_CONFIG.supabasePublishableKey)return null;const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),8000);try{const response=await fetch(`${PUBLIC_CONFIG.supabaseUrl}/functions/v1/ravradar-assistant`,{method:'POST',headers:{apikey:PUBLIC_CONFIG.supabasePublishableKey,'Content-Type':'application/json'},body:JSON.stringify({question,context:publicAssistantContext(ctx)}),signal:controller.signal});if(!response.ok)return null;return(await response.json()).answer||null;}catch{return null;}finally{clearTimeout(timer);}}
export async function askRavRadar(question,context={},options={}){const safe=String(question||'').trim().slice(0,600);if(!safe)throw new Error('Skriv et spørgsmål først.');if(options?.localOnly)return localAnswer(safe,context);return await remoteAnswer(safe,context)||localAnswer(safe,context);}
