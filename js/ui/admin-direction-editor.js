...
function pairGeometryCheck(part){
  ...
  if(angleFromPerpendicular>20)return{valid:false,message:`Punktlinjen afviger ${Math.round(angleFromPerpendicular)}° fra vinkelret på kysten.`};
  if(waterSide*landSide>=0)return{valid:false,message:'Land- og havpunkt ligger ikke på hver sin side af kysten.'};
  return{valid:true,message:`Vinkelret kystkryds kontrolleret · afvigelse ${Math.round(angleFromPerpendicular)}°.`};
}
...
export function createDirectionEditor(host,{zones,coastalParts,reviews,saveNow,saveDraft}){
  ...
  const startPlacement=(key,text)=>{placementMode=key;const help=host.querySelector('#directionPlacementHelp');help.textContent=text;help.hidden=false;};host.querySelector('#setSeaPoint').onclick=()=>startPlacement('waterPoint','Klik nu på det rigtige sted i havet på kortet.');host.querySelector('#setLandPoint').onclick=()=>startPlacement('landPoint','Klik nu på det rigtige sted ved stranden/landet på kortet.');
