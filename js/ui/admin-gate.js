import { authEnabled,currentSession,signInWithPassword,signOut,getCurrentRole,testConnection } from '../services/auth-service.js';
...
async function start(){
 ...
 try{const role=await getCurrentRole();if(role!=='owner'){await signOut();return showMessage('Adgang nægtet',`Denne konto har rollen ${esc(role||'ukendt')}. Kun RavRadars ejer kan åbne administrationen.`)};root.hidden=true;shell.hidden=false;await import('./admin-app.js');}
 catch(e){showLogin(e.message)}
}
function showSetup(){showMessage('Supabase mangler','RavRadar skal forbindes én gang. Åbn den enkle opsætningsside, indsæt Project URL og Publishable key, og test forbindelsen.',`<a class="admin-button" href="supabase-setup.html">Åbn Supabase-opsætning</a>`)}
