import { calculateRavScore, scoreRating } from "../core/score-engine.js?v=4.0.264";
import { selectBestTimeForDay } from "../core/best-time-selector.js?v=4.0.264";
...
function stateExplanationPanel(result) {
  ...
  return `<section class="state-explanation"><p class="eyebrow dark">Hvad skete før nu?</p><h3>${escapeHtml(phaseLabels[state.phase]||state.phase)}</h3><p>${escapeHtml(state.summary)}</p>${metrics.length?`<div class="state-metrics">${metrics.join('')}</div>`:''}${facts?`<details><summary>Se forløbet time for time</summary><ul>${facts}</ul><p class="muted">RavRadar bruger det dokumenterede forløb i den aktuelle score.</p></details>`:''}</section>`;
}
  ...
  if (forecastSection) {
    ...
    const render = index => {
      ...
      detail.innerHTML = `<div class="forecast-selected"><div><h4>${capitalize(dayLabel(`${day.date}T12:00:00`))} ${dateLabel(`${day.date}T12:00:00`)}</h4>${best.recommended?`<p>Bedste beregnede tidspunkt: <b>${best.isNow?"Lige nu":hourLabel(h.time)}</b></p><p class="muted">Dette tidspunkt har dagens højeste samlede RavScore${best.isNow?" blandt timerne fra nu og frem":""}. Hvis to tidspunkter har samme score, bruges vandstanden til at vælge mellem dem.</p>${best.candidates?.length>1?`<details class="best-time-comparison"><summary>Se sammenligningen</summary><ol>${best.candidates.slice(0,5).map(candidate=>`<li><span>${candidate.isNow?"Lige nu":hourLabel(candidate.time)}</span><b>RavScore ${candidate.score}</b></li>`).join("")}</ol></details>`:""}`:`<p>Der kan ikke vælges et bedste tidspunkt, fordi der mangler data. Se timeprognosen i stedet.</p>`}</div><div class="score-badge ${r.level}"><strong>${r.available?r.score:"–"}</strong><span>RavScore</span></div></div>
        ...
        <div class="component-list compact metric-sized">${componentDetails("Søgeforhold","huntability",r,"Hvor let det er at lede på den valgte måde. Ved søgning i vandet tæller især vinden og de små bølger, som kan gøre det sværere at lyse gennem vandet. RavScore vurderer ravmuligheden, ikke om turen er sikker.")}${componentDetails("Transport mod kysten","transport",r,"Om strømmen fører rav mod denne kystdel, langs kysten eller væk fra den. Ved kraftig strøm væk fra kysten kan transportscoren ende på nul.")}${componentDetails("Rav i bevægelse","release",r,"Om tidligere bølgeenergi kan have løsnet rav fra havbunden og holdt det i bevægelse. Virkningen kan fortsætte, efter at det hårdeste vejr er ovre.")}</div>${r.available ? coastTransportExplanation(r) : ""}
...
export function showZoneInfo(element, zone, result, condition, mode, options = {}) {
  ...
  const componentHtml = result.available ? `<div class="component-list metric-sized">${componentDetails("Søgeforhold","huntability",result,"Hvor let det er at lede på den valgte måde. Ved søgning i vandet tæller især vinden og de små bølger, som kan gøre det sværere at lyse gennem vandet. RavScore vurderer ravmuligheden, ikke om turen er sikker.")}${componentDetails("Transport mod kysten","transport",result,"Om strømmen fører rav mod denne kystdel, langs kysten eller væk fra den. Ved kraftig strøm væk fra kysten kan transportscoren ende på nul.")}${componentDetails("Rav i bevægelse","release",result,"Om tidligere bølgeenergi kan have løsnet rav fra havbunden og holdt det i bevægelse. Virkningen kan fortsætte, efter at det hårdeste vejr er ovre.")}</div>` : `<div class="metric-grid"><div class="metric"><span>Søgeforhold</span><strong>–/100</strong></div><div class="metric"><span>Transport mod kysten</span><strong>–/100</strong></div><div class="metric"><span>Rav i bevægelse</span><strong>–/100</strong></div></div>`;
