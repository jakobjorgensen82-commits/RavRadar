...
export function selectCandidates(base,audit,owner){
  const auditById=new Map(audit.parts.map(row=>[row.partId,row])), decisions=owner.decisions??{};
  const selected=[];
...
async function main(){
  ...
  const work=val('--work','.geometry-v2-work'),auditFile=val('--audit',path.join(work,'national-inner-water-candidate-audit.json')),ownerFile=val('--owner','data/geometry-v2/national-owner-coastal-review-2026-08-11.json'),out=val('--output-dir','.owner-review/national-inner-water-follow-up');
  const [coastal,partitions,names,points,shadow,audit,owner]=await Promise.all(['national-coastal-parts.geojson','national-locality-partitions.geojson','national-local-part-name-suggestions.json','national-local-part-point-pairs.json','national-shadow-score-validation.json'].map(file=>read(path.join(work,file))).concat([read(auditFile),read(ownerFile)]));
  const base=buildReview({coastal,partitions,names,points,shadow}),parts=selectCandidates(base,audit,owner);
