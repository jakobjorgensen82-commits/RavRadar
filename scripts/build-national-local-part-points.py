...
def build(zones,parts_geo,locality_geo,names,name_audit,policy):
    zone_map={f["properties"]["id"]:f["properties"] for f in zones.get("features") or []};features={f["properties"].get("partId") or f["properties"].get("proposalId"):f for f in (parts_geo.get("features") or [])+(locality_geo.get("features") or [])};candidate_map={r["partId"]:r.get("officialPlaceCandidates") or [] for r in name_audit.get("parts") or []};rows=[]
    for named in names.get("parts") or []:
        ...
        rows.append({"zoneId":named["zoneId"],"finalPartId":named["finalPartId"],"suggestedName":named["suggestedName"],"status":"private-point-pair-proposed" if not reasons else "blocked-point-pair-evidence","coastReferencePoint":lonlat(mid),"landPoint":lonlat(land_point) if not reasons else None,"waterPoint":lonlat(water_point) if not reasons else None,"onshoreDirectionDeg":onshore if not reasons else None,"landWitness":{"officialCandidateId":land_candidate["id"],"name":land_candidate["primaryName"],"distanceM":round(mid.distance(land),1),"normalProjectionM":round(land_projection,1)} if land_candidate else None,"marineWitness":{"source":marine_source,"officialCandidateId":marine_candidate["id"] if marine_candidate else None,"name":marine_candidate["primaryName"] if marine_candidate else None,"point":lonlat(marine) if marine_source.startswith("official") else marine_value,"distanceM":round(mid.distance(marine),1),"normalProjectionM":round(water_projection,1)} if marine is not None and water_projection is not None else None,"unresolvedNormalCandidates":unresolved,"blockingReasons":sorted(set(reasons)),"weatherSamplingEnabled":False,"stateEnabled":False,"scoreEnabled":False,"automaticActivationAllowed":False})
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--work-dir",type=Path,default=ROOT/".geometry-v2-work");p.add_argument("--zones",type=Path,default=ROOT/"data"/"zones.geojson");p.add_argument("--policy",type=Path,default=ROOT/"data"/"geometry-v2"/"national-local-part-point-policy.json");p.add_argument("--self-test",action="store_true");a=p.parse_args()
    if a.self_test:self_test();return
    report=build(load(a.zones),load(a.work_dir/"national-coastal-parts.geojson"),load(a.work_dir/"national-locality-partitions.geojson"),load(a.work_dir/"national-local-part-name-suggestions.json"),load(a.work_dir/"national-coastal-part-name-audit.json"),load(a.policy));(a.work_dir/"national-local-part-point-pairs.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(f"Private nationale punktpar: {report['proposedPointPairCount']}/{report['finalPartCount']} foreslået, {report['blockedPointPairCount']} blokeret.")
