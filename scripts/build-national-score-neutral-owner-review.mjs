...
export function buildReview({coastal,partitions,names,points,shadow}){
  ...
  const scoredIds=shadow.parts.map(p=>p.partId),scored=new Set(scoredIds);
  if(!features.length||features.length!==names.finalPartCount||features.length!==points.finalPartCount||names.parts.length!==features.length||points.parts.length!==features.length||byId.size!==features.length||nameById.size!==features.length||pointById.size!==features.length)throw new Error('Final geometri, navne og punktpar er ikke 1:1');
  if(scored.size!==scoredIds.length)throw new Error('Shadow-score indeholder dublerede kystdele');
  const proposedCount=points.parts.filter(p=>p.status==='private-point-pair-proposed').length,blockedCount=points.parts.length-proposedCount;
  if(points.proposedPointPairCount!=null&&points.proposedPointPairCount!==proposedCount)throw new Error('Punktparrapportens foreslåede antal er inkonsistent');
  if(points.blockedPointPairCount!=null&&points.blockedPointPairCount!==blockedCount)throw new Error('Punktparrapportens blokerede antal er inkonsistent');
  for(const partId of scored){const p=pointById.get(partId);if(!p||p.status!=='private-point-pair-proposed')throw new Error(`${partId}: shadow-score mangler gyldigt punktpar`);}
  if(shadow.scoredPartCount!=null&&shadow.scoredPartCount!==scored.size)throw new Error('Shadow-scorens antal scorede dele er inkonsistent');
  if(shadow.unscoredPartCount!=null&&shadow.unscoredPartCount!==proposedCount-scored.size)throw new Error('Shadow-scorens antal delvist dækkede dele er inkonsistent');
  ...
  const expected={complete:scored.size,partial:proposedCount-scored.size,blocked:blockedCount};
  if(Object.keys(expected).some(k=>counts[k]!==expected[k])||Object.values(counts).reduce((sum,n)=>sum+n,0)!==rows.length)throw new Error(`Inkonsistent national reviewfordeling: ${JSON.stringify({actual:counts,expected})}`);
...
export function selfTest(){
  const feature=(id,zone='Z')=>({type:'Feature',properties:{zoneId:zone,partId:id,lengthKm:1},geometry:{type:'LineString',coordinates:[[8,55],[8.1,55.1]]}}),ids=Array.from({length:835},(_,i)=>`p${i}`),coastal={features:ids.map(id=>feature(id))},partitions={features:[]};
  const names={finalPartCount:835,parts:ids.map((id,i)=>({zoneId:`Z${i%210}`,finalPartId:id,suggestedName:`Del ${i}`,nameStatus:'private-official-name-suggestion'}))};
  const points={finalPartCount:835,proposedPointPairCount:832,blockedPointPairCount:3,parts:ids.map((id,i)=>({finalPartId:id,status:i<3?'blocked':'private-point-pair-proposed',blockingReasons:i<3?['test']:[]}))};
  const shadow={status:'passed-private-national-shadow-score-validation',scoreChanged:false,publicRuntimeChanged:false,scoredPartCount:828,unscoredPartCount:4,parts:ids.slice(3,831).map(partId=>({partId}))};
  const report=buildReview({coastal,partitions,names,points,shadow}),html=renderOwnerReviewHtml(report);if(report.partCount!==835||report.statusCounts.complete!==828||report.statusCounts.partial!==4||report.statusCounts.blocked!==3||html.includes('"score":')||!html.includes('7 kræver kontrol')||!html.includes('Alle 835')||!html.includes('Luftfoto'))throw new Error('National owner-review self-test fejlede');
  let rejected=false;try{buildReview({coastal,partitions,names,points:{...points,proposedPointPairCount:831},shadow})}catch{rejected=true}if(!rejected)throw new Error('National owner-review accepterede inkonsistent punktoptælling');
  rejected=false;try{buildReview({coastal,partitions,names:{...names,parts:[...names.parts,names.parts[0]]},points,shadow})}catch{rejected=true}if(!rejected)throw new Error('National owner-review accepterede dubleret navneinput');
