#!/usr/bin/env python3
"""Verify one COMPLETE Copernicus range seal without exposing private rows."""
...
def inspect(
    ...
    if any(collection.get(key) != value for key, value in expected.items()):
        raise RuntimeError("Private Copernicus COMPLETE seal does not match the exact target/DMI gap matrix")
    return {
        "cachePresent": True,
        "completeRangePresent": True,
        "productionReferenceAt": reference,
        "requiredPairCount": collection["requiredPairCount"],
...
def main() -> int:
    args = arguments()
    state = inspect(args.shadow, args.registry, args.dmi, args.targets, args.at)
    write_outputs(args.github_output, state)
    if args.require_complete and not state["completeRangePresent"]:
        raise RuntimeError("The exact COMPLETE Copernicus range seal is required but absent")
    if state["completeRangePresent"]:
        print(
            "Private Copernicus range seal is COMPLETE: "
