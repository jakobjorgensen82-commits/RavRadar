...
for(const [document_key,file] of Object.entries(assets)){
 ...
 const r=await fetch(`${url}/rest/v1/admin_documents?on_conflict=document_key`,{method:'POST',headers:{apikey:key,...(key.startsWith('sb_secret_')?{}:{Authorization:`Bearer ${key}`}), 'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({document_key,payload,updated_by:null})});
