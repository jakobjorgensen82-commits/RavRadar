...
const publicRavScoreProfileId='RRS-COASTAL-CAUSAL-CHAIN-1';
function assertPublicRavScoreSelection(payload,label='ravscore-profile-selection'){
 const version=String(payload?.sourceVersion||'');
 const valid=/^\d+\.\d+\.\d+$/.test(version)
  &&payload?.schemaVersion==='3.0.0'
  &&payload?.switchVersion==='RAVSCORE-PROFILE-SWITCH-4.0.306'
  &&payload?.requestedProfileId===publicRavScoreProfileId
  &&payload?.modelProfileId===publicRavScoreProfileId
  &&payload?.rollbackProfileId===null
  &&payload?.publicModelEnabled===true
  &&payload?.prePublicWarmupAccepted===false
  &&payload?.automaticActivationAllowed===false
  &&payload?.publicAvailabilityPolicy==='ravscore-local-fail-closed'
  &&payload?.legacyPublicFallbackAllowed===false
  &&String(payload?.status||'').startsWith('owner-approved-integrated-next-ravscore-')
  &&Boolean(String(payload?.activationAuthority||'').trim())
  &&Boolean(String(payload?.evidence?.ownerReviewDecisionId||'').trim());
 if(!valid)throw new Error(`${label} er ikke den komplette integrerede RavScore-kontrakt`);
 return payload;
}
...
for(const [document_key,file] of Object.entries(assets)){
 ...
 if(document_key==='ravscore-profile-selection')assertPublicRavScoreSelection(payload,'Lokal ravscore-profile-selection');
...
assertPublicRavScoreSelection(ravScoreSelectionLocal,'Lokal ravscore-profile-selection');
assertPublicRavScoreSelection(ravScoreSelectionCentral,'Central ravscore-profile-selection');
