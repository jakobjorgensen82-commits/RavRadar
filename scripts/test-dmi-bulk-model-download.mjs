...
assert.match(bulk, /PARSER_VERSION = 8/);
...
assert.match(workflow, /RavRadar\/4\.0\.27/);
...
assert.match(bulk, /reserved_wind_rank = 0 if collection == "harmonie_dini_sf"/);
assert.match(bulk, /scheduleCoverageBeforeRun/);
assert.match(bulk, /windReservationActive/);
