...
const nationalOwnerReview = await fs.readFile('scripts/build-national-score-neutral-owner-review.mjs', 'utf8');
const nationalAdminRoundtrip = await fs.readFile('scripts/validate-national-admin-roundtrip.mjs', 'utf8');
...
assert.match(workflow, /node scripts\/build-national-score-neutral-owner-review\.mjs/);
...
for (const marker of ['passed-private-national-score-neutral-owner-review','coverageStatusColorsNotScores','partScoresForbidden','rawWeatherForbidden','automaticActivationAllowed']) assert.ok(nationalOwnerReview.includes(marker), `National ejer-review mangler ${marker}`);
for (const marker of ['passed-private-national-admin-roundtrip-and-rollback','protectedRuntimeUnchanged','tempDocumentAbsentAfterRollback','rawAdminPayloadStored','automaticActivationAllowed']) assert.ok(nationalAdminRoundtrip.includes(marker), `National admin-roundtrip mangler ${marker}`);
