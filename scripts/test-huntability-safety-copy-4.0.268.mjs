...
assert.ok(ui.includes('Hvor let det er at lede på den valgte måde'), 'Søgeforhold skal forklares som praktisk søgning');
...
assert.ok(assistant.includes("if(intent==='safety')return KNOWLEDGE.safety"), 'Et direkte spørgsmål om sikkerhed skal stadig kunne besvares');
