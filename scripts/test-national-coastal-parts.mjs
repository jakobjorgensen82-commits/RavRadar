...
for(const x of ['proposedName") is not None','inventedConnectionCount','EXPECTED_ZONE_COUNT=211','zoneCount")!=EXPECTED_ZONE_COUNT'])assert.ok(validator.includes(x),`Validator mangler ${x}`);
assert.ok(source.includes('EXPECTED_ZONE_COUNT=211'),'Delgeneratoren skal følge den aktuelle nationale 211-zonepolitik');
