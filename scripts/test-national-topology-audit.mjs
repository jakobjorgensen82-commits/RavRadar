...
for(const marker of ['EXPECTED_ZONE_COUNT=210','zoneCount")!=EXPECTED_ZONE_COUNT','selectedExclusionCount','automaticActivationAllowed','manual-review-required']) assert.ok(validator.includes(marker),`Validator mangler ${marker}`);
assert.ok(source.includes('EXPECTED_ZONE_COUNT=210'),'Topologianalysen skal følge den aktuelle nationale 210-zonepolitik');
