...
const updateWeather = fs.readFileSync('scripts/update-weather.mjs', 'utf8');
...
requireMarkers(inspectJob, [
  ...
  '--cadence-policy data/current-regional-proxy-policy.json',
  '--before-attestation',
  '--after-attestation',
  '--before-bundle',
  '--after-bundle',
...
const orderedSteps = [
  'Restore the latest compact schema-4 RavScore checkpoint',
  'Restore protected compact RavScore checkpoint when cache is absent',
  'Resolve the one-time Candidate G bootstrap gate',
  'Verify exact sealed inspection artifact identity',
  'Apply the descriptor-bound one-time Candidate G reconstruction',
  ...
  'Build compact schema-4 RavScore continuation checkpoint after final gates',
  'Save compact schema-4 RavScore continuation checkpoint after final gates',
...
const normalFallbackReport = step('Report skipped refresh');
assert.ok(
  normalFallbackReport.includes("steps.preflight.outputs.should_run != 'true'")
    && !normalFallbackReport.includes('candidate_g_gap_reconstruction_mode'),
  'Normal drift skal have et rent friskheds-no-op uden en skjult Candidate G-fallbackvej.',
);

const retiredCandidateRecoveryStepNames = [
  ...
  'Build compact Candidate G continuation checkpoint',
  'Save compact Candidate G continuation checkpoint before final gates',
];
const legacyCandidateRecoveryPresent = retiredCandidateRecoveryStepNames.some(name => workflow.includes(`name: ${name}`));
if (legacyCandidateRecoveryPresent) {
for (const name of [
  'Inspect verified Candidate G continuation recovery',
  'Inspect failed-run Candidate G gap checkpoint recovery',
  'Restore latest compact Candidate G continuation checkpoint',
...
for (const retiredName of retiredCandidateRecoveryStepNames) {
  assert.ok(!workflow.includes(`name: ${retiredName}`), `Den integrerede produktionsvej må ikke genaktivere ${retiredName}.`);
}

const integratedContinuationRestore = step('Restore the latest compact schema-4 RavScore checkpoint');
requireMarkers(integratedContinuationRestore, [
  "if: steps.private-runtime-state.outputs.available != 'true'",
  'key: ravscore-continuation-schema4-v2-${{ github.run_id }}-${{ github.run_attempt }}',
  'ravscore-continuation-schema4-v2-',
], 'Schema-4 continuation-cache restore');
assert.ok(!integratedContinuationRestore.includes('candidate-g-continuation-checkpoint'), 'Schema-4 restore må ikke bruge Candidate G-checkpointkeys.');

const protectedIntegratedRestore = step('Restore protected compact RavScore checkpoint when cache is absent');
requireMarkers(protectedIntegratedRestore, [
  "steps.ravscore-checkpoint-cache.outputs.cache-matched-key == ''",
  'node scripts/protected-ravscore-continuation-checkpoint.mjs',
  '--restore',
  '--target-reference "$RAVRADAR_PRODUCTION_TARGET_HOUR"',
], 'Protected schema-4 restore');

const integratedContinuationBuild = step('Build compact schema-4 RavScore continuation checkpoint after final gates');
requireMarkers(integratedContinuationBuild, [
  "if: steps.preflight.outputs.should_run == 'true' && steps.weather.outcome == 'success'",
  'node scripts/ravscore-continuation-checkpoint.mjs',
  '--save',
  '--source data/live/conditions.json',
], 'Schema-4 checkpoint build');
const integratedContinuationSave = step('Save compact schema-4 RavScore continuation checkpoint after final gates');
requireMarkers(integratedContinuationSave, [
  "if: steps.preflight.outputs.should_run == 'true' && steps.weather.outcome == 'success'",
  'uses: actions/cache/save@v6',
  'ravscore-continuation-schema4-v2-${{ github.run_id }}-${{ github.run_attempt }}',
], 'Schema-4 checkpoint save');
assert.ok(!integratedContinuationBuild.includes('candidate-g-continuation-checkpoint')
  && !integratedContinuationSave.includes('candidate-g-continuation-checkpoint'),
'Den delte continuation-kæde må kun gemme den aktive schema-4-model.');
assert.ok(position('name: Run release governance gate after refreshed data validation')
  < position('name: Build compact schema-4 RavScore continuation checkpoint after final gates'),
'Schema-4 checkpointet må først mintes efter fuld validate og releasegate.');

for (const marker of [
  'RECONSTRUCTED_TRANSPORT_EVIDENCE_TRUST_STATUS',
  'CANDIDATE_G_RECONSTRUCTED_STATE_SCHEMA_VERSION',
  'transportEvidence.filter(isReconstructedTransportEvidence)',
  'Integrated RavScore cutover refuses Candidate G source with active reconstructed evidence trust',
  'Integrated RavScore cutover refuses Candidate G schema 2.1 or reconstructed transport samples',
]) {
  assert.ok(updateWeather.includes(marker), `Den integrerede zero-synthetic-cutovergate mangler ${marker}`);
}
...
requireMarkers(deploy, [
  ...
  "if: github.ref == 'refs/heads/main' && needs.build-and-prepare.outputs.should_deploy == 'true'",
  'id: deployment',
  'url: ${{ steps.integrated-normalization-deployment.outputs.page_url || steps.deployment.outputs.page_url }}',
