...
for(const [zoneId,fullZone] of Object.entries(full.zones||{})){
 ...
 for(const mode of ['waders','beach']){
  ...
  const hours=fullZone.forecast?.hourly||[];for(const index of [0,Math.floor(hours.length/2),hours.length-1].filter(i=>i>=0)){
   ...
   const pb=calculateRavScore({mode,zone,weather:detailsFile.zones[zoneId].forecast.hourly[index],history:pub.history||{}});
...
console.log(`OK: progressiv offentlig runtime bevarer scorer og reducerer startpakken fra ${fullBytes} bytes til ${publicBytes} bytes.`);
