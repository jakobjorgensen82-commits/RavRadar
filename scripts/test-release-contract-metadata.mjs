...
const LOCAL_UNAVAILABLE_CUTOVER_BINDING_CHECK =
  'node scripts/build-local-unavailable-cutover-binding-migration.mjs';
const PUBLIC_RUNTIME_ORACLE_BINDING_CHECK =
...
const POST_CUTOVER_CURRENT_HOLD_BINDING_CHECK =
  'node scripts/build-post-cutover-current-hold-binding-migration.mjs';
const VALID_DATA_BEFORE_LOCAL_MISSING_BINDING_CHECK =
...
const MEASURED_WARMUP_CHECKPOINT_CHECK =
  'node scripts/build-measured-warmup-checkpoint-migration.mjs';
const PUBLIC_HOUR_DELIVERY_BINDING_CHECK =
  'node scripts/build-public-hour-delivery-binding-migration.mjs';
const INTEGRATED_MODEL_BINDING_SUCCESSOR_CHECK =
...
const RELEASE_METADATA_TEST_COMMAND = [
  ...
  MEASURED_WARMUP_CHECKPOINT_CHECK,
...
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
    file => file === 'scripts/build-measured-warmup-checkpoint-migration.mjs',
  ).length,
  1,
  'Release gate must run the measured-warmup checkpoint successor check exactly once',
);
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
    file => file === 'scripts/build-public-hour-delivery-binding-migration.mjs',
  ).length,
  1,
  'Release gate must run the public-hour delivery binding successor check exactly once',
);
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
...
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
    file => file === 'scripts/build-local-unavailable-cutover-binding-migration.mjs',
  ).length,
  1,
  'Release gate must run the local-unavailable cutover binding check exactly once',
);
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
    file => file === 'scripts/build-public-runtime-oracle-binding-migration.mjs',
  ).length,
  1,
  'Release gate must run the public-runtime oracle binding check exactly once',
);
assert.equal(
  RELEASE_GATE_TEST_FILES.filter(
    file => file === 'scripts/build-h0-reference-recovery-binding-migration.mjs',
  ).length,
  1,
...
assert.equal(
  ...
  (await computeCandidateGRollbackBundle()).modelBundleSha256,
);
