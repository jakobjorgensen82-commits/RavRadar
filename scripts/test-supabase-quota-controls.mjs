...
for(const token of ['new.payload is not distinct from old.payload','protected-asset-manifest','coastal-point-staging-status','ravscore-continuation-checkpoint'])assert.ok(schema.includes(token),`Nye Supabase-installationer mangler ${token}`);
