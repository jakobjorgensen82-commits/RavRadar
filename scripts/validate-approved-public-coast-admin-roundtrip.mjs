...
async function main(){
  ...
  const read=async documentKey=>(await request(`?document_key=eq.${encodeURIComponent(documentKey)}&select=document_key,payload,version,updated_at`,{},`godkendt kyst-roundtrip: læs ${documentKey}`))?.[0]??null;
  ...
  try{
    ...
    await request('',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({document_key:tempKey,payload:{...draft,phase:'create'}})},'godkendt kyst-roundtrip: opret midlertidig kladde');created=true;
    ...
    await request(`?document_key=eq.${encodeURIComponent(tempKey)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({payload:{...draft,phase:'rollback-ready'}})},'godkendt kyst-roundtrip: opdater midlertidig kladde');
    ...
    await request(`?document_key=eq.${encodeURIComponent(tempKey)}`,{method:'DELETE',headers:{Prefer:'return=representation'}},'godkendt kyst-roundtrip: slet midlertidig kladde').catch(()=>{});deleted=!(await read(tempKey));
  }
