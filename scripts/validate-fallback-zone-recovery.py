...
ALLOWED = {"DK-B07-19", "DK-B08-12", "DK-B08-18", "DK-B08-19", "DK-B10-14", "DK-B08-10", "DK-B08-17"}
TARGET_RECOVERY_ZONES = {"DK-B07-19", "DK-B08-12", "DK-B08-18", "DK-B08-19", "DK-B10-14"}
...
def main():
    ...
    if "DK-B10-16" in zones:
        errors.append("Den slettede Fejø/Femø-zone er genoplivet")
    planned_zone_ids = {row.get("zoneId") for row in (plan.get("zones") or [])}
    if planned_zone_ids != TARGET_RECOVERY_ZONES:
        errors.append(f"Fallbackplanen har forkert målzonebestand: {sorted(planned_zone_ids)}")
    if set(plan.get("deletedZonesPreserved") or []) != {"DK-B02-14", "DK-B10-16"}:
        errors.append("Fallbackplanen bevarer ikke begge centralt slettede zoner")
    ...
    for part_id, source_owner in EXPECTED_MOVE_OWNERS.items():
        ...
        target_owner = planned_moves.get(part_id)
        if target_owner in {None, source_owner}:
            errors.append(f"{part_id} mangler en ny, anden hovedzoneejer")
        elif owners == [target_owner]:
            completed_moves += 1
        elif owners != [source_owner]:
            errors.append(
                f"{part_id} findes hverken hos oprindelig ejer {source_owner} "
                f"eller færdig målzone {target_owner}: {owners}"
            )
    ...
    for part_id, owner in EXPECTED_REPLACED_OWNER.items():
        ...
        if not owners:
            ...
            errors.append(f"{part_id} findes hos en uventet ejer: {owners}")
    ...
    for part_id in ids:
        ...
        if not row:
            ...
            continue
        if row.get("status") == "private-point-pair-proposed":
            if not row.get("landPoint") or not row.get("waterPoint"):
                errors.append(f"Aktivt punktpar mangler land- eller vandpunkt: {part_id}")
        elif row.get("status") == "blocked-point-pair-evidence":
            alternatives = row.get("unresolvedNormalCandidates") or []
            if row.get("landPoint") is not None or row.get("waterPoint") is not None or len(alternatives) != 2:
                errors.append(f"Blokeret punktpar er ikke fail-closed: {part_id}")
        else:
            errors.append(f"Ukendt punktstatus: {part_id}")
    ...
    for owner, parts in (active_document.get("zones") or {}).items():
        for part in parts:
            ...
            for feature, candidate_geometry in metric:
                if (
                    part.get("partId") in active_candidate_ids
                    and part.get("partId") == feature["properties"]["partId"]
                    and owner == feature["properties"]["zoneId"]
                ):
                    continue
    ...
    report = {
        ...
        "blockedPointPairCount": sum(part_id in points and points[part_id].get("status") == "blocked-point-pair-evidence" for part_id in ids),
        "ownershipMoveCount": len(planned_moves),
        "completedOwnershipMoveCount": completed_moves,
        "replacementPartCount": len(planned_replacements),
        "completedReplacementPartCount": completed_replacements,
