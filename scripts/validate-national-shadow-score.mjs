...
import { buildBlendedRegimeMemory, buildCurrentTransportPotential, normalizeMemoryTrackCausally, signedDirectionalForce } from '../js/core/ravscore-regime-memory.js';
...
function candidateGMemoryByTime(marineHours,localWind,part){
  const samples=marineHours.map(hour=>{
    ...
    const currentSpeedMps=Math.hypot(cu,cv),currentAlignment=directionalAlignment(toward(cu,cv),onshore);
    return {time:hour.time,currentSpeedMps,currentAlignment,currentForce:signedDirectionalForce({magnitude:currentSpeedMps,alignment:currentAlignment}),waveForce:signedDirectionalForce({magnitude:Math.max(0,waveHeight)**2*Math.max(0,wavePeriod),alignment:directionalAlignment(norm(waveFrom+180),onshore)}),windForce:signedDirectionalForce({magnitude:Math.hypot(wu,wv),alignment:directionalAlignment(toward(wu,wv),onshore)})};
  }).filter(Boolean).sort((a,b)=>Date.parse(a.time)-Date.parse(b.time));
  const transportPotential=buildCurrentTransportPotential(samples);
  const result={};
  for(const [key,track] of Object.entries(candidateGTracks)){
    ...
    result[key]=new Map(samples.map((sample,index)=>[sample.time,{current:current[index].boundedState,wave:wave[index].boundedState,directWind:directWind[index].boundedState,transportPotential:transportPotential[index].transportPotential,outboundEpisodeEffectiveHours:transportPotential[index].outboundEpisodeEffectiveHours,outboundEpisodeLossPoints:transportPotential[index].outboundEpisodeLossPoints,actualOutboundTransport:transportPotential[index].actualOutboundTransport}]));
  }
  return result;
}
...
function buildCandidateShadow(partRows,contract){
  const rows=partRows.flatMap(part=>part.snapshots.flatMap(snapshot=>Object.entries(snapshot.modes).map(([mode,result])=>({zoneId:part.zoneId,partId:part.partId,time:snapshot.time,mode,currentRegime:result.currentRegime,active:result.score,activeFinal:result.finalScore,activeRuleCount:result.activeRuleCount,activeRuleMatchCount:result.activeRuleMatchCount,candidateGRuleMatchCount:result.candidateGRuleMatchCount,noDirectRuleMatchCount:result.noDirectRuleMatchCount,wadersLimitRuleMatchCount:result.wadersLimitRuleMatchCount,windLedWadersRuleMatchCount:result.windLedWadersRuleMatchCount,currentLedOutflow8RuleMatchCount:result.currentLedOutflow8RuleMatchCount,...result.shadowCandidates}))));
  const byMode=Object.fromEntries(['waders','beach'].map(mode=>{const selected=rows.filter(row=>row.mode===mode);return [mode,{activeToA:summarizeDeltas(selected,'active','candidateA'),activeToB:summarizeDeltas(selected,'active','candidateB'),activeToC:summarizeDeltas(selected,'active','candidateC'),activeToD:summarizeDeltas(selected,'active','candidateD'),activeToE:summarizeDeltas(selected,'active','candidateE'),activeToG24:summarizeDeltas(selected,'active','candidateG24'),activeToG5050:summarizeDeltas(selected,'active','candidateG5050'),activeToG48:summarizeDeltas(selected,'active','candidateG48'),activeFinalToG5050Final:summarizeDeltas(selected,'activeFinal','candidateG5050Final'),activeFinalToGNoDirectWindFinal:summarizeDeltas(selected,'activeFinal','candidateGNoDirectWindFinal'),activeFinalToGWadersLimitFinal:summarizeDeltas(selected,'activeFinal','candidateGWadersLimitFinal'),activeFinalToGWindLedWadersFinal:summarizeDeltas(selected,'activeFinal','candidateGWindLedWadersFinal'),activeFinalToGCurrentLedOutflow8Final:summarizeDeltas(selected,'activeFinal','candidateGCurrentLedOutflow8Final'),g5050BaseToFinal:summarizeDeltas(selected,'candidateG5050','candidateG5050Final'),noDirectBaseToFinal:summarizeDeltas(selected,'candidateGNoDirectWind','candidateGNoDirectWindFinal'),wadersLimitBaseToFinal:summarizeDeltas(selected,'candidateGWadersLimit','candidateGWadersLimitFinal'),windLedWadersBaseToFinal:summarizeDeltas(selected,'candidateGWindLedWaders','candidateGWindLedWadersFinal'),currentLedOutflow8BaseToFinal:summarizeDeltas(selected,'candidateGCurrentLedOutflow8','candidateGCurrentLedOutflow8Final'),windLedWadersToCurrentLedOutflow8:summarizeDeltas(selected,'candidateGWindLedWaders','candidateGCurrentLedOutflow8'),noDirectToWadersLimit:summarizeDeltas(selected,'candidateGNoDirectWind','candidateGWadersLimit'),wadersLimitToWindLedWaders:summarizeDeltas(selected,'candidateGWadersLimit','candidateGWindLedWaders'),eToG5050:summarizeDeltas(selected,'candidateE','candidateG5050'),g24ToG48:summarizeDeltas(selected,'candidateG24','candidateG48'),noDirectToG5050:summarizeDeltas(selected,'candidateGNoDirectWind','candidateG5050'),aToB:summarizeDeltas(selected,'candidateA','candidateB'),bToC:summarizeDeltas(selected,'candidateB','candidateC'),cToD:summarizeDeltas(selected,'candidateC','candidateD'),dToE:summarizeDeltas(selected,'candidateD','candidateE')}];}));
  ...
  const candidateIds=['candidateA','candidateB','candidateC','candidateD','candidateE','candidateG24','candidateG5050','candidateG48','candidateGNoDirectWind','candidateGWadersLimit','candidateGWindLedWaders','candidateGCurrentLedOutflow8','candidateG5050Final','candidateGNoDirectWindFinal','candidateGWadersLimitFinal','candidateGWindLedWadersFinal','candidateGCurrentLedOutflow8Final'];
  ...
  for(const marine of marineInput.series){
    ...
    for(const hour of marine.hours){
      ...
      for(const mode of ['waders','beach']){
        ...
        const windLedWaders=evaluateRavScoreCandidateG({mode,zone,weather,history},{variantId:'G-50-50-NO-DIRECT-WIND-WADERS-WIND-LED',memory:candidateGMemory.candidateG5050.get(hour.time)});
        const currentLedOutflow8=evaluateRavScoreCandidateG({mode,zone,weather,history},{variantId:'G-CURRENT-LED-OUTFLOW-8-WADERS-WIND-LED',memory:candidateGMemory.candidateG5050.get(hour.time)});
        ...
        const windLedWadersRuleResult=evaluateRules({rules:activeRules,zone,mode,weather,history,baseScore:windLedWaders.score});
        const windLedWadersFinal=mode==='waders'&&windLedWadersRuleResult.score!==null?Math.min(windLedWadersRuleResult.score,windLedWaders.components.huntability):windLedWadersRuleResult.score;
        const currentLedOutflow8RuleResult=evaluateRules({rules:activeRules,zone,mode,weather,history,baseScore:currentLedOutflow8.score});
        const currentLedOutflow8Final=mode==='waders'&&currentLedOutflow8RuleResult.score!==null?Math.min(currentLedOutflow8RuleResult.score,currentLedOutflow8.components.huntability):currentLedOutflow8RuleResult.score;
        modes[mode]={score:result.score,finalScore:activeFinal.score,activeRuleCount:activeRules.length,activeRuleMatchCount:activeFinal.matches.length,candidateGRuleMatchCount:candidateGFinal.matches.length,noDirectRuleMatchCount:noDirectFinal.matches.length,wadersLimitRuleMatchCount:wadersLimitRuleResult.matches.length,windLedWadersRuleMatchCount:windLedWadersRuleResult.matches.length,currentLedOutflow8RuleMatchCount:currentLedOutflow8RuleResult.matches.length,components:result.components,contributions:result.explanation?.contributions,dominantPathway:result.explanation?.mobilisationDiagnostics?.dominantPathway??null,currentRegime:currentRegime(weather,zone),currentLedActualOutboundTransport:currentLedOutflow8.diagnostics.candidateGActualOutboundTransport,shadowCandidates:{...shadow.candidateScores,...candidateGResults,candidateGNoDirectWind:noDirect.score,candidateGWadersLimit:wadersLimit.score,candidateGWadersLimitHuntability:wadersLimit.components.huntability,candidateGWindLedWaders:windLedWaders.score,candidateGWindLedWadersHuntability:windLedWaders.components.huntability,candidateGCurrentLedOutflow8:currentLedOutflow8.score,candidateGCurrentLedOutflow8Huntability:currentLedOutflow8.components.huntability,candidateGCurrentLedOutflow8TransportPotential:currentLedOutflow8.diagnostics.candidateGCurrentLedTransportPotential,candidateG5050Final:candidateGFinal.score,candidateGNoDirectWindFinal:noDirectFinal.score,candidateGWadersLimitFinal:wadersLimitFinal,candidateGWindLedWadersFinal:windLedWadersFinal,candidateGCurrentLedOutflow8Final:currentLedOutflow8Final}};
      }
...
export function selfTest(){
  ...
  if(!report.candidateShadow.modelIds?.candidateA||!report.candidateShadow.modelIds?.candidateD||!report.candidateShadow.modelIds?.candidateG5050||!report.candidateShadow.modelIds?.candidateGWadersLimit||!report.candidateShadow.modelIds?.candidateGWindLedWaders||!report.candidateShadow.modelIds?.candidateGCurrentLedOutflow8||!Object.values(report.parts[0].snapshots[0].modes.waders.shadowCandidates).every(Number.isFinite))throw new Error('Kandidat-shadow mangler stabile modeller eller scorer');
  if(report.candidateShadow.byMode.waders.activeToG5050.count!==2||report.candidateShadow.byMode.beach.noDirectToG5050.count!==2||report.candidateShadow.byMode.waders.activeFinalToGCurrentLedOutflow8Final.count!==2)throw new Error('Candidate G national shadow mangler tracks eller no-direct-wind/current-led kontrol');
  if(report.candidateShadow.centralRuleChain.activeRuleCount!==1||report.candidateShadow.centralRuleChain.activeMatchedContextCount!==4||report.candidateShadow.centralRuleChain.candidateGMatchedContextCount!==4||report.candidateShadow.centralRuleChain.currentLedOutflow8MatchedContextCount!==4||report.candidateShadow.byMode.waders.g5050BaseToFinal.meanDelta!==2)throw new Error('Candidate G national shadow afspiller ikke den centrale slutregelkaede');
  if(report.candidateShadow.centralRuleChain.wadersLimitMatchedContextCount!==4||report.candidateShadow.centralRuleChain.windLedWadersMatchedContextCount!==4||!report.candidateShadow.centralRuleChain.wadersHuntabilityLimitAppliedAfterRules)throw new Error('Waders-varianterne afspiller ikke regler og jagtbarhedsloft i den dokumenterede rækkefølge');
  for(const part of report.parts)for(const snapshot of part.snapshots){const waders=snapshot.modes.waders.shadowCandidates,beach=snapshot.modes.beach.shadowCandidates;if(waders.candidateGWadersLimitFinal>waders.candidateGWadersLimitHuntability||waders.candidateGWindLedWadersFinal>waders.candidateGWindLedWadersHuntability||waders.candidateGCurrentLedOutflow8Final>waders.candidateGCurrentLedOutflow8Huntability)throw new Error('Waders-slutscore overstiger jagtbarheden');if(beach.candidateGWadersLimit!==beach.candidateGNoDirectWind||beach.candidateGWindLedWaders!==beach.candidateGNoDirectWind)throw new Error('Waders-varianterne ændrer strandscore');}
