...
function buildCandidateShadow(partRows,contract){
  ...
  const candidateIds=['candidateA','candidateB','candidateC','candidateD','candidateE','candidateG24','candidateG5050','candidateG48','candidateGNoDirectWind','candidateGWadersLimit','candidateGWindLedWaders','candidateG5050Final','candidateGNoDirectWindFinal','candidateGWadersLimitFinal','candidateGWindLedWadersFinal'];
  ...
  for(const marine of marineInput.series){
    ...
    for(const hour of marine.hours){
      ...
      for(const mode of ['waders','beach']){
        ...
        modes[mode]={score:result.score,finalScore:activeFinal.score,activeRuleCount:activeRules.length,activeRuleMatchCount:activeFinal.matches.length,candidateGRuleMatchCount:candidateGFinal.matches.length,noDirectRuleMatchCount:noDirectFinal.matches.length,wadersLimitRuleMatchCount:wadersLimitRuleResult.matches.length,windLedWadersRuleMatchCount:windLedWadersRuleResult.matches.length,components:result.components,contributions:result.explanation?.contributions,dominantPathway:result.explanation?.mobilisationDiagnostics?.dominantPathway??null,currentRegime:currentRegime(weather,zone),shadowCandidates:{...shadow.candidateScores,...candidateGResults,candidateGNoDirectWind:noDirect.score,candidateGWadersLimit:wadersLimit.score,candidateGWadersLimitHuntability:wadersLimit.components.huntability,candidateGWindLedWaders:windLedWaders.score,candidateGWindLedWadersHuntability:windLedWaders.components.huntability,candidateG5050Final:candidateGFinal.score,candidateGNoDirectWindFinal:noDirectFinal.score,candidateGWadersLimitFinal:wadersLimitFinal,candidateGWindLedWadersFinal:windLedWadersFinal}};
      }
...
export function selfTest(){
  ...
  if(!report.candidateShadow.modelIds?.candidateA||!report.candidateShadow.modelIds?.candidateD||!report.candidateShadow.modelIds?.candidateG5050||!report.candidateShadow.modelIds?.candidateGWadersLimit||!report.candidateShadow.modelIds?.candidateGWindLedWaders||!Object.values(report.parts[0].snapshots[0].modes.waders.shadowCandidates).every(Number.isFinite))throw new Error('Kandidat-shadow mangler stabile modeller eller scorer');
