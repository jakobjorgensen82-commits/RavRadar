...
def validate(report,geo,waters):
    ...
    for key in ("riverNarrowLinePartRejectedCount","riverShortLinePartRejectedCount"):
        if report.get(key)!=sum(r.get(key,0) for r in zones) or report.get(key,0)<0:fail(f"National vandløbsfravalgstæller er inkonsistent: {key}")
